...
Портал отправляет письма транзакционным SMTP smtp.bz. Разговор идёт по SMTP, а не по HTTP API
провайдера: смена провайдера должна быть правкой `env`, а не правкой кода.

**Домен отправителя — `auto.su10.ru`**, тот же, на котором живёт портал.
...
### 5.2 Настройки в панели провайдера
