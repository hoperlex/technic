...
### 5.1 Дополнительные каналы отправки

Всё выше описывает **основной** канал (`default`) — им уходят задания водителям, сводки и письма про
доступ. Канал сверх него нужен, когда письмо обязано уйти **от другого ящика**: почтовые службы
отвергают чужой `From` ответом 550, а SPF и DKIM домена подписывает его собственный сервер. Первый
такой случай — служба ремонта оргтехники: письма по заявкам уходят с её ящика и ей же адресованы
(план [office-equipment-mail-and-history-plan.md](office-equipment-mail-and-history-plan.md),
Р83–Р90).

Канал описывается своим набором переменных в том же `prod.env`; ключ (`repair`) известен порталу —
реестр в `packages/contracts/src/mail-accounts.ts`, и письмо выбирает канал по нему:

```env
MAIL_ACCOUNT_REPAIR_HOST=m.example.ru
MAIL_ACCOUNT_REPAIR_PORT=465
MAIL_ACCOUNT_REPAIR_SECURE=true          # 465 — implicit TLS; для 587 было бы false
MAIL_ACCOUNT_REPAIR_AUTH_METHOD=CRAM-MD5 # «зашифрованный пароль» почтовых клиентов; иначе LOGIN
MAIL_ACCOUNT_REPAIR_USER=repair@example.ru
MAIL_ACCOUNT_REPAIR_PASSWORD=
MAIL_ACCOUNT_REPAIR_FROM="Ремонт оргтехники <repair@example.ru>"
MAIL_ACCOUNT_REPAIR_MAX_PER_MINUTE=20
```

Что важно знать про каналы:

- **Канал считается объявленным по любой своей переменной**, а настроенным — когда есть сервер,
  учётка, пароль и отправитель. Объявленный наполовину роняет старт воркера с именами недостающих
  переменных: канал, выглядящий заведённым, но не умеющий отправлять, — худший из вариантов.
- **Пара «порт + защита» проверяется на старте.** Перепутанная не ломается сразу: соединение виснет
  до таймаута, и выглядит это как «рассылка не работает» — ночью, на первом письме.
- **Потолок писем в минуту у каждого канала свой.** У корпоративного сервера пределы не те, что у
  транзакционного провайдера, и общий счётчик душил бы один канал ради другого. Не задан — берётся
  `MAIL_MAX_PER_MINUTE`.
- **Незаданный канал — не ошибка.** Письма этого канала ждут настройки (worker откладывает их на 15
  минут и пишет об этом в лог), остальные каналы работают. Отладочная отправка такой канал показывает,
  но выбрать его не даёт: письмо легло бы в очередь, а человек считал бы, что проверил отправку.
- **DNS домена нового ящика — обязательная часть настройки.** SPF должен разрешать его сервер, DKIM —
  подписывать письма, DMARC — не отбивать их. Без этого письма уходят, но осядут в спаме.
- **Порядок выката: миграции раньше воркера.** Воркер, в отличие от API, стартовой сверки миграций
  не делает, а запрос письма называет колонки `reply_to` и `account`. Выкаченный раньше миграции, он
  получал бы `column … does not exist` на каждом письме и уводил бы письма в `dead`.

Проверяется канал отладочной отправкой: «Администрирование → Рассылки → Отладка», поле «Канал
отправки». Письмо уйдёт с отправителя канала и через его сервер — это единственный способ убедиться
в настройке до первого настоящего события.
