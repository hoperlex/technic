...
Рабочий план, не ADR. Решения, принятые при постановке, помечены **Р** — их обоснование переедет
в ADR 0051 вместе с реализацией.
...
**Р10. Удаление склада — деактивация (`is_active`),** как у объектов и отделов: на склад сошлются
заявки, и строку придётся сохранить. Хард-удаления нет.
...
| `DELETE /warehouses/:id` | `directories.write` | деактивация (Р10)                                           |
...
Аудит — `warehouse.create` / `warehouse.update` / `warehouse.deactivate`, как у отделов.
