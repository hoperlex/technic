...
function footer(label = 'Инструкция • оргтехника и заявки • 24.08.2026') {
  return (
...
function page1() {
  ...
  const cards = [
    ...
    },
    {
      ...
      soft: C.purpleSoft,
      title: 'Расходники',
      body: 'Коды, цвета, совместимые модели, наличие и неизменяемый журнал каждого движения остатка.',
      tag: 'СКЛАД',
    },
    {
      n: '3',
      ...
      soft: C.orangeSoft,
      title: 'Новый цикл',
      body: 'Поимённые исполнители, виза ИТ по смете, заморозка, очереди решений и автозакрытие.',
      tag: 'ЗАЯВКИ',
    },
  ];
  cards.forEach((card, i) => {
    ...
    b += text(x + 26, 943, i === 0 ? 'Справочники → Оргтехника' : i === 1 ? 'Картриджи и тонеры' : 'Обслуживание → Заявки', 14, {
      fill: card.color,
      weight: 600,
    });
  });
  ...
  b += text(55, 1435, 'Редакция: 24 августа 2026', 15, { fill: C.faint });
  b += text(1068, 1435, '15 страниц', 15, { fill: C.faint, anchor: 'end' });
  b += footer('Пользовательская инструкция • 24.08.2026');
  return document(b);
}
...
function page2() {
  let b = header(
    ...
    'Что изменилось',
    '1. Карта обновления',
    'Три изменения связаны: модель аппарата связывает карточку техники с подходящим расходником, а заявка описывает работу с этой техникой.',
  );
  ...
  const blocks = [
    ...
    },
    {
      ...
      soft: C.orangeSoft,
      points: [
        'Единый путь: Новая → Назначена → В работе → Решена → Закрыта; смета — ветка ремонта.',
  ...
  for (const block of blocks) {
    b += rect(55, block.y, 1013, 315, { fill: '#fff', stroke: C.line, r: 16, shadow: true });
    b += rect(55, block.y, 12, 315, { fill: block.color, r: 6 });
...
function page7() {
  let b = header(
    ...
    'Список отвечает на три вопроса: какой код заказать, к каким аппаратам подходит позиция и сколько единиц сейчас есть на складе.',
  );
  ...
  const widths = [130, 250, 100, 90, 100, 100, 100, 80];
  b += tableHeader(82, 402, widths, ['Код', 'Наименование', 'Цвет', 'Наличие', 'Модели', 'В парке', 'Активность', '']);
  b += tableRow(82, 444, widths, ['Д0000337741', 'Тонер Ricoh 201 (шт)', '—', '12', '3', '68', 'Активен', '№  ✎'], { fill: '#fcfdff', weights: [500, 600, 400, 700] });
  b += tableRow(82, 504, widths, ['Б0000014256', 'Картридж Ricoh Type 1270D', 'чёрный', '0', '2', '71', 'Активен', '№  ✎'], { colors: [C.text, C.text, C.text, C.red] });
  b += tableRow(82, 564, widths, ['Д0000337810', 'Тонер Kyocera TK-3160', '—', '4', '1', '12', 'Активен', '№  ✎'], { fill: '#fcfdff' });
  b += tableRow(82, 624, widths, ['Д0000341142', 'Тонер Ricoh MP C2503', 'голубой', '2', '1', '3', 'Активен', '№  ✎']);
  b += tableRow(82, 684, widths, ['Д0000341143', 'Тонер Ricoh MP C2503', 'жёлтый', '0', '1', '3', 'Погашен', '№  ✎'], { fill: C.graySoft, colors: [C.text, C.text, C.text, C.red, C.text, C.text, C.faint] });
  b += paragraph(82, 781, 'Остаток правится отдельной кнопкой «№» — у каждого изменения спрашивается причина. «В парке» считается в вашей области и не определяет совместимость.', 925, 13, { fill: C.muted, lineHeight: 19 });
  b += button(820, 845, 202, 'Добавить расходник', { primary: true, compact: true, icon: '+' });

  b += text(55, 980, 'Как пользоваться фильтрами', 24, { fill: C.ink, weight: 700 });
  b += callout(55, 1020, 315, 200, '1', 'Поиск', 'Введите часть наименования или код номенклатуры. Например: «Pantum» или «Д0000337741».', { fill: C.bluePale, stroke: '#bae0ff' });
  b += callout(404, 1020, 315, 200, '2', 'Модель', 'Выберите модель аппарата, чтобы увидеть только совместимые позиции — по настроенным связям.', { fill: C.purpleSoft, stroke: '#d3adf7', dotFill: C.purple });
  b += callout(753, 1020, 315, 200, '3', 'Нет в наличии', 'Покажет нулевые остатки. Отсортируйте по «Наличию», чтобы подготовить заказ поставщику.', { fill: C.redSoft, stroke: '#ffccc7', dotFill: C.red });
  ...
  b += paragraph(82, 1337, 'Чтение доступно вместе со справочником оргтехники. «Добавить/Редактировать» требует ведения номенклатуры; «Изменить остаток» — отдельного складского права. У кладовщика может не быть права менять код и совместимость.', 940, 15, { fill: C.text, lineHeight: 22 });
  b += footer();
  return document(b);
}
...
function page8() {
  let b = header(
    ...
    'Создавайте отдельную позицию на каждый код и цвет. Остаток не задаётся при создании — первое число тоже проводится через журнал с причиной.',
  );
  ...
  b += text(82, 1040, 'Журнал остатка', 18, { fill: C.ink, weight: 700 });
  b += rect(82, 1062, 540, 95, { fill: '#fff', stroke: C.line, r: 8 });
  b += text(103, 1095, 'Остаток ещё не меняли', 14, { fill: C.faint });
  ...
  b += callout(715, 840, 353, 182, '4', 'Гасите, не стирайте', 'Погашенная позиция не предлагается для новых операций, но остаток, совместимость и журнал сохраняются.', { fill: C.orangeSoft, stroke: '#ffd591', dotFill: C.orange });
  b += callout(715, 1045, 353, 180, '5', 'Удаляйте только пустую историю', 'Если остаток уже менялся, запись удалить нельзя: исправление проводится новым событием, а не очисткой журнала.', { fill: C.redSoft, stroke: '#ffccc7', dotFill: C.red });
  ...
  b += paragraph(82, 1348, 'Нажмите «Изменить остаток», укажите фактическое количество и причину «поступление по счёту …» или «начальный пересчёт». Только после этого наличие станет ненулевым.', 940, 15, { fill: C.text, lineHeight: 22 });
  b += footer();
  return document(b);
}
...
function page9() {
  let b = header(
    ...
    '8. Складской журнал',
    'Остаток — не редактируемое поле, а последовательность событий. Поэтому портал всегда спрашивает новое значение и объяснение изменения.',
  );
  ...
  const steps = [
    ...
    ['5', 'Сохраните', 'Одинаковое число не создаёт пустое событие; реальная правка попадает в журнал.'],
  ];
  ...
  b += text(55, 930, 'Журнал остатка', 24, { fill: C.ink, weight: 700 });
  b += rect(55, 968, 1013, 355, { fill: '#fff', stroke: C.line, r: 14, shadow: true });
  b += tableHeader(82, 997, [165, 145, 370, 225], ['Дата и время', 'Было → стало', 'Причина', 'Автор / источник']);
  b += tableRow(82, 1039, [165, 145, 370, 225], ['24.08.2026 11:42', '12 → 18', 'Поступление по счёту 1245', 'Иванов И. И.'], { fill: '#fcfdff', weights: [400, 700] });
  b += tableRow(82, 1097, [165, 145, 370, 225], ['24.08.2026 15:10', '18 → 17', 'Выдано на АЛ13', 'Петрова А. С.']);
  b += tableRow(82, 1155, [165, 145, 370, 225], ['25.08.2026 09:02', '17 → 19', 'Исправлен пересчёт, найдено 2 шт.', 'Петрова А. С.'], { fill: '#fcfdff' });
  b += paragraph(82, 1263, 'Строки журнала не редактируют и не удаляют. Ошибку исправляют следующим движением с новой причиной — история остаётся честной.', 905, 13, { fill: C.muted, lineHeight: 19 });

  b += rect(55, 1360, 1013, 68, { fill: C.redSoft, stroke: '#ffccc7', r: 10 });
  b += text(82, 1403, 'Нельзя:', 15, { fill: C.red, weight: 700 });
  b += text(155, 1403, 'затирать чужую правку, вводить отрицательный остаток или оставлять изменение без причины.', 15, { fill: C.text });
  b += footer();
  return document(b);
}
...
function page11() {
  let b = header(
    ...
    'Ремонт и будущая заявка на расходники используют один язык. «Диагностика» слилась с «В работе», а входная «Согласована ИТ» больше не является шагом нового цикла.',
  );
  ...
  b += text(700, 536, 'необязательная ветка ремонта', 12, { fill: C.gold, weight: 600 });
  ...
  const desc = [
    ...
    ['В работе', 'Исполнитель делает работу; для ремонта может подготовить и предъявить смету.', C.orange],
...
function page12() {
  ...
  const roles = [
    ...
    },
    {
      ...
      actions: ['принимает назначенную компании заявку', 'ведёт смету, комментарий и результат', 'подшивает закрывающий документ', 'не принимает собственную работу за заказчика'],
    },
  ];
  roles.forEach((role) => {
...
function page13() {
  let b = header(
    ...
    '12. Ветка ремонта',
    'Смета — необязательная ветка ремонта. Согласование привязано к ревизии: новое предъявление делает старые подписи неактуальными, не стирая историю.',
  );
  ...
  b += text(130, 735, 'Внешний сервис + ремонт', 19, { fill: C.red, weight: 700 });
  ...
  b += bullet(605, 850, 'ремонт выполняет свой сотрудник', 390, { color: C.green, size: 14, lineHeight: 20 });
...
function page14() {
  ...
  b += callout(826, 888, 242, 190, '4', 'Документы', 'Сервисные ремонты, где до предъявления работы ещё нужен акт, счёт или талон.', { fill: C.orangeSoft, stroke: '#ffd591', dotFill: C.orange });
...
function page15() {
  let b = header(
    ...
    '14. Граница готовности',
    'Инструкция описывает целевой пользовательский поток. Перед публикацией пользователям сверьте две завершаемые волны с фактической сборкой.',
  );
  ...
  const ready = [
    ...
    'картриджи и тонеры, совместимость, остаток и журнал;',
  ...
  ready.forEach((item, i) => {
    const x = i < 3 ? 82 : 590;
    const y = i < 3 ? 342 + i * 55 : 342 + (i - 3) * 76;
    b += bullet(x, y, item, i < 3 ? 470 : 430, { color: C.green, size: 14, lineHeight: 20, checkmark: true });
  });
  ...
  b += callout(82, 670, 450, 160, '1', 'Заявка вида «Расходники»', 'Контракты и серверная модель подготовлены, но в текущей веб-форме ещё нет выбора вида и строк номенклатуры.', { fill: '#fff', stroke: '#ffd591', dotFill: C.orange });
  b += callout(558, 670, 482, 160, '2', 'Автоматическое списание', 'Не включайте в пользовательский сценарий, пока форма выдачи и списание по заявке не появились и не прошли приёмку.', { fill: '#fff', stroke: '#ffd591', dotFill: C.orange });
  ...
  b += footer('Инструкция • целевой пользовательский поток • 24.08.2026');
  return document(b);
}
...
const fixedDate = new Date('2026-08-24T12:00:00+05:00');
