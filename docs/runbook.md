...
## Первичное наполнение справочника водителей

Разовая операция, не шаг деплоя. Кадровая выгрузка приезжает JSON-файлом и в git не попадает:
ФИО, дата рождения и СНИЛС живых людей в репозитории не хранятся (ADR 0037 п. 12–13). Файл
кладут рядом с `prod.env` и монтируют в контейнер только на время запуска.
...
# 2. Разбор и отчёт без записи: СНИЛС с контрольной суммой, даты, категории по справочнику
...
# 3. Завести (повторный запуск ничего не дублирует — ключ человека СНИЛС)
docker compose -f deploy/docker-compose.yml -p technic --profile tools run --rm seed-drivers

# 4. Убрать персональные данные с диска VPS
sudo shred -u /etc/technic-portal/drivers.json
```

Скрипт заводит человека, специализацию «Водитель», трудовое отношение и удостоверение с
