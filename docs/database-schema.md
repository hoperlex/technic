...
- **vehicle_types** — иерархический справочник типов/подтипов ТС в одной таблице: `kind_id` (FK, restrict), `parent_id` (self), `code` (unique), `name`, `description`, `is_selectable`, `sort_order`, `is_active`. Составной FK `(parent_id, kind_id)` держит родителя и дочерний тип в одном виде; глубина 2 уровня и `is_selectable` — на уровне приложения. Наполнение (9 родителей + 22 подтипа) — миграция `0010`; активность рассчитана по работавшей технике. Обоснование — `docs/adr/0001-vehicle-classification.md`, `docs/adr/0002-vehicle-classification-population.md`.
