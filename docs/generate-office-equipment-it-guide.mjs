...
function page2() {
  ...
  b += bullet(
    582,
    1392,
    'Смена и добавление исполнителя статус не меняют: работающая заявка остаётся «В работе» (ADR 0187).',
    460,
...
function page3() {
  ...
  b += note(
    ...
    'Отказ из «В работе» не открыт. Есть «Вернуть в «Новую»» — это откат моего же «принял в работу»: причины он не спрашивает, исполнителей с заявки не снимает (я остаюсь в списке), и после него отказ снова доступен. Если заявку передали мне уже работающей, статус так и останется «В работе» (ADR 0187): вернуть её в «Новую» может только тот, кто её ведёт, — попросите его.',
...
function page4() {
  let b = header(
    ...
    '3. Деньги и бумаги',
    'Объём работ — не статус и бывает только под сервисной компанией: его предъявляют, оставаясь «В работе». Документы подшивают по видам, и один из них решает, закроются ли работы сервисной компании.',
  );
  ...
  b += paragraph(
    630,
    854,
    'Объёма работ нет вовсе у вида «Расходники» и у ремонта, который веду я сам: согласовывать там нечего и не у кого, а заявка закрывается датой выполнения и необязательным «Что сделали».',
    438,
    14,
    { fill: C.muted, lineHeight: 21 },
  );
