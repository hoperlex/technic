...
function page1() {
  ...
  b += tag(55, 70, 'КРАТКАЯ ИНСТРУКЦИЯ · 10 СТРАНИЦ', {
    fill: C.blue,
    color: '#fff',
    size: 14,
    pad: 16,
...
function page9() {
  let b = header(
    9,
    '8. Ключи и правила',
    'Чтобы портал узнавал аппарат сам',
    'Само письмо находит карточку только по серийному номеру. Всё остальное работает через ключ опознания, который человек заводит один раз на аппарат.',
  );

  b += text(55, 240, 'Три двери, и все ведут к одному', 22, { fill: C.ink, weight: 700 });
  const doors = [
    {
      color: C.blue,
      fill: C.bluePale,
      title: 'Из карточки',
      body: 'Блок «Ключи опознания» → «Добавить ключ». Письма ждать не нужно: накопленные он подберёт сразу.',
    },
    {
      color: C.cyan,
      fill: C.cyanSoft,
      title: 'Из очереди',
      body: 'Кнопка «Привязать» у письма: тот же ключ, только значение подставлено из самого письма.',
    },
    {
      color: C.green,
      fill: C.greenSoft,
      title: 'Файлом на весь парк',
      body: 'Обмен справочником «Ключи опознания аппаратов»: род, значение, инвентарный номер, примечание.',
    },
  ];
  doors.forEach((door, i) => {
    const x = 55 + i * 336;
    b += rect(x, 275, 310, 215, { fill: door.fill, stroke: `${door.color}55`, r: 16 });
    b += numberDot(i + 1, x + 38, 318, door.color, 18);
    b += text(x + 70, 325, door.title, 19, { fill: C.ink, weight: 700 });
    b += paragraph(x + 26, 372, door.body, 258, 14, { fill: C.text, lineHeight: 21, maxLines: 6 });
  });

  b += rect(55, 520, 1013, 120, { fill: C.orangeSoft, stroke: '#ffd591', r: 16 });
  b += text(87, 563, 'Снятие ключа не отменяет записанное', 19, { fill: C.orange, weight: 700 });
  b += paragraph(
    87,
    598,
    'Новые письма по снятому ключу опознаваться не будут, а показания, которые уже легли в карточку, останутся: убрать их отдельным действием портал пока не умеет.',
    950,
    15,
    { fill: C.text, lineHeight: 22, maxLines: 2 },
  );

  b += text(55, 700, 'Правила разбора: если аппарат подписывает поля по-своему', 22, {
    fill: C.ink,
    weight: 700,
  });
  b += paragraph(
    55,
    740,
    'Портал читает письма встроенными метками. Аппарат, который пишет иначе, — не повод ждать выката: правило заводят в портале, и оно старше метки.',
    990,
    16,
    { fill: C.muted, lineHeight: 23, maxLines: 2 },
  );

  const fields = tableMock(
    55,
    790,
    [
      { title: 'Что заполняете', w: 250 },
      { title: 'Смысл', w: 763 },
    ],
    [
      [
        'Что достаём',
        'Ключ опознания (серийник, имя, сетевое имя) или показание (счётчик, остаток)',
      ],
      ['Метка или выражение', 'Как поле подписано в письме; выражение — когда метки нет вовсе'],
      ['Где искать', 'Тема, текст, таблицы письма, вложения — или везде'],
      ['Когда', 'Ограничения по профилю, отправителю и теме: правило пишут под конкретный формат'],
      ['Порядок', 'Первым срабатывает меньший номер'],
    ],
    { rowHeight: 52, wrapCells: true },
  );
  b += fields.svg;

  const y = 790 + fields.height + 40;
  b += rect(55, y, 496, 190, { fill: '#fff', stroke: '#b7eb8f', r: 16, shadow: true });
  b += circle(107, y + 52, 24, { fill: C.green });
  b += check(94, y + 54, '#fff', 4.5);
  b += text(148, y + 58, 'Проверьте на письме', 19, { fill: C.ink, weight: 700 });
  b += paragraph(
    87,
    y + 100,
    'Кнопка «Проверить» показывает, что именно вынулось и какую карточку это опознало бы. Ничего не записывает — нажимать можно не думая.',
    432,
    14,
    { fill: C.text, lineHeight: 21, maxLines: 4 },
  );

  b += rect(572, y, 496, 190, { fill: C.graySoft, stroke: C.line, r: 16 });
  b += text(604, y + 48, 'Ненужное правило выключают', 19, { fill: C.ink, weight: 700 });
  b += paragraph(
    604,
    y + 88,
    'По выключенному видно, почему письма читались именно так. Удалить совсем можно только правило, при жизни которого писем ещё не разбирали.',
    432,
    14,
    { fill: C.text, lineHeight: 21, maxLines: 4 },
  );
  b += footer('Портал «Техник» • ключи и правила • 18.09.2026');
  return document(b);
}
