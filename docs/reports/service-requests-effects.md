...
**Состояние на портале: закрыто 10.08.2026.** Было: все места мутаций модуля
(`features/assign-service`, `features/estimate-editor`, `features/estimate-approval`,
`features/service-complete`, `features/service-accept`, `pages/service/*`) гасили ровно один
ключ — `serviceRequestKeys.root`, и открытая рядом карточка единицы показывала вчерашнюю историю
ремонтов до перезагрузки страницы. Теперь рядом гасится `officeEquipmentKeys.root`; мест оказалось
12, а не 11 — в `ArchiveTab` их два. Причина записана в докблоке `serviceRequestKeys`: там её
увидит автор следующей мутации, не разыскивая карточку оргтехники.
