...
**Номера взяты по факту: `0197` (M8), `0198` (M9), `0199` (M10).** Ориентир, стоявший здесь при
написании плана (`0185`–`0187`), устарел к 24.08.2026 весь: `0185` занял поток вывоза, `0186` —
наша же M12 выпуска 3, `0187` — автозапчасти. Правило «номер берётся свободным на момент создания
и сверяется прямо перед ним» (§13) отработало и тут.
...
**M11 — не понадобилась.** Запись выпуска заводят ради читателя журнала обновлений, а выпуск 2
для него не меняет ничего: мёртвых статусов он не видел, визу ставит тем же окном, приёмку — той
же кнопкой. Сказать было бы нечего, а пустая строка в ленте выпусков хуже её отсутствия. Раз все
три выпуска уехали одним накатом, работа описана записью выпуска 48 (`0193`).
...
**Решение заказчика 24.08.2026: все три выпуска катятся ОДНИМ накатом.** Основание — в боевой базе
по модулю одна тестовая заявка, переводить нечего, а окно выката проходит в согласованное время,
когда модулем никто не пользуется.

Риск назван и принят: пока сервисы не перезапущены, старый код работает на contract-схеме и на визе
ИТ, на приёмке и на попытке завести `it_approved` получит отказ базы. Плата — **невозможность
отката кода**: на схеме выпуска 2 старый код не работает вовсе.

Условие §11.3 при этом выполнено ПО ПОСТРОЕНИЮ, а не по наблюдению, и это проверено машинно:
целевых статусов у шести коридоров восемь, мёртвых среди них нет; все упоминания `it_approved` и
`diagnostics` в коде — чтение. Неделя наблюдения, которую требовал план, добавила бы к этому знанию
ноль. **Отдельно найдено и исправлено**: портал в двух местах ОТПРАВЛЯЛ мёртвые статусы
(`prompts.decline` слал `it_approved`, `rejectEstimate` — `diagnostics`), и первое было не legacy,
а дефектом выпуска 1 — коридор отката из «Назначена» ведёт в «Новую», и сервер такой переход не
принимал уже сейчас.
