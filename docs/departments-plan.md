...
**Отличия от плана по факту работы.** Перечень типов заявки переехал в `contracts/enums.ts`:
коридор ролей задан по типу, а `vehicle-requests.ts` сам импортирует `permissions.ts` — обратная
ссылка замкнула бы круг. Вкладка «На объекте» скрывается у ролей, которым недоступна спецтехника:
иначе она показывала бы пустой список всегда. Фильтра по отделу в срезе «На объекте» нет по той
же причине.
