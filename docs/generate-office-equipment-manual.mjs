...
function page6() {
  ...
  b += callout(715, 634, 353, 190, '3', 'Описание — словами', 'Одно поле у обоих видов. Расходники заявки заявитель не подбирает: состав заполнит исполнитель, которому везти.', { fill: C.greenSoft, stroke: '#b7eb8f', dotFill: C.green });
...
function page7() {
  ...
  b += paragraph(82, 1296, 'Заявитель говорит словами: «закончился чёрный тонер, печатать нечем». Позиции справочника подбирает тот, кому везти, — действием «Заполнить расходники» в карточке. Заказчик состав видит и не правит.', 950, 15, { fill: C.text, lineHeight: 22 });
...
function page10() {
  ...
  b += paragraph(82, 1370, 'У расходников его нет: картридж берут со своего склада, согласовывать нечего и не у кого, а вместо него — «Заполнить расходники». Нет его и у ремонта, который ведёт свой сотрудник: такую заявку закрывают датой выполнения и необязательным «Что сделали», стоимость внутренних работ не фиксируется.', 950, 14, { fill: C.muted, lineHeight: 21 });
  b += footer();
  return document(b);
}
