 ...
 */
const ALLOWED = {
  // Очередь «Письма устройств» (план `docs/office-equipment-mail-telemetry-plan.md`, §10): тост
  // здесь про ИСХОД действия, а не про незаполненное поле. Сервер ответил, что часть писем пачки
  // применить было нечем (снимка нет или он нечитаем), — пометить этим полем нечего, а молчать
  // нельзя: человек иначе уверен, что очередь разобрана целиком.
  'features/device-mail-review/model/actions.ts':
    'исход пачечной привязки: часть писем осталась без применения, поля у этого нет',
  // Реестр ключей (план `docs/office-equipment-mail-identity-ui-plan.md`, §6.1): тот же исход и по
  // той же причине — ключ заведён, а часть накопленных писем применить было нечем.
  'features/device-mail-identities/model/actions.ts':
    'исход применения ключа: часть писем осталась без применения, поля у этого нет',
  // Опрос аппарата по сети (решение `docs/adr/0205-device-network-poll.md`): формы здесь нет вовсе
  // — одна кнопка «Получить данные», — а тост пересказывает ИСХОД опроса словами сервера. Молчание
  // сети, чужой серийник, отсутствие счётчика и незнакомая единица — законные исходы (пп. 2, 4—6), а
  // не отказ по полю: помечать нечего, а молчать нельзя — нажавший иначе решит, что показание
   ...
   * и оба говорят про ДЕЙСТВИЕ, а не про поле.
   *
   * Очередь проверки: 409 означает «пока вы читали строку, решение принял коллега» — обычный исход
   * одновременной работы, а не отказ по заполненному. Помечать нечем: все шесть полей верны, а
   * лечится это перезапросом карточки, который тост и сопровождает.
   *
   * Отказ заведения: сервер нашёл аппарат в справочнике по названному номеру. Когда карточка в
   * области человека, портал сам подставляет её в поле и говорит об этом словами — пометка «здесь
   * ошибка» на поле, которое портал только что заполнил ЗА человека, была бы прямой неправдой.
   * Когда карточка вне его области, исправить в форме нечего вовсе: аппарат существует, но
   * числится за чужим подразделением, и ответ — не «поправьте», а «напишите в ИТ-службу».
   */
  'features/candidate-review/model/decisions.ts':
    'очередь проверки: 409 — решение принял другой проверяющий, поля тут ни при чём',
  'pages/service/serviceRequestFailure.ts':
    'дубль в справочнике: единицу подставили за человека либо она вне его области',
