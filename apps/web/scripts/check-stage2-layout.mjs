 ...
 *
 * Краснеет не тогда, когда опустел инвентарь выше, а когда пройдена СВОЯ волна — перевод ключей на
 * фабрики слайсов. Прежде эти два события были связаны одним условием, и снос последнего
 * legacy-файла разом объявил бы нарушением 45 файлов, ключи которых никто и не обещал тронуть в
 * той работе: разрез API и сведение ключей — разные волны, и вторая меняет ячейки кэша, то есть
 * видна человеку на экране.
 *
 * Признак сделан явным, а не выводимым: когда волна ключей пройдёт, здесь ставят `true`, и с этого
 * дня сырой ключ в экране роняет прогон. Пока флаг `false`, остаток виден заметкой каждый прогон —
 * чтобы о нём не забыли.
 */
const KEYS_WAVE_DONE = false;
...
if (stringKeyOwners.length > 0) {
  const done = existsSync(entitiesDir) && KEYS_WAVE_DONE;
