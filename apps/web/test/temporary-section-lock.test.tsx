 ...
 *
 * Пока модуль «Орг.техника» дорабатывают, раздел не показывается никому, кроме администратора,
 * держателей надстроек оргтехники («Оргтехника: ведение» и «Оргтехника: ИТ-служба») и сервисной
 * компании, которая по этим заявкам и работает. Права ролей
...
/**
 * Сервисная компания: роль `operator` плюс тип контрагента `service` — профиль «Сервисный центр»,
 * который наборами не выдаётся вовсе. Раздел у неё единственный, и заплатка его не гасит.
 */
const service = () =>
  authUser({ id: 'user-service', role: 'operator', counterpartyType: 'service' });
...
describe('«Орг.техника» скрыта до запуска у всех, кроме своих', () => {
  ...
  it('сервисная компания видит раздел: заплатка его не отнимает у исполнителя', async () => {
    renderPortal(service(), '/office-equipment');
    // Раздел у подрядчика единственный: спрятав его, заплатка оставила бы учётку без портала
    // («Разделы портала вам пока не назначены») — а заявки исполняет он.
    expect(await screen.findByText(sectionPage('Орг.техника'))).toBeDefined();
    expect(screen.getByText('Орг.техника')).toBeDefined();
  });
