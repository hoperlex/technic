...
describe('нижняя навигация на мобильном повторяет права роли', () => {
  it('администратор — все разделы', () => {
    expect(mobileNavLabels('admin')).toEqual([
      'Вывоз мусора',
      'Заказ ТС',
      // Механизация (ADR 0152) — четвёртый модуль заявок, и в меню он встаёт рядом с
      // остальными заявками, а не в хвосте: стартовым разделом он последний, но это другой порядок.
      'Механизация',
      'Путевые листы',
  ...
  /**
   * «Орг.техника» у менеджера и диспетчера есть по правам, хотя модуля заявок на обслуживание у них
   * нет: с появлением вкладки «Техника» раздел открывают два права (план
   ...
   *
   * ⚠️ В меню его сейчас нет: пока модуль дорабатывают, раздел скрыт временной заплаткой
   * (`src/auth/temporarySectionLock.ts`) у всех, кроме администратора и держателей надстроек
   * оргтехники. На запуске заплатку снимают, и строка `'Орг.техника'` возвращается во все списки
   * ниже — прав ролей она не трогала.
   */
  it('менеджер — без администрирования', () => {
    expect(mobileNavLabels('manager')).toEqual([
      ...
      'Путевые листы',
      'Гараж',
      // ⚠️ «Орг.техника» скрыта до запуска временной заплаткой
      // (`src/auth/temporarySectionLock.ts`): строку возвращают вместе с её снятием.
      'Справочники',
    ]);
  });
  ...
  it('штаб — три модуля заявок, без справочников', () => {
    expect(mobileNavLabels('shtab')).toEqual([
      'Вывоз мусора',
      'Заказ ТС',
      'Механизация',
      // ⚠️ «Орг.техника» скрыта до запуска временной заплаткой
  ...
  it('руководитель строительства — те же модули, что у штаба (ADR 0031)', () => {
    expect(mobileNavLabels('rukstroy')).toEqual([
      'Вывоз мусора',
      'Заказ ТС',
      'Механизация',
      // ⚠️ «Орг.техника» скрыта до запуска временной заплаткой
  ...
  // Комендант ведёт вывоз мусора и механизацию: заказ автотехники не его модуль, а виброплиту с
  // генератором на площадке заказывает он же. Спрошено у заказчика прямо (01.09.2026) — ответ
  // «пусть будет доступна как для заявителя»: механизация открыта ему как заказчику, то есть
  // заводить и править свои заявки может, а ход их решают те, кто их исполняет (план, Р9).
  it('комендант — вывоз мусора и механизация: заказ ТС не его модуль', () => {
    expect(mobileNavLabels('commandant')).toEqual(['Вывоз мусора', 'Механизация']);
  });
  ...
  it('наблюдатель — все модули заявок, смотреть их можно и с телефона (ADR 0033)', () => {
    expect(mobileNavLabels('observer')).toEqual([
      'Вывоз мусора',
      'Заказ ТС',
      'Механизация',
      // ⚠️ «Орг.техника» скрыта до запуска временной заплаткой
