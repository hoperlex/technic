...
/**
 * Тот же недельный лист, но сокращённый: заказ закрыли фактической датой, и портал отнял у бланка
 * дни с конца (Р12 плана «закрытие фактической датой»).
 *
 * Номер прежний, статус прежний, замены нет вовсе — «не такой, каким выдан» видно только по следу
 * правки. Признак `isCorrection` поднимает сервер третьим источником (`period_trimmed_at`), и в
 * фикстуре он стоит именно так: операции коррекции у сокращённого листа нет, а метка есть.
 */
const TRIMMED: WaybillDto = {
  ...WEEK,
  id: 'w-3',
  number: 'ЭСМ-00000005',
  // Действует по среду, а в графе «Период работы» напечатано воскресенье: расхождение бумаги и
  // портала намеренное (Р13), и подсказка обязана назвать обе даты.
  periodTo: '2026-09-02',
  periodToOriginal: '2026-09-06',
  trimmedAt: '2026-09-03T07:00:00.000Z',
  trimReason: 'машина ушла с объекта раньше',
  isCorrection: true,
  files: [],
};
...
describe('журнал путевых листов: три бланка в одной таблице', () => {
  ...
  /*
   * Сокращение периода (Р12, Р13). Отдельной метки у него нет намеренно — решение заказчика: в
   * журнале стоит та же «коррекция», потому что бухгалтерии важно не то, каким механизмом бланк
   * привели к нынешнему виду, а то, что он не такой, каким выдан.
   *
   * Значит, весь смысл переносит подсказка, и проверяется она текстом целиком: метка без объяснения
   * у листа, который не аннулирован и никем не заменён, читается как ошибка портала.
   */
  it('сокращённый лист помечен коррекцией, а подсказка называет обе даты и причину', async () => {
    renderJournal([WEEK, TRIMMED]);
    await waitFor(() => expect(screen.getByText('ЭСМ-00000005')).toBeDefined());

    // Метка стоит у сокращённого и не стоит у соседа по той же неделе: иначе проверка прошла бы и
    // на портале, который метит всё подряд.
    expect(within(rowOf('ЭСМ-00000004')).queryByText('коррекция')).toBeNull();
    const tag = within(rowOf('ЭСМ-00000005')).getByText('коррекция');

    fireEvent.mouseEnter(tag);
    expect(
      await screen.findByText(
        'Период сокращён до 02.09.2026, в бланке напечатано по 06.09.2026. Причина: машина ушла с объекта раньше',
      ),
    ).toBeDefined();
  });

  it('печать сокращённого листа предупреждает, что бланк выйдет не копией лежащего на площадке', async () => {
    renderJournal([WEEK, TRIMMED]);
    await waitFor(() => expect(screen.getByText('ЭСМ-00000005')).toBeDefined());

    // Повторная печать даёт укороченный период: снимок приведён к факту, и второй экземпляр с тем
    // же номером законно отличается от первого (Р13). Сказано это у самой кнопки — там, где
    // человек и решает печатать.
    fireEvent.mouseEnter(
      within(rowOf('ЭСМ-00000005')).getByRole('button', { name: 'Печать бланка' }),
    );
    // Текстом целиком, а не куском: подсказка называет две даты и говорит, где стоит каждая, —
    // перепутанные местами, они утверждают обратное, и проверка по одному обороту этого не поймает.
    expect(
      await screen.findByText(
        'Печать бланка. Период листа сокращён: напечатается нынешний конец — 02.09.2026, а в выданном экземпляре стоит 06.09.2026. Это не копия того листа, что лежит на площадке, а тот же номер с укороченным периодом.',
      ),
    ).toBeDefined();

    // У несокращённого листа подсказка прежняя: объяснение стоит там, где ему есть что объяснять.
    fireEvent.mouseEnter(
      within(rowOf('ЭСМ-00000004')).getByRole('button', { name: 'Печать бланка' }),
    );
    expect(await screen.findByText('Печать бланка')).toBeDefined();
  });
