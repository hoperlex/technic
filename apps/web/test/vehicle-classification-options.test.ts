 ...
 *
 * Ключи у списков разные, и это проверяется здесь же: форма называет пару `тип:категория`, потому
 * что выбирает одну конечную позицию, а фильтр — самодостаточный `t<тип>` / `c<категория>`, потому
 * что набирает несколько позиций сразу и каждая обязана читаться сама по себе.
 */
function position(
...
describe('classificationFilterGroups (фильтр списка)', () => {
  it('у типа с категориями впереди стоит он сам целиком, и значение — ключ фильтра', () => {
    const [special] = classificationFilterGroups([crane25, crane130, drill]);
    expect(special!.options).toEqual([
      { value: 'tcrane', label: 'Автокраны — все категории' },
      { value: 'cc25', label: 'Автокраны, г/п 25 т' },
      { value: 'cc130', label: 'Автокраны, г/п 130 т' },
      // Тип без категорий сам конечная позиция — «весь тип» ему не добавляется.
      { value: 'tdrill', label: 'Ямобур' },
    ]);
  });
  ...
  it('вариант «весь тип» — один на тип, сколько бы категорий у него ни было', () => {
    ...
    expect(special!.options.filter((o) => o.value === 'tcrane')).toHaveLength(1);
  });
