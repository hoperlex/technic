...
/** Исполнитель: права `serviceRequests.assign` у него нет — назначать он не вправе (Р5). */
const EXECUTOR: AuthUser = serviceExecutor();

/**
 * Уже назначенная заявка: и свой сотрудник поимённо, и сервисная компания строкой (Н5).
 *
 * Статус у неё «Новая», а не снятая «Назначена» (Р1): назначение перестало быть переходом, и
 * «назначенность» держит теперь СОСТАВ исполнителей, а не колонка статуса. Фикстура потому и
 * называется по составу — она и есть единственное основание всего, что ниже.
 */
...
/**
 * Что окно обещает при замене — и чего больше не обещает (ADR 0187).
 *
 * Прежде работающей заявке оно говорило «Заявка вернётся в «Новую»», и это было правдой: ручка
 * состава откатывала статус. Откат снят по просьбе администраторов и «Ведения» оргтехники, и
 * предупреждение обязано измениться вместе с ним — обещание хода, которого не будет, читалось бы
 * как поломка портала. Сказано при этом ПРЯМО, что статус останется: человек, помнящий прежнее
 * поведение, иначе пошёл бы поднимать заявку обратно после каждой замены.
 */
describe('предупреждение окна о цене замены', () => {
  async function openAssign(request: ServiceRequestDto): Promise<HTMLElement> {
    renderTab(OPERATOR, [request]);
    await openCard();
    fireEvent.click(assignButton()!);
    return await waitFor(() => wrapOf('Изменить исполнителей'));
  }

  it('у работающей заявки обещает сохранённый статус, а не возврат в «Новую»', async () => {
    const modal = within(await openAssign(assignedRequest({ status: 'in_work' })));

    expect(modal.getByText(/Статус не изменится/)).toBeDefined();
    expect(modal.queryByText(/вернётся в «Новую»/)).toBeNull();
    // Якорь: сама цена замены названа по-прежнему — пропал ровно откат, а не всё предупреждение.
    expect(modal.getByText(/объём работ/i)).toBeDefined();
  });

  it('у «Новой» про статус не сказано ничего: двигать его неоткуда', async () => {
    const modal = within(await openAssign(assignedRequest()));

    expect(modal.queryByText(/Статус не изменится/)).toBeNull();
    expect(modal.getByText(/отсчёт ожидания начнётся заново/)).toBeDefined();
  });
});
