 ...
 *
 * Сеть подменяется на уровне HTTP, а не модулем `@entities/session`: утверждение «запроса не
 * было» о подменённом модуле говорит лишь то, что портал сейчас разложен по таким файлам, а
 * проверять надо контракт подтверждения — тот же, по которому портал разговаривает с сервером на
 * самом деле.
 */
