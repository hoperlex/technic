 ...
 * (`SERVICE_ASSIGNER_TRANSITIONS`) ради ПЕРЕназначения — `PUT /:id/executors` сам возвращает
 * заявку в «Новую» (Р5). А пункт «Вернуть в „Новую“» уходит в `PATCH /:id/status`, за которым
 * стоит страж `serviceRequests.status`, и у профиля «Системный администратор» его нет: прямой
 * опрос сервера отвечал ему 403 «Недостаточно прав для смены статуса», пока портал этот пункт
 * рисовал.
 *
