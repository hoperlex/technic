 ...
 *
 * Первое — вкладка «Дни работ» заказа техники на объект: карточка монтирует её сама, а
 * планирование дня и снятие его с рейса живут внутри `VehicleRequestDays` собственными мутациями.
 * Условие их доступности — `vehicleRequests.status && waybills.read`, то есть **ровно то же**, каким
...
/**
 * Заказ техники на объект в работе на своей машине со сроком — тот, у кого дни и бывают.
 *
 * Здесь стоял ЛИНЕЙНЫЙ заказ, и подпись к нему утверждала: «только у такого и бывает вкладка „Дни
 * работ“». Утверждение снято решением заказчика (ADR 0207 п. 1): дни перестали быть привилегией
 * линейного типа ТС — 4-П за день просят и у машины, которая весь срок стоит на площадке, — и
 * вкладка открыта любому заказу техники на объект.
 *
 * Признак оставлен ВЫКЛЮЧЕННЫМ намеренно, и это не мелочь оформления: заказ, которому вкладку
 * прежде не давали вовсе, и есть доказательство снятого правила. На линейном заказе старое и новое
 * поведение совпадают, и возврат замка такой сценарий проспал бы целиком — все проверки читалки
 * ниже идут теперь на нелинейном заказе.
 */
...
/*
 * Кому вкладка вообще достаётся. Отдельной проверки у этого не было: фикстура была линейной, и
 * вкладка приходила вместе с ней — вопрос «а нелинейному?» имел ответ в правиле, а не в сценарии.
 *
 * После ADR 0207 п. 1 ответ сменился на противоположный, и вопрос стал самостоятельным: вкладку
 * даёт ВИД заявки — заказ техники на объект, — а признак линейности заказанного типа ТС её больше
 * не воротит. Пара значений `isLinear` здесь и есть утверждение: одно значение доказывало бы ту
 * половину правила, которая была верна и до решения.
 */
describe('кому карточка даёт вкладку «Дни работ»', () => {
  it('любому заказу техники на объект: линейность типа ТС её не решает', async () => {
    const { unmount } = renderCard(ON_SITE);
    expect(await screen.findByRole('tab', { name: 'Дни работ' })).toBeDefined();
    unmount();

    renderCard({ ...ON_SITE, isLinear: true });
    expect(await screen.findByRole('tab', { name: 'Дни работ' })).toBeDefined();
  });
});

describe('дни заказа техники на объект в читалке заявки', () => {
  it('показывает, каким рейсом едет какой день, но ни планировать, ни снимать не даёт', async () => {
    renderCard(ON_SITE, { readOnly: true });
    await openDaysTab();
 ...
 */
describe('дни заказа техники на объект заказчику', () => {
  it('вкладку открывает и без права на путевые листы, а планирования в ней не даёт', async () => {
    ...
    renderCard(ON_SITE, { user: rukstroy });
    await openDaysTab();
