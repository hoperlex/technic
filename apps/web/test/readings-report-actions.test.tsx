 ...
 * полем тела отправки, и уходит той же `submit` с теми же числами. С приходом топлива «те же»
 * стало утверждением о пяти числах, а не о трёх (ADR 0163, Н6): оба остатка бака возвращаются
 * обратно как есть, иначе подпись под аномалией одометра оказалась бы правкой чужих литров.
 *
 * **Подпись строки в перестановке смен несёт и топливо** (Н8): строка, где сдали одни литры,
 * больше не называется «числа не заполнены» — иначе две утренние смены не различить, а
 * переставляют их именно по числам.
 *
...
describe('карточка отчёта дня: порядок смен', () => {
  ...
  /**
   * Топливная строка в подписи не пуста (ADR 0163, Н8).
   *
   * Переставляют смены ровно тогда, когда их две и различить их надо по числам, — а `readingLabel`
   * до сих пор подписывал строку одними счётчиками. Утренняя строка, где водитель сдал только
   * литры, называлась при этом «числа не заполнены»: человек видит две одинаково безымянные смены
   * и переставляет их наугад, а перестановка переписывает разности обоих отчётов.
   *
   * Метки в подписи короткие и обязательные: все три числа в литрах, и «46 · 120 · 85,5 л» без
   * «нач./запр./кон.» неразличимо ровно так же, как и пустота.
   */
  it('строку, где сдано только топливо, не называет пустой', async () => {
    const fuelOnly: VehicleReadingJournalDto = {
      ...JOURNAL_DTO,
      items: [
        // Вторая смена дня: счётчиков нет вовсе, литры есть — законное утреннее состояние.
        journalRow({
          itemId: 'item-9',
          shiftOrder: 2,
          reportId: 'r-2',
    ...
    await openOrder({ [JOURNAL]: () => json(fuelOnly) });
    const modal = dialog('Порядок смен — КамАЗ · А111АА777, 14.08.2026');

    // Числа стоят в подписи с короткими метками и в порядке смены — после счётчиков.
    expect(within(modal).getByText('нач. 46 л · запр. 120 л · кон. 85,5 л')).toBeDefined();
    // И «пустой» такая строка больше не называется: сказать так о смене, где литры сданы, значит
    // предложить переставлять смены вслепую.
    expect(within(modal).queryByText('числа не заполнены')).toBeNull();
  });
...
describe('карточка отчёта дня: подтверждение аномалии', () => {
      ...
      items: [
        {
          ...
          reading: {
            ...
            odometerKm: 128400,
            engineHours: 1240.5,
            // Оба остатка бака входят в тело наравне со счётчиками, даже пустые (ADR 0163, Н6):
            // в контракте они необязательны, и пропуск ключа сервер понял бы как «оставить как
            // есть» — тело перестало бы описывать то показание, под которым ставят подпись.
            fuelStartLiters: null,
            fuelFilledLiters: 120,
            fuelEndLiters: null,
            comment: 'сдал вечером',
  ...
  /**
   * Главный караул Н6: подтверждение возвращает ОБА остатка ровно теми же числами.
   *
   * Подтверждение аномалии — скрытая полная перезапись: своей ручки у него нет, и портал шлёт
   * сохранённое показание обратно целиком, подняв признак нужного счётчика. Сервер сравнивает
   * присланное с хранимым и, не найдя разницы, правки не пишет — подпись при этом ставит. Значит
   * любое расхождение в теле превращает подтверждение в **правку чужого числа**: с историей, с
   * требованием причины и с пересчётом цепочки.
   *
   * Двух способов ошибиться ровно два, и оба тихие: потерять ключ (сервер прочтёт «оставить как
   * есть», и тело перестанет описывать подтверждаемое показание) или подставить `null` вместо
   * сохранённого числа (сервер прочтёт «очистить», и остатки исчезнут в момент подписи под
   * аномалией одометра). Поэтому в фикстуре здесь настоящие литры, а тело сверяется целиком.
   */
  it('подтверждение возвращает оба остатка бака теми же числами, а не пустыми', async () => {
    const withFuel = reportDto({
      items: [
        {
          ...ITEM,
          reading: { ...READING, fuelStartLiters: 46, fuelFilledLiters: 120, fuelEndLiters: 85.5 },
        },
      ],
    });
    const http = await openCard({ [REPORT]: () => json(withFuel), [SUBMIT]: () => json(withFuel) });
      ...
      items: [
        {
          ...
          reading: {
            ...
            odometerKm: 128400,
            engineHours: 1240.5,
            // Три числа топлива уходят обратно как есть: подписывают аномалию ОДОМЕТРА, и литры
            // при этом не трогают ни в одну сторону.
            fuelStartLiters: 46,
            fuelFilledLiters: 120,
            fuelEndLiters: 85.5,
            comment: 'сдал вечером',
