...
describe('вход приводит в раздел роли, а не на служебную форму', () => {
  it('оператор сервисной компании попадает в «Орг.техника», а не на смену пароля', async () => {
    ...
    /*
     * ⚠️ Пока модуль дорабатывают, «Орг.техника» скрыта временной заплаткой
     * (`src/auth/temporarySectionLock.ts`) у всех, кроме администратора и держателей надстроек
     * оргтехники, — и у этой учётки раздел единственный, поэтому портал отвечает ей экраном
     * «разделов нет». Проверяемое здесь при этом держится: на форму смены пароля вход её не
     * уводит, а именно это и было тем самым багом.
     *
     * На запуске заплатку снимают, и обе строки возвращаются к прежнему виду:
