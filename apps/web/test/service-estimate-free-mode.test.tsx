...
describe('гарантийный ремонт без оплаты работает как прежде', () => {
  ...
  it('пустой состав + кнопка — предъявление уходит своей ручкой, без записи состава', async () => {
    ...
    // `docs/office-equipment-on-site-and-invoice-estimate-plan.md`): гарантийный ремонт называется
    // `mode`, а не отрицанием соседнего формата. Сверяется тело ЦЕЛИКОМ (`toEqual`, не `объект с
    // лишним`) — именно так ловится забытое старое поле: схема ручки строгая, и тело с обоими
    // признаками сервер отбивает как «формат задан дважды».
