...
describe('новый маршрут: бланк без граф прицепа уносит и сами графы', () => {
  it('смена тягача на легковую очищает подставленный полуприцеп, а не прячет его', async () => {
     ...
     *
     * Сегодняшним числом, а не записанным в код: у прошедшего дня форма спрашивает ещё и причину
     * заднего числа (ADR 0101), и жёсткая дата, побывшая будущей, однажды становится прошлой — тест
     * начинает падать на поле, к прицепам отношения не имеющем, в день, когда его никто не трогал.
     */
    typeDate('Дата рейса', dayjs().format('DD.MM.YYYY'));
