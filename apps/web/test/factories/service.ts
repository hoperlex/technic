...
export function serviceRequest(overrides: Partial<ServiceRequestDto> = {}): ServiceRequestDto {
  ...
  const request: ServiceRequestDto = {
    ...
    audience: 'finance',
    /*
     * Сторона заказчика читателя (Н8): «эта заявка моей площадки или моего отдела». Умолчание —
     * `false`, и выбрано оно по той же мерке, что `audience` выше, но в другую сторону: истина
     * открывала бы правку и удаление КАЖДОЙ фикстуре, включая сценарии про сисадмина на чужой
     * площадке, ради которых поле и заведено. Сценарий «заявка моей области» ставит его явно.
     */
