 ...
 * сервер (`OFFICE_EQUIPMENT_SELECTOR_SEARCH_MIN`), а мок отвечает на набранное так же, как ответил
 * бы на него сервер по своей области, — сценариям выдачи проверять тут нечего.
 *
 * Портал с недавних пор это число ПОКАЗЫВАЕТ — подсказкой под полем и текстом пустого списка, — но
 * по-прежнему не решает им ничего и запроса не режет: обе строки читают ту же константу контрактов.
 * Проверяются они своим тестом (`service-search-hint.test.tsx`), а не через эту фабрику.
 */
