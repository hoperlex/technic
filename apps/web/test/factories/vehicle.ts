...
export function vehicleRequest(
  ...
  return {
    ...
    route: null,
    /*
     * Бумага заявки (`hasActiveWaybill` DTO; ADR 0207, последствие «заказ на объект с выписанным
     * дневным листом не откатывается в „Новую“ без аннулирования»): считается всеми тремя
     * способами попасть в лист — грузовой строкой, днём и перегоном, — и именно им карточка
     * решает, открывать ли окно отката. У заказа на объект своего рейса не бывает, так что
     * `route: null` этого вопроса не закрывает: признаки разные, и писать здесь оба обязательно.
     *
     * Умолчание «нет»: по умолчанию заявка «Новая», бланков ей не выписывали. Сценарии выданной
     * бумаги включают признак сами.
     */
...
export function freightRequest(
  ...
  return {
    ...
    completion: null,
    route: null,
    // Бумага заявки (ADR 0207): у «Новой» грузоперевозки листа нет. Сценарию, которому лист нужен,
    // мало включить `route.hasWaybill` — тот отвечает про заморозку рейса, а этот про саму заявку.
    hasActiveWaybill: false,
    files: [],
