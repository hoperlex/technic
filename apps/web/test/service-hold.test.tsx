...
describe('что можно с отложенной заявкой', () => {
  ...
  it('оператору — только возобновление, перемещение техники и отмена', async () => {
    ...
    /*
     * Ровно три пункта. Правки нет — `on_hold` не входит в список правимых статусов; срочности нет
     * — сервер отвечает на неё 422 (Р119); повтор письма снят целиком (ADR 0162). Перемещение
     * техники тоже ушло из меню списка: аппарат по-прежнему ездит независимо от статуса заявки, но
     * записывают переезд из карточки, у поля «Какой аппарат», — там же, где читают реквизиты.
     *
     ...
     */
    expect(await rowActionLabels()).toEqual(['Возобновить', 'Обсуждение', 'Отменить заявку']);
  });
  ...
  it('администратору к ним добавляется снос в архив — его сервер пускает в любом статусе', async () => {
    ...
    // видел по `serviceRequests.estimate`, а оператор не видел вовсе. Перемещения нет ни у кого:
    // из меню списка оно снято (ADR 0162).
