...
describe('что можно с отложенной заявкой', () => {
  ...
  it('оператору — только возобновление, перемещение техники и отмена', async () => {
    ...
    /*
     * Ровно три пункта. Правки нет — `on_hold` не входит в список правимых статусов; срочности
  ...
  it('администратору к ним добавляется снос в архив — его сервер пускает в любом статусе', async () => {
    ...
    //
    // «Примечание исполнителя» стоит здесь по той же таблице Р110: писать его отложенной разрешено,
    // и у администратора оно есть, потому что право `serviceRequests.estimate` у него есть, — а у
    // оператора выше того же пункта нет ровно потому, что этого права у оператора нет.
    expect(await rowActionLabels()).toEqual([
      'Возобновить',
