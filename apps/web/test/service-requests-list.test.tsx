...
function renderTab(user: AuthUser, items: ServiceRequestDto[], over: RouteMap = {}): HttpMock {
  const http = mockHttp({
    ...
    // Справочники фильтров: объекты и отделы видны обеим сторонам, типы оргтехники — только тому,
    // у кого есть право справочника (сервису он закрыт, Р7).
    'GET /objects': () => json(list([objectDto()])),
    'GET /departments': () => json(emptyList()),
    'GET /counterparties': () => json(emptyList()),
    /*
     * Сам перечень единиц список заявок больше не спрашивает: колонка «Гарантия» питается строкой
     * заявки (Ф3). Маршрут оставлен в моках НАМЕРЕННО — им проверяется, что запроса нет: пропади
     * мок, забытое обращение падало бы «незамоканным запросом», то есть по другой причине и с
     * другим текстом, а счётчик `countOf` показывал бы ноль в обоих случаях.
     */
 ...
 * ручки `PATCH /:id/it-approval` нет, фичи `it-approval` нет, коридора визы нет вовсе (таблицу
 * `SERVICE_IT_TRANSITIONS` сняли вместе с уборкой мёртвого права из наборов), а пункта «Решение ИТ
 * по смете» портал не рисует ни в одном статусе. Проверять там нечего:
...
/**
 * Колонка «Гарантия» в столбце «Техника» (Ф3 плана `docs/office-equipment-candidate-plan.md`).
 *
 * До фикса срок приходил ОТДЕЛЬНЫМ запросом всего справочника — страницей в 500 строк, — и
 * проверять там было нечего: колонка показывала то, что успело поместиться. Проверяемое теперь
 * утверждение другое и состоит из двух половин, которые врозь ничего не значат: срок берётся из
 * САМОЙ СТРОКИ заявки, и справочник ради колонки не спрашивается ВОВСЕ. Останься второй запрос
 * живым — потолок в 500 карточек вернулся бы вместе с ним, и колонка снова замолчала бы у части
 * заявок молча.
 *
 * Три состояния разведены намеренно: тег со сроком, прочерк («срок в карточке не заведён») и
 * отсутствие тега («спрашивать не у чего» — предмета у заявки нет). Слейся два последних — портал
 * отвечал бы «гарантии нет» там, где карточки не существует.
 */
describe('колонка «Гарантия» питается строкой заявки', () => {
  /** Единица со сроком: остальные поля — те же, что у фабрики, разница ровно в гарантии. */
  const equipmentWith = (warrantyUntil: string | null) => ({
    id: 'oe-1',
    name: 'Kyocera M3145',
    serialNumber: 'SN-1',
  ...
  /**
   * Ячейка столбца «Техника» целиком: гарантия рисуется третьей строкой в ней же, и проверять её
   * по всему экрану нельзя — прочерк рисуют и соседние столбцы.
   */
  const equipmentCell = (text: string): HTMLElement => {
    const cell = screen.getAllByText(text)[0]?.closest('td');
    if (!cell) throw new Error(`ячейки «${text}» в таблице нет`);
    return cell as HTMLElement;
  };

  it('срок берётся из выдачи списка, а справочник не спрашивается вовсе', async () => {
    const http = renderTab(OPERATOR, [serviceRequest({ equipment: equipmentWith('2099-12-31') })]);
    await screen.findByText('СО-14');

    expect(equipmentCell('Kyocera M3145').textContent).toContain('до 31.12.2099');
    /*
     * Ноль обращений к справочнику — вторая половина утверждения. Мок маршрута в `renderTab`
     * оставлен намеренно: пропади он, тест падал бы «незамоканным запросом», то есть по другой
     * причине и с другим текстом, а здесь нужно видеть именно счётчик.
     */
    expect(http.countOf('GET /office-equipment')).toBe(0);
  });

  it('срок не заведён — прочерк, а не «гарантии нет»', async () => {
    // `null` внутри блока предмета: карточка есть, талона у неё нет. `WarrantyTag` рисует на это
    // прочерк, и выдумывать за портал отрицательный ответ он не станет.
    renderTab(OPERATOR, [serviceRequest({ equipment: equipmentWith(null) })]);
    await screen.findByText('СО-14');

    const cell = equipmentCell('Kyocera M3145');
    expect(cell.textContent).toContain('—');
    expect(cell.querySelector('.ant-tag')).toBeNull();
  });

  it('у заявки без аппарата гарантии нет вовсе — ни тега, ни прочерка', async () => {
    /*
     * Тот же ответ приходит и на заявку с непроверенным кандидатом (план кандидата, Р2): блока
     * предмета у неё нет, потому что нет карточки, — и «нечего показать» выражено отсутствием
     * блока, а не значением поля.
     */
    renderTab(OPERATOR, [serviceRequest({ equipment: null })]);
    await screen.findByText('СО-14');

    const cell = equipmentCell('Без аппарата');
    expect(cell.querySelector('.ant-tag')).toBeNull();
    expect(cell.textContent).not.toContain('—');
  });

  it('подрядчику колонка молчит — и молчит по решению сервера, а не по недосмотру портала', async () => {
    /*
     * РЕШЕНИЕ ВЛАДЕЛЬЦА 04.09.2026. Живой срок карточки видит тот, кому открыт справочник, а
     * сервисной компании он закрыт намеренно (Р7, `COUNTERPARTY_TYPE_PERMISSIONS.service`):
     * реквизиты нужной единицы приезжают ей снимком заявки, а живая колонка парка — нет.
     * Закрывает поле СЕРВЕР, в единственной сборке блока предмета, — поэтому в строке подрядчика
     * приезжает `null`, и фикстура здесь именно такая.
     *
     * ЧТО ПРОВЕРЯЕТСЯ ИМЕННО ЗДЕСЬ, на портале: получив `null`, он рисует прочерк — и не идёт
     * добирать срок в справочник (нулевой счётчик обращений держит первый случай раздела, общий на
     * всех читателей). Иначе закрытое сервером поле вернулось бы боковой дверью, а с ним и потолок
     * в 500 карточек.
     */
    renderTab(EXECUTOR, [assignedServiceRequest({ equipment: equipmentWith(null) })]);
    await screen.findByText('СО-14');

    expect(equipmentCell('Kyocera M3145').querySelector('.ant-tag')).toBeNull();
    expect(equipmentCell('Kyocera M3145').textContent).toContain('—');
  });
});
