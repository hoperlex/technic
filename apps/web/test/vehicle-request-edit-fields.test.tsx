 ...
 *    редактора, а не заявки, — и сохранение переносило заявку с отдела на площадку. Полей стало
 *    одно на обе оси («Объект/отдел», план `docs/department-requests-plan.md`, Р2), и правило
 *    осталось тем же: показано то, что у заявки, а не то, чем заказывает открывший её;
...
describe('форма правки заявки на ТС: поля заявки, а не роли', () => {
  ...
  it('у грузоперевозки отдела в подборе стоит её отдел, а не ось редактора', async () => {
    renderTab();
    ...
    expect(labels).not.toContain('Отдел');
