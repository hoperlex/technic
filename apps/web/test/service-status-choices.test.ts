...
describe('полнота вверх: коридор не остаётся без пункта (ADR 0161, решение 1)', () => {
   ...
   * исполнителями.
   *
   * Второе — дуга `in_work → new` у субъекта с `assign` без `serviceRequests.status`: в коридор её
   * отдаёт таблица распределения, а пункт возврата уходит в ручку статуса. Оно вычитается прямо в
   * сравнении, рядом с причиной.
   *
  ...
  it('на распределённой заявке пункты покрывают коридор целиком', () => {
    for (let i = 0; i < ACCESS_PROFILES.length; i += 1) {
      ...
      for (const status of SERVICE_REQUEST_STATUSES) {
        ...
        const assignment = serviceExecutorAssignment(request, user);
        /*
         * ВТОРОЕ ЗАКОННОЕ РАСХОЖДЕНИЕ, и оно закреплено случаем ниже. Дугу `in_work → new` отдаёт
         * коридору `SERVICE_ASSIGNER_TRANSITIONS`, то есть право `serviceRequests.assign`, а
         * единственный её пункт — «Вернуть в «Новую»» — уходит в `PATCH /:id/status`. У субъекта с
         * `assign` без `serviceRequests.status` — то есть у профиля «Системный администратор» —
         * пункта поэтому нет, и это не забытый `toStatus`: сервер отвечал ему 403 «Недостаточно
         * прав для смены статуса», пока пункт рисовался (проверено прямым опросом).
         *
         * Переназначение в этом расхождении больше не участвует: статуса оно не меняет (ADR 0187) и
         * `toStatus` у своего пункта не несёт вовсе.
         */
  ...
  it('дуга «В работе» → «Новая» без права статуса остаётся без пункта: за ней ручка «Ведения»', () => {
    ...
    // Коридор дугу отдаёт — её даёт право распределения, хотя ходит по ней только ручной возврат.
    expect(
      allowedServiceStatusTransitions(
        'in_work',
        sysadmin,
