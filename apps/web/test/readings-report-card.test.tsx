 ...
 *
 * **Топлива три числа, и они в порядке смены** (ADR 0163, Р2, Р7): остаток бака на её начало,
 * заправленное за неё, остаток на конец. Форма принимает все три наравне со счётчиками — правило
 * «хотя бы одно из пяти», а не «хотя бы один счётчик», — и подставляет сохранённые литры в поля:
 * показание уходит целиком, и форма, их не знающая, стирала бы оба при всякой правке одометра.
 *
...
/**
 * Показание с полной тройкой топлива (ADR 0163): остаток бака на начало смены, заправленное за
 * неё, остаток на конец. Числа разные и неокруглённые нарочно — по ним видно, какое из трёх встало
 * не под свою подпись.
 */
...
/**
 * Отчёт из одной строки с заданным показанием. Топливные караулы смотрят на одну строку намеренно:
 * подписи счётчиков в карточке повторяются от строки к строке, и «первая попавшаяся такая подпись»
 * назвала бы соседнюю смену.
 */
const oneItem = (reading: VehicleReadingDto): DriverReportDto =>
  reportDto({ items: [{ ...FILLED_ITEM, reading }], discrepancies: [] });
...
/** Подписи чисел показания в карточке: по ним и проверяется порядок группы. */
const COUNTER_LABEL = /^(Одометр|Моточасы|Топливо на начало|Заправлено|Топливо на конец)$/u;

/**
 * Число под подписью: у `Counter` подпись и значение — соседние ячейки одного `Space`. Проверять
 * их парами существенно: «в карточке где-то есть 46 л» прошло бы и при переставленных подписях,
 * а перепутанные «начало» и «конец» читаются как обычная смена, а не как ошибка.
 */
function counterValue(label: string): string {
  const cell = within(card()).getByText(label).closest('.ant-space-item') as HTMLElement;
...
describe('карточка отчёта дня: что показано', () => {
  ...
  /**
   * Три числа топлива вместо одного «Заправлено» (ADR 0163, Р2).
   *
   * Караул держит порядок и парность подписей, а не сам факт трёх чисел. Причина простая: все три
   * в литрах, подписи различаются одним словом, и переставленные «начало» с «концом» дают на
   * экране осмысленную с виду смену — ошибку такого рода глазами не ловят. Порядок один на все
   * экраны и на кабину: по ходу смены.
   */
  it('печатает три числа топлива в порядке смены, каждое под своей подписью', async () => {
    renderGarage(intakeRoute('&report=r-1'), { [REPORT]: () => json(oneItem(FUEL_READING)) });
    expect(await screen.findByText('Отчёт — Петров П. П., 14.08.2026')).toBeDefined();

    // Группа идёт после счётчиков и в порядке смены: уровень бака, поток за смену, уровень бака.
    expect(
      within(card())
        .getAllByText(COUNTER_LABEL)
        .map((node) => node.textContent),
    ).toEqual(['Одометр', 'Моточасы', 'Топливо на начало', 'Заправлено', 'Топливо на конец']);

    expect(counterValue('Топливо на начало')).toBe('46 л');
    expect(counterValue('Заправлено')).toBe('120 л');
    expect(counterValue('Топливо на конец')).toBe('85,5 л');

    // Прироста у топлива нет намеренно (Р1): разность остатков соседних смен и была бы расходом,
    // расходом, который считает сверка с нормой (ADR 0194). У счётчиков прирост при этом на месте.
    expect(within(card()).getByText('прирост 8 400 км')).toBeDefined();
    expect(within(card()).queryByText(/прирост .+ л$/u)).toBeNull();
  });

  it('у показания без топлива под каждой из трёх подписей прочерк, а не ноль', async () => {
    const noFuel = {
      ...READING,
      fuelStartLiters: null,
    ...
    renderGarage(intakeRoute('&report=r-1'), { [REPORT]: () => json(oneItem(noFuel)) });
    expect(await screen.findByText('Отчёт — Петров П. П., 14.08.2026')).toBeDefined();

    // Ноль в баке — утверждение о машине («бак пуст»), которого никто не делал: литров просто не
    // передали. У техники без указателя уровня их не будет вовсе, и это законный день.
    expect(counterValue('Топливо на начало')).toBe('—');
    expect(counterValue('Заправлено')).toBe('—');
    expect(counterValue('Топливо на конец')).toBe('—');
    expect(within(card()).queryByText('0 л')).toBeNull();
  });
});
...
describe('карточка отчёта дня: внесение за работника', () => {
      ...
      items: [
        {
          ...
          reading: {
            ...
            engineHours: 1240.5,
            // Оба остатка бака уходят явными ключами, даже пустые (ADR 0163, Р13). Пропуск ключа
            // сервер понял бы как «оставить как есть», а форма их показывала — значит пустое поле
            // здесь решение вносящего, а не незнание сборки.
            fuelStartLiters: null,
            fuelFilledLiters: 120,
            fuelEndLiters: null,
            comment: 'сдал вечером',
      ...
      items: [
        {
          ...
          reading: {
            ...
            odometerKm: null,
            engineHours: 812.5,
            // Пустые остатки уходят ключами, а не пропуском: см. ADR 0163, Р13.
            fuelStartLiters: null,
            fuelFilledLiters: null,
            fuelEndLiters: null,
            comment: '',
  ...
  /**
   * Оба остатка бака вносятся за работника, и топливной строки хватает, чтобы показание
   * состоялось (ADR 0163, Р7).
   *
   * Правило формы — «хотя бы одно из пяти», а не «хотя бы один счётчик»: у техники без одометра и
   * без моточасов литры — единственное, что вообще можно передать, и требование счётчика закрыло
   * бы ей ввод совсем. Обратная половина правила караулится тут же: пустая форма отправку не
   * пускает, иначе «хотя бы одно» превратилось бы в «ничего».
   */
  it('вносит один только остаток бака: топливной строки хватает, пустой формы — нет', async () => {
    const http = renderGarage(intakeRoute('&report=r-1'), { [SUBMIT]: () => json(reportDto()) });
    expect(await screen.findByText('Отчёт — Петров П. П., 14.08.2026')).toBeDefined();

    await openForm('Внести показание — ЭСМ-2 № 000123');
    fireEvent.click(saveButton('ЭСМ-2 № 000123'));

    // Отказ относится ко всем пяти полям сразу и приходит с путём первого из них.
    expect(
    ...
    fireEvent.change(field('ЭСМ-2 № 000123', 'Топливо в баке на начало смены, л'), {
      // Запятая нормализуется на вводе: у литров она встречается чаще, чем у одометра.
      target: { value: '46,5' },
    });
    fireEvent.change(field('ЭСМ-2 № 000123', 'Топливо в баке на конец смены, л'), {
      target: { value: '30' },
    });
    fireEvent.click(saveButton('ЭСМ-2 № 000123'));
    ...
    expect(http.lastCall(SUBMIT)!.body).toEqual({
      version: 3,
      // Первый ввод объяснять нечего: причина остаётся пустой, и поля её в форме не было.
      reason: '',
      items: [
        {
          itemId: 'item-2',
  ...
  /**
   * Правка сохранённого остатка требует причины — и не задевает соседние числа (Р7, Р13).
   *
   * Вторая половина здесь важнее первой. Показание отправляется целиком, и остатки уходят явными
   * ключами: форма, которая их не подставила бы в поля, при всякой правке одометра молча стирала
   * бы оба — своим же честным `null`. Поэтому караул сперва смотрит, что сохранённые литры видны
   * в форме, и только потом — что уехало на сервер.
   */
  it('правка сохранённого остатка идёт с причиной и не стирает соседние числа', async () => {
    const http = renderGarage(intakeRoute('&report=r-1'), {
      [REPORT]: () => json(oneItem(FUEL_READING)),
      [SUBMIT]: () => json(reportDto()),
    });
    expect(await screen.findByText('Отчёт — Петров П. П., 14.08.2026')).toBeDefined();

    await openForm('Поправить показание — Р-142');
    // Форма открывается сохранёнными литрами — правят их, а не заполняют заново.
    expect(field('Р-142', 'Топливо в баке на начало смены, л').value).toBe('46');
    expect(field('Р-142', 'Заправлено за смену, л').value).toBe('120');
    expect(field('Р-142', 'Топливо в баке на конец смены, л').value).toBe('85.5');

    fireEvent.change(field('Р-142', 'Топливо в баке на конец смены, л'), {
      target: { value: '92' },
    });
    fireEvent.click(saveButton('Р-142'));

    // Чужое число — оно и есть чужое: остаток бака правят с объяснением наравне с одометром.
    expect(await screen.findByText('Чужое число меняют с объяснением')).toBeDefined();
    expect(http.countOf(SUBMIT)).toBe(0);

    fireEvent.change(field('Р-142', 'Причина правки'), {
      target: { value: 'водитель списал уровень до заправки' },
    });
    fireEvent.click(saveButton('Р-142'));
    ...
    expect(http.lastCall(SUBMIT)!.body).toEqual({
      version: 3,
      reason: 'водитель списал уровень до заправки',
      items: [
        {
          itemId: 'item-1',
          reading: {
            kind: 'values',
            // Все нетронутые числа — те же, до последнего знака: правили одно поле из пяти.
            odometerKm: 128400,
            engineHours: 1240.5,
            fuelStartLiters: 46,
            fuelFilledLiters: 120,
