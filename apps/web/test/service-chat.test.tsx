...
describe('бейджи раздела: «ждут меня» и «мне написали» — числа разные', () => {
  ...
  it('счётчик непрочитанного спрашивают только у тех, кому видны заявки', async () => {
    // У диспетчера раздел открыт правом на технику (`officeEquipment.read`), а заявок нет вовсе:
    // ручка ответила бы ему отказом на каждый вход. Менеджер сюда больше не годится — круг
    // заказчика выдан ему ролью (ADR 0181), и счётчик он спрашивает наравне со штабом.
    const http = renderMenu(authUser({ role: 'dispatcher' }));
