...
// Правило форм заявок: новую заявку назначают не раньше чем на завтра — тем же правилом
// сервер проверяет создание (`minRequestDateKey` в контрактах). Отсчёт ведётся по Москве, а не
// по поясу браузера: у диспетчера из другого региона своя граница суток, и «завтра» у него
...
/** Минимальная дата новой заявки: завтра по МСК. Она же — значение по умолчанию. */
...
/**
 * Дата раньше завтрашней по МСК — для `DatePicker.disabledDate` в форме создания заявки.
