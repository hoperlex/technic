 ...
 * они разъезжаются.
 *
 * **1 → 2 (выпуск заявки без аппарата).** Первый и пока единственный подъём, и повод у него ровно
 * тот, ради которого механизм заводили: `equipment` и `object` в ответе о заявке стали
 * необязательными, а сборки контракта 1 читают их без проверки — первая же заявка без аппарата
 * роняет у них список и карточку. Отсюда обязанность выката: подняв это число, поднять и пол
 * `MIN_CLIENT_CONTRACT` до 2 — отдельным действием и уже после того, как новая статика
 * раздаётся (порядок фаз — в `docs/runbook.md`).
 */
