...
export function mechRequestViewFields({
  ...
  /*
   * Виза площадки (план визы, Р1) — строкой карточки, а не одним тегом состояния: тег отвечает
   * «ждёт», а карточка обязана ответить «кто и когда подписал». Поле показывается ВСЕГДА, включая
   * пустое: у «Новой» это её главный вопрос, у взятой в работу — основание договорённости, а у
   * закрытых заявок старше визы честный прочерк объясняет, почему подписи нет.
   */
  fields.push({
    key: 'approval',
    label: 'Виза площадки',
    children: request.approvedAt
      ? `${formatDateTime(request.approvedAt)} · ${request.approvedByName ?? '—'}`
      : 'ждёт визы',
  });
