 ...
 *
 * Отдельным файлом от самого списка (`RequestTripsBlock.tsx`) и от вкладки: здесь нет ни одного
...
/**
 * Ездка сохранённой заявки в строку **новой** — для копии заявки (ADR 0173).
 *
 * От `tripToForm` отличается одним: снимается `id`. Копия — это новые ездки, номера им назначит
 * сервер; уедь `id` в тело заведения, схема отвергла бы его как лишнее поле (`.strict()`), а
 * пройди он — заявка ссылалась бы на чужие строки.
 *
 * Время подачи, в отличие от `repeatTrip`, **сохраняется**, и это не разнобой, а разные вопросы.
 * Там из одной ездки делают шесть, и шесть машин за смену едут по графику, а не одновременно;
 * здесь повторяют тот же заказ другим днём — «первая в 8:00, вторая в 14:00» и есть то, что
 * копируют. День в момент не входит вовсе (`scheduledTime` — часы и минуты), поэтому сдвиг
 * подачи копии переносит время следом сам и границу дня (`tripsOutOfRequestDay`) не ломает.
 */
