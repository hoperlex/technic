import { Button, Space, Spin, Tabs, Tag, Tooltip, Typography } from 'antd';
...
interface Props {
  ...
  /**
   * Сменить машиниста внутри срока заявки и посмотреть состав по датам (план
   * `docs/assignment-periods-plan.md`, §9). Не передана — действие этой заявке недоступно: у
   * линейного заказа машиниста называют при выписке каждого листа, а не на заявке (ADR 0100 §6),
   * на арендной машине бланк ведёт арендодатель, у закрытой заявки состав уже предъявлен счётом, —
   * и распоряжается им не всякий, кто карточку читает.
   */
  onChangeMachinist?: (r: VehicleRequestDto) => void;
  /**
...
export function VehicleRequestViewModal({
  ...
  /**
   * Контакт водителя видит каждый, кто открыл заявку (ADR 0122): машину встречает заказчик, и
   * звонить он должен сам. Своего права у строки нет — его заменяет право на заявку, которым
   * закрыта и ручка; запрос отдельный потому, что полем DTO контакт не является.
   */
              ...
              <Space size={[16, 4]} wrap>
                ...
                {asksDriver ? (
                  <Space size={8} wrap>
                    ...
                    {/* Смена машиниста — рядом с человеком, а не в подвале окна: подвал отвечает
                        за заявку целиком, а меняют здесь одно решение — кто работает и с какого
                        числа (план `docs/assignment-periods-plan.md`, §9). Оттуда же открывается
                        «Состав по датам»: вопрос «кто работал в марте» задают, глядя на эту
                        строку, а она отвечает только про сегодня. */}
                    {onChangeMachinist && (
          ...
          {request.requestType === 'special_equipment' ? (
            <Tabs
              items={[
                ...
                // Дни работ (ADR 0100 решение 8) — место, где дни ведут: и по одному, и пачкой
                // на весь период. Вкладка открыта ЛЮБОМУ заказу техники на объект, а не только
                // линейному (ADR 0207 решение 1): 4-П за день просят и у машины, которая неделю
                // стоит на площадке, и прятать дверь по признаку типа значило бы отвечать «такой
                // бумаги не бывает» там, где её выписывают. Заказу, которому дни всё же не
                // положены — арендному, не взятому в работу, — таблица объяснит это словами
                // сервера (`blocker`), а не пустотой: причина приходит вместе с ней.
                //
                // Своего права у вкладки нет (ADR 0122): «кто и на чём выйдет ко мне в четверг» —
                // вопрос заказчика, а планирование дней держит `canPlan` внутри таблицы.
                {
                  key: 'days',
                  label: 'Дни работ',
                  ...
                  // диспетчер, заглянувший в заявку из рейса, получил бы рабочий планировщик.
                  // Сама вкладка остаётся — она и отвечает на «каким рейсом едет какой день»
                  // (план §3.5).
