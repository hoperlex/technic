 ...
 *
 * `null` — «этой строки денег не показываем» (ADR 0160, Р11), и он ОТЛИЧАЕТСЯ от прочерка. Прочерк
 * утверждает «суммы нет», а у редуцированной строки суммы может быть сколько угодно: сервер
 * обнулил `estimatedTotalAmount` и `completion.totalAmount` не потому, что их не было, а потому,
 * что читателю их не положено. Ровно для этого различия в DTO и заведено поле `audience` (решение
 * 4 ADR): без него ячейка честно врала бы прочерком. Пустая ячейка не утверждает ничего.
 */
export function amountLabel(request: ServiceRequestDto): { value: string; hint: string } | null {
  if (request.audience === 'requester') return null;
...
/**
 * Есть ли в выдаче хоть одна строка, деньги которой читателю положены (ADR 0160, Р11).
 *
 * Столбец «Сумма» — решение на всю таблицу, а аудитория — свойство строки, и мостом между ними
 * служит только эта проверка. У чистого заявителя финансовой строки не бывает ни одной, и столбца
 * у него нет вовсе; у исполнителя он появляется от первой же назначенной заявки, а редуцированные
 * соседи молчат пустой ячейкой (`amountLabel` → `null`).
 *
 * Пустая выдача считается финансовой намеренно, и это не поблажка: страница, которая ещё едет или
 * ничего не нашла, о читателе не сообщает ничего, а снятый на это время столбец возвращался бы с
 * приходом данных — таблица дёргалась бы на каждой перелистовке и на каждом отборе. Заголовок над
 * пустой таблицей дешевле: под ним всё равно нет ни одной ячейки.
 *
 * Здесь, а не в модуле колонок, по той же причине, что и `amountLabel`: это ответ на вопрос «что
 * здесь на самом деле показано», и файл колонок упирается в предел длины.
 */
