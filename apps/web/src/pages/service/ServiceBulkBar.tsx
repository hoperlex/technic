...
export function useServiceBulk(
   ...
   *
   * И восстанавливается он ТОЛЬКО СВОЕМУ ХОЗЯИНУ. Рабочее место в конторе бывает общим, а выход
   * гасит серверную сессию, но не хранилище вкладки: без имени в записи следующий вошедший увидел
   * бы «Продолжить» над чужим выбором строк и выполнил бы его от своего имени. Идемпотентность
   * сервера здесь не спасает — ключ пачки привязан к автору, и для второго человека тот же ключ
   * означает новую команду, а не повтор. Чужую запись чтение сносит само (`readServiceBulkRun`).
   */
  const userId = user?.id;
  useEffect(() => {
    const run = readServiceBulkRun(userId);
    if (!run) return;
    setKeys(run.body.rows.map((row) => row.id));
    setFrozen(scope);
    setTarget({ kind: 'restored', run });
    // Ровно один раз на учётку, а не на монтирование: пока сессия поднимается, `userId` пуст, и
    // единственный проход пришёлся бы на это окно — брошенная пачка не вернулась бы вовсе. За
    // отбором эффект по-прежнему не следит: `scope` ему в зависимости не нужен.
