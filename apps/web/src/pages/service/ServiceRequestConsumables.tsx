 ...
 *
 * ПОДПИСИ ГОВОРЯТ «РАСХОДНИКИ», А НЕ «НОМЕНКЛАТУРА» (план
 * `docs/office-equipment-card-and-list-cleanup-plan.md`, Р13; просьба заказчика 08.09.2026, п. 4).
 * «Номенклатура» — слово учётной системы: заказчик, у которого кончился тонер, ищет глазами
 * расходники, а не номенклатуру. Переименовано при этом только НАЗВАНИЕ СОСТАВА ОДНОЙ ЗАЯВКИ —
 * заголовок окна и уведомление об успехе; предметный термин остался на месте («Код номенклатуры» в
 * плейсхолдере поиска и подписи поля ниже, набор полномочий «Оргтехника: номенклатура», справочник
 * позиций). Глобальная замена слова по репозиторию переименовала бы и справочник, и право, и код
 * учётной системы — то есть развела бы портал с бумагами склада.
 *
 * Имя «Расходники» занято вкладкой РАЗДЕЛА, где позиции ведут и считают остаток (находка Н6), и
 * прилагательного окно не заводит: раздел и карточка — разные экраны, а рядом в карточке стоит
 * «Объём работ», и пара «Объём работ / Расходники» читается как ответ на один вопрос «что по
 * заявке пойдёт».
 */
export function ServiceRequestConsumablesModal({
  ...
  /**
   * Модель аппарата заявки — по ней подбираются позиции (Н10).
   *
   * Спрашивается тем же поиском по справочнику, каким технику ищет человек (Ф1), и по тем же
   * приметам — номерам из снимка заявки: с переводом отбора на сервер выдача перестала быть
   * справочником целиком, и «найти в загруженном списке» больше не работает — единица заявки в
   * полусотне строк по алфавиту лежать не обязана. Карточкой по идентификатору не спрашиваем:
   * ответ `GET /office-equipment/:id` несёт ещё и ленту обслуживания с гарантиями ремонтов (§8.2)
   * — тяжёлый ответ ради одного поля, которого в нём и не видно.
   *
   * Справочник открыт не всякому — сервисной компании он закрыт целиком (Р7), — и тогда модель
   * просто неизвестна, а поле показывает весь перечень расходников.
   */
  const equipment = request?.equipment ?? null;
  const { data: found = [] } = useQuery({
    ...officeEquipmentOptionsQuery(
      equipment ? equipment.serialNumber || equipment.inventoryNumber || equipment.name : undefined,
    ),
    enabled: !!equipment && can('officeEquipment.read'),
  });
  // Сверка по идентификатору обязательна: поиск по номеру снимка может принести и однофамильцев —
  // карточку с тем же номером в другом парке, — а модель нужна ровно у аппарата этой заявки.
  const modelId = equipment ? found.find((o) => o.value === equipment.id)?.modelId : undefined;
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      message.success('Состав расходников сохранён');
  ...
  return (
    <FormModal
      title={request ? `Расходники заявки ${request.displayNumber}` : 'Расходники'}
      open={!!request}
      onCancel={onClose}
