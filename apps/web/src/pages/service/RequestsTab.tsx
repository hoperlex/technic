...
export function RequestsTab() {
  ...
  /**
   * Разрешено ли РАСПОРЯЖАТЬСЯ ЭТОЙ ЗАПИСЬЮ — править её и уводить в архив. Два предиката
   * контрактов, те же, что стоят на серверных дверях (`assertServiceRequestEditable` и
   * `assertServiceRequestDeletable`; план профилей оргтехники, Р6 и Н8; план свободного объёма
   * работ и области исполнителя, Р11).
   *
   * ОНИ ПЕРЕМНОЖАЮТСЯ, А НЕ ЗАМЕНЯЮТ ДРУГ ДРУГА. Первый отвечает «в моей ли эта заявка ОБЛАСТИ
   * ДЕЙСТВИЙ» и ловит случай, мимо которого проходит второй: бывшему исполнителю на СВОЕЙ площадке
   * страж заказчика скажет «да» по `inCustomerScope`, а сервер ответит 403 — он не назначен и не
   * автор. Второй отвечает «моя ли это ЗАПИСЬ»: сторона заказчика по своей оси либо авторство.
   *
   * Второй копии правил портал не заводит: оба предиката живут в контрактах, признаки собирает
   * общий адаптер карточки. Сужают они троих — держателя набора «Заявитель» у роли без оси,
   * сисадмина со сквозной областью и снятого исполнителя; у прочих ответ прежний. Стоят рядом с
   * правом, а не вместо: право отвечает «положено ли действие вообще», предикаты — «на этой ли
   * строке». Подшивка документов зовёт их порознь — см. `ServiceRequestDocuments`.
   */
