 ...
 *
 * До неё окна перехода — назначение сервиса, согласование сметы, закрытие работ, приёмка —
 * показывали номер заявки в заголовке и больше ничего о предмете. Человек, открывший окно из
 * списка, помнит, что нажал; человек, пришедший по ссылке из письма или из очереди, — нет, а
 * решение принимает именно он: везти ли мастера на площадку, за какие деньги и к какому аппарату.
 *
 * Объект и место стоят здесь наравне с номером и техникой намеренно: это единственное место, где
 * «Согласовать смету на 7 100 ₽» перестаёт быть решением вслепую — видно, что чинят и где оно
 * стоит. Реквизиты берутся из снимка заявки, а не из справочника: карточку могли переименовать и
 * перенести, а решение принимается по тому, что чинили тогда.
 *
