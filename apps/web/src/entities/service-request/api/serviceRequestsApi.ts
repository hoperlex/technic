 ...
 * перехода, а портал не имеет возможности отправить объём работ туда, где его не ждут.
 * `changeStatus` остаётся отмене и административным откатам — у них из содержания только причина.
 *
 ...
 *
 * Ответ у переходов один и тот же — обновлённая заявка: следующее действие (предъявить объём
 * работ сразу после его правки) требует новой версии, а перезапрашивать карточку ради номера
 * версии значило бы гонку с самим собой.
 */
export const serviceRequestsApi = {
  ...
  /**
   * Состав объёма работ целиком: это документ, и «добавить строку» без остальных строк не имеет
   * смысла. Поэтому PUT, а не POST по строке. Имена ручек и полей при этом прежние (`estimate`,
   * Р17): переименовалось слово для человека, а не схема базы и матрица прав.
   *
   * Ручка отвечает 409, пока висит непогашенное предъявление (Р9): статуса-замка больше нет, и
   * править цифры под открытым окном согласования нельзя — согласующий подписал бы не то, что
   * видел. Ключ от замка один — «вернуть в правку» ниже.
   */
   ...
   * переводит заявку на сторону согласования. Повторное предъявление заперто тем же 409 (Р9):
  ...
  /**
   * Согласование и отклонение — одна ручка: у них одно право, одна область и один момент. Статуса
   * согласие не двигает (Р8) — гасит предъявление, и заявка остаётся «В работе»; отказ, наоборот,
   * закрывает её в «Отменена» и несёт с собой причину и решение — «что делаем вместо» (Р12).
   */
  decideEstimate: (id: string, body: ApproveServiceEstimateInput) =>
    patch<ServiceRequestDto>(id, '/estimate/approval', body),
  /**
   * Возврат объёма работ в правку (Р9) — единственный путь изменить состав после предъявления.
   * Снимает обе отметки: подпись согласования (как раньше) и непогашенное предъявление, — поэтому
   * ручка отпирает и правку состава, и повторное предъявление сразу.
   */
   ...
   * «добавить одну позицию» без остальных заставляло бы сервер угадывать, снимали ли что-то. Пустой
   * список
  ...
  /*
   * Визы отдела ИТ здесь больше нет (ADR 0145, Р10): подпись под объёмом работ одна, и ставит её
   * назначенный сотрудник — `decideEstimate`. Ручка `/it-approval` снята и с сервера; клиентский
   * метод, оставленный «на всякий случай», звал бы её в 404.
   *
   * Поля визы у заявки при этом живы и читаются: подпись, поставленная до этого выпуска, — правда
   * о том, что тогда решили, и карточка обязана её показывать.
   */
