 ...
 * соседних модулей (Р36). Заявка здесь не «висит вообще», а висит **в текущем ожидании**: стороны
 ...
 *
 * Считается по `statusChangedAt`, но меряет он не статус (Р4). После слияния статусов (Р2)
 * половина смен ответственной стороны идёт БЕЗ смены статуса — назначение, предъявление объёма
 * работ, согласование, — и колонку сервер обнуляет тогда, когда меняется тот, кого ждут: сторона
 * либо человек, которому передали работу. Иначе заявка, вчера переданная новому исполнителю,
 * показывала бы «в работе 9 дней» про чужое ожидание. Имя колонки при этом прежнее: переименование
 * ради уточнения смысла стоило бы миграции, а смысл живёт здесь.
 */

const DAY = 86_400_000;

/** Сколько полных суток заявка стоит в текущем ожидании (Р4). */
...
export function serviceStatusLine(
  request: Pick<
    ...
    //
    // `estimateFormat` — ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО И ОСТАЛЬНЫЕ ПОЛЯ ОЧЕРЕДИ, и перечислен он здесь
    // ради компилятора, а не ради чтения: поле карточки необязательное, поэтому объект без него
    // подходит под `Pick` молча — и подпись состояния считалась бы по планке наследия («счёт
    // закрывает») у заявки, где очередь списка и кнопка «Закрыть работы» отвечают иначе. Сегодня
    // все зовущие передают карточку целиком и расхождения нет; появилось бы оно в день, когда
    // подпись позовут сокращённым объектом, — то есть без единой ошибки сборки.
    | 'status'
    | 'waitingOn'
    | 'holdReason'
    | 'files'
  ...
  /*
   * Подпись «Вам: …» берётся по паре «статус + кого ждут», а не по одному статусу (волна В6), и
   * после слияния (Р2) без пары не обойтись вовсе: статусов стало меньше, а состояний столько же.
   * «Новая» ждёт то распределения (`operator`), то что за неё возьмутся (`service`); «В работе» —
   * то подписи под предъявленным объёмом работ (`approval`, Р3), то самих работ. Подпись по одному
   * статусу звала бы «выполнить и закрыть работы» того, от кого ждут подписи.
   *
   * Признаки Р2 (`hasExecutors`, `estimatePendingRevision`) сюда не передаются намеренно: сторону
   * по ним уже посчитал сервер (`serviceRequestWaitingOn`), и второй разбор тех же колонок на
   * портале разошёлся бы с очередью МОЛЧА — подпись звала бы к одному шагу, а бейдж «ждут меня»
   * отбирал бы заявки по другому правилу. Поэтому `serviceStepLabelFor` принимает готовый ответ
   * очереди вторым аргументом.
   *
   * Чужой ход подписан стороной (`serviceWaitingOnLabels`) — там шаг называть незачем: смотрящему
   * важно, кого ждут, а не что именно тот сделает. Новая сторона `approval` в словаре заведена
   * («Ждёт согласования»), и своего перечня сторон портал не держит: разъехаться им негде.
   */
  return mine
