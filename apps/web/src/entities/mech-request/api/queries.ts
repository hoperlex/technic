 ...
 *
 * Перечень идёт прямым `apiFetch` мимо `@entities/counterparty` — то же решение и по той же
 * причине, что у сервисных компаний: так велит граница слоёв, а не незаконченный переезд, и
 * «починка» на импорт слайса её ломает. Причина записана один раз, в
 * `entities/service-request/api/queries.ts`; повторять её здесь значило бы завести второй носитель
 * одного правила — а расходятся такие копии молча.
 */
