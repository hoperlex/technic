...
/**
 * Что арендуют — наименование модели (ADR 0156).
 *
 * Ссылка на справочник, а не строка заявки, и это порядок, а не предпочтение: модель могли
 * переименовать, и заявка обязана показывать её нынешнее имя — снимками у портала хранятся только
 * деньги.
 *
 * Прочерк здесь появился уборкой Э3 и означает ровно то, что показывает: **названия техники у этой
 * заявки нет**. Прежде на его месте стояло написание, с которым заявку когда-то завели; заказчика
 * спросили про такие заявки прямо, и он выбрал снять колонку, потеряв написания. Придумывать им
 * подпись вместо прочерка («не указана», «до справочника») значило бы прятать цену его решения от
 * того, кто эту заявку открыл; что именно потеряно, отвечает журнал аудита
 * (`mech_request.kind_dropped`), а не экран.
 */
export function mechModelLabel(row: Pick<MechRequestDto, 'mechModelName'>): string {
  return row.mechModelName ?? '—';
}
