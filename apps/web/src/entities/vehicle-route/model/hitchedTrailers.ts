...
// ── Э8: одно правило подстановки на все окна (план §14) ──

/**
 * Что делать с графами прицепа, когда окно узнало закрепление машины (Р20).
 *
 * Исходов три, а не два. Подстановка умела только **ставить**, и смена машины оставляла в форме
 * прицеп прежней: рейс уезжал с чужим госномером, а напечатанный чужой госномер читается как
 * правда — в отличие от пустой графы, которую допишут от руки. Очистка — такой же законный исход,
 * как подстановка, и названа она здесь наравне с ней.
 */
export type TrailerGraphsAction = 'keep' | 'substitute' | 'clear';

/** Решение подстановки: что делать с графами и ставить ли галочку по типу машины. */
export interface TrailerSubstitution {
  graphs: TrailerGraphsAction;
  /**
   * Ставить ли галочку «с прицепом» по типу машины (§4.4 (а)). Отдельным полем, а не четвёртым
   * исходом `graphs`: галочка говорит о категории прав и о бланке, а не о том, чем машина сегодня
   * укомплектована, — и графы этим решением не трогаются вовсе. `keep` при этом обязан быть
   * настоящим бездействием: пока галочка ставилась внутри него, «не трогать запись» означало
   * «не трогать, кроме галочки», и рейс, заведённый голым тягачом, получал прицеп обратно.
   */
  tractorDefault: boolean;
}

/**
 * Заполнены ли графы прицепа.
 *
 * Со срезанными пробелами: строка из одних пробелов — не описанный прицеп, а след правки, и
 * считать её заполненной графой значит запереть подстановку пробелом. Тем же `trim`, каким графы
 * сравнивает `graphsAreHitched`.
 */
export function trailerGraphsFilled(graphs: Partial<TrailerGraphs>): boolean {
  return [
    graphs.trailer1Model,
    graphs.trailer1RegNumber,
    graphs.trailer2Model,
    graphs.trailer2RegNumber,
  ].some((value) => (value ?? '').trim() !== '');
}

/**
 * Правило подстановки — одно на все пять окон (Р20).
 *
 * **Что считается «своим» у записи.** Описанный прицеп (графы) либо прямое «без прицепа» (снятая
 * галочка). Галочка при пустых графах не описывает ничего: она говорит «прицеп будет», а какой —
 * говорят графы. До этой работы она считалась своими графами наравне с ними, и последствие било
 * ровно по той машине, ради которой заводили реестр: у седельного тягача галочка встаёт сама
 * (§4.4 (а)), значит «своё» у него истинно всегда, значит правка рейса не подставляла закрепление
 * **никогда** — рейс оставался с пустыми графами, а бланк печатал пустые линии.
 *
 * **Зеркальная половина того же дефекта.** Рейс с осознанно снятой галочкой (голый тягач в ремонт
 * и обратно) «своим» не считался вовсе и получал подстановку вместе с галочкой обратно. Теперь
 * «без прицепа» — такое же решение записи, как заполненные графы, и подстановка его не отменяет.
 *
 * **Смена машины сильнее всего.** Графы описывают уже не ту единицу: закрепление у новой есть —
 * подставить, нет — очистить. Спрятанные графы очистка забирает тоже: поля прицепа при снятой
 * галочке со страницы уходят, но значения в форме остаются (`preserve` у rc-field-form), и
 * пережившая смену машины пара «марка + госномер» вернулась бы на экран вместе с галочкой.
 *
 * **Тип машины на судьбу граф не влияет.** Он решает только галочку, и только там, где её некому
 * было поставить: у заводимого рейса и у машины, на которую только что сменили.
 */
export function trailerSubstitution(input: {
  /** За машиной что-то закреплено — то есть подставлять есть что. */
  hasHitched: boolean;
  /** Окно правит существующую запись и не вытесняет её граф (`keepOwnGraphs` блока). */
  keepOwnGraphs: boolean;
  /** Машину в форме сменили после открытия окна. */
  vehicleChanged: boolean;
  /** Галочка «с прицепом» в форме прямо сейчас. */
  withTrailer: boolean;
  /** Хоть одна графа прицепа непуста (`trailerGraphsFilled`). */
  graphsFilled: boolean;
  /**
   * Машина — седельный тягач (§4.4 (а)); `undefined` — тип ещё не известен.
   *
   * Необязателен потому, что графы от типа не зависят вовсе: их решение спрашивают, не дожидаясь
   * справочника типов, — живое закрепление не должно стоять из-за медленного или упавшего списка.
   * Неизвестный тип означает ровно одно: галочку по типу не ставим.
   */
  isTractor?: boolean;
}): TrailerSubstitution {
  const { hasHitched, keepOwnGraphs, vehicleChanged, withTrailer, graphsFilled, isTractor } = input;

  if (hasHitched) {
    // Смена машины и окна заведения подставляют без оговорок: вытеснять в первом случае нечего
    // (графы уже не о той единице), а во втором — некого (записи ещё нет).
    if (vehicleChanged || !keepOwnGraphs) return { graphs: 'substitute', tractorDefault: false };
    // Открытая запись: своё не вытесняется, пустое при стоящей галочке — заполняется.
    if (graphsFilled || !withTrailer) return { graphs: 'keep', tractorDefault: false };
    return { graphs: 'substitute', tractorDefault: false };
  }

  // Закрепления нет: новой подстановки не бывает (§4.2.2, пункт 2), но чужие графы после смены
  // машины обязаны уйти — их оставлял в форме прежний код.
  if (vehicleChanged) return { graphs: 'clear', tractorDefault: isTractor === true };
  // Открытая запись неприкосновенна целиком, включая галочку: её поставил или снял человек.
  if (keepOwnGraphs) return { graphs: 'keep', tractorDefault: false };
  return { graphs: 'keep', tractorDefault: isTractor === true };
}
