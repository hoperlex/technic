...
export function ServiceAcceptModal({
   ...
   * закрывает» — то есть подсказка молчала бы у заявки, которой акта как раз и не хватает. Формат
   * сервер отдаёт (Э3); `?? null` остаётся ответом на карточку СТАРОГО приложения, где поля нет, —
   * там планка сегодняшняя, из трёх видов.
   */
