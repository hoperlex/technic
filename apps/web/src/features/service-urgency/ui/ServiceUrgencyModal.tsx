    ...
    >
      {request && (
        <Space orientation="vertical" size={12} style={{ width: '100%' }}>
          ...
          {clearing ? (
            /*
             * Плашка обещает ровно то, что человек увидит после нажатия (план
             * `docs/office-equipment-card-and-list-cleanup-plan.md`, Р3, находка Н2). Прежний
             * заголовок — «Заявка перестанет стоять первой в очередях и в письмах» — после снятия
             * подъёма срочных (Р1) стал неправдой наполовину: первой она не стоит и сейчас, а в
             * теме письма приставка «СРОЧНО ·» есть и никуда не денется у уже отправленных.
             * Обещание, которое сбывается через раз, хуже отсутствующего: следующий раз человек
             * не поверит и второй половине.
             */
            <Alert
              type="warning"
              showIcon
              title="С заявки снимется отметка срочности"
              // Наблюдаемый эффект, а не обещание про очередь: маркер, отбор и тема письма — три
              // места, где снятие видно сразу; уже ушедшие письма названы отдельно, иначе первый
              // же лежащий в почте «СРОЧНО ·» прочтётся как «не сработало».
              description={`Маркер исчезнет из списка и карточки, заявка выйдет из отбора «Только срочные», а новые письма по ней уйдут без приставки «СРОЧНО ·» — уже отправленные не меняются. ${
