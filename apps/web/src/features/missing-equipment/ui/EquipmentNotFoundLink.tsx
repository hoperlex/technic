 ...
 * `docs/office-equipment-candidate-plan.md`, §9; Э5 плана
 * `docs/office-equipment-request-subject-plan.md`).
 *
 ...
 *   2. иначе `officeEquipment.propose` — плашка с ДВУМЯ выходами сразу: «Заполнить самостоятельно»
 *      (окно «Сообщить об аппарате» — шесть реквизитов наблюдения, которые проходят проверку и
 *      только решением человека с `write` становятся карточкой, Р1) и «Написать в техподдержку».
 ...
 *      выхода у всех.
 *
 * **Почему у второй ветви два выхода, а не один.** Просьба заказчика (Э5): человек, которому
 * разрешено сообщить о технике, не обязан этого уметь и не обязан хотеть — аппарат стоит в другом
 * корпусе, шильдик заклеен, номера не видно. Единственная кнопка «Сообщить об аппарате» ставила
 * такого человека перед формой, которую нечем заполнить, а поддержку прятала за отсутствующей
 * ссылкой. Плашка поэтому объясняет и главный путь («Ведение» заведёт карточку по вашим данным,
 * приняв заявку в работу), и запасной — и оба видны сразу, без выбора «либо-либо».
 *
...
export function EquipmentNotFoundLink({
  ...
  const openBranch = () => {
    // Карточку заводят сразу окном: у того, кто ведёт справочник, выход из тупика ровно один, и
    // плашка перед ним была бы лишним нажатием. У двух других ветвей выходов больше — там сперва
    // разворачивается объяснение.
  ...
  return (
    <div style={{ marginTop: -12, marginBottom: 16 }}>
      ...
      {/* Подсказка разворачивается по нажатию, а не висит под полем всегда: у большинства заявок
          ...
          У заявленного аппарата её нет вовсе: сообщение уже составлено, и плашка «как быть, если
          техники нет» рядом с плашкой «аппарат на проверке» отвечала бы на вопрос, на который
          человек уже ответил. */}
      {branch === 'report' && hintOpen && !draft && (
        <Alert
          type="info"
          showIcon
          style={{ marginTop: 8 }}
          title="Техники нет в справочнике"
          description={
            <>
              <Typography.Paragraph type="secondary" style={{ marginBottom: 8 }}>
                Опишите аппарат сами — модель с шильдика, номер и место. Карточку заведёт «Ведение»
                по вашим данным, когда примет заявку в работу. Если описать нечем — напишите в
                техподдержку.
        ...
        <Alert
          ...
          description={
            <>
              ...
              {/* Готовый текст обращения — только здесь: в этой ветви поддержка единственный выход,
                  и писать его человеку пришлось бы самому. Рядом с «Заполнить самостоятельно» он
                  отодвигал бы вниз ту самую кнопку, ради которой плашку и показывают. */}
              <Typography.Paragraph
                copyable={{ text }}
      ...
      {branch !== 'create' && (
        /* То же окно контактов, что открывает каркас портала: другого канала поддержки нет, и
           ...
           Стоит на обеих ветвях без карточки: у держателя `propose` поддержка — второй выход, а
           не отсутствующий (Э5). */
