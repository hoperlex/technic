...
export function useServiceRequestCustomer({
   ...
   *
   * Спрашивается она ПО КОДАМ НАБОРОВ — тем же аргументом, что на сервере (план профилей
   * оргтехники, Р9): учётка приносит их с собой (`AuthUser.grantCodes`), и второй источник ответа
   * порталу больше не нужен. Прежде здесь стояли `user.addons`: ответ тот же (коды системных
   * наборов совпадают с кодами надстроек по построению), но надстройки уходят на шаге 1e ADR 0106,
   * и место, решающее по ним, пережило бы шаг молча — не отказом, а пропавшим пунктом в списке.
   * Своего предиката области здесь по-прежнему не заводится: вторая копия правила разошлась бы с
   * серверной так же тихо.
   */
  const wideScope = hasModuleWideScope(user?.grantCodes, 'serviceRequests');
