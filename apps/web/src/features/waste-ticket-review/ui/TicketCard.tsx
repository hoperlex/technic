      ...
      <Space orientation="vertical" size={6} style={{ width: '100%' }}>
        ...
        {field(
          ...
          ticket.needsReviewFields.includes('issuedOn') ? (
            // Подписи с транскрипцией у спорной даты нет намеренно (ADR 0166, п. 7): рядом уже
            // стоят два кандидата, и третий текст без объяснения, к какому из них он относится,
            // только запутает. Само значение при этом сохранено — для диагностики, а не для показа.
            <Disputed field="issuedOn" label="дату" candidates={ticket.candidates} />
          ) : (
            <>
              {/* По-русски, как на бланке: 17.08.2026. `YYYY-MM-DD` — это формат хранения и
                  обмена, и человеку, сверяющему с бумагой, он читается как чужая запись. */}
              {ticketDate(ticket.issuedOn)}
              {ticket.issuedOnRaw !== '' && (
                /*
                 * Транскрипция под датой (ADR 0166, п. 7). Сказано «OCR в графе», а не «на
                 * бланке», и разница существенная: `issuedOnRaw` — такой же ответ модели, как и
                 * сама дата, а не независимое свидетельство о бумаге. Обещай подпись содержимое
                 * листа, расхождение с датой человек читал бы как доказательство ошибки чтения —
                 * тогда как ошибиться могла и сама транскрипция.
                 *
                 * Пусто у ручных талонов и у машинных прошлых разборов: тогда строки нет вовсе.
                 * Ручная правка даты транскрипцию не меняет — она относится к скану, а не к
                 * решению человека.
                 */
                <Typography.Text type="secondary" style={{ display: 'block', fontSize: 12 }}>
                  OCR в графе: {ticket.issuedOnRaw}
                </Typography.Text>
              )}
            </>
          ),
