 ...
 * пережили переход на чеки (план `docs/auto-part-receipts-plan.md`, Р2, Р3):
 *
 *   — **аннулирование вместо удаления** у акта с движениями склада (Р6). Правило известно заранее,
 *     по `hasPartMovements` DTO — и держится именно на нём, а не на строках акта: строки портал
 *     больше не читает вовсе, а движения складского журнала их пережили. Узнавать правило из 409
 *     после нажатия «Удалить» человек не должен;
 *   — **акт, названный в адресе** вторым ключом `record` (Р14): открытая по такой ссылке сводка
 *     прокручивается к документу, а не просто к машине, и раскрывает его строку.
 */
...
export function VehicleMaintenanceBlock({
  ...
  /** Акт, который аннулируют: окно спрашивает причину и называет последствие (Р6). */
  ...
  const remove = useMutation({
    ...
    onError: (e) => {
      ...
      /*
       * По акту успели пройти движения, пока журнал был на экране: кнопка «Удалить» стояла по
  ...
  /**
   * Акт, названный вторым ключом адреса (Р14): ссылка бывает не на машину, а на документ, и
   * открытая по ней сводка обязана показать нужную строку среди десятков — прокруткой и
   * раскрытием.
   *
