 ...
 *
 * Причину переназначения окно спрашивает, но не требует (Р6 плана свободного объёма работ):
 * половина замен идёт по отпускам и сменам зон ответственности, где объяснять нечего.
 *
 * Переходом назначение быть перестало (Р5) — статуса оно не меняет, а состав пишет и кладёт строку
 * истории `from = to`. **Исключений больше нет** (ADR 0187): работающая заявка остаётся «В работе»
 * и при замене исполнителя, и при добавлении второго. Прежде окно обещало здесь возврат в «Новую»,
 * и обещание было честным — ручка действительно откатывала статус; теперь такой строки в
 * предупреждении нет, потому что откатывать нечего.
 */
export function AssignServiceModal({
  ...
  /*
   * Кандидаты в поимённые исполнители спрашиваются по праву НАЗНАЧЕНИЯ, а не по праву на перечень
   * учёток. У ручки `GET /service-requests/executor-candidates` страж — `serviceRequests.assign`
   * (`access-manifest.ts`), и заведена она была ровно затем, чтобы поле наполнялось у того, кто
   * заявки распределяет.
   *
   * ЗДЕСЬ СТОЯЛ `users.manage`, И ЭТО БЫЛ ДЕФЕКТ. Проверка осталась от прежнего решения, когда
   * кандидатов брали из общего `GET /users`; комментарий ссылался на него уже после того, как
   * ручку заменили. Право `users.manage` есть только у администратора портала — и у «Ведения» с
   * «ИТ-службой», то есть у обоих, кто назначает, список кандидатов приходил пустым. Назначить
   * можно было только тех, кто уже назначен, а себя в исполнители не поставить вовсе.
   *
  ...
  const mutation = useMutation({
    ...
    onSuccess: (res) => {
       ...
       *
       * Молчат три исхода, а не один. `not_needed` — это правка состава, которая никого не
       * назначила (сняли компанию, оставив исполнителя), и красная строка про «письма нет» была бы
       * тревогой на ровном месте. `event_off` — рубильник события выключен администратором
       * намеренно (§5.10 плана `docs/office-equipment-mail-expansion-plan.md`): назначивший этого
       * не поправит и поправлять не должен, а красная строка читалась бы как поломка. Список тот
       * же, что у `reportServiceMail`, — дотянуться до него отсюда нельзя по причине выше.
       */
      if (res.mail !== 'queued' && res.mail !== 'not_needed' && res.mail !== 'event_off') {
  ...
  return (
    ...
    >
      ...
      {/*
       * Что именно теряет заявка при замене — до нажатия, а не после.
       *
       * Возврата в «Новую» здесь больше нет (ADR 0187), и работающей заявке сказано об этом прямо:
       * человек, помнящий прежнее поведение, обязан видеть, что статус останется на месте, — иначе
       * он по привычке пойдёт поднимать заявку обратно в «В работе» после каждой замены.
       */}
      {!first && (
        <Alert
          type="warning"
          ...
          title="У снятого исполнителя заявку заберут"
          description={
            request?.status === 'in_work'
              ? 'Статус не изменится — заявка останется «В работе», и новый исполнитель продолжит её с этого места. Объём работ и согласование прежнего исполнителя будут стёрты, а отсчёт ожидания начнётся заново. Оставшиеся назначенные продолжают вести заявку.'
      ...
      <Form form={form} layout="vertical" onFinish={(v) => mutation.mutate(v)}>
        ...
        {/* Страховка, а не обычный путь: окно открывают кнопкой, которая сама спрашивает право
            назначения, — значит ветка недостижима, пока эту кнопку не покажут кому-то ещё. Оставлена
            затем, чтобы пустой список объяснялся словами, а не читался как «своих исполнителей нет
            вовсе»: молчание в этом месте уже однажды стоило дефекта. */}
        {!mayListUsers && (
          <Typography.Paragraph type="secondary" style={{ marginTop: -12 }}>
            Список сотрудников виден тому, кто распределяет заявки; здесь остаются только уже
            назначенные. Сервисная компания выбирается всегда.
          </Typography.Paragraph>
        )}
        {/* ПРИЧИНА ЗАМЕНЫ НЕОБЯЗАТЕЛЬНА (план
            `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р6; просьба заказчика
            от 09.09.2026 дословно: «убрать звёздочку с причины замены»).

            Обязательной она была замком, который держал не то: заменяют исполнителя сплошь и рядом
            по обстоятельствам, которые обеим сторонам и так известны — человек в отпуске, заявку
            передали ближе к площадке, — и требование объясниться превращалось в набор слова
            «замена» ради прохода формы. Такая история хуже пустой: она выглядит объяснением, не
            будучи им.

            ПРЕДУПРЕЖДЕНИЕ НИЖЕ ОСТАЁТСЯ, и это разные вещи: причина — про прошлое («почему
            сменили»), предупреждение — про последствие («у снятого заявку заберут»). Снять второе
            вместе с первым значило бы убрать единственное место, где о последствии сказано до
            нажатия.

            В истории пустая причина показывается прочерком, а не пропадает строкой (ответ В8):
            событие «Сервис заменён» было, и молчание в его строке читалось бы как потерянный
            текст. */}
        {!first && (
          <Form.Item
            name="reason"
            label="Причина замены"
            extra="Необязательно. Уйдёт в историю заявки: по ней и разбирают, почему исполнителей сменили"
          >
