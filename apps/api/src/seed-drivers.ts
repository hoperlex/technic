  ...
  if (report.created.length > 0 && !report.dryRun) {
    console.log(
      `\nУ заведённых удостоверений пустые серия, номер и сроки: в выгрузке их нет. Пока они ` +
        `пустые, водитель в отбор под машину не попадает: без номера удостоверения путевой лист ` +
        `печатается недействительным. Реквизиты вносит администратор заменой документа в карточке.`,
    );
  }
}
