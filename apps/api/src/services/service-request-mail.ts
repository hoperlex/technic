  ...
  if (intent.event === 'service_request_assigned') {
    ...
    for (const target of intent.comment?.targets ?? []) {
      ...
      if (target === 'service' && hasServiceSide(side)) {
        /**
         * Сторона сервиса у реплики — такая же, как у остальных событий, включая общий ящик
         * компании (уточнение заказчика 04.09.2026). Ящик подрядчика получает письма по ТЕМ
         * заявкам, которые ему назначены, — и обсуждение по ним такое же его дело, как задание и
         * бумаги: у компании без учёток в портале это единственный способ прочитать реплику.
         *
         * Отбор «только свои заявки» держится сам собой: сторона считается по
         * `serviceCounterpartyId` заявки, и в письма чужой заявки этот ящик не попадает никогда.
         */
