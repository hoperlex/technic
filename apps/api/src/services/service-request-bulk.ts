...
import { writeServiceAccessDenial } from '../lib/service-access-audit';
import type { ServiceManifestRouteKey } from '../lib/service-access-manifest';
...
/**
 * АДРЕС ПОПЫТКИ В ЖУРНАЛЕ ОТКАЗОВ — ПАКЕТНАЯ РУЧКА, А НЕ ТА ОДИНОЧНАЯ, ЧЕЙ ШАГ ОТБИЛ СТРОКУ.
 *
 * Подставить сюда ключ одиночной ручки заманчиво — отбивает строку тот же страж, — но это была бы
 * неправда о самом важном: обращение было ОДНО, пакетное, и разбирают такой случай как одно
 * обращение с пятьюдесятью адресами, а не как пятьдесят прямых заходов в карточки. Какой операцией
 * шли строки, при этом не теряется: о ней отвечает `bulkOperationId` и таблица пачек.
 *
 * Тип — ключ манифеста области (`service-access-manifest.ts`), где путь пакетной ручки уже
 * объявлен: перепиши его кто-нибудь там, не поправив здесь, — упрётся в компилятор, а не в журнал
 * с адресом несуществующей ручки.
 */
const BULK_ROUTE: ServiceManifestRouteKey = 'POST /api/v1/service-requests/bulk';
...
async function runRow(params: {
  ...
  try {
    ...
    if (e instanceof BulkLeaseLost) throw e;
    /*
     * ОТКАЗ ДОСТУПА ОСТАВЛЯЕТ ТОТ ЖЕ СЛЕД, ЧТО И У ОДИНОЧНОЙ РУЧКИ (Р6 и критерий К6 плана
     * `docs/office-equipment-executor-access-audit-plan.md`) — и пишет его пачка сама.
     *
     * ПОЧЕМУ НЕ ХУК, КОТОРЫЙ ПИШЕТ ВСЕ ОСТАЛЬНЫЕ. Он ловит отказ по дороге НАРУЖУ, а отсюда наружу
     * уходит `200` с отчётом: до `onError` перехваченный отказ не доходит никогда. Пока записи здесь
     * не было, пачка была дырой ровно в том случае, ради которого журнал и заводили, — за одно
     * обращение перебиралось до пятидесяти чужих идентификаторов, и не оставалось ни строки.
     *
     * ЗАПИСЬ У КАЖДОЙ ОТКАЗАННОЙ СТРОКИ, А НЕ ОДНА НА ПАЧКУ: журнал отвечает на вопрос «кто стучался
     * в ЭТУ заявку», и сводная строка не ответила бы на него ни по одной из пятидесяти. Приписка
     * пачки при этом та же, что у записей об успешных строках (Р8), — «эти пятьдесят сделаны одним
     * движением» проверяется сложением по одному идентификатору.
     *
     * ТРАНЗАКЦИИ СТРОКИ ЗДЕСЬ УЖЕ НЕТ: отказ её и откатил, — а `writeAudit` ходит мимо неё, в общий
     * пул, и потому запись откат переживает. Отчёта журнал не меняет ничем: исход строки считается
     * ниже прежним порядком, а сбой самой вставки `writeAudit` глотает — «не записалось» не имеет
     * права стать «не отказано».
     *
     * ЗАПИСЬ ИДЁТ ДО ОТМЕТКИ О СТРОКЕ, и это выбор в пользу журнала: строка, у которой отметку
     * перехватил takeover, повторится другим заходом и оставит вторую запись о том же отказе.
     * Повтор в журнале читается как повтор попытки — неточно, но не опасно; обратный порядок
     * терял бы записи молча, а «отказа не было» — единственная ошибка, которой этому событию
     * делать нельзя.
     */
    if (e instanceof ServiceAccessDenied) {
