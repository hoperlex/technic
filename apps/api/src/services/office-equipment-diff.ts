...
export const officeEquipmentFieldLabels: Record<string, string> = {
  ...
  isActive: 'Активна',
  /*
   * Гарантия поставщика в `changes` не приходит и прийти не может — диф её намеренно выносит
   * отдельным полем (`warrantyChange`), чтобы одно действие не давало в ленте две строки. Подпись
   * здесь всё равно нужна: блок «Ручные правки» (план истории тремя блоками, Р3) приводит это
   * изменение к общей форме и показывает строкой блока, а выгрузка читает подписи отсюда же.
   * Без строки на экране и в файле стояло бы сырое имя поля.
   */
  warrantyUntil: 'Гарантия поставщика',
};
