 ...
 *
 * ПОЧЕМУ ОН ЖИВЁТ ЗДЕСЬ, А НЕ В ОТЧЁТЕ. Предикат — листовое правило: его читают и дверь записи
 * ({@link requireHistoryPopulationReady}), и сводка готовности
 * ([assignment-readiness.ts](./assignment-readiness.ts)), и прогон бэкфилла
 * ([scripts/assignment-backfill.ts](../../scripts/assignment-backfill.ts)). Пока он лежал в модуле
 * сводки, дверь записи зависела от отчёта, а отчёт — от двери: исполняемый цикл на одну константу.
 * Сводка берёт его отсюда и переизлучает наружу, так что зависимость односторонняя.
 *
 * Копия этого предиката жила в прогоне бэкфилла; теперь он берёт её отсюда. Две копии предиката
 * cutover означали бы, что «готово» у прогона и «готово» у отчёта считаются по разным множествам
 * заявок, — а различить это по числам нельзя.
 */
