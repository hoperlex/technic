 ...
 * ни было их значение для операции — приёмник и источник переноса, рейсы дней заказа на объект,
 ...
 * рейса (`POST /vehicle-routes`) и постановка дня заказа на объект (`openDayRoute`).
 */
 ...
 *
 * Считаются все три способа попасть в бланк: строкой состава рейса — грузовым талоном или днём
 * заказа техники на объект, — основанием рейса-перегона (ADR 0057) и листом, выписанным ещё вне
 * маршрутов (`legacyWaybillOf`). Аннулированный лист не держит: испорченный бланк списан, и работа
 * заявки к нему больше не относится.
 *
 * Держит откат тот документ, который печатает работу ИЗ РЕЙСА, и дневной 4-П держит его ровно так
 * же, как грузовой (ADR 0207, «Последствия»). Ничего ради этого не добавлялось: день стоит в
 * составе той же строкой, что и грузовой талон. Изменилось другое — дневные листы теперь бывают у
 * ЛЮБОГО заказа техники на объект, а не только у линейного (ADR 0207 §1), и отказ встречает
 * заказы, у которых прежде такой бумаги не заводилось вовсе.
 *
 * ЭСМ-2 откату по-прежнему не мешает, но причина этому не «второй бумаги на ту же работу взяться
 * неоткуда»: с ADR 0207 §2 за один день работы их как раз бывает две — недельный ЭСМ-2 и дневной
 * 4-П. Решает не число бумаг, а привязка: ЭСМ-2 висит на самой заявке, а не на рейсе (миграция
 * 0087). Откат у него ничего не отбирает и во второй такой лист заявку не уводит — дыра ADR 0050
 * открывается только там, где бумагу печатает состав рейса. Держать им откат значило бы запретить
  ...
  const [row] = await reader
    ...
    .where(
      and(
        ...
        // неперенесённой истории: недельный лист висит на самой заявке, и рейса нет у НЕГО — дни
        // той же недели с ADR 0207 в рейсах как раз стоят и закрываются своими дневными 4-П.
 ...
 * днями заказов на объект в порядке объезда (`waybillTaskRows`, Р11), и первая ездка перестала
 * быть ответом на «что напечатать». **Задание водителю** ушло с этапом 8: письмо и кабинет собираются по точкам
...
export async function requestsByRoute(
  ...
  const rows = await reader
    .select({
      ...
      // День заказа техники на объект, ради которого строка стоит в рейсе (ADR 0100,
      // миграция 0127); NULL — грузоперевозка: день несёт сам рейс.
  ...
  for (const row of rows) {
    ...
    list.push({
      ...
      // Погрузка, разгрузка и груз есть только у грузоперевозки: у дня заказа на объект вместо них
 ...
 * миграции 0127 больше нет: у грузоперевозки рейс по-прежнему один (день несёт сам рейс), а заказ
 * техники на объект стоит в стольких рейсах, сколько дней распланировано. Оставленная рядом со
 ...
 * было.
 *
 * Строка ищется парой «рейс + заявка», и второго ответа здесь не бывает даже у заказа, ведомого
 * днями: первичный ключ состава — та же пара, а значит в одном рейсе заявка стоит ровно одной
 * строкой. День возвращается ради вызывающего: снятая строка это либо грузовая заявка, либо день
 * заказа на объект, и в журнале эти два события читаются по-разному.
 *
 ...
 *
 * Берутся **все**: строки состава (у грузоперевозки одна, у заказа на объект по одной на каждый
 * распланированный день) и перегоны, а сверх них — рейс, названный в теле запроса
 * (`extraRouteIds`): перевод в работу и смена машины кладут заявку в готовый маршрут, и он обязан
 * быть взят тем же проходом, иначе две встречные перестановки «A→B» и «B→A» возьмут одну пару в
 * разных порядках.
 *
 ...
 *
 * Дни заказов переезжают сами: их день физически равен дню рейса (составной FK с `ON UPDATE
 ...
 * Заказы на объект попадают в тот же перечень — «какие заявки уехали вместе с рейсом» вопрос один,
...
/**
 * Уедут ли стоящие в рейсе дни заказов вместе с ним — до того, как рейс тронулся с места.
 *
 ...
 *
 * «Линейное» в имени — от прежнего правила: признак типа ТС дверь дней больше не воротит, и день
 * бывает у любого заказа техники на объект (ADR 0207 §1). Имя оставлено общим с контрактами
 * (`syncLinearRouteDays`, `LinearDaySubject`): переименовать половину — значит завести два словаря
 * на одно понятие.
 */
async function assertLinearDaysMovable(
  ...
  const rows = await tx
    ...
    .from(vehicleRouteRequests)
    .innerJoin(vehicleRequests, eq(vehicleRequests.id, vehicleRouteRequests.requestId))
    // Срок заказа — у детали спецтехники; день бывает только у заказа техники на объект, чем бы
    // ни был его тип ТС (ADR 0207 §1).
    .leftJoin(
      specialEquipmentRequestDetails,
 ...
 *
 * `workDate` — день заказа техники на объект, ради которого строка встаёт в рейс (ADR 0100 §2;
 * линейность его больше не решает — ADR 0207 §1); `null` — грузоперевозка, у которой дня нет
 * вовсе: его несёт сам рейс. Проверять равенство дня и даты рейса здесь незачем — этого не даст
 * база (составной FK, миграция 0127).
 */
