 ...
 * (`requireHistoryEquipment`), и второй поход за той же строкой стоил бы шестого запроса ради
 * значения, которое лежит в руке. `objectId` нужен признаку расхождения площадок ниже.
 *
 * БЮДЖЕТ ЗАПРОСОВ — ПЯТЬ НА СТРАНИЦУ, И НЕ БОЛЬШЕ (Р10, К8), при любом числе строк:
 ...
 *   5. подтверждения заявленного места (`confirmedPlaceByRequest`).
 *
 * ПЯТАЯ ДОГРУЗКА ПРИШЛА ПОЗЖЕ ОСТАЛЬНЫХ, и потому счёт здесь долго стоял на четырёх. Признак
 * «заявляли не то место» дорос до третьего члена уже после блоков — соседней работой (план
 * перемещения из карточки заявки, п. 12 и Р8), — а считать его обязан тот же модуль, что и DTO
 * заявки: два понятия расхождения, посчитанные порознь, разъехались бы молча. Первая редакция Р10
 * обещала уложить признак в первый запрос коррелированным `NOT EXISTS`, но у драйвера такой
 * подзапрос в списке столбцов собирается неверно (см. ниже), и цена ошибки здесь выше цены пятого
 * запроса, который идёт параллельно остальным и цепочку не удлиняет.
 *
 * Пустая страница стоит одного запроса: догрузки по пустому списку идентификаторов не идут никуда.
 *
 * Число держит не эта запись, а db-тест (`equipment-history-blocks.db.test.ts`, §10.1, тест 11): он
 * считает выражения SQL обёрткой на соединении пула и сверяет страницу в одну строку со страницей в
 * двадцать — равные числа и означают, что скрытого `N+1` нет.
 *
...
export async function loadEquipmentRequestsPage(
  ...
  // Четыре догрузки одной пачкой и параллельно: вход у всех четырёх один — идентификаторы
  // страницы, и ждать их по очереди незачем.
