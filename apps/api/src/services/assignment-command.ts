 ...
 *
 * ПОЧЕМУ ЭТО ЭКСПОРТ, А НЕ ЧАСТНОЕ ДЕЛО КАРКАСА. Б5 писался про **все двери, зовущие общий план**,
 * а каркасом из них ходят шесть; ещё три зовут ту же бумагу мимо него — ручная выписка ЭСМ-2,
 * смена статуса и старая дверь смены техники ([vehicle-requests.ts](../routes/vehicle-requests.ts)).
 * Уровень у них обязан быть тот же самый объект, а не та же строка, набранная второй раз: разойтись
 * два таких значения могут только молча и только бумагой, а сверить их будет нечем — в ответе
 * портала уровень изоляции не виден.
 */
export const SNAPSHOT_ISOLATION = { isolationLevel: 'repeatable read' } as const;
