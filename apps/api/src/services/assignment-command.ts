 ...
 * (шаг 12) и состояний готовности (Р26). Их приносят двери волны 3.2 — каркас лишь держит для них
 * места в правильном порядке. Шаг 12 двери занимают по-своему: команда машиниста исполняет им
 * недельную сверку (паритет старой бумаги, этап 3), периодная коррекция — снятие подписей по
 * `approvalClearRange`. Исполнителя **отрезкового** плана нет ни у одной; когда он появится, туда
 * встанет `applyEsm2SyncPlanAndAudit(scopedPlan, context)`, и **вместе с ним** уйдут шесть внешних вызовов
 * `auditEsm2Sync` после транзакции ([waybill-esm2.ts](./waybill-esm2.ts)) — иначе останется либо
 * старое нетранзакционное окно, либо двойное событие. В этой волне они не трогаются намеренно:
 * удалять вызовы, которым ещё нечем заменить работу, значит выкатить дыру.
 *
...
export interface AssignmentCommandSpec<TPlan, TApplied, TPaper> {
   ...
   *
   * Занята она пока наполовину: команда машиниста исполняет здесь недельную сверку, периодная
   * коррекция — снятие подписей, а сверка **отрезкового** плана и постусловие ждут своего
   * исполнителя (этап 5). Порядок внутри шага значим сам по себе: `frozen` у операции задним числом — отказ,
   * откатывающий транзакцию, и проверять его после снятия подписей значило бы откатывать уже
   * сделанную работу.
   */
 ...
 * 12  сверка бумаги и всё, что за ней                     — дверь (`syncPaper`), пока пусто
...
export async function runAssignmentCommand<TPlan, TApplied, TPaper = void>(
  ...
  const command = async (): Promise<AssignmentCommandOutcome<TApplied, TPaper>> =>
    executor.transaction(async (raw): Promise<AssignmentCommandOutcome<TApplied, TPaper>> => {
      ...
      // 12. Бумага и всё, что за ней: недельная сверка у команды машиниста, снятие подписей у
      //     периодной коррекции. Дверь без бумажных последствий колбэка не даёт вовсе.
