...
export interface ReassignPlan {
   ...
   *
   * Окно их показывает, а боевая половина двери по ним **требует** подпись — но спрашивает её не
   * здесь: тело запроса в расчёт не приходит вовсе, и живёт проверка в маршруте
   * (`PATCH /:id/assignment`), которому этот набор и передаётся готовым. Пустой список здесь был бы
   * неправдой дважды: предупреждения у бумаги есть, и подтверждать их человеку пришлось бы вслепую.
   */
 ...
 *
 * `issues` дверь показывает, а рукопожатие по ним спрашивает маршрут — там, где бумагу выпускает
 * сам план (`read_mode = history`): боевая половина старой двери живёт в
 * `PATCH /:id/assignment`, тело запроса сюда не приходит, и проверке место рядом с телом. Показать
 * предупреждения при этом обязан предпросмотр: человек должен видеть, с какими пробелами в
 * документах уйдёт бумага, до того как подпишет.
 */
