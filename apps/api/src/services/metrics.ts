...
import { assignmentRetryCounters } from './assignment-retry';
 ...
 *   главным риском многопоточности;
 * - **повторы протокола В4 и их исчерпания** — первое растёт в норме и означает, что конкуренция
 *   разобрана без участия человека; второе обязано быть нулём и по нему же подбирается потолок
 *   повторов (`ASSIGNMENT_RETRY_ATTEMPTS`).
 *
...
export async function collectMetrics(): Promise<string> {
  ...
  return render([
    ...
    {
      /*
       * Протокол повторов на `40001` (план `assignment-periods-plan.md`, В4): сколько раз
       * транзакция команды пересчитывалась заново, потому что её снимок устарел под конкурентной
       * записью.
       *
       * Метрика РАСТУЩАЯ В НОРМЕ, и это про неё главное: повтор — штатный исход конкуренции, а не
       * происшествие. Смотреть на неё нужно в паре с исчерпаниями ниже: ненулевые повторы при
       * нулевых исчерпаниях означают, что протокол работает и людям об этом знать незачем.
       *
       * Метка `door="preview"` — предпросмотры всех дверей одной строкой: их повтор выбрасывает
       * только расчёт, тогда как повтор боевой команды выбрасывает работу под блокировкой заявки.
       */
      name: 'technic_assignment_retries',
      help: 'Повторы транзакций истории назначения на конфликте сериализации (с запуска процесса)',
      type: 'counter',
      values: assignmentRetryCounters().map((c) => ({
        labels: { door: c.door },
        value: c.retries,
      })),
    },
    {
      /*
       * Исчерпания потолка повторов: человек получил `503` с `Retry-After` вместо результата.
       *
       * Отдельная метрика, а не метка у прежней: у них разные нормы. Норма повторов — «растёт»,
       * норма исчерпаний — **ноль**, и одна метрика на оба состояния прятала бы второе в тени
       * первого. Ненулевое значение читается по закону спайка (§4.3): потолок `N` покрывает `W ≤ N`
       * одновременных писателей, поэтому исчерпания означают либо что боевое `W` выше настроенного
       * потолка (`ASSIGNMENT_RETRY_ATTEMPTS`), либо что общая строка портала — счётчик номеров
       * бланков ЭСМ-2 — перегружена массовым прогоном, который спайк велел делать однопоточным.
       *
       * Те же случаи считает и `technic_assignment_command{outcome="serialization"}`: там они
       * видны рядом с прочими исходами двери, здесь — рядом с числом повторов, по которому и
       * подбирается потолок.
       */
      name: 'technic_assignment_retry_exhausted',
      help: 'Исчерпания потолка повторов истории назначения: ответ 503 с Retry-After (с запуска процесса)',
      type: 'counter',
      values: assignmentRetryCounters().map((c) => ({
        labels: { door: c.door },
        value: c.exhaustions,
