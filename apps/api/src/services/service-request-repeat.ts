...
/**
 * Сама заявка `R` там, где её приходится доставать отдельно: пакетный счёт соединяется с ней по
 * идентификатору, ссылка «предыдущие» читает её реквизиты подзапросом (`subjectColumn`).
 *
 * И то и другое ради одного: аппарат и дату заведения брать колонками базы, а не значениями,
 * проехавшими через JS, — там `timestamptz` теряет микросекунды, и потребители правила разъезжались
 * бы на границе окна.
 */
 ...
 *
 * Два реквизита и идентификатор. Вид (Р3) и аппарат (условие 1) — здесь потому, что ими решается
 * ПРИМЕНИМОСТЬ правила, а её считают в JS, до всякого SQL.
 *
 * ДАТЫ ЗАВЕДЕНИЯ В СУБЪЕКТЕ НЕТ, хотя окно отсчитывается именно от неё (условие 4): все три запроса
 * читают её колонкой по идентификатору — соединением либо подзапросом (`subjectColumn`). Держать
 * рядом поле, которого ни один из них не читает, значило бы завести тихую ловушку: подделанная в
 * нём дата не сдвинула бы границы окна ни на микросекунду, и на это однажды понадеялись бы.
 *
 * Всё остальное — статус самой `R`, её область, её содержание — правилу безразлично: повтором
 * бывает и новая заявка, и давно закрытая.
 */
 ...
 * приходит из трёх разных мест — из соединения по строке `VALUES` (пакетный счёт), из колонок
 * внешнего запроса (отбор `repeat=true`) и из подзапроса по идентификатору (ссылка «предыдущие»);
 * условие при этом обязано остаться одним.
 */
...
/**
 * Реквизит заявки `R` КОЛОНКОЙ БАЗЫ там, где самой `R` в запросе нет, — скалярным подзапросом по её
 * идентификатору.
 *
 * Так устроена ссылка «предыдущие»: её основная таблица — сами `P`, соединять их с `R` незачем, а
 * подставить дату заведения значением из JS нельзя: `Date` заканчивается миллисекундой, и обе
 * границы окна уезжали бы вниз на микросекундный хвост — ровно там, где метка и отбор читают ту же
 * дату колонкой (см. сборку пакетного запроса).
 *
 * Подзапрос НЕ КОРРЕЛИРОВАН — он не смотрит ни на одну колонку внешнего запроса, — поэтому
 * планировщик считает его однажды на весь запрос, а ловушка коррелированного подзапроса в drizzle
 * (Н6) сюда не достаёт. Строки по идентификатору нет — реквизиты приходят `NULL`, условия
 * обращаются в неизвестность, и выдача пуста: исчезнувшую заявку предшественниками не наделяют.
 */
function subjectColumn(id: string, column: PgColumn): SQL {
  return sql`(SELECT ${column} FROM ${serviceRequests} ${CUR} WHERE ${CUR.id} = ${id}::uuid)`;
}
 ...
 *
 * ИЗ JS СЮДА УЕЗЖАЕТ ТОЛЬКО ИДЕНТИФИКАТОР — как и в пакетный счёт. Аппарат и дату заведения запрос
 * читает колонками самой `R` (`subjectColumn`), и это не украшение симметрии: подставленная через
 * `Date` дата теряет микросекунды, обе границы окна съезжают вниз, — а метка считает по колонке.
 * Заявка, заведённая в ту же миллисекунду, в которую закрыли предшественницу, получала бы тогда
 * «Повтор ×1» и пустой список по ссылке — расхождение того самого класса, ради которого правило и
 * сведено в один builder (К3).
 *
 * ЧТЕНИЕ `R` ПОДЗАПРОСОМ ОБЛАСТИ НЕ СПРАШИВАЕТ, И ЭТО НЕ ДЫРА. Вопрос «есть ли такая заявка у
 * этого читателя» задан и закрыт раньше — маршрут читает `R` под общим предикатом видимости и
 * отвечает `404`; подзапрос же отвечает ровно на «от какой даты считать окно», а сами `P` область
 * читателя отбирает внутри правила (условие 5).
 *
...
export function serviceRequestRepeatPreviousWhere(
  ...
  return repeatMatchWhere(
    ...
    serviceRequests,
    {
      ...
      equipmentId: subjectColumn(subject.id, CUR.officeEquipmentId),
      createdAt: subjectColumn(subject.id, CUR.createdAt),
    },
    window,
  );
}
