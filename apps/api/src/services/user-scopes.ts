...
import { revokeAllForUsersTx } from '../auth/sessions';
...
/**
 * Площадки отделов учётки (ADR 0062, кардинальность — ADR 0144) — производная область: в её
 * пределах роль отдела ведёт вывоз мусора. Считается из справочника, а не хранится у учётки:
 * объекты задаются отделу, и второе хранилище разошлось бы с ним при первой же правке.
 *
 * **Источник — таблица связи `department_construction_objects`, а не колонка
 * `departments.construction_object_id`.** Площадок у отдела теперь столько, сколько нужно
 * (ADR 0144, решение 1), и колонка осталась лишь совместимой проекцией набора на один релиз: при
 * наборе из нескольких она равна `NULL`, то есть чтение по ней отдало бы гарантийному отделу
 * пустую область. Механика доступа при этом не меняется ни строкой — `Principal
 * .departmentObjectIds` был массивом с самого начала, менялось только то, откуда он берётся
 * (ADR 0144, решение 3).
 *
 ...
 *
 * `ORDER BY` внутри агрегата — не косметика. Порядок строк без него определяет план запроса, а не
 * запрос, и один и тот же принципал приходил бы с одинаковым набором в разном порядке: `/auth/me`
 * менял бы ответ от запроса к запросу, ломая сравнение снимков на клиенте и в тестах, а разница
 * «набор изменился» считалась бы по перестановке. Сортировка по самому идентификатору, а не по
 * коду или имени объекта: набор уходит в `IN` и в токен, где человеку он не показывается, а
 * join ради порядка стоил бы запросом на каждом обращении к принципалу.
 */
export const departmentObjectIdsExpr = sql<string[]>`(
  SELECT coalesce(
    array_agg(DISTINCT dco.construction_object_id ORDER BY dco.construction_object_id),
    '{}'
  )
  FROM user_departments ud
  JOIN department_construction_objects dco ON dco.department_id = ud.department_id
  WHERE ud.user_id = ${users.id}
)`;
 ...
 * каждом запросе и иначе дожил бы до истечения с прежней областью.
 *
 * Возвращает учётки, чьи refresh-сессии обязаны быть отозваны следом — `revokeAllForUsersTx` той
 * же транзакцией (ADR 0144, решение 6). Отзыв не делается прямо здесь намеренно: функция называет
 * ровно одно действие — «версия доступа отдела сдвинулась», — и она же берёт блокировку строк
 * учёток, то есть первую половину порядка `users` → `refresh_sessions` (Р12). Вторую половину
 * ставит вызывающий, и ставит непосредственно за ней: `replaceDepartmentObjects` — сразу,
 * загрузка справочника файлом — в своей транзакции.
 */
...
/** Текущий набор площадок отдела — по образцу `objectIdsOfUser`, той же осью с другой стороны. */
export async function objectIdsOfDepartment(tx: Tx, departmentId: string): Promise<string[]> {
  const rows = await tx
    .select({ id: departmentConstructionObjects.constructionObjectId })
    .from(departmentConstructionObjects)
    .where(eq(departmentConstructionObjects.departmentId, departmentId));
  return rows.map((r) => r.id);
}

/**
 * Порог набора площадок (ADR 0144, Р5) — от опечатки в клиенте, а не от рабочего случая: площадок
 * у отдела единицы. Проверяется здесь, у единственного писателя, а не только схемой тела: писателей
 * два — карточка и загрузка справочника файлом, — и обязательство, живущее у одного, второму не
 * достанется. Значение то же, что в `departmentObjectIdsSchema` контракта; расходиться им нельзя,
 * и если порог однажды меняют, меняют оба места.
 */
const DEPARTMENT_OBJECTS_LIMIT = 50;

/** Разница набора площадок — **кодами** объектов: её читает человек в журнале, а не машина. */
export interface DepartmentObjectsDiff {
  /** Коды выданных отделу площадок, по алфавиту. Пусто — не добавляли. */
  added: string[];
  /** Коды снятых площадок, по алфавиту. Пусто — не снимали. */
  removed: string[];
}

/**
 * Набор не менялся. Отдельным значением, потому что читателей у него трое: сама функция (набор
 * прислали, но он совпал с текущим) и оба писателя (поля в запросе не было вовсе — «не трогать»).
 * Все три случая означают для журнала одно и то же — площадки этой правкой не двигались, — и
 * собирать пустые массивы порознь значило бы дать им разойтись.
 *
 * Значение общее на всех: его только читают и сериализуют в журнал — дописывать в него ничего
 * нельзя, иначе пустая разница перестанет быть пустой у следующего вызывающего.
 */
export const NO_OBJECT_CHANGES: DepartmentObjectsDiff = { added: [], removed: [] };

/**
 * Заменяет набор площадок отдела целиком (ADR 0144) — **единственный писатель набора**.
 *
 * Писателей у набора два: карточка справочника (`routes/departments.ts`) и загрузка справочника
 * файлом (`directory-transfer/defs/org.ts`). Поэтому здесь лежит всё, что обязано случиться при
 * смене области отдела, — блокировка, разница, проекция, `authVersion` и отзыв сессий, — а не
 * раздаётся обязательствами по вызывающим: обязательство, живущее у одного из двух, второму не
 * достанется, и первый же импорт сменил бы область молча, не погасив ни одной сессии (Р3).
 *
 * Возвращает разницу кодами объектов для предметного аудита (Р10): нигде больше она не хранится —
 * карточка отдела показывает только «сейчас», и вопрос «кто и когда снял отделу эту площадку»
 * отвечается единственно журналом.
 *
 * Порядок шагов ниже — не стилистика, каждый стоит там, где стоит, по своей причине.
 */
export async function replaceDepartmentObjects(
  tx: Tx,
  departmentId: string,
  objectIds: string[],
  actorUserId: string,
): Promise<DepartmentObjectsDiff> {
  /*
   * 1. Канонизация — та же и по той же причине, что у `replaceUserObjects`: схема допускает
   *    одинаковые UUID в присланном списке. Упасть на них вставка не упадёт — дубль проглотит
   *    `onConflictDoNothing`, — и вред тише: сравнение «изменилось ли» врёт на длине (`[A, A]`
   *    против текущего `{A}` читается изменением, гасит сессии и пишет пустой аудит), проекция в
   *    колонку считает набор из двух и ставит `NULL` вместо единственной площадки, а в журнал
   *    доступа уезжает `added: [A, A, B]`. Всё это закреплено db-тестом набора.
   */
  const ids = [...new Set(objectIds)];

  /*
   * 2. Блокировка строки отдела. Без неё два параллельных изменения читают один и тот же старый
   *    набор: итог остаётся верным (побеждает записавший последним), но разница второго считается
   *    от устаревшего состояния — и в журнал уходит «добавил X», когда X уже был. Блокируется
   *    отдел, а не таблица связи: у набора нет строки, которую можно занять, пока он пуст.
   *
   *    Отсутствие строки — 404 здесь, а не у вызывающего: маршрут проверяет отдел до правки, но
   *    импорт заводит и правит записи пакетом, и «отдел исчез между чтением и записью» обязан
   *    отвечать одинаково с обеих сторон.
   */
  const [locked] = await tx
    ...
    .for('update');
  if (!locked) throw err.notFound('Отдел не найден');

  // 3. Текущий набор и разница — уже под блокировкой, то есть от состояния, которое до конца
  //    транзакции никто не сдвинет.
  const before = new Set(await objectIdsOfDepartment(tx, departmentId));
  const addedIds = ids.filter((id) => !before.has(id));
  const removedIds = [...before].filter((id) => !ids.includes(id));

  /*
   * 4. Существование объектов и порог. Активность **не** проверяется намеренно — тем же правилом,
   *    что у объектов учётки (`assertAllObjectsExist`): привязка описывает зону ответственности, а
   *    не готовность площадки принимать заявки прямо сейчас, и деактивация объекта не должна молча
   *    обрезать область сотрудникам отдела (Р4).
   *
   *    Порог спрашивается ПЕРВЫМ: он не ходит в базу и отвечает на «список слишком длинный» тем
   *    самым словом, а проверка существования на списке из сотни неизвестных идентификаторов
   *    ответила бы «Объект не найден» — правдой, из которой человек не поймёт, что дело в длине.
   */
  if (ids.length > DEPARTMENT_OBJECTS_LIMIT) {
    const text = `Площадок у отдела не больше ${DEPARTMENT_OBJECTS_LIMIT}`;
    throw err.badRequest(text, { constructionObjectIds: text });
  }
  await assertAllObjectsExist(tx, ids);

  /*
   * Набор не изменился — не пишем ничего. Это не оптимизация: перезапись набора теми же строками
   * подменила бы `created_by`/`created_at` привязок, то есть историю «кто и когда выдал отделу эту
   * площадку», а `authVersion` и отзыв сессий выкинули бы из портала всех сотрудников отдела из-за
   * правки, их области не касавшейся (правка названия шлёт набор целиком — он в теле всегда).
   */
  if (addedIds.length === 0 && removedIds.length === 0) return NO_OBJECT_CHANGES;

  /*
   * 5. Проекция в колонку `departments.construction_object_id` (Р9) — единственный объект, когда в
   *    наборе ровно один, иначе `NULL`. Колонка живёт один релиз ради предыдущей версии кода,
   *    которая набора не знает и читает только её.
   *
   *    ⚠ ШАГИ 5 И 6 МЕСТАМИ НЕ МЕНЯТЬ. На колонке висит триггер совместимости
   *    `department_object_sync_trg` (миграция 0221): он ловит запись колонки и ЗЕРКАЛИТ её в
   *    таблицу связи — сносит набор целиком и кладёт вместо него одну строку проекции. Он не
   *    отличает запись старого кода от нашей и не должен: его задача — не потерять правку
   *    предыдущей версии в окне выката. Поэтому проекция пишется ПЕРВОЙ, а полный набор ложится
   *    ПОСЛЕ неё, поверх того, что оставил триггер. Переставь шаги местами — и набор из нескольких
   *    площадок схлопнулся бы в одну (или в ноль) сразу после записи, причём молча и только на
   *    боевой схеме: без триггера тесты на голой таблице этого не покажут.
   */
  await tx
    .update(departments)
    .set({
      constructionObjectId: ids.length === 1 ? ids[0]! : null,
      updatedAt: new Date(),
    })
    .where(eq(departments.id, departmentId));

  // 6. Замена набора — целиком, приёмом `replaceUserObjects`: клиент прислал то, что должно
  //    остаться, разницу посчитал сервер.
  await tx
    .delete(departmentConstructionObjects)
    .where(eq(departmentConstructionObjects.departmentId, departmentId));
  if (ids.length > 0) {
    await tx
      .insert(departmentConstructionObjects)
      .values(
        ids.map((constructionObjectId) => ({
          departmentId,
          constructionObjectId,
  ...
  /*
   * 7. Область сменилась — гасим доступ, и обязательны оба действия, потому что отвечают они на
   *    разное: `authVersion + 1` обесценивает уже выданные access-токены (иначе прежняя область
   *    дожила бы до их истечения), а отзыв refresh-сессий не даёт молча выписать новый токен по
   *    живой сессии.
   *
   *    Порядок именно такой: `markDepartmentScopeChanged` блокирует строки учёток (`UPDATE users`),
   *    и только после этого трогаются `refresh_sessions` — единый порядок блокировок
   *    `users` → `refresh_sessions` (Р12), встречный к которому даёт взаимную блокировку с входом
   *    и ротацией токена, то есть случайные 500 в портале.
   *
   *    Отзыв идёт В ЭТОЙ ЖЕ транзакции (Р6, пересмотр прежнего правила «чистка сессий живёт вне
   *    транзакции»): после коммита он негарантирован — правка записана, `UPDATE` по сессиям упал,
   *    и повторный прогон импорта изменений уже не увидит, — а откат правки обязан откатывать и
   *    отзыв: если область не сменилась, гасить нечего.
   */
  const members = await markDepartmentScopeChanged(tx, departmentId);
  await revokeAllForUsersTx(tx, members);

  /*
   * 8. Разница — кодами объектов: журнал читает человек, и «снята площадка 0f3a-…» ему не
   *    отвечает ни на что. Снятые объекты заведомо существуют (внешний ключ `RESTRICT` не даёт
   *    удалить объект, пока он числится площадкой), выданные проверены шагом 4, поэтому запрос
   *    один на обе половины разницы.
   */
  const rows = await tx
    .select({ id: constructionObjects.id, code: constructionObjects.code })
    .from(constructionObjects)
    .where(inArray(constructionObjects.id, [...addedIds, ...removedIds]));
  const codeById = new Map(rows.map((r) => [r.id, r.code]));
  // Идентификатор запасным значением: строки в аудите не должно недоставать даже там, где объект
  // исчез между записью и чтением кодов, — «что-то безымянное сняли» хуже, чем сырой UUID.
  const codes = (list: string[]): string[] =>
    list.map((id) => codeById.get(id) ?? id).sort((a, b) => a.localeCompare(b, 'ru'));
  return { added: codes(addedIds), removed: codes(removedIds) };
}
 ...
 * учётки. Назначение руководителем того, кто уже состоял в отделе, области не меняет, но в набор
 * затронутых попадает: лишний отзыв токена дешевле забытого, а связь у него всё равно изменилась.
 *
 * **Сессии гасит эта функция, а не маршрут** (ADR 0144, решение 6). Раньше отзыв висел на
 * `routes/departments.ts` после коммита; с переводом площадок на транзакционный отзыв маршрутный
 * `revokeScopeChanged` снят целиком — иначе половина правок гасила бы сессии дважды, а вторая
 * половина не гасила бы вовсе. Не перенести отзыв сюда означало бы оставить правку ОДНИХ
 * руководителей (набор площадок не трогали, `replaceDepartmentObjects` не звался) без отзыва
 * сессий вовсе: снятый руководитель дообновлял бы токен по живой сессии до её истечения.
 */
export async function replaceDepartmentHeads(
  ...
  // …а одного `authVersion` не хватило бы наоборот: живая refresh-сессия прозрачно выдаст новый
  // access-токен с новой версией, и человек останется в портале — просто с другой областью. Отзыв
  // идёт следом за `UPDATE users` и в той же транзакции: порядок блокировок
  // `users` → `refresh_sessions` (Р12), откат правки откатывает и отзыв (Р6).
  await revokeAllForUsersTx(tx, affected);
  return affected;
}
