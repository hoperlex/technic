...
/**
 * Дни заказа техники на объект (ADR 0100): чтение плана по дням и сверка его с самой заявкой.
 *
 * Заведены дни были ради линейной техники, но правило с тех пор шире: день срока кладут в рейс у
 * любого заказа техники на объект — и у линейного, и у того, что стоит на площадке неделями, —
 * потому что 4-П на весь период выписывают и тому и другому. Признак линейности решает другое
 * (режим ЭСМ-2, занятость в гараже) и на перечень дней не влияет. Дней не бывает только у
 * грузоперевозки: там рейс и есть сама работа.
 *
...
export interface LinearRequestState extends LinearDaySubject {
   ...
   * и расхождение с этой законно у любого заказа техники на объект: на разные дни срока выходят
   * разные единицы, и отказ такому дню отправил бы выезд мимо портала. Поэтому расхождение
   * помечается (`otherVehicle`), а не отклоняется; линейная техника была лишь первым, ради кого
   * эту пометку завели.
   */
 ...
 * планирования. Отличие ровно одно: машину нового рейса называет человек, потому что у заказа
 * техники на объект назначение — машина по умолчанию, а на разные дни выходят разные единицы
 * (ADR 0100 §4).
 *
 ...
 *
 * Наружу вынесена ради выписки 4-П на весь период (`vehicle-request-day-batch.ts`): там одной и той
 * же машиной заводится рейс на каждый день срока, и спрашивать про неё внутри цикла значило бы
 * читать одну и ту же строку полсотни раз, а отказать — посреди уже заведённых рейсов, каждый из
 * которых успел сжечь свой номер «Р-» из последовательности. Поэтому проверка стоит один раз и до
 * цикла: машина у пакета одна, и отказ по ней обязан прийти до первой записи.
 */
export async function assertDayRouteVehicle(tx: Tx, vehicleId: string): Promise<void> {
  const [row] = await tx
    .select({
      ownership: vehicles.ownership,
...
export async function loadRequestDays(
  ...
  /*
   * Дни выводятся из срока у любого заказа техники на объект — линейность на это не влияет:
   * 4-П на весь период выписывают и нелинейному заказу, а значит дни срока у него законны. Пустой
   * срок остаётся у грузоперевозки: у неё момент подачи, а не отрезок календаря, и дней не бывает
   * вовсе (`linearDaysBlocker`) — построить ей таблицу по сроку значило бы предложить действие,
   * которое сервер отклонит.
   *
   * Подставить сюда пустой срок заказу, у которого он есть, нельзя ещё и потому, что тогда всё
   * распланированное уходит в `outOfTerm`: день внутри собственного срока показался бы «за сроком»,
   * а рядом стоящая сверка считает такие дни обречёнными.
   *
   * Распланированное при этом показывается всегда — даже у отменённой заявки и у заказа,
   * переведённого на арендную машину: снятые дни исчезнут сами (сверка), а оставшийся в выданном
   * бланке день обязан быть виден.
   */
  const term =
    request.requestType === 'special_equipment' ? request : { dateFrom: undefined, dateTo: null };
 ...
 * два правила: общий запрет `linearDaysBlocker` (тип заявки, архив, статус, принадлежность машины,
 * наличие срока) и подённая граница `isShiftDayInTerm`. Поэтому передаётся
 ...
 *
 * Нужен ради холостого хода сверки: она зовётся пятью местами, и у заявки, дни которой ни разу не
 * ставили в рейс, обязана кончаться первым же запросом. Читать ради этого субъект допустимости
 * (пять join'ов по заявке) значило бы платить за него на каждом переводе статуса любой заявки
 * портала.
 */
 ...
 * запрос отвечает пустотой у всех заявок, кроме тех, чьи дни уже стоят в рейсах: `work_date` у
 * остальных NULL.
 *
