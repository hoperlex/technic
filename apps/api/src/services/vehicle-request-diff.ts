...
/**
 * Как машина названа в истории: подпись из справочника, её категория и арендодатель — без
 * последнего две машины одной категории у разных арендодателей в истории неразличимы.
 *
 * Категория пишется потому, что технику подбирают по типу, а не по ТТХ (ADR 0045): вышла машина
 * заказанной категории или соседней — видно только по ней, а подпись собственной машины это
 * госномер. Пишется всегда, а не только при расхождении: дифф сравнивает два назначения и
 * заказанной категории не знает, да и «чем именно закрыли заявку» интересно само по себе.
 * Незаполненная категория и категория, которая и есть подпись, не дублируются.
 */
function vehicleValue(a: VehicleRequestAssignmentDto): string {
  const title = assignmentTitle(a);
  return [title, a.categoryName !== title ? a.categoryName : null, a.lessorName]
    .filter(Boolean)
    .join(' · ');
}
