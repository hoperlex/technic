...
function fieldChangesOf(metadata: unknown): RequestChangeDto[] {
  ...
  return (
    raw
       ...
       *
       * РАЗБИРАЕТСЯ `c.files`, А НЕ САМА ЗАПИСЬ: `fileEntriesOf` ждёт МАССИВ пар, и переданный ему
       * объект изменения проваливает первую же проверку `Array.isArray` — молча, ответом
       * `undefined`. Правило имени тогда не получает ни одного идентификатора и гасить ему нечего:
       * имя запертого документа уходит в ленту истории заявки целым. Ошибка не отзывается ни типом
       * (вход объявлен `unknown` — иначе общий разбор чужого JSON не написать), ни падением, и
       * ловит её только сцена оргтехники в `file-quarantine-history.db.test.ts`.
       */
      .map((c) => {
        const files = fileEntriesOf(c.files);
        // Без пар запись отдаётся как лежит, с парами — уже очищенными: ровно то же, что делает
        // `changesOf` общей ленты, и разойтись этим двум разборам нельзя.
