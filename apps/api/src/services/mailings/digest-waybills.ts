 ...
 *
 * Исключение ровно одно: распланированный день заказа на объект. Начато оно линейной техникой
 * (ADR 0100 §12) — недельного листа у неё может не быть вовсе, ЭСМ-2 такому заказу выписывают по
 * требованию, а работа идёт днями, — но признаком линейности дни быть перестали: 4-П выписывается
 * на каждый день срока любого заказа на объект, и отбор по листам потерял бы такую работу целиком
 * и молча.
 *
 * Поэтому у таблицы «Техника на объектах» два источника, и во втором строка стоит в том числе без
 * документа: день, на который лист ещё не выписан, печатается номером рейса с пометкой. Это не
 * отмена правила выше, а его цена — молчаливая потеря в сводке хуже лишней строки: несделанная
 * работа выглядит как несуществующая.
 */
...
const ONSITE_HEAD = [
  // «Документ», а не «Лист ЭСМ-2»: у дневных строк в графу приходят и номера дневных 4-П, и
 ...
 * таблицу. Порядок общий — объект, потом первый день строки: недельный лист и распланированные дни
...
interface OnsiteRow {
   ...
   * (машина + машинист). У дневных строк поля нет — неделя у них не документ, а набор выездов, и
   * разрезать её нечему.
   */
 ...
 * выписывается только по нему (`waybill-esm2.ts`), а день строки состава (`work_date`) бывает
 * только у него же: у грузоперевозки день несёт сам рейс, и в строке состава он NULL (миграция
 * 0127). Линейность здесь ни при чём — дни заводятся любому заказу на объект, а отдела не бывает
 * ни у линейного, ни у обычного. Второй CHECK, `vehicle_requests_customer_check`, при этом
 * гарантирует, что объект у такой заявки заполнен всегда, — поэтому и `leftJoin` на справочник
 * объектов промахнуться не может.
 *
...
/**
 * Техника на объектах: недельные листы ЭСМ-2 и распланированные дни заказов, попавшие в окно.
 *
 ...
 *
 * Сложить их можно только потому, что пересечься они не могут: неделя, у которой внутри окна есть
 * распланированные дни, из первой половины отсеяна (`esm2Rows`), и заказ приходит в таблицу ровно
 * одним способом. Без этого правила заказ, которому дни и недельный лист заведены оба, вставал бы
 * в письмо двумя строками про одну и ту же работу.
 */
 ...
 *
 * Осознанное последствие правила «неделя уступает дням» (`esm2Rows`): у заказа, чьи дни попали в
 * окно, недельных строк в таблице нет вовсе, и пояснения про смену машиниста внутри недели он не
 * получит. Пояснять там и нечего — двух строк, которые читались бы как задвоение, тоже нет, а сама
 * смена видна в дневных строках прямо: машину называет отдельная строка каждой единицы, а
 * сменившихся за дни машинистов — графа «Машинист» (`driversCell`).
 */
 ...
 *
 * Половина уступающая: заказ, чьи дни распланированы внутри окна, отсюда уходит целиком и
 * показывается дневными строками (условие в `where` ниже).
 */
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        // решение 12 из портала и убрало. Условие держится и тогда, когда дней в окне нет вовсе:
        // ЭСМ-2 такому заказу выписывают по требованию, и его номер — про моточасы недели, а не
        // про то, кто и чем работает в эти дни. Признак читается у **заказанного** типа
        // (ADR 0100 §1), а не у типа машины листа: как ведётся заказ, решает заказ, а не
        // подобранная под него единица.
        //
        ...
        // Неделя, у которой внутри окна есть распланированные дни, уступает строкам этих дней.
        // Дни бывают у **любого** заказа на объект — портал выписывает 4-П на каждый день срока и
        // нелинейному тоже, — поэтому одной линейности выше уже мало: без этого условия такой
        // заказ пришёл бы в письмо дважды, строкой недели и строками своих дней.
        //
        // Уступает именно неделя, потому что дневная строка несёт больше: машину, вышедшую в этот
        // день (она бывает не той, что назначена заявке), номер её 4-П либо пометку «лист не
        // выписан» и водителя дня. У дня без листа документа нет вовсе, и потеряй письмо дневную
        // половину — пропал бы сигнал «машина завтра выйдет без бумаги», ради которого сводку и
        // читают накануне. Цена обратная и названа прямо: номер недельного ЭСМ-2 у такого заказа
        // в письме не печатается, за ним идут в журнал листов или в карточку заявки — ссылка на
        // неё стоит в той же строке.
        //
        // Дни считаются окном письма, а не сроком заказа, и теми же границами, какими их отбирает
        // вторая половина (`linearDayRows`). Разойдись эти два условия — заказ либо исчез бы из
        // письма целиком, либо вернулся бы в него двумя строками, то есть ровно тем, от чего
        // условие и заведено.
        //
        // Отсюда же граница правила, видная только в окне шире суток: неделя, распланированная
        // наполовину, покажется своими днями, а её нераспланированный хвост в письмо не попадёт.
        // У сводки на день такого состояния нет — день либо распланирован, либо строку даёт
        // неделя, — а в широком окне это честнее обратного: строка недели сказала бы «машина
        // работает всю неделю» там, где на остаток недели не поставлено ни рейса, ни листа.
        sql`NOT EXISTS (
          SELECT 1 FROM vehicle_route_requests vrr
          WHERE vrr.request_id = ${vehicleRequests.id}
            AND vrr.work_date BETWEEN ${ctx.windowFrom}::date AND ${ctx.windowTo}::date
        )`,
        vehicleRequestScope(ctx),
      ),
    )
...
/**
 * Второй источник: распланированные дни заказов на объект, попавшие в окно (ADR 0100 §12).
 *
 * Условия линейности здесь нет и никогда не было, и это не упущение: день кладётся в маршрут дня
 * (`vehicle_route_requests.work_date`, миграция 0127) и печатается в 4-П этого рейса — начинали
 * так вести линейный заказ, а теперь 4-П выписывается на каждый день срока любого заказа на
 * объект. Половина показывает всякий такой день, а недельная строка того же заказа ей уступает
 * (`esm2Rows`): день знает машину своего рейса, номер её листа и водителя этого дня, а неделя —
 * только бланк недели.
 *
 * Единственное место письма, где строка стоит без документа, и это осознанная плата. Недельного
 * листа у заказа может не быть вовсе — линейному ЭСМ-2 выписывают по требованию, — и отбор по
 * листам потерял бы работу целиком и молча; день, на который лист ещё не выписан, показывается
 * номером рейса с пометкой — в этом и польза сводки, видно, что машина завтра выйдет без бумаги.
 *
