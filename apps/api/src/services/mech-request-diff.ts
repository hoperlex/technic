...
/**
 * Виза площадки и её отзыв (`mech_request.approve`, `mech_request.approval_revoke`; план
 * `docs/mechanization-approval-and-grants-plan.md`, Р12).
 *
 * Обычная пара «было → стало» по одному ключу `approval`: у подписи «было» есть — её отсутствие, —
 * и лента карточки читается ровно как жизнь заявки («— → Иванов И. И.», потом «Иванов И. И. → —»).
 * Различает два события вид истории, а не имя ключа: поле одно, а действий два — тем же приёмом
 * живёт пара «выдача и её снятие».
 *
 * Имя подписавшего, а не его идентификатор: история читается человеком, и `uuid` в ней означал бы
 * запись, по которой нельзя ответить на вопрос «кто согласовал» без второго запроса.
 *
 * **Событие — единственный носитель факта** (Р12, строгий аудит): после отзыва колонки подписи
 * пусты, и что она была, кто её поставил и когда — не помнит больше ничто.
 */
export function mechApprovalChanges(
  before: string | null,
  after: string | null,
): RequestChangeDto[] {
  const diff = changeSet();
  diff.changed('approval', before ?? EMPTY, after ?? EMPTY);
  return diff.changes;
}
