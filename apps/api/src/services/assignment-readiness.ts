 ...
 *
 * ГДЕ ОН СХОДИТСЯ С ДВЕРЬЮ. Предикат Р20 спрашивают обе стороны. Дверь активации — своей
 * проверкой `requireHistoryPopulationReady` ([assignment-mode.ts](assignment-mode.ts)), и она идёт
 * первой в ряду, раньше меток загрязнения и дня пересчёта. Сводка — тем же
 * {@link ASSIGNMENT_READINESS_POPULATION} и той же формой счёта (`empty` с назначением, `empty`
 * без назначения, `materialized`), но заранее и без блокировок. Одно правило на две стороны — это
 * решение, а не совпадение: пока предикат жил только здесь, заявка в состоянии `empty` проходила
 * дверь насквозь (меток у неё нет, а из запроса устаревания состояние `empty` исключено явно — у
 * него и `validated_on` нет по ограничению схемы), и отчёт отвечал за дверь. Теперь за себя
 * отвечает дверь, а отчёт задаёт тот же вопрос до окна выката.
 *
 * Расхождение при этом остаётся односторонним, и направление важно: отчёт может сказать «не
 * готов» там, где дверь пропустит, но не наоборот. Обратное расхождение было бы ложным зелёным, и
 * держит его тест ([assignment-report.db.test.ts](../../test/assignment-report.db.test.ts)).
 *
