...
const REFERENCING_TABLE_LABELS: Record<string, string> = {
  ...
  /*
   * Отдел ссылается на объект дважды: таблицей связи и совместимой проекцией
   * `departments.construction_object_id` (ADR 0144, решение 4). При наборе из ОДНОЙ площадки
   * `RESTRICT` первым срабатывает у колонки, и без этой строки человек получил бы безликое «на
   * запись ссылаются другие данные». Подпись говорит про площадки, а не про отделы вообще:
   * единственная ссылка `departments` на справочник объектов — эта колонка. Строка уходит вместе
   * с колонкой следующим релизом; путь пользователя при этом другой — маршрут объекта отказывает
   * раньше и подробнее, и сюда доходит только прямой `DELETE` мимо него.
   */
  departments: 'площадки отделов',
  // Площадки отдела держат объект `RESTRICT` (ADR 0144, решение 2): каскад снял бы площадку у
  // целого отдела молча — мимо карточки, в которой её задавали. Маршрут объекта отказывает раньше
  // и подробнее, эта строка — на случай, когда до ограничения всё же дошли.
  department_construction_objects: 'площадки отделов',
  // Заявка механизации держит модель ссылкой `ON DELETE RESTRICT` (ADR 0156, миграция 0251):
  // заявка — это история аренды, и снос строки справочника не имеет права унести из неё предмет.
  // Без этой подписи держатель справочника читал бы «на запись ссылаются другие данные» и не знал
  // бы, где искать.
  mech_requests: 'заявки механизации',
  ...
  // прицепов с остальным порталом (рейс и лист хранят текст-снимок, а не ключ). Живых таких
  // ссылок портал больше не оставляет: списание, уход машины в архив и перевод её на «форму № 3»
  // привязку снимают (план `docs/vehicle-trailers-plan.md`, §4.2.3). Досюда доходит то, что легло
  // в базу раньше этого правила или помимо портала, — и человек обязан прочитать, что именно
  // держит машину, а не общее «ссылаются другие данные».
  vehicle_trailers: 'прицепы, закреплённые за машиной',
