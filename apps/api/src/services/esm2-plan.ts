 ...
 *
 * С правилом `trim` (Р5, Р6 плана `docs/vehicle-request-actual-end-date-plan.md`) к ним добавился
 * ещё один — «сократить»: лист, которому хватает правки последнего дня. Он не «сошёлся» и не
 * «сгорел», и путать его ни с тем, ни с другим нельзя: в `kept` он спрятал бы правку строгого
 * бланка, в `cancel` — сжёг бы номер, которого сокращение не расходует.
 */
export interface Esm2SheetPlan {
  ...
  kept: string[];
  /**
   * Листы, которым хватает сокращения периода (Р6): состав тот же, начало то же, конец ближе, а
   * отнятые дни не нужны никакому другому ожиданию. Номер не расходуется, замены не выписывается,
   * графа замен у такого листа пуста — документ остаётся собой и лишь теряет дни с конца.
   *
   * Сокращаются только листы **внутри** области сверки, и отдельного условия для этого нет: лист
   * вне области отобран раньше, а новый конец лежит внутри прежнего периода, то есть внутри той
   * же области по построению.
   *
   * Поле **обязательное** (Э3): его читает исполнитель правки, и необязательность означала бы
   * дверь, которая молча ничего не сокращает — план посчитан, сокращать есть что, а `undefined`
   * прошёл бы через `?? []` как «нечего». Три двери, державшие пустой план литералом
   * (`EMPTY_SHEET_PLAN` в `assignment-period.ts`, `assignment-crew.ts`, `assignment-correction.ts`),
   * называют пустой список сами.
   */
  trim: Esm2Trim[];
  /**
 ...
 * не трогается.
 *
 * Исключение из «не правится никогда» ровно одно и названо поимённо (Р5, Р6 плана
 ...
 * работа не знала. Условий пять, и нужны все сразу: состав совпал теми же сверками, что решают
 * `kept` против `cancel`; начало не двигается; новый конец не раньше начала и строго раньше
 * прежнего; лист целиком внутри области сверки; **отнятые дни не нужны никакому другому
 * ожиданию**. Всё прочее — по-прежнему пара «аннулировать плюс выписать»: сменилась машина или
 * человек, сдвинулось начало, период расширяется. **Правка умеет только отнимать дни**, и
 * раздвинуть выданный бланк она не может ни при каких входных данных.
 *
 * Пятое условие и отделяет закрытие от разреза. У закрытия фактической датой хвост уходит в
 * пустоту: работы в эти дни не было и бумаги на них не будет ни у кого. У смены машиниста со
 * среды хвост забирает соседний документ — и вот там правка запрещена. Причина не в записи, а в
 * бумаге: печатный экземпляр остаётся с прежним, невырезанным периодом (Р13, решение В8), и после
 * правки на площадке лежали бы два действующих бланка, у обоих в графе «Период работы» стояла бы
 * среда, и один и тот же день можно было бы вписать дважды. Аннулирование этого не допускает:
 * старый бланк изымается из оборота целиком.
 *
 ...
 *
 * Ожидания здесь считаются по **разрезу состава**, то есть по режиму `auto`: портал сам решает,
 * сколько бумаги нужно заявке. У заказа с бумагой по требованию решения такого нет, и ожидания ему
 * считает {@link esm2RequestedSheetPlan} — тем же телом сверки и из уже выписанного.
 */
export function esm2SheetPlan(
  ...
  return esm2PlanOfWanted(
    wantedSheets(esm2PaperSegments(segments, term, context.ownershipByVehicle)),
    existing,
    context,
  );
}
...
/**
 * Ожидания заказа с бумагой **по требованию** — из уже выписанного, подрезанного сроком
 * (ADR 0100 §5, Р14; дыра Э10 плана `docs/vehicle-request-actual-end-date-plan.md`).
 *
 * ЗАЧЕМ ОТДЕЛЬНЫЙ СПОСОБ. У линейного заказа недели называет человек при выписке, а не срок
 * заявки: разреза, из которого выводится «сколько бумаги нужно», у него нет вовсе. Спроси мы
 * ожидания у отрезков — портал выписал бы линейному заказу недели, которых никто не просил, а у
 * заказа, ушедшего в работу без машиниста, не нашёл бы ожиданий ни на один день и **сжёг бы**
 * выданные по просьбе бланки: отрезок без человека бумаги не ожидает (`wantedSheets`). Оба исхода
 * — порча строгой отчётности, и оба молчаливые.
 *
 * ПРАВИЛО ТО ЖЕ, ЧТО У НЕДЕЛЬНОЙ СВЕРКИ (`esm2RequestedPeriods` контрактов), и переписано оно
 * здесь не ради второй редакции, а ради состава: недельная сверка отвечает периодами, а разрезу
 * нужен полный лист — с машиной и человеком, — иначе `sheetMatchesWanted` не с чем сверять.
 * Состав берётся **из самого листа**, и это ровно то же, что делает недельная сторона, отказываясь
 * сверять у `on_demand` машину и человека (ADR 0100 §6, §7): неделю на объекте закрывает та
 * единица и тот машинист, которых выбрал человек, а не назначение заявки. Отсюда и следствие,
 * ради которого всё затевалось: сокращение срока не жжёт номер, а **правит период** — состав
 * совпал с самим собой, начало то же, конец ближе.
 *
 * Подрезка сроком и разрез месячной границей — те же и в том же порядке, что у контрактной
 * функции: неделя, целиком выпавшая из срока, в ответ не входит (её лист аннулируется без
 * замены), а пересечённая возвращается подрезанной. Расширять неделю подрезка не умеет
 * намеренно: продлённый срок линейному заказу новых листов не приносит.
 */
export function esm2RequestedSheets(
  existing: readonly Esm2ExistingSheet[],
  term: AssignmentTerm,
): Esm2WantedSheet[] {
  // Пустая дата окончания — однодневный срок: так её читают и `esm2Periods`, и подрезка контрактов.
  const last = term.dateTo || term.dateFrom;
  if (!term.dateFrom || last < term.dateFrom) return [];

  const wanted: Esm2WantedSheet[] = [];
  for (const sheet of existing) {
    const from = sheet.periodFrom < term.dateFrom ? term.dateFrom : sheet.periodFrom;
     ...
     * лист внутри своей недели по построению.
     */
    for (const period of esm2Periods(from, to)) {
      wanted.push({
        ...
        driver: { state: 'set', personId: sheet.driverPersonId },
        // Арендной бумаги у `on_demand` не бывает: аренда даёт режим `none`, и листа тут нет вовсе.
        responsibility: 'portal',
      });
    }
  }
...
/**
 * План листов заказа с бумагой **по требованию**: те же три исхода, что и у разреза, но ожидания
 * приходят из выписанного, а не из состава (ADR 0100 §5).
 *
 * Сверка, замки и область — общие с `esm2SheetPlan` и одним телом: режимы отличаются ровно
 * набором ожиданий, и вторая копия «что делать с выданным листом» разошлась бы с первой на первой
 * же правке правила.
 *
 * Новых недель этот план не заводит по построению: каждое ожидание вырезано из уже существующего
 * листа, и выписка достаётся лишь тому куску, чей лист сверка гасит целиком (сдвинулось начало
 * срока — документ другой, и правкой это не решается). Ровно так же поступает и недельная сторона.
 */
export function esm2RequestedSheetPlan(
  existing: readonly Esm2ExistingSheet[],
  term: AssignmentTerm,
  context: Esm2PlanContext,
): Esm2SheetPlan {
  return esm2PlanOfWanted(esm2RequestedSheets(existing, term), existing, context);
}

/**
 * Сверка ожиданий с выписанным — общее тело обоих режимов бумаги.
 *
 * Ожидания сюда приходят готовыми и больше ни о чём не спрашиваются: откуда они взялись — из
 * разреза состава (`auto`) или из просьбы человека (`on_demand`) — знает вызывающий, а «что делать
 * с выданным листом» у режимов одно и то же (ADR 0100 §5: режим меняет набор недель, и только его).
 */
function esm2PlanOfWanted(
  wanted: readonly Esm2WantedSheet[],
  existing: readonly Esm2ExistingSheet[],
  context: Esm2PlanContext,
): Esm2SheetPlan {
  ...
  /** Листы, не совпавшие с ожиданием точно: им во втором проходе решается «сократить или сжечь». */
  const pending: Esm2ExistingSheet[] = [];
  /**
   ...
   *
   * Проходов два, и второй не оптимизация, а условие правильности: сокращать лист можно только в
   * то ожидание, за которым ещё не стоит документа, а «не стоит» знает лишь **весь** первый
   * проход. Реши мы это одним циклом — лист пн–вс, встреченный раньше своего точного двойника
   * пн–вт, сократился бы в занятое ожидание, и у заявки оказалось бы два действующих документа на
   * одни и те же дни.
   *
   * В `freed` отнятые дни не попадают, и это прямое следствие пятого условия: сокращение
   * разрешено только тогда, когда хвоста не ждёт ни одно ожидание, — значит и выписывать на эти
   * дни нечего. `freed` заведён под другое: он открывает выписку замены там, где документ **сгорел
   * целиком**. Ослабь кто-нибудь пятое условие — освобождение хвоста придётся вернуть сюда же,
  ...
  for (const sheet of pending) {
    ...
    burned.push({ from: sheet.periodFrom, to: sheet.periodTo });
  }
  ...
  // Ожидания возвращаются копией: набор пришёл параметром, а читатели плана вправе считать его
  // своим — общий массив связал бы два режима бумаги одной ссылкой без всякой на то причины.
  return { wanted: [...wanted], cancel, issue, trim, kept, locked, outOfScope };
}
...
export function sheetMatchesWanted(sheet: Esm2ExistingSheet, wanted: Esm2WantedSheet): boolean {
  ...
  return sameSheetComposition(sheet, wanted);
}

/**
 * В какое ожидание можно укоротить этот лист — или `null`, если нельзя (Р6).
 *
 * Функция одна на все пять условий нарочно: разложи их по двум предикатам — и первый же вызывающий
 * позовёт один без другого, а цена такой забывчивости — действующий бланк, разошедшийся с работой.
 *
 * Три условия про границы: первый день не двигается (сдвинутое начало — другой документ, и правкой
 * это не решается), новый последний день не раньше первого (лист без дней не сокращают, а
 * аннулируют) и строго раньше прежнего (равенство — это `kept`, а больше — расширение, которого
 * правка не умеет вовсе).
 *
 * Состав спрашивается **той же** функцией, что и у совпадения: сокращение — это правка периода, а
 * не поблажка по машине и человеку. Отсюда же и поблажка `unknown`, доставшаяся ей заодно: лист
 * приблизительно восстановленного дня сокращается наравне с прочими, потому что графа человека в
 * нём напечатана, а сокращение её не трогает.
 *
 * Пятое условие — про хвост: отнятые дни не должны быть нужны **никакому** другому ожиданию. Им
 * закрытие фактической датой отделяется от разреза недели. У закрытия хвост уходит в пустоту:
 * работы в эти дни не было, бумаги на них не будет ни у кого. У смены машиниста со среды хвост
 * ср–вс забирает соседний документ — и правка там запрещена, потому что печатный экземпляр
 * остаётся с прежним, невырезанным периодом (Р13, решение В8): на площадке лежали бы два
 * действующих бланка, у обоих в графе «Период работы» стояла бы среда, и один день можно было бы
 * вписать дважды. Аннулирование этого не допускает — старый бланк уходит из оборота целиком.
 *
 * Ожидания считаются все, включая невыписываемые (`unknown`, Р19): «на эти дни бумага ожидается»
 * — это про дни, а не про то, хватит ли портала на её выписку сегодня.
 *
 * `covered` — ожидания, за которыми уже стоит документ. Это состояние плана, а не свойство пары,
 * и приходит оно параметром: сократить лист в занятое ожидание значит завести второй документ на
 * те же дни.
 */
export function sheetTrimTarget(
  sheet: Esm2ExistingSheet,
  wanted: readonly Esm2WantedSheet[],
  covered: ReadonlySet<Esm2WantedSheet>,
): Esm2WantedSheet | null {
  const target = wanted.find(
    (expected) =>
      !covered.has(expected) &&
      expected.from === sheet.periodFrom &&
      expected.to >= expected.from &&
      expected.to < sheet.periodTo &&
      sameSheetComposition(sheet, expected),
  );
  if (!target) return null;
  const tail: Esm2Period = { from: shiftDateKey(target.to, 1), to: sheet.periodTo };
  return wanted.some((expected) => periodsOverlap(expected, tail)) ? null : target;
}

/** Состав листа против состава ожидания: машина всегда, человек — кроме `unknown` (Р19). */
function sameSheetComposition(sheet: Esm2ExistingSheet, wanted: Esm2WantedSheet): boolean {
...
/**
 * Сокращаемый лист в исполняемом плане: тот же номер, тот же состав, новый последний день (Р5, Р6).
 *
 * Период здесь — **прежний**, каким лист стоит в базе сейчас, а `newTo` — каким он станет. Обе
 * даты нужны каждому потребителю плана: исполнителю — чтобы записать `period_to_original` и
 * погасить в снимке числа дней, ушедших за границу; предпросмотру — чтобы показать человеку «было
 * → стало»; журналу — чтобы объяснить, почему бланк перестал быть равен выданному. Считать
 * «прежнее» вторым чтением у каждого из них значило бы завести три ответа на один вопрос, и
 * разошлись бы они на первой же гонке.
 *
 * Номер (`displayNumber`) стоит рядом по той же причине, что и у аннулируемого: правку бланка
 * строгой отчётности журнал обязан назвать так, как она напечатана, а не идентификатором строки.
 *
 * Графы замен у сокращения нет и быть не может: номер не менялся, замены не выписывалось, и
 * искать сокращённому листу предшественника или наследника не в чем — документ остался собой.
 */
export interface Esm2ScopedTrim {
  waybillId: string;
  displayNumber: string;
  /** Период листа **до** правки: им же и проверяется, что правка отнимает дни, а не добавляет. */
  period: Esm2Period;
  /** Новый последний день: строго внутри прежнего периода и не раньше его первого дня (Р6). */
  newTo: string;
}
...
export interface Esm2ScopedPlan {
  ...
  /**
   * Третий исход рядом с «сжечь» и «выписать» (Р5): листы, которым хватает правки последнего дня.
   *
   * Список обязательный и в графе замен не участвует. В `cancel` такой лист попасть не может по
   * построению — он не гаснет, — а значит, и предшественником нового номера не станет: связь
   * замены ищется только среди сгоревших.
   */
  trim: Esm2ScopedTrim[];
}
 ...
 *
 * Правки (`trim`) проходят четвёртым списком и по первому правилу: тот же канонический порядок
 * `(from, waybillId)`, что и у аннулируемых. Порядок им нужен ровно затем же, зачем `issue` —
 * ключи: план хешируется отпечатком предпросмотра, и порядок выборки из базы менял бы отпечаток
 * у неизменившегося плана. Ни ключей, ни связей замены правка не получает — номер не менялся.
 */
export function esm2ScopedPlan(params: {
  ...
  /**
   * Сокращаемые листы. Список обязателен, а не «необязателен и по умолчанию пуст»: вызывающих у
   * сборки двое — недельная сверка и отрезковый план, — и умолчание позволило бы одному из них
   * молча растерять правки, оставив бумагу несошедшейся без единого признака.
   */
  trim: readonly Esm2ScopedTrim[];
  ...
  const trim = [...params.trim].sort((a, b) =>
    a.period.from < b.period.from
      ? -1
      : a.period.from > b.period.from
        ? 1
  ...
  return { scope: normalizeRangeSet(params.scope), cancel, issue, trim };
}
...
export function esm2ScopedPlanOfSheetPlan(params: {
  ...
  return esm2ScopedPlan({
    ...
    /*
     * Прежний период правки читается из уже прочитанных листов — тем же способом, что и период
     * сгорающего, и по той же причине: после правки `period_to` в строке уже новый, и «каким лист
     * был» второе чтение не вернёт. Пустые границы у листа, которого в карте нет, — та же
     * защитная форма, что и у `cancel`: план, назвавший чужой лист, обязан упасть на исполнении,
     * а не тихо сократить документ в неизвестно какой день.
     */
    trim: params.plan.trim.map((item) => {
      const sheet = byId.get(item.waybillId);
      return {
        waybillId: item.waybillId,
        displayNumber: params.displayNumbers.get(item.waybillId) ?? item.waybillId,
        period: {
          from: sheet?.periodFrom ?? '',
          to: sheet?.periodTo ?? '',
        },
