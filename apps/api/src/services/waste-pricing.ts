 ...
 * Оператор выбирает, чей прайс применить; без него берётся минимальная цена (ADR 0023).
 ...
 * Между операторами выбор тот же, что и в подборе по виду (ADR 0023).
 * Возвращает null, если для пары прайс не задан.
 */
 ...
 * (ADR 0022) — цена берётся по виду «Самосвал». Прайс у каждого оператора свой (ADR 0023):
