...
/**
 * Снимок цены для сохранения в заявке. Для нетарифицируемой операции (любая контейнерная —
 * установка, замена, снятие; ADR 0019) возвращает пустой снимок — так поля цены гарантированно
 * очищаются при смене типа заявки. Сумму заявки считает БД (generated-колонка `amount`).
 */
