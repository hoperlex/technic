...
import { assertMachinistSelectable, findMachinist, type MachinistOption } from './drivers';
...
async function issueEsm2Waybill(
   ...
   * на эту неделю», и ответ на него был разный: ручная выписка отказывала словами, а сверка
   * печатала бланк без фамилии машиниста — то есть недействительный документ строгой отчётности,
   * и узнавали об этом в бухгалтерии заказчика.
   *
   * Строки в `persons` нет — случай теоретический: `waybills.driver_person_id` и
   * `assignment_periods.driver_person_id` держит внешний ключ `restrict`, и стереть человека
  ...
  if (!prepared.machinist) {
    ...
    throw err.unprocessable(
      'Машиниста нет в справочнике — карточки с таким идентификатором не существует, и путевой лист на него не выписывается',
  ...
  /*
   * Снятая карточка листу не мешает, и это решение, а не упущение (план `machinist-card-removal`,
   * Р6; ADR 0164 в редакции от 14.09.2026).
   *
   * Здесь человек уже назван — либо формой, которая спросила его годность сама
   * (`assertMachinistSelectable` в двери), либо самой бумагой заявки, которую портал продолжает
   * вести. Отказать второму значило бы остановить заявку целиком: ни продлить, ни сократить, ни
   * закрыть, а машиниста у продления не спрашивают вовсе — починить такую заявку из портала нечем.
   * Ровно так прод и встал 14.09.2026: карточку сняли когда-то, а упёрлись в неё на первом же
   * продлении недели.
   *
   * Различать здесь вызывающего больше не нужно и не следует: правило о том, кого **можно назвать**,
   * живёт в дверях явного выбора, а этой точке остался её собственный инвариант — в снимке не бывает
   * пустой графы ФИО. Держится он тем, что мягкое удаление сохраняет и ФИО, и табельный номер.
   *
   * След остаётся в журнале сервера: событие редкое, а разбор «почему бумага идёт на снятого» без
   * него начинается с нуля. Человеку об этом говорит плашка карточки заявки, а не лог.
   */
  if (machinist.cardDeleted) {
    logger.warn(
      {
        requestId: params.requestId,
        vehicleId: prepared.vehicleId,
      ...
      'esm2: лист выписан на снятую карточку машиниста',
    );
  }
...
export async function buildEsm2SyncPlan(
  ...
  params: {
    ...
    /**
     * Срок, каким он **станет**: план считается по нему, а не по записанному в базе.
     *
     * Нужен там, где последствия спрашивают до правки — у диалога удаления карточки машиниста
     * (план `machinist-card-removal`, Р7): у полностью оформленного заказа с ожидающим продлением
     * нынешний план пуст, а виза по недельной заявке бумагу родит. Не передан — нынешний срок из
     * базы, и это единственный случай, когда предполагать нечего.
     */
    assumeDateTo?: string | null;
    /**
   ...
   */
  // Срок, по которому считается набор недель: предполагаемый, если о нём попросили, иначе
  // записанный. Подставляется он только когда назван (`undefined` — «не спрашивали»), а не по
  // истинности: пустой `date_to` у однодневного заказа — законное значение.
  const dateTo = params.assumeDateTo === undefined ? request.dateTo : params.assumeDateTo;
  const wanted =
    mode === 'none' || !request.dateFrom
      ? []
      : mode === 'auto'
        ? esm2Periods(request.dateFrom, dateTo)
        : esm2RequestedPeriods(sheets, request.dateFrom, dateTo);
  /*
...
export async function syncEsm2Waybills(
  ...
  params: {
    ...
    /**
     * Рукопожатия, принятые **дверью** (шаг 8, Б4): `issueKey` строкой → отпечаток набора.
     *
     * Приходят от старой двери смены техники: предупреждения ей показывает план, подпись она
     * требует сама (`PATCH /vehicle-requests/:id/assignment`), а бумагу по-прежнему выпускает эта
     * сверка — и без этого поля выписанный лист уходил бы с умолчанием «не проверяли» сразу после
     * того, как человек всё подтвердил (Р21).
     *
     * Ключ тот же самый, что показан окну: и предпросмотр, и эта сверка нумеруют выписки одним
     * каноном — индексом в плане, отсортированном по `(from, to, vehicleId, driverPersonId)`
     * ([esm2-plan.ts](./esm2-plan.ts)). Прав подпись не даёт и ничего не разрешает: исполнитель
     * ставит `acknowledged` только там, где отпечаток сошёлся с **пересчитанным** набором, а
     * разошедшийся оставляет умолчание колонки.
     */
    acknowledgements?: Readonly<Record<string, string>> | undefined;
  },
  ...
  const applied = await applyEsm2SyncPlanAndAudit(tx, scoped, {
    ...
    // Подписи двери, если она их спрашивала: лист обязан помнить, под чем он вышел (Р21).
...
export async function issueEsm2OnDemand(
  ...
  /*
   * Машинист здесь назван **формой**, и потому спрашивается именно здесь (план
   * `machinist-card-removal`, Р6). Общая точка выпуска этого правила не держит и держать не может:
   * через неё ходит и сверка, которая человека не выбирает, а наследует, — и одно правило на обе
   * дороги останавливало бы заявку вместо того, чтобы не пустить чужой выбор (ADR 0164 в редакции
   * от 14.09.2026).
   *
   * Периоды передаются **все сразу** (Р9). Переходную неделю месячного разреза закрывают два бланка
   * одной транзакцией, и человек, годный только августовскому, не годится просьбе целиком: отказ на
   * втором откатил бы первый, и состояния «на август выдали, на сентябрь нет» всё равно не вышло бы,
   * зато номер сгорел бы зря.
   */
  await assertMachinistSelectable(tx, params.driverPersonId, periods);
