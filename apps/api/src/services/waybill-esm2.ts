...
async function issueEsm2Waybill(
   ...
   * разница не в строгости, а в том, кто назвал человека.
   *
   * Ручной вход спрашивает машиниста у формы — и человека, которого в справочнике больше нет,
   * принимать не должен: список выбора его не предлагает, а пришёл он из чужой открытой вкладки.
   *
   * Сверка человека не выбирает вовсе: она переоформляет бумагу заявки и берёт того, кто уже
   * напечатан в сгоревшем листе или в прежнем листе заявки (`machinistFor`). Отказ здесь означал
   * не «выберите другого», а «продлить срок нельзя»: заявка, у машиниста которой удалили карточку,
   * переставала жить целиком — не продлевалась, не сокращалась, не закрывалась, — и починить это
   * из портала было нечем. Именно так это и проявилось на проде: карточку сняли когда-то, а
   * упёрлись в неё на первом же продлении недели.
   *
   * Недействительным бланк от этого не становится: удаление у человека мягкое, ФИО и табельный
   * номер в карточке остались, и графы листа заполняются ими же. Ради чего писался ADR 0164 —
   * пустая графа ФИО — здесь не возникает.
   */
  if (machinist.cardDeleted) {
    if (params.requester.by === 'human') {
      throw err.unprocessable(
        `Карточка машиниста (${machinist.fullName}) удалена из справочника — выберите другого человека или восстановите карточку`,
        { driverPersonId: 'Выберите машиниста' },
      );
    }
    logger.warn(
...
export async function issueEsm2OnDemand(
  ...
  // Своей проверки машиниста здесь больше нет (ADR 0164): человека без карточки отвергает общая
  // точка выпуска — тем же текстом и до взятия номера, — а сверка ходит только через неё. Две
  // копии одного правила разошлись бы ровно так, как уже разошлись однажды: ручная выписка
  // отказывала, а сверка печатала бланк с пустой графой ФИО.
  //
  // Переходная неделя от этого ничего не теряет. Оба её бланка выписываются одной транзакцией, и
  // отказ на первом откатывает второй: состояния «на август выдали, на сентябрь нет» не бывает.
