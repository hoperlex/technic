...
export async function sideRecipients(
  ...
  /**
   * Общий ящик компании из карточки — часть стороны `service` у ВСЕХ событий модуля: ящик
   * подрядчика получает письма по тем заявкам, которые ему назначены, и обсуждение по ним такое же
   * его дело, как задание и закрывающие бумаги (уточнение заказчика 04.09.2026).
   *
   * Параметр оставлен точкой расширения на случай события, которому ящик организации не адресован:
   * выключать его надо осознанно и с объяснением, потому что у компании без единой учётки в
   * портале это единственный адрес, по которому она вообще что-либо узнаёт.
   */
