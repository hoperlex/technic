...
export const mailKindEnum = pgEnum('mail_kind', [
  ...
  /**
   * Сообщение о технике, которой нет в справочнике (план `docs/office-equipment-candidate-plan.md`,
   * §10; миграция M2 волны кандидатов). Два вида на два события: «сообщение ждёт проверки» и
   * «решение принято». Свой вид каждому обязателен по общему правилу очереди: заведение пары
   * «кандидат + заявка» ставит рядом `service_request_waiting_it`, и вид, притворившийся соседним,
   * молча подавил бы его письмо по паре `(kind, dedupe_key)`.
   */
...
export const userGrants = pgTable(
  'user_grants',
  {
    ...
    origin: text('origin').notNull().default('manual').$type<'manual' | 'migration' | 'backfill'>(),
...
export const officeEquipmentConsumables = pgTable(
  'office_equipment_consumables',
  {
    ...
    /**
     * Потребность (Р13 ADR 0146, миграция 0227): сколько позиции хотим держать на полке. Не
     * минимум и не норматив — намерение, которое правят по обстоятельствам, поэтому своего журнала
     * у неё нет (в отличие от остатка): правка идёт обычным аудитом карточки, как цвет и
     * комментарий.
     *
     * Ноль значит «не следим», а не «нужно ноль штук»: такие позиции в предзаполнение закупки не
     * попадают вовсе — иначе форма предлагала бы заказать всё, чего нет на складе, включая то, что
     * сознательно не держат.
     *
     * Дефицит (`max(0, потребность − остаток − уже заказанное)`) не хранится: он меняется от
     * каждого движения склада и от каждой чужой закупки, и считает его сервер одним местом (Р15).
     */
  ...
  (t) => ({
    ...
    // Отрицательной потребности не бывает: «нужно минус два» — не отрицательное намерение, а
    // ошибка ввода. Правило то же и по той же причине, что у остатка соседней строкой.
    requiredNonNegative: check(
      'office_equipment_consumables_required_check',
...
/**
 * Плановая закупка расходников (Р9 ADR 0146, миграция 0227): документ, которым дефицит превращают
 * в заказ снабжению. Номер показывается как «ЗК-<num>».
 *
 * СВОЕЙ ТАБЛИЦЕЙ, А НЕ ВИДОМ `serviceRequests`, и довод конкретен. Остаток расходников один на
 * компанию (граница ADR 0132: мест хранения у него нет), значит потребность, дефицит и заказ по
 * дефициту глобальны — ни площадки, ни отдела у такого документа не бывает. Модуль заявок построен
 * на обратном: на аппарате, его объекте и его отделе, которыми считается область, фильтруются
 * списки и стоят `INNER JOIN` базового запроса. Первая редакция плана впихивала глобальный документ
 * в эту модель и платила исключениями в семи местах — `CHECK` предмета, предикат области, триггер
 * исполнителей, вычислитель ожиданий, планирование писем, два коридора статусов, ручка приёмки.
 *
 * Отсюда и то, чего у закупки НЕТ: вложений, обсуждения, срочности, заморозки, писем и области
 * видимости. Видна она по праву `officeEquipmentPurchases.manage`, а не по объекту.
 *
 * ОСТАТОК ОНА НЕ ДВИГАЕТ (Р11, решение заказчика на альфу): закупка — бумага для снабжения, приход
 * заводится ручной правкой остатка. Поэтому у журнала остатка нет ни нового вида события, ни ссылки
 * сюда, а порядок «сначала приход, потом закрытие» держится галочкой в форме закрытия и записан
 * принятым операционным риском, а не инвариантом.
 */
export const officeEquipmentPurchases = pgTable(
  'office_equipment_purchases',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    /** Сквозной человекочитаемый номер (отображается как «ЗК-<num>»), как у заявок и заказов ТС. */
    num: integer('num').generatedAlwaysAsIdentity(),
    /**
     * Цикл из четырёх состояний (Р10): `new` → `in_work` (бумага у снабжения) → `closed`; из первых
     * двух — `cancelled` с обязательной причиной. Закрытая и отменённая конечные: ошибку исправляют
     * новой закупкой, а не переписыванием прошлой.
     *
     * Текстом с `CHECK`, а не своим `enum`: `DROP VALUE` в Postgres нет, и мёртвое значение
     * оставалось бы в типе навсегда — этот модуль уже носит четыре таких (0197 и 0224).
     *
     * Переход пишется УСЛОВНОЙ ЗАПИСЬЮ (`UPDATE … WHERE id = ? AND status = ?`) с обязательным
     * `ROW_COUNT = 1`: ноль означает «состояние уже другое» и отвечает 409. Без этого параллельные
     * «Закрыть» и «Отменить» перетёрли бы друг друга, и в базе оказалась бы закрытая закупка с
     * заполненной причиной отмены.
     */
    status: text('status')
      .$type<'new' | 'in_work' | 'closed' | 'cancelled'>()
      .notNull()
      .default('new'),
    /**
    ...
    comment: text('comment').notNull().default(''),
    /**
     * Версия содержимого (Р18). Состояние и версия отвечают на РАЗНЫЕ вопросы: состояние — «можно
     * ли править», версия — «не изменил ли документ кто-то другой», и одним полем на два не
     * ответить. Условия по статусу хватает переходам и не хватает правке полей: она идёт внутри
     * одного статуса, где статус неподвижен. Двое открыли черновик на 10, первый поставил 12 и
     * сохранил, второй ставит 8 — снимок Р17 у второго сходится (внешнее состояние не менялось), и
     * «8» молча перетёрло бы «12».
     *
     * Растёт при каждой правке и входит в условие записи; «Провести» несёт её тоже — иначе в
     * снабжение уезжает не тот состав, который человек видел на экране. Считается с единицы:
     * заведённый документ — это уже первая редакция.
     */
    contentVersion: integer('content_version').notNull().default(1),
    /**
     * Пары переходов, по одной на каждый ход. Историю закупки держат ОНИ, а не журнал аудита, и
     * решение это принято из-за свойства общего помощника: `writeAudit` глотает ошибку записи
     * (`catch` + `logger.error`), то есть гарантией истории не является. У остального портала это
     * осознанный компромисс — журнал не должен ронять выписанный путевой лист, — но карточка,
     * показывающая жизненный путь документа из такого источника, показывала бы его с дырами.
     * Колонка пишется той же транзакцией, что и статус, и лента карточки полна по построению.
     * Аудит пишется тоже, но строгий (`writeAuditTx`): закупка двигает деньги снабжения.
     *
     * `restrict` у всех четырёх — тот же довод, что у автора события журнала остатка: кто завёл,
     * провёл, закрыл и отменил закупку, не должно теряться вместе с учёткой, а `set null` заставил
     * бы карточку отвечать «неизвестно» на единственный вопрос, ради которого колонка заведена.
     */
    ...
    /**
     * Причина отмены обязательна (Р10), но колонкой `notNull` этого не сказать: у неотменённой
     * закупки причины нет. Пустая строка означает «отмены не было», `null` рядом с ней был бы
     * вторым именем того же состояния, а связывает её со статусом `CHECK` ниже.
     */
     ...
     * нельзя. Отличие от образца одно: там ключ ложится на существующую строку дня, здесь строка
     * ещё создаётся, — отсюда уникальное ограничение и разбор `23505` снаружи транзакции.
     *
     * Оба поля обязательны: ручка новая, legacy-клиентов у неё нет, а необязательный ключ означал
     * бы, что защита работает у тех, кто её попросил. `uuid` — ключ описывает попытку отправки,
     * портал генерирует его сам, и тип отбивает мусор в заголовке раньше маршрута.
     */
  ...
  (t) => ({
    // Инварианты держит БАЗА, а не порядок вызовов: ручной `UPDATE`, скрипт обслуживания или
    // забытая ветка маршрута оставили бы документ в состоянии, которого не бывает, — закрытым с
    // причиной отмены или отменённым без причины. Отдельными ограничениями, а не одной матрицей
    // (как у дня водителя): имя сработавшего читает человек, и «нарушено ..._cancelled_check»
    // говорит, что именно не сошлось.
    statusKnown: check(
      'office_equipment_purchases_status_check',
      sql`${t.status} IN ('new', 'in_work', 'closed', 'cancelled')`,
    ),
    // Пара закрытия заполнена РОВНО у `closed`. Обе стороны равенства важны: «заполнено только у
    // закрытой» не запрещает закрытую без автора, а «у закрытой заполнено» — автора закрытия у
    // живой.
    closedPair: check(
      'office_equipment_purchases_closed_check',
      sql`(${t.status} = 'closed') = (${t.closedBy} IS NOT NULL)
          AND (${t.status} = 'closed') = (${t.closedAt} IS NOT NULL)`,
    ),
    // Тройка отмены «кто, когда, почему» — ровно у `cancelled`. Причина меряется `btrim`: строка из
    // пробелов это отсутствие причины, набранное иначе.
    cancelledTriple: check(
      'office_equipment_purchases_cancelled_check',
      sql`(${t.status} = 'cancelled') = (${t.cancelledBy} IS NOT NULL)
          AND (${t.status} = 'cancelled') = (${t.cancelledAt} IS NOT NULL)
          AND (${t.status} = 'cancelled') = (btrim(${t.cancelReason}) <> '')`,
    ),
    // Проведение ходит парой, у «Новой» пусто, у «В работе» и «Закрытой» заполнено. У ОТМЕНЁННОЙ —
    // свободно, и это не послабление, а единственное честное правило: отменить можно и из «Новой»
    // (проведения не было), и из «В работе» (было), а по статусу `cancelled` эти два случая уже не
    // различить — прежнее состояние в шапке не хранится.
    submittedPair: check(
      'office_equipment_purchases_submitted_check',
      sql`(${t.submittedBy} IS NULL) = (${t.submittedAt} IS NULL)
          AND (${t.status} <> 'new' OR ${t.submittedBy} IS NULL)
          AND (${t.status} NOT IN ('in_work', 'closed') OR ${t.submittedBy} IS NOT NULL)`,
    ),
    // Ноль означал бы документ без единой редакции, а он создаётся сразу с составом.
    contentVersionPositive: check(
      'office_equipment_purchases_content_version_check',
      sql`${t.contentVersion} >= 1`,
    ),
    // Пустой отпечаток сделал бы ключ бессмысленным: повтор с ЛЮБЫМ телом совпал бы с ним и вернул
    // бы чужую закупку как свою.
    fingerprintNotBlank: check(
      'office_equipment_purchases_fingerprint_check',
      sql`btrim(${t.idempotencyFingerprint}) <> ''`,
    ),
    /**
     * Уникальность ключа — ОБЫЧНАЯ и С ЯВНЫМ ИМЕНЕМ. Обычная, а не частичная: ключ обязателен, и
     * частичный индекс по одной колонке был бы и слабее, и лукавее — он обещал бы, что ключа может
     * не быть. Пара с автором, а не ключ сам по себе: ключ описывает попытку конкретного человека.
     *
     * Имя задано руками, и это условие работоспособности маршрута, а не стиль. Гонку двух запросов
     * с одним ключом, но непересекающимися позициями, блокировки строк расходников не разводят
     * вовсе — разводит только это ограничение, и ловится оно снаружи уже прерванной транзакции по
     * паре «код + имя»: `pg.code === '23505' && pg.constraint === '…_idempotency_unique'` (приём
     * `vehicle-request-days.ts`, `user-email.ts`, `waste-types.ts`). `23505` в той же транзакции
     * способны дать и уникальность номера, и пара «закупка + позиция», и строгий аудит; перехвати
     * маршрут код целиком — настоящий дефект базы вернулся бы клиенту как успешный повтор, то есть
     * ЧУЖОЙ закупкой вместо ошибки.
     */
    idempotencyUnique: unique('office_equipment_purchases_idempotency_unique').on(
      t.createdBy,
      t.idempotencyKey,
    ),
    // Номер уникален, как у заявок и заказов ТС: идентичность раздаёт значения без повторов, но
    // уникальности не обещает (`OVERRIDING SYSTEM VALUE` и правка счётчика существуют), а «ЗК-14»
    // на двух документах сделала бы спор со снабжением неразрешимым.
    numUnique: uniqueIndex('office_equipment_purchases_num_unique').on(t.num),
    /**
     * Главный индекс модуля — под вопрос «сколько по этой позиции уже заказано в ОТКРЫТЫХ закупках»
     * (Р15): его задают на каждой странице перечня расходников и в каждом предзаполнении формы.
     *
     * Частичный по ровно тому условию, каким открытость и определена: закрытые и отменённые
     * выпадают из него навсегда, открытых в любой момент единицы, и стоимость вопроса не растёт
     * вместе с архивом (замер: 16 КБ против 88 КБ у полного индекса по номеру на 3008 закупках).
     * По `id`, а не по `status`, — спрашивают у него именно идентификаторы: планировщик ведёт
     * соединение от открытых закупок и берёт индекс index-only, `Heap Fetches: 0`.
     *
     * Условие записано тем же `IN`-списком, что и в запросах, и переписывать его в маршруте нельзя:
     * частичный индекс применяется, только если предикат запроса влечёт предикат индекса, а
     * `status <> 'closed' AND status <> 'cancelled'` этой проверки не проходит — вышел бы верный
     * ответ по неверному плану.
     */
    openIdx: index('office_equipment_purchases_open_idx')
      .on(t.id)
      .where(sql`${t.status} IN ('new', 'in_work')`),
  }),
);

/**
 * Строки закупки (Р17): позиция, количество и СНИМОК РАСЧЁТА, из которого это количество вышло.
 */
export const officeEquipmentPurchaseItems = pgTable(
  'office_equipment_purchase_items',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    /**
     * `cascade`: строки — это состав документа, а не его история. Тот же довод, что у строк заявки
     * на расходники. Историю каскад не уносит: отдельной таблицей её у закупки нет, она живёт
     * колонками переходов, то есть в самой удаляемой строке.
     */
    purchaseId: uuid('purchase_id')
      .notNull()
      .references(() => officeEquipmentPurchases.id, { onDelete: 'cascade' }),
    /**
     * `restrict`: позицию, попавшую в закупку, удалить нельзя — иначе «заказали 12» осталось бы без
     * ответа на вопрос «чего именно 12». Погашенная позиция при этом законна и в старых закупках
     * остаётся: гашение означает «больше не покупаем», а не «этого никогда не заказывали».
     */
    consumableId: uuid('consumable_id')
      .notNull()
      .references(() => officeEquipmentConsumables.id, { onDelete: 'restrict' }),
    /** Сколько заказываем; человек правит это число руками — отсюда и отдельное от `suggested`. */
    quantity: integer('quantity').notNull(),
    /**
     * Снимок расчёта на момент отправки, и это главная группа колонок строки. Потребность плавающая
     * (Р13), остаток движется каждый день, «уже заказано» меняется от каждой соседней закупки — без
     * снимка через месяц на вопрос «почему заказали двенадцать» ответить НЕЧЕМ: сегодняшняя
     * потребность к тому решению отношения уже не имеет, и пересчёт задним числом дал бы другое
     * число при полностью верных данных. Приём тот же, что снимок предмета в заявке.
     *
     * Четырьмя колонками, а не одним `jsonb`: по ним считают («заказали больше предложенного —
     * почему?»), сравнивают и рисуют столбцы формы.
     */
    requiredSnapshot: integer('required_snapshot').notNull(),
    stockSnapshot: integer('stock_snapshot').notNull(),
    alreadyOrderedSnapshot: integer('already_ordered_snapshot').notNull(),
    /**
     * Что предложил портал: `max(0, потребность − остаток − уже заказанное)` на момент отправки.
     * Хранится рядом со слагаемыми, а не выводится из них: формула Р15 может смениться, и старые
     * строки пересчитались бы по новому правилу, соврав о прошлом решении.
     */
    suggestedQuantity: integer('suggested_quantity').notNull(),
    createdAt: createdAt(),
  },
  (t) => ({
    // Заказывают хотя бы штуку: «заказать ноль» — отсутствие строки, а не строка.
    quantityPositive: check(
      'office_equipment_purchase_items_quantity_check',
      sql`${t.quantity} > 0`,
    ),
    // Снимок неотрицателен целиком. Ноль в любом из четырёх чисел законен и осмыслен («потребность
    // 0» — позицию дописали руками, «предложено 0» — предлагать было нечего, а заказать решили всё
    // равно), а отрицательное — не свойство склада, а ошибка вычислителя.
    snapshotNonNegative: check(
      'office_equipment_purchase_items_snapshot_check',
      sql`${t.requiredSnapshot} >= 0
          AND ${t.stockSnapshot} >= 0
          AND ${t.alreadyOrderedSnapshot} >= 0
          AND ${t.suggestedQuantity} >= 0`,
    ),
    // Одна строка на позицию: две строки «Тонер Ricoh 201» в одной закупке — не два расходника, а
    // ошибка формы, и сумма по ним удвоила бы «уже заказано» у соседа.
    pairUnique: unique('office_equipment_purchase_items_unique').on(t.purchaseId, t.consumableId),
    /**
     * Вход со стороны позиции: проверка `restrict` при попытке удалить расходник и «что вообще
     * заказывали по этой позиции» в её карточке. Без индекса каждый из них означал бы seq scan по
     * строкам всех закупок за все годы; тот же индекс и по той же причине стоит у строк заявки.
     *
     * В МИГРАЦИИ У НЕГО ЕСТЬ `INCLUDE (purchase_id, quantity)`, которого здесь нет: построитель
     * индексов drizzle 0.45 включённых колонок не умеет вовсе. Расхождение безвредно — схема здесь
     * зеркало, DDL берётся из `apps/api/drizzle/*.sql`, — но выдумывать вместо него составной ключ
     * нельзя: он поменял бы порядок сравнения и стоимость вставки, то есть зеркало разошлось бы с
     * базой по существу, а не по одной необязательной строке.
     *
     * В самом расчёте дефицита планировщик этот индекс НЕ берёт: открытых закупок мало, и он ведёт
     * соединение от них (см. `openIdx`). Заведён он ради двух случаев выше.
     */
    consumableIdx: index('office_equipment_purchase_items_consumable_idx').on(t.consumableId),
  }),
);

/**
 * Кандидат на добавление техники (план `docs/office-equipment-candidate-plan.md`, Р3; миграция
 * 0263): сообщение заявителя «в кабинете 214 стоит аппарат, которого нет в справочнике».
 *
 * СВОЕЙ ТАБЛИЦЕЙ, А НЕ НЕАКТИВНОЙ СТРОКОЙ `office_equipment` (Р1), и довод здесь тот же, что у
 * закупки: у кандидата другая область видимости и другие правила жизни. Постановка требует, чтобы
 * до подтверждения запись не попадала ни в селекторы, ни в статистику, ни в интеграции, ни в чужую
 * область. Строкой парка это выполнялось бы перечислением читателей — список, карточка, обмен
 * файлом, счётчики моделей и расходников, гарантии, история, — а такой список невозможно закрыть:
 * он открыт для каждой будущей ручки модуля. Здесь требование выполнено тем, что записи в парке
 * ещё нет.
 *
 * Отдельно к этому счёту: номера парка уникальны среди ЖИВЫХ строк, безотносительно `is_active`
 * (`office_equipment_serial_unique`). Кандидат-строка парка занимала бы номер, и опечатавшийся
 * заявитель блокировал бы оператору заведение настоящей карточки — снять блокировку можно было бы
 * только удалением чужой записи. Свои частичные индексы ниже держат тот же замок среди ОЖИДАЮЩИХ и
 * освобождают номер решением.
 *
 * Плата за выбор названа вслух: это ВТОРАЯ ТОЧКА, где заводится карточка техники. Смягчает её Р14 —
 * подтверждение зовёт тот же помощник вставки (`services/office-equipment-write.ts`), что и маршрут
 * справочника, а не свою копию проверок.
 */
export const officeEquipmentCandidates = pgTable(
  'office_equipment_candidates',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    /**
     * Четыре состояния (Р3): `pending` — ждёт проверки; `confirmed` — по сообщению завели новую
     * карточку; `duplicate` — аппарат уже был в справочнике и кандидат объединён с существующей
     * карточкой; `rejected` — аппарата в названном месте нет либо сообщение недостоверно. Все три
     * решения конечны: «передумал» оформляется новой парой «кандидат + заявка», а не переписыванием
     * прошлой.
     *
     * Текстом с `CHECK`, а не своим `enum`: `DROP VALUE` в Postgres нет, и мёртвое значение
     * осталось бы в типе навсегда — модуль уже носит четыре таких (0197 и 0224). Тип значения — из
     * контрактов, а не переписанный сюда союз: те же четыре слова стоят в `CHECK` ниже и в
     * `OFFICE_EQUIPMENT_CANDIDATE_STATUSES`, и третий список разошёлся бы с ними молча.
     *
     * Меняется он УСЛОВНОЙ ЗАПИСЬЮ (`UPDATE … WHERE id = ? AND status = 'pending' AND
     * content_version = ?`) с обязательным `ROW_COUNT = 1` (Р11): ноль означает «решение уже принято
     * или форма устарела» и отвечает 409. Без этого «Подтвердить» и «Отклонить», нажатые
     * одновременно, перетёрли бы друг друга, и в базе оказался бы подтверждённый кандидат с
     * причиной отказа.
     */
    status: text('status').$type<OfficeEquipmentCandidateStatus>().notNull().default('pending'),
    /**
     * Тип — ссылкой на обычный справочник: перечень из десятка строк, и без типа проверяющий не
     * знает, какую форму открывать. `restrict`, как у карточки парка: тип, на который ссылается
     * хоть одно сообщение, не удаляют, а гасят.
     */
    equipmentTypeId: uuid('equipment_type_id')
      .notNull()
      .references(() => officeEquipmentTypes.id, { onDelete: 'restrict' }),
     ...
     * должен. Ссылку на модель проставляет проверяющий в форме подтверждения, и живёт она уже в
     * заведённой карточке, а не здесь: заявленное остаётся заявленным.
     */
    declaredModel: text('declared_model').notNull(),
    serialNumber: text('serial_number').notNull().default(''),
    inventoryNumber: text('inventory_number').notNull().default(''),
    /**
     * Физическое место аппарата — площадка. Обязательно: у сообщения о технике адрес есть всегда,
     * его называет сам заявитель, и он же становится объектом связанной заявки (Р5).
     */
    objectId: uuid('object_id')
      .notNull()
      .references(() => constructionObjects.id, { onDelete: 'restrict' }),
    /**
     * «каб. 214». Обязательно и непусто — в отличие от карточки парка, где место бывает пустым:
     * там единицу уже нашли и завели, а здесь место — единственное, по чему проверяющий физически
     * найдёт аппарат, чтобы посмотреть на шильдик своими глазами.
     */
     ...
     * Снимком, а не соединением с сегодняшними привязками учётки: переведённый в другой отдел
     * человек не должен уносить свои прошлые сообщения из чужой очереди — тот же приём и тот же
     * довод, что у трёх снимков области в заявке.
     *
     * `NULL` законен и означает «у автора отделов нет» (администратор, роль без привязок), а не
     * «неизвестно»: у таких кандидатов отдельская ось просто не работает, и видны они по объекту.
     */
    ...
    createdBy: uuid('created_by')
      ...
      .references(() => users.id, { onDelete: 'restrict' }),
    /**
     * Идемпотентность заведения пары «кандидат + заявка». Приём и оба поля — от закупки (её Р17) и,
     * через неё, от кабинета водителя (ADR 0103): ключ описывает ПОПЫТКУ отправки, отпечаток —
     * саму команду. Тот же ключ с тем же отпечатком возвращает уже созданную пару без второго
     * аудита и второго письма, тот же ключ с другим телом — это другая команда под старым ключом, и
     * молча подтверждать её нельзя.
     *
     * Здесь у приёма есть своя тонкость, которой у закупки нет: одинаковое тело одновременно
     * конфликтует и по ключу, и по номеру ожидающего кандидата, а PostgreSQL не обещает, какой из
     * уникальных индексов сообщит об этом первым. Поэтому маршрут разбирает `23505` по ИМЕНИ
     * ограничения и сначала перечитывает победителя по ключу — имена всех трёх индексов заданы
     * руками именно ради этого разбора.
     */
    idempotencyKey: uuid('idempotency_key').notNull(),
    /**
     * Отпечаток НОРМАЛИЗОВАННОГО тела заявки целиком, включая кандидата и `fileIds`: строки
     * обрезаны, необязательные значения приведены к одному виду, множества идентификаторов
     * отсортированы и дедуплицированы. Нормализация обязательна до отпечатка — иначе переставленные
     * местами вложения той же заявки дали бы «другую команду под тем же ключом», и честный повтор
     * получил бы 409.
     */
    idempotencyFingerprint: text('idempotency_fingerprint').notNull(),
    /**
     * Версия содержимого (Р11, Р12). Статус и версия отвечают на РАЗНЫЕ вопросы, и одним полем на
     * два не ответить: статус — «принято ли решение», версия — «не изменил ли строку кто-то
     * другой». Статуса не хватает именно правке реквизитов: она идёт ВНУТРИ `pending`, где статус
     * неподвижен и ничего не стережёт, — двое проверяющих, открывших одну строку очереди, перетёрли
     * бы исправления друг друга молча.
     *
     * Растёт и при правке, и при решении, и входит в условие обеих записей. Считается с единицы:
     * отправленный кандидат — это уже первая редакция свидетельства.
     */
    contentVersion: integer('content_version').notNull().default(1),
    createdAt: createdAt(),
    /**
     * Правка реквизитов проверяющим до решения (Р12): «заявитель списал `O` вместо `0`». Автору
     * правка закрыта после отправки намеренно — кандидат есть свидетельство о том, что человек
     * видел в кабинете, и переписанное задним числом свидетельство ничего не доказывает.
     *
     * `set null` здесь, а не `restrict` соседей: «кто поправил опечатку» — служебный след, ради
     * которого держать учётку уволенного не нужно, а «кто сообщил» и «кто решил» — предмет самой
     * записи. Пустая пара означает «сообщение приняли как есть».
     */
    updatedBy: uuid('updated_by').references(() => users.id, { onDelete: 'set null' }),
    updatedAt: updatedAt(),
    /**
     * Кто и когда решил. Пара пишется той же транзакцией, что и статус, и вместе с парой заведения
     * и парой правки образует ленту жизни кандидата — её строят ЭТИ колонки, а не журнал аудита
     * (довод закупки: общий `writeAudit` глотает ошибку записи и гарантией истории не является).
     * Строгий аудит решения при этом пишется тоже, но он отвечает на другой вопрос — «что вообще
     * происходило в портале».
     */
    decidedBy: uuid('decided_by').references(() => users.id, { onDelete: 'restrict' }),
    decidedAt: timestamp('decided_at', { withTimezone: true }),
    /**
     * Причина решения. Обязательна у отказа (`…_reason_check`) и уходит заявителю ДОСЛОВНО — и в
     * карточку заявки, и в письмо (Р15, В5): общее «сообщение отклонено, подробности в портале»
     * экономило бы проверяющему выбор слов ценой второго действия человека. Пустая строка означает
     * «причины не было», `null` рядом с ней был бы вторым именем того же состояния.
     */
     ...
     * отдельного признака происхождения у карточки парка поэтому НЕТ (В6): он был бы третьим
     * хранением одного факта и разошёлся бы с этой ссылкой на первом же объединении, где карточка
     * заведена вовсе не по сообщению.
     *
     * `restrict`: карточку, на которую ссылается решение, нельзя снести насовсем, не потеряв ответ
     * «откуда она взялась». Мягкое удаление ссылку не трогает — оно и не должно.
     */
    resultEquipmentId: uuid('result_equipment_id').references(() => officeEquipment.id, {
      onDelete: 'restrict',
    }),
  },
  (t) => ({
    // Словарь состояний. Значение вне четвёрки — не «новый статус», а опечатка скрипта.
    statusKnown: check(
      'office_equipment_candidates_status_check',
      sql`${t.status} IN ('pending', 'confirmed', 'duplicate', 'rejected')`,
    ),
    // Хотя бы один номер — тот же инвариант, что у парка, и по той же причине: по номеру аппарат
    // опознают. Здесь у него есть и вторая работа: по номеру ищется дубль на всех трёх рубежах
    // (Р10), и сообщение без обоих номеров не с чем сверить.
    identity: check(
    ...
    // Пустой отпечаток сделал бы ключ бессмысленным: повтор с ЛЮБЫМ телом совпал бы с ним и вернул
    // бы чужую пару «кандидат + заявка» как свою.
    fingerprintNotBlank: check(
    ...
    // решённого заполнено» — автора решения у ожидающего. Второе равенство держит саму пару: время
    // без автора и автор без времени — половина ответа на вопрос, ради которого колонки заведены.
    decisionPair: check(
      'office_equipment_candidates_decision_check',
      sql`(${t.status} = 'pending') = (${t.decidedBy} IS NULL)
          AND (${t.decidedBy} IS NULL) = (${t.decidedAt} IS NULL)`,
    ),
    // Результат есть РОВНО у двух положительных исходов. Отсюда и обязательный порядок шагов в
    // транзакции подтверждения (Р13): карточка вставляется ДО перевода статуса, иначе промежуточное
    // состояние «confirmed без результата» не прошло бы это ограничение, — а отказ, наоборот,
    // ссылки на карточку иметь не может, потому что карточки не появилось.
    resultPresence: check(
      'office_equipment_candidates_result_check',
      sql`(${t.status} IN ('confirmed', 'duplicate')) = (${t.resultEquipmentId} IS NOT NULL)`,
    ),
    // Отказа без причины не бывает (Р15): её читает заявитель, и «отклонено» без слов через месяц
    // читается как «передумали». У остальных исходов причина свободна — подтверждение объясняет
    // себя самой заведённой карточкой.
    reasonPresence: check(
      'office_equipment_candidates_reason_check',
      sql`${t.status} <> 'rejected' OR btrim(${t.decisionReason}) <> ''`,
    ),
    /**
     * ГОНКУ ДВУХ ЗАЯВИТЕЛЕЙ ЛОВЯТ ЭТИ ДВА ИНДЕКСА, А НЕ SELECT МАРШРУТА (Р10, рубеж 2). Двое в одну
     * минуту сообщают об одном принтере: проверка «нет ли такого кандидата» у обоих проходит, а
     * отбивает второго уникальность — его транзакция откатывается целиком, и маршрут переводит
     * `23505` в 409 «Этот аппарат уже отправлен на проверку». Тот же расклад и по той же причине,
     * что у замка «одна открытая заявка на единицу».
     *
     * Частичные по `status = 'pending'`: решённый кандидат номер НЕ держит — новый факт можно
     * сообщить новой парой «кандидат + заявка», в том числе после отказа. Пустой номер не
     * сравнивается вовсе: он законен, пока заполнен второй, и «пусто = пусто» запретило бы второе
     * сообщение с одним лишь серийным.
     *
     * `upper(btrim(...))` — ровно как у номеров парка: человек набирает номер с пробелом или другим
     * регистром, и без нормализации замок обходился бы опечаткой.
     */
    serialPendingUnique: uniqueIndex('office_equipment_candidates_serial_pending_unique')
      .on(sql`upper(btrim(${t.serialNumber}))`)
      .where(sql`${t.status} = 'pending' AND btrim(${t.serialNumber}) <> ''`),
    inventoryPendingUnique: uniqueIndex('office_equipment_candidates_inventory_pending_unique')
      .on(sql`upper(btrim(${t.inventoryNumber}))`)
      .where(sql`${t.status} = 'pending' AND btrim(${t.inventoryNumber}) <> ''`),
    /**
     * Уникальность ключа — ОБЫЧНАЯ и с явным именем, приём и довод от закупки. Обычная, а не
     * частичная: ключ обязателен, и частичный индекс по одной колонке обещал бы, что ключа может не
     * быть. БЕЗ УСЛОВИЯ ПО СТАТУСУ, в отличие от двух индексов выше, и это не забывчивость:
     * потерянный успешный ответ повторяют и через час, когда кандидат уже решён, — а частичный
     * индекс ожидающих к тому времени освободился бы и создал вторую пару «кандидат + заявка».
     *
     * Пара с автором, а не ключ сам по себе: ключ описывает попытку КОНКРЕТНОГО человека.
     */
    idempotencyUnique: unique('office_equipment_candidates_idempotency_unique').on(
      t.createdBy,
      t.idempotencyKey,
    ),
    ...
    pendingIdx: index('office_equipment_candidates_pending_idx')
      .on(t.createdAt)
      .where(sql`${t.status} = 'pending'`),
    // Две оси, которыми очередь сужается до области смотрящего (Р9), и «мои кандидаты» у автора.
    // Третьей, отдельской, своего индекса пока НЕ дано осознанно: очередь площадочного и
    // отдельского оператора отбирается из `…_pending_idx` — она короткая, — а полный перебор
    // решённых сообщений отдела спрашивают редко. Заводить третий индекс до первого замера значило
    // бы платить за вставку ради вопроса, которого никто не задал.
    objectIdx: index('office_equipment_candidates_object_idx').on(t.objectId),
    authorIdx: index('office_equipment_candidates_author_idx').on(t.createdBy),
  }),
);
...
export const serviceRequests = pgTable(
  'service_requests',
  {
    ...
    // Аппарат НЕОБЯЗАТЕЛЕН с миграции 0230 (план расходников и закупки, Р5/Р7; ADR 0146,
    // решение 6): «поставьте розетку», «настройте почту новому сотруднику» и «принтер привезли,
    // инвентарного номера ещё нет» — работа модуля, у которой предмета в справочнике нет вовсе.
    // Заводит такую заявку держатель права `serviceRequests.createWithoutEquipment`, а «чья она»
    // отвечает заказчик — объект или отдел (`service_requests_subject_check` ниже).
    officeEquipmentId: uuid('office_equipment_id').references(() => officeEquipment.id, {
      onDelete: 'restrict',
    }),
    // ТРЕТИЙ СПОСОБ НАЗВАТЬ ПРЕДМЕТ (план `docs/office-equipment-candidate-plan.md`, Р4; миграция
    // 0263): аппарата в справочнике нет, и заявитель приложил к заявке сообщение о нём. Ссылка
    // своей колонкой, а не переиспользованным `office_equipment_id`: кандидат — не запись
    // справочника (Р1), и общая колонка означала бы, что предмет заявки иногда указывает не туда.
    //
    // ПАРА СТРОГО 1:1, и держит это частичный уникальный индекс ниже: один кандидат объясняет
    // предмет ровно одной исходной заявки. Второе сообщение о том же аппарате заводит СВОЕГО
    // кандидата и только после решения предыдущего — подшить второе свидетельство к чужой строке
    // нельзя: два наблюдения могут различаться местом и реквизитами, а общая строка раскрыла бы
    // второму автору чужой объект через основание видимости `related`.
    //
    // ССЫЛКА ОСТАЁТСЯ ЗАПОЛНЕННОЙ И ПОСЛЕ РЕШЕНИЯ. Заявка обязана помнить, что предмет пришёл
    // проверкой: по этой паре читается история и строится срез «заведено через проверку», и по ней
    // же снимается замок приёмки (Р16). Отсюда законная асимметрия хранимой строки: после
    // подтверждения заполнены ОБА поля — кандидат хранит происхождение, `office_equipment_id`
    // решённый предмет, — и `service_requests_subject_check` этого сочетания намеренно не
    // запрещает. Запрет «два входных способа сразу» относится только к команде создания и держится
    // схемой контракта.
    //
    // Обратное правило «у кандидата всегда есть заявка» держат единственная транзакционная команда
    // заведения (Р2) и все пути удаления; циклический обязательный FK ради него не заводится.
    // `restrict`: необратимый purge заявки удаляет её единственного кандидата той же транзакцией
    // явным шагом — молчаливый каскад унёс бы решение проверяющего вместе с ним.
    equipmentCandidateId: uuid('equipment_candidate_id').references(
      () => officeEquipmentCandidates.id,
      { onDelete: 'restrict' },
    ),
    ...
    //
    // `NOT NULL` снят той же миграцией 0230 и по той же причине: у заявки «от отдела» без аппарата
    // площадки нет ни откуда — ни из карточки единицы, ни из выбора человека. При АППАРАТЕ объект
    // по-прежнему обязателен, и держит это первая ветвь `service_requests_subject_check`, а не
    // `NOT NULL` колонки: правило «у аппарата всегда есть объект» не должно было ослабнуть от
    // того, что появились заявки без аппарата.
    equipmentObjectId: uuid('equipment_object_id').references(() => constructionObjects.id, {
      onDelete: 'restrict',
    }),
    // «Аппарат стоит не там, где написано в карточке» — ЗАЯВЛЕННЫЙ ФАКТ (Р16 ADR 0145, миграция
    // 0224): объект этой заявки указал человек галочкой в форме, а не подставила карточка единицы.
    // Хранится и не гасится ничем, как остальные снимки заявки: это свидетельство о том, что
    // заявитель видел аппарат в другом месте, а не текущее состояние справочника.
    //
    // САМО РАСХОЖДЕНИЕ ЭТОЙ КОЛОНКОЙ НЕ ХРАНИТСЯ и храниться не должно: оно вычисляется
    // соединением — `equipment_object_id` заявки против `office_equipment.object_id` карточки, — и
    // перенос единицы в справочнике гасит его сам, без второго действия и без человека, который
    // обязан не забыть. Отбор ИТ-службы — конъюнкция обоих признаков плюс «заявка открыта»:
    // расхождение заявили, оно ещё не устранено, и заявка ещё жива. Порознь оба неверны: хранимый
    // флаг сам по себе не гаснет никогда и через месяц становится списком всего, что когда-либо
    // поправляли, а одно вычисляемое расхождение у прошлогодних заявок стоит сплошь и рядом —
    // технику возят, и никто этого не заявлял.
    //
    // Справочник галочка НЕ ПРАВИТ: перенос единицы — решение ИТ-службы после проверки, а карточку
    // правит всякий заявитель, и опечатка в заявке переставляла бы аппараты по объектам.
    objectOverridden: boolean('object_overridden').notNull().default(false),
    ...
    // Вид заморозки (план освобождения от согласования, Р9; миграция 0306): `NULL` — обычное «ждём
    // запчасть», `estimate_exemption_dispute` — остановка спором о том, что подпись под объёмом
    // работ не собирали. Отдельным полем, а не разбором `hold_reason`: по виду заперты возврат из
    // заморозки, закрытие работ и приёмка, а запрет, прочитанный из свободного текста причины,
    // снимался бы правкой этого текста.
    //
    // Со статусом ограничением НЕ связан, и это требование окна выката: старый код возвращает
    // заявку из заморозки, про колонку не зная, и связка «вид есть ровно у отложенной» уронила бы у
    // него первый же возврат. Гасить вид обязана матрица сброса (этап Э5 плана) — ровно так же, как
    // она гасит `held_from_status` и причину.
    holdKind: text('hold_kind').$type<'estimate_exemption_dispute'>(),
    ...
    // Ожидание подписи под объёмом работ (Р2 ADR 0145, миграция 0224). `NULL` — ответ получен
    // (согласовали, не согласовали либо исполнитель сам отозвал предъявление), число — висит
    // предъявление ЭТОЙ ревизии. Колонка заменила собой статус «Смета на согласовании»: заявка всё
    // это время стоит в «В работе», а очередь согласования, замок на правку состава сметы и запрет
    // переназначения под висящим счётом читаются отсюда.
    //
    // СВОЯ КОЛОНКА, А НЕ ВЫВОД ИЗ СОСЕДНИХ — главное решение Р2, и оно куплено двумя ошибками.
    // Признак пробовали считать по `estimate_submitted_at` и по сравнению ревизий, и он врал на
    // СТАРЫХ данных: возврат объёма в правку снимок согласования чистит, а дату предъявления
    // оставляет; отказ по смете её тоже не чистит (ручка согласования при `approved = false` пишет
    // пустой patch). Отличить «вернули в правку» от «ждёт подписи» этими полями нечем, а
    // нормализовать их миграцией нельзя: тот же запрос через релиз накрыл бы настоящие
    // предъявления, заведённые уже по новой модели. Ошибка обоих подходов одна — признак выводился
    // из колонок, которые пишет и СТАРЫЙ код тоже. Своя колонка даёт отличие по построению: старый
    // код о ней не знает и оставляет `NULL`, а `estimate_submitted_at` сохраняет прежний смысл —
    // «когда предъявляли в последний раз», дата для показа и ничего больше.
    //
    // Мест записи ШЕСТЬ, а не два, и колонка, не проведённая через все, была бы хуже выведенного
    // признака — тот хотя бы врал предсказуемо. Четыре про очередь: предъявление ставит сюда
    // поднятую ревизию, согласование, отказ и возврат в правку гасят в `NULL`. Ещё два — про
    // ограничение ниже: передача заявки другому исполнителю и полный сброс сметы обнуляют
    // `estimate_revision` до нуля, и оставленная pending-ревизия уронила бы запись ошибкой БД.
    estimatePendingRevision: integer('estimate_pending_revision'),
    // Происхождение ожидания подписи (Р9 плана освобождения, миграция 0306): `submit` — обычное
    // предъявление, `dispute` — ожидание, открытое исходом разрешённого спора. `NULL` читается как
    // `submit`: старый код открывает ожидание, про происхождение не зная.
    //
    // Поле завелось из-за подписи в статусе «Решена». Разрешить её по статусу было нельзя:
    // `canApproveServiceEstimate` пустил бы к согласованию ЛЮБОЕ ожидание, случайно оказавшееся в
    // «Решена», — в том числе наследственное, от предъявления, переехавшего туда вместе с заявкой.
    // Происхождение сужает дверь до того единственного ожидания, которое открыл разбор спора.
    estimatePendingSource: text('estimate_pending_source').$type<'submit' | 'dispute'>(),
    ...
    // Источник подписи под объёмом работ (Р11 плана освобождения, миграция 0306): `human` —
    // согласовал человек, `auto` — подпись проставлена заявлением «Согласование не требуется» (Р1).
    // `NULL` читается как `human`: так выглядят все подписи до этого выпуска и все, что поставит
    // старый код в окне выката, — этого обязан держаться весь код выпуска A.
    //
    // У `auto` автора подписи нет и быть не может (тот же довод, что у `acceptance_source`): за
    // автоподпись не отвечает ни один человек, а подставить в автора заявителя освобождения значило
    // бы записать, что оператор сервиса согласовал смету сам себе, — ровно то, что
    // `canApproveServiceEstimate` запрещает явно.
    //
    // Связки «подпись ⇔ источник» в выпуске A нет намеренно: старый код ставит и гасит подпись, про
    // источник не зная, и равенство уронило бы у него первое же согласование. Она встаёт выпуском B
    // (этап Э9 плана, образец — миграция 0198).
    estimateApprovalSource: text('estimate_approval_source').$type<'human' | 'auto'>(),
    ...
    // Порог автоприёмки (Р9 плана освобождения, миграция 0306): раньше этого момента автозакрытие
    // заявку не берёт. Пусто у всех, кроме прошедших спор, — и пустое значение отбор считает как
    // сегодня, от `completed_at` и первого закрывающего документа.
    //
    // Явным полем, а не вычитанием времени заморозки из срока. Заявка, простоявшая в споре неделю,
    // созревала бы на первом же прогоне после возврата; «отсчёт от даты разрешения спора» тоже
    // неверен — при исходе «нужна подпись» подпись ставят через два дня, и заявка закрылась бы
    // сразу после неё, не дав никому отреагировать. Порог выставляет тот, кто знает, какой блокер
    // снят последним, и читается он одним сравнением, а не сборкой `GREATEST` из трёх источников.
    autoCloseNotBefore: timestamp('auto_close_not_before', { withTimezone: true }),
    ...
    // Решение при отказе по объёму работ (Р12 ADR 0145, миграция 0224): «что делаем вместо» —
    // «меняем аппарат, заявка на закупку заведена». От причины отказа разведено намеренно: причина
    // отвечает, ПОЧЕМУ объём не согласован («вдвое дороже нового аппарата»), и уходит комментарием
    // перехода в историю, как у всех переходов с объяснением; одним полем на два вопроса не
    // ответить, а спор по отклонённой заявке начинается именно с решения. В карточке решение стоит
    // само по себе, рядом с пометкой замены выше, а не «под причиной»: причины в DTO нет вовсе —
    // её читают на вкладке истории, где она и записана.
    //
    // Живёт только у отменённой заявки (`service_requests_rejection_resolution_check`) и снимается
    // вместе с пометкой замены при откате «Отменена → Новая»: обе объясняют, почему заявку закрыли
    // без ремонта, и обе относятся к отмене, которой после отката больше нет. Гасит их матрица
    // сброса `serviceResetOnTransition` — не сними она решение, откат упрётся в ограничение ровно
    // так же, как упирался бы в `service_requests_replacement_check`.
    rejectionResolution: text('rejection_resolution').notNull().default(''),
    comment: text('comment').notNull().default(''),
  ...
  (t) => ({
    ...
    /*
     * ПРЕДМЕТ ЗАЯВКИ НАЗВАН — двусоставно, а не «хоть что-нибудь» (миграция 0230, план расходников
     * и закупки, Р7; ADR 0146, решение 6).
     *
     * Первая ветвь закрепляет СЕГОДНЯШНЕЕ правило: есть аппарат — есть и его объект. До 0230 его
     * держали два `NOT NULL`, и снятие одного из них не должно было утащить за собой второе:
     * заявка с аппаратом, но без площадки, не бывала ни разу за всю жизнь модуля, и разрешить её
     * заодно значило бы открыть дыру там, где правку никто не просил.
     *
     * Вторая требует у заявки БЕЗ аппарата ровно одного заказчика — объект ИЛИ отдел,
     * исключающе (`<>` над двумя булевыми — это XOR). Ноль заказчиков означал бы заявку, которую
     * не видит никто, кроме автора и администратора: предикат области `serviceRequestScopeWhere`
     * читает ровно эти три колонки, и пустые все три выпадают из области любой роли. Два заказчика
     * сразу означали бы обратное — заявку, которую роль площадки и роль отдела считают своей
     * каждая, и спор «кто её ведёт» решался бы тем, кто первым открыл список.
     *
     * Условие «аппарат ИЛИ объект ИЛИ отдел», записанное первой редакцией плана, слабее ОБОИХ этих
     * правил сразу — оно пропускает и аппарат без объекта, и двух заказчиков.
     *
     * Закупки в условии нет вовсе: у неё своя таблица (`office_equipment_purchases`, Р9), и
     * исключения ради неё здесь не нужны.
     *
     * ТРЕТЬЯ ВЕТВЬ — ПРЕДМЕТ НА ПРОВЕРКЕ (план кандидатов, Р5; миграция 0263). Читается она так: у
     * заявки с кандидатом площадка названа ВСЕГДА — её называет сам заявитель, — а аппарат при этом
     * либо ещё не назначен (`pending`), либо уже проставлен решением проверяющего.
     *
     * Обязательная внешняя скобка `equipment_candidate_id IS NULL` у двух сегодняшних ветвей — не
     * оформление, а половина правила: без неё база приняла бы сразу два входных способа назвать
     * предмет. При этом СЕГОДНЯШНЕЕ ПРАВИЛО СОХРАНЕНО ДОСЛОВНО для заявок без кандидата — ослабления
     * нет ни в одной точке, и расширение безопасно для старого кода в окне выката: таких строк он
     * не пишет.
     *
     * Ветвь кандидата намеренно НЕ требует XOR объекта и отдела: заявку от лица отдела заводят и на
     * кандидата, и тогда `customer_department_id` держит заказчика, а `equipment_object_id` —
     * физическое место аппарата. Иначе сотрудник отдела после отправки потерял бы собственную
     * заявку по правилу ADR 0146. Что оба поля не назначают заявке двух хозяев, доказывает не
     * `CHECK`, а предикат области: у заявки с аппаратом эти же две колонки заполнены вместе уже
     * сегодня.
     */
    subject: check(
      'service_requests_subject_check',
      sql`(${t.equipmentCandidateId} IS NOT NULL AND ${t.equipmentObjectId} IS NOT NULL)
          OR (
            ${t.equipmentCandidateId} IS NULL
            AND (
              (${t.officeEquipmentId} IS NOT NULL AND ${t.equipmentObjectId} IS NOT NULL)
              OR (
                ${t.officeEquipmentId} IS NULL
                AND (${t.equipmentObjectId} IS NOT NULL) <> (${t.customerDepartmentId} IS NOT NULL)
              )
            )
          )`,
    ),
    ...
    // Согласование — снимок «когда и что именно»: дата без ревизии не говорит, ЧТО согласовали, а
    // ревизия без даты — что согласование вообще было. Эта пара нерушима.
    //
    // ПОДПИСЬ БЕЗ АВТОРА ЗАКОННА (Р11 плана освобождения, миграция 0306), и прежнее равенство по
    // автору снято: у автопринятия (`estimate_approval_source = 'auto'`) автора нет вовсе — за него
    // не отвечает ни один человек. Снятие попутно убирает мину, которая стояла и у обычной подписи:
    // ссылка на автора объявлена `set null`, и удаление уволенной учётки обнуляло её у согласованной
    // заявки — то есть упиралось в это ограничение (тот же разбор, что в 0198 про приёмку).
    //
    // Уцелевшее направление прежней пары: «кто-то согласовал, но когда — неизвестно» было
    // бессмыслицей и осталось ею. Строгая связка «подпись ⇔ источник» — выпуск B (этап Э9).
    approval: check(
      'service_requests_approval_check',
      sql`(${t.estimateApprovedAt} IS NULL) = (${t.approvedEstimateRevision} IS NULL)
          AND (${t.estimateApprovedBy} IS NULL OR ${t.estimateApprovedAt} IS NOT NULL)`,
    ),
    // Мягкая редакция выпуска A, дословно как у приёмки (0176): перечень значений и запрет автора у
    // автоподписи — всё, что можно потребовать, пока старый код в окне выката ставит и гасит подпись,
    // про источник не зная. Строгое «источник есть ровно у подписанной» — выпуск B (Э9).
    estimateApprovalSource: check(
      'service_requests_estimate_approval_source_check',
      sql`${t.estimateApprovalSource} IS NULL
          OR (${t.estimateApprovalSource} IN ('human','auto')
              AND (${t.estimateApprovalSource} <> 'auto' OR ${t.estimateApprovedBy} IS NULL))`,
    ),
    // Перечень значений и ничего больше: связку с `estimate_pending_revision` нельзя поставить ни в
    // одну сторону, пока в проде живёт старый код. Он открывает ожидание без происхождения
    // («ожидание без источника») и гасит ожидание, не убрав источник («источник без ожидания»), —
    // то есть в окне выката законны оба перекоса. Пустое значение читается как `submit`.
    estimatePendingSource: check(
      'service_requests_estimate_pending_source_check',
      sql`${t.estimatePendingSource} IS NULL
          OR ${t.estimatePendingSource} IN ('submit','dispute')`,
    ),
    // Перечень закрыт ценой опечатки: по виду заморозки заперты возврат из неё, закрытие работ и
    // приёмка, и незнакомое значение сняло бы все три замка молча — заявка выглядела бы обычной
    // отложенной. Связки со статусом здесь нет намеренно, причина — у самой колонки.
    holdKind: check(
      'service_requests_hold_kind_check',
      sql`${t.holdKind} IS NULL OR ${t.holdKind} IN ('estimate_exemption_dispute')`,
    ),
    ...
    // Ждать можно только подписи под ТЕКУЩЕЙ ревизией: предъявление её поднимает и тут же ставит
    // сюда тот же номер, а повторное предъявление под висящим счётом запрещено (Р9) — иначе
    // исполнитель подменил бы цифры под уже открытым окном согласования. Равенство, а не `<=` как
    // у соседа выше: согласованной бывает и устаревшая ревизия (подпись обесценивается новым
    // предъявлением), а ОЖИДАЮЩЕЙ — только та, что предъявлена сейчас.
    //
    // Старому коду ограничение не мешает и мешать не может: он про колонку не знает и оставляет
    // `NULL`, а `NULL` удовлетворяет ему при любой ревизии.
    estimatePending: check(
      'service_requests_estimate_pending_check',
      sql`${t.estimatePendingRevision} IS NULL
          OR ${t.estimatePendingRevision} = ${t.estimateRevision}`,
    ),
    ...
    // Решение при отказе — по образцу соседа выше и по той же причине: оно объясняет, почему
    // заявку закрыли, ничего не починив, и у живой заявки означало бы «делаем вместо ремонта то, о
    // чём уже договорились», пока ремонт идёт. Отсюда та же обязанность матрицы сброса —
    // гасить поле при возврате отменённой в «Новую».
    rejectionResolution: check(
      'service_requests_rejection_resolution_check',
      sql`${t.status} = 'cancelled' OR btrim(${t.rejectionResolution}) = ''`,
    ),
    numUnique: uniqueIndex('service_requests_num_unique').on(t.num),
    /**
     * Пара «заявка — кандидат» строго 1:1 (Р4), и держит её именно индекс, а не аккуратность
     * маршрута: вторая ссылка на чужого кандидата — это гонка, а гонку ловит база (тот же расклад,
     * что у замка «одна открытая заявка на единицу» ниже).
     *
     * Частичный по непустому значению: заявок БЕЗ кандидата сколько угодно, и в B-tree `NULL` не
     * равен `NULL` — то есть строгий `UNIQUE` по одной колонке дал бы тот же результат. Условие
     * записано явно затем, что оно называет размер индекса: он остаётся размером с число сообщений,
     * а не с архивом всех заявок модуля.
     *
     * `deleted_at` в условие НЕ идёт, и это решение: мягкий архив заявки связь не рвёт (Р4) — пара
     * остаётся парой, — а необратимый purge удаляет заявку вместе с её единственным кандидатом.
     * Исключи мы архивные, освободившееся место занял бы второй кандидат, и восстановление заявки
     * из архива упёрлось бы в уникальность.
     */
    candidateUnique: uniqueIndex('service_requests_equipment_candidate_unique')
      .on(t.equipmentCandidateId)
      .where(sql`${t.equipmentCandidateId} IS NOT NULL`),
    ...
    //
    // ЗАЯВОК БЕЗ АППАРАТА ОБА ИНДЕКСА ПРОПУСКАЮТ СКОЛЬКО УГОДНО, и это правильно (миграция 0230):
    // в B-tree `NULL` не равен `NULL`, поэтому строки с пустым `office_equipment_id` друг с другом
    // не сталкиваются. Запирать их нечем и незачем — «не заводите вторую розетку в тот же кабинет»
    // не инвариант схемы, а работа диспетчера модуля. `NULLS NOT DISTINCT` был бы здесь прямой
    // ошибкой: он разрешил бы одну заявку без аппарата на весь портал.
...
export const serviceRequestFiles = pgTable(
  'service_request_files',
  {
    ...
    /**
     * Роль файла в заявке (Р5 плана освобождения, миграция 0306). `closing_evidence` — вложение и
     * закрывающая бумага, то есть сегодняшнее поведение; `estimate_basis` — страница документа, на
     * котором стоит денежное решение (счёт, поданный вместо формы объёма работ).
     *
     * Роль — ВТОРАЯ половина правила «что закрывает заявку»: первая, вид документа, зависит от
     * формата действующей ревизии (у документной закрывает только акт, В10). Без роли счёт-основание
     * был бы для очереди документов той же закрывающей бумагой, что и акт, — заявка закрывалась бы
     * собственным основанием.
     *
     * Умолчание `closing_evidence` — требование окна выката: старый код подшивает файлы, про роль не
     * зная, и роль обязана получаться та же, что была до выката. Назначает её только сервер, клиент
     * не выбирает (Р5).
     */
    purpose: text('purpose')
      .notNull()
      .default('closing_evidence')
      .$type<'closing_evidence' | 'estimate_basis'>(),
    /**
     * Чьим основанием файл является — номер ревизии, а не ссылка на строку: у документной ревизии
     * страниц несколько, и связь «файл → ревизия» живёт здесь же, в самой связи. Отдельной таблицы
     * страниц нет намеренно (Р4): перенеси мы ссылку в соседнюю таблицу, правило «роль
     * `estimate_basis` бывает только у счёта и только с ревизией» перестало бы быть выразимым одним
     * `CHECK` — понадобился бы отложенный триггер, а роль и страницы смогли бы разойтись.
     *
     * Номер не произвольный: его держит составной внешний ключ на строку ревизии (ниже,
     * `estimateRevisionFk`) — `CHECK` рядом требует лишь непустоты, а ссылка на ревизию, которой нет,
     * прятала бы доказательство денежного решения, не удаляя его.
     */
     ...
     * она вторая, — иначе распознавание (Р12) привяжет предложение не к тому листу. Пусто у
     * вложений и у одностраничного основания.
     */
  ...
  (t) => ({
    ...
    purposeCheck: check(
      ...
      sql`${t.purpose} IN ('closing_evidence','estimate_basis')`,
    ),
    /**
     * Роль основания не раздаётся сама, и правило ДВУСТОРОННЕЕ (миграция 0309; в 0306 оно было
     * импликацией, и односторонним его нашла сверка — §8 плана). Три условия закрывают три разные
     * дыры.
     *
     * Вид `invoice`: без него любая подшивка — фотография поломки, акт, гарантийный талон — смогла
     * бы стать «основанием» и выпала бы из закрывающих бумаг, то есть заявка перестала бы
     * закрываться.
     *
     * РАВЕНСТВО «роль ⇔ ревизия», а не импликация. Слева направо: без ревизии счёт стал бы
     * беспризорным основанием — замок «основание не снимается никогда» (Р6) ищет страницы по
     * ревизии, и файл без неё снялся бы обычной кнопкой вместе с доказательством денежного решения.
     * Справа налево: закрывающая бумага с номером ревизии несёт поле, которое для неё не означает
     * ничего, но составной внешний ключ ниже сторожит его как доказательство — сброс сметы по
     * переназначению отбивался бы 422 ради акта, основанием никогда не бывшего, а ручка снятия тот
     * же акт при этом отдавала бы (она спрашивает роль).
     *
     * `page_no` только у основания: номер листа вне документа не означает ничего.
     */
    basisCheck: check(
      'service_request_files_basis_check',
      sql`(${t.purpose} = 'estimate_basis') = (${t.estimateRevision} IS NOT NULL)
          AND (${t.purpose} = 'estimate_basis' OR ${t.pageNo} IS NULL)
          AND (${t.purpose} <> 'estimate_basis' OR ${t.kind} = 'invoice')`,
    ),
    // Нулевая и отрицательная страница — не порядок, а опечатка: нумерация листов начинается с 1.
    pageNoCheck: check(
      'service_request_files_page_no_check',
      sql`${t.pageNo} IS NULL OR ${t.pageNo} >= 1`,
    ),
    /**
     * АДРЕС СТРАНИЦЫ ОСНОВАНИЯ ОДНОЗНАЧЕН (миграция 0309). Тройка «заявка, ревизия, лист»: ревизии
     * нумеруются внутри заявки, и «лист 2 ревизии 1» без заявки не адрес. Сегодня страницы пишет
     * один `INSERT` одной команды, и столкнуться номерам нечем, — но на этот номер обопрётся разбор
     * документа (Р12): два первых листа означали бы, что распознанное некуда класть, и не отбивалось
     * бы это ничем.
     *
     * В БД индекс объявлен с `NULLS NOT DISTINCT`: пустой `page_no` у основания схема допускает
     * (одностраничный счёт порядка листов не имеет), а без модификатора `NULL <> NULL` — и два
     * беспорядковых основания одной ревизии прошли бы оба, то есть ровно тот неоднозначный адрес.
     * Драйвер модификатора не выражает, поэтому здесь индекс описан без него: источник истины — SQL
     * миграции (так же описан `vehicles_rental_offer_unique`).
     */
    basisPageUnique: uniqueIndex('service_request_files_basis_page_unique')
      .on(t.requestId, t.estimateRevision, t.pageNo)
      .where(sql`${t.purpose} = 'estimate_basis'`),
    ...
    /**
     * Отбор закрывающих документов после появления роли (Р5, миграция 0306): предикат спрашивает
     * «есть ли у этой заявки бумага такого вида в такой роли», и обе половины правила теперь в
     * ключе — `EXISTS` закрывается индексом, не заглядывая в таблицу. Вопрос задают семь читателей
     * (Н8 плана), двое из них — SQL-ем в очереди и в отборе автозакрытия, то есть в фоновом воркере,
     * где медленный план заметят последним.
     *
     ...
     * индекса. Соседний `service_request_files_doc_idx` остаётся: он меньше и отвечает тем, кто про
     * роль не спрашивает вовсе.
     */
    closingIdx: index('service_request_files_closing_idx').on(t.requestId, t.kind, t.purpose),
    /**
     * СТРАНИЦА ОСНОВАНИЯ ЗНАЕТ СУЩЕСТВУЮЩУЮ РЕВИЗИЮ (замок Р6 п. 2, миграция 0306). Единственное, что
     * сторожит ключ, — отсутствие ссылки в пустоту: `service_request_files_basis_check` требует лишь
     * НЕПУСТОГО номера, и `estimate_revision = 99` оставил бы страницу счёта висеть на небывшем
     * предъявлении. Замок «основание не снимается никогда» ищет страницы по ревизии — по номеру,
     * которого нет, он не нашёл бы ничего, то есть доказательство денежного решения пропало бы, не
     * будучи удалённым.
     *
     * УДАЛЕНИЕ САМОЙ СВЯЗИ КЛЮЧОМ НЕ ЗАПРЕЩЕНО И ЗАПРЕЩЕНО БЫТЬ НЕ МОЖЕТ: внешний ключ сторожит
     * ссылаемую строку, а не ссылающуюся. Запрет снятия страницы-основания — проверка в ручке под той
     * же блокировкой, которой идёт удаление (Р6 п. 1), и ключ её не заменяет.
     *
     * `restrict`, а не `cascade` — в отличие от заявления и спора ниже, и разница не в строгости.
     * У тех своей ссылки на заявку нет, их строки уходят только каскадом от ревизии, и `restrict`
     * сломал бы `records.purge`. У связи файла ссылка на заявку своя и тоже каскадная, а `purge` —
     * один `DELETE FROM service_requests`: обе ветви каскада уходят одним оператором, и проверка
     * `restrict` (отложенный до конца оператора триггер) находит таблицу уже пустой. Проверено
     * исполнением на PostgreSQL 17 и 16. `cascade` же был бы хуже отсутствия ключа: удаление строки
     * ревизии руками унесло бы с собой страницы основания — молча.
     *
     * КЛЮЧ НЕМЕДЛЕННЫЙ (`DEFERRABLE` drizzle не выражает, а расхождение этого файла с базой дороже
     * удобства), и потому он ЗАДАЁТ ПОРЯДОК ВСТАВОК в команде предъявления: строка ревизии пишется
     * раньше страниц. Обратный порядок отказывает на первом же документном предъявлении.
     */
    estimateRevisionFk: foreignKey({
      columns: [t.requestId, t.estimateRevision],
      foreignColumns: [
        serviceRequestEstimateRevisions.requestId,
        serviceRequestEstimateRevisions.revision,
      ],
      name: 'service_request_files_estimate_revision_fk',
    }).onDelete('restrict'),
  }),
);

/**
 * Ревизии объёма работ: формат предъявления и его реквизиты (Р4 плана
 * `docs/office-equipment-on-site-and-invoice-estimate-plan.md`, миграция 0306).
 *
 * ЗАЧЕМ ТАБЛИЦА. До этой волны ревизия была числом в самой заявке (`estimate_revision`), а её
 * содержимым — строки сметы. Документное предъявление эту картину ломает впервые: у его ревизии
 * строк нет вовсе — счёт приложен, содержимое системе пока неизвестно (В5, В9), — и значит формат,
 * автора и будущую разобранную сумму класть больше некуда.
 *
 * ФОРМАТ ХРАНИТСЯ, А НЕ ВЫВОДИТСЯ. Догадка по наличию файла или по пустоте строк разошлась бы с
 * правдой на первой же раскладке «Ведения»: она переиздаёт документную ревизию в построчную, и
 * признак обязан это пережить. От формата зависит, какая бумага закрывает заявку (Р5: у документной
 * — только акт), то есть ошибка в нём открывает дверь, а не портит надпись.
 *
 * НОСИТЕЛЬ НЕ ЗАВИСИТ ОТ СПОСОБА СОГЛАСОВАНИЯ. Счёт подают и с обычной подписью — освобождение от
 * подписи лишь ссылается на ревизию соседней таблицей, а данные документа живут здесь в любом
 * случае. Прежняя редакция плана держала их в таблице освобождений, и там они исчезали вместе с
 * отказом от автоподписи.
 */
export const serviceRequestEstimateRevisions = pgTable(
  'service_request_estimate_revisions',
  {
    requestId: uuid('request_id')
      ...
      .references(() => serviceRequests.id, { onDelete: 'cascade' }),
    /**
     * Номер — тот же, что в `service_requests.estimate_revision`; суррогатного `id` нет намеренно.
     * На ревизию ссылаются трое — связь файла, заявление об освобождении и спор, — и все трое знают
     * её именно номером: с суррогатом каждая ссылка требовала бы второго запроса, чтобы узнать, ту
     * ли ревизию она назвала.
     */
    revision: integer('revision').notNull(),
    format: text('format').notNull().$type<'items' | 'document' | 'warranty'>(),
    /**
     * Действующая ревизия ровно одна (частичный уникальный индекс ниже), прежние — `superseded`.
     * Состояние отдельным полем, а не сравнением с `estimate_revision` заявки: отозванная возвратом
     * в правку (`reopen`) ревизия остаётся тем, на чём стояло денежное решение (Р6), и «последняя по
     * номеру» перестаёт означать «действующая».
     */
    state: text('state').notNull().default('active').$type<'active' | 'superseded'>(),
    /**
     * Автор предъявления. Парного `estimate_submitted_by` в заявке нет (Н5), и теперь он не нужен:
     * автор каждой ревизии стоит в её собственной строке.
     *
     * `set null`, а не `restrict`: увольнение не должно упираться в денежный след, а сам след —
     * формат, время, сумма — остаётся правдой и без имени. Тот же довод, что у `estimate_approved_by`
     * и у источника приёмки (0198): имя события живёт в аудите.
     */
    submittedBy: uuid('submitted_by').references(() => users.id, { onDelete: 'set null' }),
    submittedAt: timestamp('submitted_at', { withTimezone: true }).notNull().defaultNow(),
    /**
     * Сумма ревизии — снимок, а не производная: у построчной её считает сервер из строк, у
     * документной её нет до разбора документа (Р2). Ноль вместо неизвестного ставить нельзя — он
     * читается как «работы бесплатны», тот же запрет, что у итога по акту.
     */
    totalAmount: numeric('total_amount', { precision: 14, scale: 2 }),
    /**
     * Реквизиты счёта: при подаче их не требуют (В13), номер и дату положит разбор документа (Р12).
     * `NULL`, а не пустая строка: «в счёте номера нет» и «счёт ещё не разобран» — разные факты, и
     * дубли счетов система до разбора не ловит вовсе (цена решения названа в плане вслух).
     */
  ...
  (t) => ({
    pk: primaryKey({ columns: [t.requestId, t.revision] }),
    formatCheck: check(
      'service_request_estimate_revisions_format_check',
      sql`${t.format} IN ('items','document','warranty')`,
    ),
    stateCheck: check(
      'service_request_estimate_revisions_state_check',
      sql`${t.state} IN ('active','superseded')`,
    ),
    // Нулевая ревизия — это «предъявления не было»: счёт ревизий начинается с первого предъявления.
    revisionPositive: check(
      'service_request_estimate_revisions_revision_check',
      sql`${t.revision} >= 1`,
    ),
    ...
    amountNonNegative: check(
      'service_request_estimate_revisions_amount_check',
      sql`${t.totalAmount} IS NULL OR ${t.totalAmount} >= 0`,
    ),
    /**
     * Одна действующая ревизия на заявку — индексом, а не аккуратностью маршрута: вторая `active` —
     * это гонка двух предъявлений, а от неё правило «какая бумага закрывает заявку» расходится молча
     * и в сторону лишнего (предикат прочитает ту ревизию, которую вернёт план запроса).
     *
     * Частичный: `superseded` у заявки сколько угодно, и индекс остаётся размером с число живых
     * предъявлений, а не с историей модуля.
     */
    activeUnique: uniqueIndex('service_request_estimate_revisions_active_unique')
      .on(t.requestId)
...
/**
 * Заявление об освобождении от подписи — чекбокс «Согласование не требуется» (Р1, Р3, миграция
 * 0306): кто заявил, под какой ревизией, с каким пояснением и что из этого вышло.
 *
 * ЗАЯВЛЕНИЕ И ИСХОД РАЗВЕДЕНЫ намеренно. Заявляет оператор сервисной компании (В1), а исход считает
 * сервер по рубильнику: `applied` — подпись проставлена, ожидание не открывалось; `observed` —
 * рубильник выключен, заявление записано, подпись собирают как обычно. Исход пишется в обоих
 * случаях: до включения рубильника служба обязана видеть, сколько таких заявлений и на какие суммы
 * приходит, не теряя при этом ни одной подписи.
 *
 * Денежного контроля у освобождения нет вовсе — ни лимита, ни политики, ни ловли дублей (Р13,
 * решения заказчика В2, В12, В13), — и эта таблица остаётся единственным следом, по которому
 * освобождения разбирают постфактум. Отсюда же требование писать её В ТОЙ ЖЕ транзакции, что
 * предъявление: аудит модуля пишется после `COMMIT`, и сбой между ними оставил бы денежное решение
 * без единой записи о том, кто его принял.
 */
export const serviceRequestEstimateExemptions = pgTable(
  'service_request_estimate_exemptions',
  {
    ...
    // событие с именем остаётся в аудите. Сам факт освобождения без имени правдой быть не перестаёт.
    declaredBy: uuid('declared_by').references(() => users.id, { onDelete: 'set null' }),
    declaredAt: timestamp('declared_at', { withTimezone: true }).notNull().defaultNow(),
    // Пояснение необязательно: перечня оснований нет (В4), «по условиям договора» портал не пишет
    // вовсе (Р1) — политики, которую он обещал бы, в системе не существует. Этой строкой исполнитель
    // скажет «мелкий ремонт на месте», если захочет.
    note: text('note').notNull().default(''),
    outcome: text('outcome').notNull().$type<'applied' | 'observed'>(),
    createdAt: createdAt(),
  },
  (t) => ({
    /**
     ...
     * исхода под одну подпись.
     */
    pk: primaryKey({ columns: [t.requestId, t.revision] }),
    /**
     * Ссылка на ревизию составная, и она же причина, по которой собственной ссылки на заявку у
     * таблицы нет: освобождение без ревизии не существует — оно заявлено ПОД конкретным
     * предъявлением. `cascade` продолжает цепочку «заявка → ревизия → заявление»: необратимое
     * удаление заявки (`records.purge`) обязано пройти целиком, а не упереться в след. `restrict`
     * здесь был бы прямой ошибкой — он останавливает удаление даже тогда, когда ссылающаяся строка
     * уходит тем же каскадом.
     */
    revisionFk: foreignKey({
      columns: [t.requestId, t.revision],
      foreignColumns: [
        serviceRequestEstimateRevisions.requestId,
        serviceRequestEstimateRevisions.revision,
      ],
      name: 'service_request_estimate_exemptions_revision_fk',
    }).onDelete('cascade'),
    // Исходов ровно два: `fallback_to_human` прежней редакции исчез вместе с лимитом (В2) — ветка,
    // которая никогда не срабатывает, врёт отчёту убедительнее, чем её отсутствие.
    outcomeCheck: check(
      'service_request_estimate_exemptions_outcome_check',
      sql`${t.outcome} IN ('applied','observed')`,
    ),
  }),
);

/**
 * Спор об освобождении (Р9, миграция 0306): «Ведение» не согласно с тем, что подпись не собирали, и
 * останавливает заявку до разбора. Механика остановки — `on_hold` с
 * `hold_kind = 'estimate_exemption_dispute'`; исход разрешения решает, куда заявка вернётся.
 *
 * СУРРОГАТНЫЙ КЛЮЧ, в отличие от заявления выше, и это решение, а не небрежность. Спор по одной и
 * той же ревизии бывает не один: исход «оставить освобождение» заявку не меняет, и через неделю, с
 * новыми доводами, её оспаривают снова. С парным ключом второй спор затёр бы первый — то есть стёр
 * бы, кто настаивал раньше и чем это кончилось, а именно эта история и есть весь постфактумный
 * контроль над освобождениями (Р13). Открытым при этом бывает ровно один (индекс ниже).
 */
export const serviceRequestEstimateDisputes = pgTable(
  'service_request_estimate_disputes',
  {
    ...
    // Причина обязательна: спор останавливает работающую заявку, и разрешать его без сказанного
    // вслух основания пришлось бы догадкой о том, что именно оспаривали.
    reason: text('reason').notNull(),
    state: text('state').notNull().default('open').$type<'open' | 'resolved'>(),
    // Исход: `keep` — освобождение остаётся, `require_signature` — автоподпись снята и ожидание
    // открыто заново (с происхождением `dispute`), `cancel` — заявка отменена. Пусто у открытого.
    outcome: text('outcome').$type<'keep' | 'require_signature' | 'cancel'>(),
  ...
  (t) => ({
    // Та же составная ссылка и тот же `cascade`, что у заявления: спор всегда о конкретном
    // предъявлении, и своей ссылки на заявку ему не нужно.
    revisionFk: foreignKey({
      columns: [t.requestId, t.revision],
      foreignColumns: [
        serviceRequestEstimateRevisions.requestId,
      ...
      name: 'service_request_estimate_disputes_revision_fk',
    }).onDelete('cascade'),
    reasonNotBlank: check(
      'service_request_estimate_disputes_reason_not_blank_check',
      sql`btrim(${t.reason}) <> ''`,
    ),
    stateCheck: check(
      'service_request_estimate_disputes_state_check',
     ...
     *
     * Якорь пары — время, а не автор: `resolved_by` обнуляется увольнением (`set null`), и равенство
     * по автору уронило бы удаление учётки — ровно та мина, которую сняли у подписи выше.
     */
    resolution: check(
      'service_request_estimate_disputes_resolution_check',
      sql`(${t.state} = 'resolved') = (${t.outcome} IS NOT NULL)
          AND (${t.state} = 'resolved') = (${t.resolvedAt} IS NOT NULL)
          AND (${t.outcome} IS NULL OR ${t.outcome} IN ('keep','require_signature','cancel'))
     ...
     * остановки, и разрешение первого сняло бы заморозку при живом втором — заявка уехала бы в
     * приёмку со спором, который никто не закрывал. Частичный: разрешённых у заявки сколько угодно.
     */
    openUnique: uniqueIndex('service_request_estimate_disputes_open_unique')
      .on(t.requestId)
      .where(sql`${t.state} = 'open'`),
    // Карточка показывает историю споров заявки, а каскад от ревизии ходит по этой же паре: ключ по
    // суррогатному `id` на оба вопроса не отвечает вовсе.
    requestIdx: index('service_request_estimate_disputes_request_idx').on(t.requestId, t.revision),
  }),
);
...
export const files = pgTable(
  'files',
  {
    ...
    /**
     * Карантин (Р6 плана освобождения, миграция 0306): ошибочно загруженный секретный или чужой
     * документ перестаёт выдаваться по ссылке, но остаётся в хранилище. Снять его обычным путём
     * нельзя — основание денежного решения не снимается никогда, — а «доказательство скрыто по
     * обращению» и «доказательства не было» суть разные факты, и второй из них был бы неправдой.
     *
     * Отдельное поле, а не `deleted_at` и не `status`: удаление уносит объект, а статус загрузки
     * отвечает на другой вопрос — дошёл ли файл до хранилища. Запрет обязан читаться в
     * `canAccessFile` РАНЬШЕ любой ветки видимости, включая автора файла: сегодня ссылку выдаёт
     * `GET /files/:id/download` по обычной видимости заявки, и «скрытый» документ продолжал бы
     * скачиваться по прямой ссылке.
     */
     ...
     *
     * ПАРЫ С `quarantined_at` У НЕГО НЕТ, И ЭТО РЕШЕНИЕ, А НЕ ПРОПУСК. Ограничение «карантин ⇒ хеш
     * есть» читается правильным, но оно ставит доступность ХРАНИЛИЩА условием закрытия доступа: хеш
     * считается потоком из S3, и на недоступном хранилище карантин стало бы невозможно поставить
     * вовсе. А ставят его по инциденту — в карточку заявки попали чужие персональные данные, — и
     * ждать, пока починят сеть, здесь нельзя. Поэтому порядок обратный: сначала закрываем доступ,
     * потом считаем хеш и дописываем его; недосчитанный хеш остаётся пустым и видимым в аудите как
     * «посчитать не удалось», а не как «карантин не состоялся».
     */
...
export const wasteTickets = pgTable(
  'waste_tickets',
  {
    ...
    /**
     * Написание даты, как его прочитала модель в графе: «17.08.25», «17 авг 26», «17.08»
     * (ADR 0166, миграция 0280). Рядом с `issuedOn`, а не вместо него: век двузначного года
     * выбирает портал по дате заявки, и человеку, сверяющему талон с бумагой, нужны обе величины —
     * что написано и что из этого получилось.
     *
     * Пустая строка — написания нет: у ручного талона его не бывает вовсе, у машинных талонов
     * прошлых разборов модель о нём не спрашивали. `NULL` здесь завёл бы третье состояние, которое
     * интерфейсу нечем отличить от пустого.
     */
...
export const wasteTicketFieldEvents = pgTable(
  ...
  (t) => ({
    ...
    // Скан живёт вместе с наблюдением; индекс обслуживает `file_is_linked` при каждой уборке файла.
    fileIdx: index('waste_ticket_field_events_file_idx')
      .on(t.fileId)
...
export const vehicleRequestCompletions = pgTable(
  'vehicle_request_completions',
  {
    ...
    completedBy: uuid('completed_by')
      ...
      .references(() => users.id, { onDelete: 'restrict' }),
    completedAt: timestamp('completed_at', { withTimezone: true }).notNull().defaultNow(),
    // ── Фактическая дата закрытия (план `docs/vehicle-request-actual-end-date-plan.md`, Р2, Р23) ──
    /**
     * Фактический конец работ: день, которым заказ закрыт. Той же командой он становится и сроком
     * заявки (Р4), поэтому сразу после закрытия совпадает с `vehicleRequests.dateTo` — и снимком
     * заведён именно поэтому, а не вопреки: `dateTo` живёт дальше своей жизнью (откат в работу,
     * продление, следующее сокращение), а вопрос «каким числом закрыли» задают к закрытию.
     *
     * Пусто у трёх множеств сразу, и все три законны: у закрытий до этой волны, у грузоперевозки
     * (дату спрашивают только у заказа спецтехники, Р3) и у арендодательской ветви, которая срока
     * не двигает вовсе (Р16). Тип наружу поэтому `string | null`, а не строка.
     */
    endedOn: date('ended_on', { mode: 'string' }),
    /**
     * Каким срок был **до** закрытия. Пара с `endedOn` отвечает «было 09.08, закрыли 05.08» — и
     * продолжает отвечать после любой следующей правки срока, когда разность из соседних полей
     * уже не восстановить. Тем же приёмом живёт снимок досрочного завершения
     * (`vehicleRequestEarlyEndings.previousDateTo`).
     *
     * Пишется **эффективный** конец срока, а не сырое поле заявки: пустая `dateTo` означает
     * однодневный заказ с концом `dateFrom` — так её читают и срез, и подписи присутствия, — и
     * запись сырого `null` уронила бы CHECK «оба или ни одного» на самом обычном заказе на день.
     */
  ...
  (t) => ({
    ...
    // Пара или ничего. «Закрыли 05.08» без «было 09.08» не отвечает на вопрос, ради которого
    // снимок и заведён, а «было 09.08» без факта не объясняет ничего. Второе условие — граница
    // самого понятия: закрыть позже утверждённого срока нельзя, это продление, и на новые дни
    // нужна бумага (Р15). Равенство законно и обыденно — заказ, отработанный ровно по сроку,
    // закрывается своим последним днём и бумаги не трогает.
    endedOnCheck: check(
      'vehicle_request_completions_ended_on_check',
      sql`(${t.endedOn} IS NULL) = (${t.previousDateTo} IS NULL)
        AND (${t.endedOn} IS NULL OR ${t.endedOn} <= ${t.previousDateTo})`,
    ),
...
export const waybills = pgTable(
  'waybills',
  {
    ...
    // ── След сокращения периода (план `docs/vehicle-request-actual-end-date-plan.md`, Р12) ──
    // Заказ закрывают фактической датой, и неделя ЭСМ-2 приводится к ней **правкой того же
    // бланка**, а не перевыпиской: номер не сжигается, площадка не расписывается заново за уже
    // отработанные дни (Р5, Р6). Цена — лист перестаёт быть равен тому, каким его выдали, и
    // объяснить расхождение обязана сама строка.
    /**
     * Каким лист был **выдан**: `periodTo` на момент выписки. Заполняется первым сокращением и
     * дальше не трогается никогда, в том числе повторным (Р12). В этом весь её смысл: `periodTo`
     * отвечает «до какого числа лист действует сейчас», а «до какого числа его выписали» после
     * правки не восстановить ниоткуда — номер тот же, снимок `data` приведён к факту, а печатный
     * экземпляр остался на площадке прежним (Р13). Переписать её второй правкой значило бы
     * терять этот ответ при каждом следующем сокращении.
     */
    periodToOriginal: date('period_to_original', { mode: 'string' }),
    /**
     * Когда лист приведён к нынешнему виду. Тройка «когда, кто, почему» вместе со ссылкой на
     * операцию описывает **последнюю** правку, а не первую: прежние читаются в событиях
     * `waybill.esm2_sync` и в снимках операций коррекции, и второй их копии здесь не заводим —
     * она разъехалась бы с ними при первом же расхождении.
     */
    periodTrimmedAt: timestamp('period_trimmed_at', { withTimezone: true }),
    /**
     * Кто сократил. RESTRICT — как у `issuedBy` и `cancelledBy`: правку бланка строгой отчётности
     * обязано пережить увольнение того, кто её сделал.
     */
    periodTrimmedBy: uuid('period_trimmed_by').references(() => users.id, { onDelete: 'restrict' }),
    /**
     * Почему период укоротили: «Заказ ТС-123 закрыт фактической датой 05.08.2026». Причина лежит
     * своей колонкой рядом с `cancelReason` и `correctionReason` по той же причине, что и они:
     * двух разных ответов на вопрос «почему этот бланк не такой, каким выдан» быть не должно.
     * Отсылкой к журналу операций тут не обойтись — обычное закрытие сегодняшним днём операции не
     * создаёт вовсе (Р8), и причина у такого листа осталась бы ненаписанной.
     *
     * `NOT NULL DEFAULT ''` — форма прочих текстовых граф листа: «причины нет» здесь пустая
     * строка, и CHECK связывает её пустоту с отсутствием правки.
     */
    periodTrimReason: text('period_trim_reason').notNull().default(''),
    /**
     * Операция журнала коррекций, в рамках которой лист сокращён, — у **любой неординарной**
     * правки, а не только у заднего числа: условие то же, каким сегодня связывают лист отмена и
     * выпуск (`context.kind !== 'ordinary'` в `services/waybill-esm2.ts`). Исход
     * `assignment_tail` — такая же операция журнала, как `crew`, и сокративший лист обязан на неё
     * сослаться.
     *
     * Третья ссылка на операцию рядом с `correctionId` (кто лист породил) и `cancelCorrectionId`
     * (кто списал), и свести их в одну нельзя: у листа бывают все три сразу — рождён
     * перевыпиской, через неделю сокращён, ещё через неделю списан.
     *
     * Обычная последующая правка ссылку **снимает**: она обязана описывать ту же, последнюю
     * правку, что и тройка «когда, кто, почему», иначе объясняла бы нынешний вид листа чужим
     * действием.
     */
    periodTrimCorrectionId: uuid('period_trim_correction_id').references(
      () => waybillCorrections.id,
      { onDelete: 'restrict' },
    ),
    version: integer('version').notNull().default(0),
    createdAt: createdAt(),
  ...
  (t) => ({
    ...
    // Правка периода — комплект или ничего (Р12). Держит это схема, а не сервис, по той же
    // причине, по какой схема держит причину перевыписки: сокращение приезжает из общего
    // исполнителя бумаги, а зовут его несколько дверей — закрытие, правка срока, обе ветви
    // досрочного завершения, — и достаточно одной забыть причину или автора, чтобы в журнале
    // появился бланк, не такой, каким выдан, и без объяснения. Это ровно то состояние, ради
    // закрытия которого заводился журнал коррекций.
    //
    // Условия по порядку:
    //   · «когда» и «кто» ходят парой — метка без автора не отвечает, с кого спрашивать;
    //   · «когда» и «каким выдан» ходят парой — без прежнего конца метка не показывает правку,
    //     а без метки прежний конец нечем датировать;
    //   · причина непуста ровно у сокращённого — и пуста у всех прочих: у листа, который не
    //     правили, объяснение правки означало бы, что правку потеряли;
    //   · ссылка на операцию бывает только у сокращённого — обратное означало бы лист, который
    //     операция правила, а следов правки нет;
    //   · и, наконец, `period_to_original` строго больше нынешнего конца: равенство назвало бы
    //     сокращённым лист, который не сокращали, а меньшее — расширение выданного бланка,
    //     которого правка не умеет ни при каких входных данных (Р6).
    //
    // `period_to IS NOT NULL` в последнем условии — сверх текста плана, и намеренно. Без него
    // сравнение у листа без периода даёт NULL, а NULL в CHECK проходит: сокращённым мог бы
    // числиться рейсовый 4-П, у которого недели нет вовсе (`waybills_form_source_check`).
    // Сокращают только ЭСМ-2, и ограничение обязано это говорить, а не молчать трёхзначной
    // логикой.
    periodTrimCheck: check(
      'waybills_period_trim_check',
      sql`(${t.periodTrimmedAt} IS NULL) = (${t.periodTrimmedBy} IS NULL)
        AND (${t.periodTrimmedAt} IS NULL) = (${t.periodToOriginal} IS NULL)
        AND (${t.periodTrimmedAt} IS NULL) = (btrim(${t.periodTrimReason}) = '')
        AND (${t.periodTrimmedAt} IS NOT NULL OR ${t.periodTrimCorrectionId} IS NULL)
        AND (${t.periodToOriginal} IS NULL
          OR (${t.periodTo} IS NOT NULL AND ${t.periodToOriginal} > ${t.periodTo}))`,
    ),
    // Третий источник метки «коррекция» в журнале листов (Р12) — рядом с двумя такими же
    // частичными индексами по ссылкам на операцию. Сокращённых листов в журнале доля малая, и
    // условие держит индекс размером с эту долю, а не с таблицей.
    periodTrimmedIdx: index('waybills_period_trimmed_idx')
      .on(t.periodTrimmedAt.desc())
      .where(sql`${t.periodTrimmedAt} IS NOT NULL`),
  }),
);
...
export const moduleMailEventSettings = pgTable(
  ...
  (t) => ({
    ...
    // совпадать с `MODULE_MAIL_EVENTS` и с CHECK миграций 0258 и M3 волны кандидатов (там он
    // заменяется целиком: значения в `CHECK` не дописываются, ограничение снимается и ставится
    // заново одной транзакцией).
    eventCheck: check(
      ...
      sql`${t.event} IN ('service_request_waiting_it', 'service_request_cancelled',
                         ...
                         'service_request_comment', 'office_equipment_candidate_pending',
                         'office_equipment_candidate_decided')`,
    ),
  }),
);

// ── Рубильник приёма (план `docs/office-equipment-request-subject-plan.md`, Р10, миграция 0293) ──
//
// Соседняя таблица выше отвечает «уходит ли письмо по событию»; эта — «принимаем ли мы такие данные
// вообще». Приём тот же (ключ → флаг), сущность другая, и слиты они не будут: общий `CHECK` собрал
// бы два несвязанных перечня, а админка писем показывала бы строку, которая писем не шлёт.
//
// Заведена ради одной находки ревью (Р11): миграция прав применяется ДО перезапуска приложения, и в
// это окно старый код уже принимает кандидатов. Рубильник переворачивает порядок — код едет
// закрытым, а открывает его `UPDATE` по работающему серверу.
//
// Полным резолвером флагов это не является: пилотных областей, аудита переключения и
// административной ручки здесь нет — значит нет и колонок `updated_by`/`version`, которые у
// почтовой настройки стоят из-за её экрана. Строка без экрана описывала бы несуществующую ручку.
export const featureFlags = pgTable(
  'feature_flags',
  {
    /** Ключ строки — сам рубильник: второму рубильнику одного ключа взяться неоткуда. */
    key: text('key').primaryKey().$type<FeatureFlagKey>(),
    /** Умолчание выключенное: ключ без строки обязан молчать, а не открывать приём. */
    isEnabled: boolean('is_enabled').notNull().default(false),
    updatedAt: updatedAt(),
  },
  (t) => ({
    // Перечень обязан совпадать с `FEATURE_FLAGS` в контрактах и с `CHECK` в базе (завела 0293,
    // второй ключ дописала 0300, третий и четвёртый — 0307). Причина та же, что у рубильника писем:
    // лишняя строка выглядит настройкой («рубильник включён»), пока настоящий ключ сидит без строки
    // и молчит fail-closed, — то есть выключение выглядело бы включением. Новый ключ приходит со
    // своей строкой, то есть со своей миграцией: без строки `UPDATE … SET is_enabled = true`
    // обновляет ноль рядов и молчит, и оператор уходит с уверенностью, что включил.
    //
    ...
    //
    // Третий и четвёртый — объём работ документом и освобождение от подписи (план
    // `docs/office-equipment-on-site-and-invoice-estimate-plan.md`, Р2, Р3 и §7; миграция 0307).
    // Ключей ДВА, а не один, и это решение плана: подача счётом законна и с человеческой подписью,
    // поэтому слитый ключ открывал бы автопринятие суммы тем же `UPDATE`, которым включают
    // документную подачу, а выключение подачи гасило бы работающий сценарий вместе с ней.
    keyCheck: check(
      'feature_flags_key_check',
      sql`${t.key} IN ('office_equipment_candidate_intake', 'service_request_executor_scope',
        'service_estimate_document_mode', 'service_estimate_exemption')`,
    ),
  }),
);
...
export type OfficeEquipmentPurchaseRow = typeof officeEquipmentPurchases.$inferSelect;
export type OfficeEquipmentPurchaseItemRow = typeof officeEquipmentPurchaseItems.$inferSelect;
