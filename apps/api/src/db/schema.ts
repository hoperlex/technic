...
export const serviceRequestFiles = pgTable(
  ...
  (t) => ({
    ...
    /**
     * Роль основания не раздаётся сама: `estimate_basis` бывает только у счёта и только у связи,
     * знающей свою ревизию. Оба условия держат разные дыры. Без вида любая подшивка — фотография
     * поломки, акт, гарантийный талон — смогла бы стать «основанием» и выпала бы из закрывающих
     * бумаг, то есть заявка перестала бы закрываться. Без ревизии счёт стал бы беспризорным
     * основанием: замок «основание не снимается никогда» (Р6) ищет страницы по ревизии, и файл без
     * неё снялся бы обычной кнопкой вместе с доказательством денежного решения.
     */
    basisCheck: check(
      'service_request_files_basis_check',
      sql`${t.purpose} <> 'estimate_basis'
          OR (${t.kind} = 'invoice' AND ${t.estimateRevision} IS NOT NULL)`,
    ),
