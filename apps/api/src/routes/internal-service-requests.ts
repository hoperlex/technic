 ...
 * `service-requests.ts`). Все прочие читатели спрашивают `closingKindsForFormat` и передают ему
 * формат действующей ревизии, а отбор пачки спросить его не может: формат у каждой строки свой, и
 * одним перечнем видов на всю выборку правило Р5 не выражается. Сегодня перечень отвечает планкой
 * наследия, и поведение этим не меняется; Э3 ОБЯЗАН перевести отбор на формат — и `FIRST_DOCUMENT_AT`
 * вместе с ним, иначе срок созревания считался бы от счёта, который документную заявку не закрывает.
 * Без этой пометки копия разойдётся с контрактами молча: заявка закроется сама по бумаге, которой
 * планка уже не снимается.
 */
