...
/*
 * Снимок и повтор — трём дверям этого файла, зовущим общий бумажный план мимо каркаса команд
 * (решение Б5 плана `docs/assignment-periods-plan.md`, §7 «все двери, зовущие общий план», и
 * протокол повторов В4).
 *
 * Уровень изоляции берётся из каркаса тем же значением (`SNAPSHOT_ISOLATION`), а не переписывается
 * здесь строкой: две редакции одного уровня разошлись бы молча и разошлись бы бумагой. Обёртка
 * повторов — оттуда же и по той же причине, по какой она одна на портал: `40001` под новой
 * изоляцией перестал быть гипотезой, и дверь без обёртки отдавала бы человеку пятисотку за чужую
 * очередь.
 */
import { withAssignmentRetry } from '../services/assignment-retry';
/*
...
// Дни заказа техники на объект (ADR 0100, ADR 0207): чтение плана и сверка его с заявкой. Сверка стоит рядом со
...
/*
 * Метки трёх дверей этого файла в счётчике повторов (Б5, В4).
 *
 * Метка — на **дверь**, а не на файл: потолок повторов подбирается по числу одновременных
 * писателей у конкретной двери (спайк, §4.3), и смена статуса с ручной выпиской бланка делят разве
 * что строку серии номеров. Имена — те же, какими эти двери зовутся в остальном модуле (`DOORS`
 * бэкстопа, отпечаток `assignment-reassign`), чтобы графики повторов и отказов совмещались по одной
 * и той же метке, а не по двум написаниям одной двери.
 */
const ESM2_ON_DEMAND_DOOR = 'request-esm2';
const REQUEST_STATUS_DOOR = 'request-status';
const REQUEST_ASSIGNMENT_DOOR = 'request-assignment';
...
const requestSelect = {
  ...
  /**
   * Стоит ли заявка в действующем листе хоть каким-нибудь способом — грузовой строкой состава,
   * днём (`work_date`) или перегоном.
   *
   * Соседний `routeHasWaybill` на этот вопрос не отвечает и никогда не отвечал: он про **этот**
   * рейс, тот единственный, у которого дня нет (join выше отобран `work_date IS NULL`). У заказа
   * техники на объект такой строки не бывает вовсе — его бумага висит на рейсах дней, — и портал,
   * спрашивавший блокер отката им, открывал окно, собирал причину и получал 409 сервера.
   *
   * Правило одного места: условие повторяет `activeWaybillOfRequest`
   * ([vehicle-routes.ts](../services/vehicle-routes.ts)) — ту самую функцию, которой откат в
   * «Новую» и запрещают (`ROLLBACK_WAYBILL_MESSAGE`). Разойдись они — портал снова начал бы
   * обещать действие, которое сервер не даст. Способов попасть в бумагу столько же: лист рейса,
   * в составе которого стоит заявка (строкой дня или грузовой строкой), лист рейса-перегона
   * (`source_request_id` рейса, ADR 0057) и лист, выписанный ещё вне маршрутов (`legacyWaybillOf`:
   * `route_id IS NULL`, бланк не ЭСМ-2).
   *
   * Ветвей здесь три, а не две, и это та же проверка, разложенная по `OR`: у функции состав и
   * перегон стоят одним условием внутри соединения листа с рейсом, а здесь каждая половина ходит
   * от **заявки** — `vehicle_route_requests(request_id)` и `vehicle_routes(source_request_id)`
   * своими индексами, лист по `waybills_route_unique`. Написанные как у функции, они заставляли
   * Postgres перебирать все действующие листы на каждую строку списка (проверено `EXPLAIN`):
   * ответ тот же, цена — полный проход по журналу листов на каждую заявку страницы.
   *
   * Аннулированный лист не держит: испорченный бланк списан, и работа заявки к нему больше не
   * относится. ЭСМ-2 не держит тоже — в первые две ветви он не попадает сам (рейса у него нет,
   * `route_id` пуст, и `JOIN` его отбрасывает), а в третьей отсечён по бланку.
   *
   * Выражением того же `SELECT`, что и `routeHasWaybill`, а не добором на страницу: признак у
   * строки один, и второй проход по списку ради булева ответа не нужен. Корреляция читается
   * колонками внешнего запроса — таблиц во `FROM` здесь два десятка, и переписывание списка
   * столбцов, которое губит такие подзапросы в односоставных выборках, сюда не приходит.
   */
  hasActiveWaybill: sql<boolean>`(EXISTS (
    SELECT 1 FROM ${vehicleRouteRequests} arr
    JOIN ${waybills} aw ON aw.route_id = arr.route_id AND aw.status <> 'cancelled'
    WHERE arr.request_id = ${vehicleRequests.id}
  ) OR EXISTS (
    SELECT 1 FROM ${vehicleRoutes} ar
    JOIN ${waybills} arw ON arw.route_id = ar.id AND arw.status <> 'cancelled'
    WHERE ar.source_request_id = ${vehicleRequests.id}
  ) OR EXISTS (
    SELECT 1 FROM ${waybillRequests} lwr
    JOIN ${waybills} lw ON lw.id = lwr.waybill_id
    WHERE lwr.request_id = ${vehicleRequests.id}
      AND lw.route_id IS NULL
      AND lw.form_code <> 'esm2'
      AND lw.status <> 'cancelled'
  ))`.as('has_active_waybill'),
...
function baseQuery() {
  return (
    db
      ...
      // миграция 0127). Условие обязательное, а не украшение: заказ техники на объект стоит в
      // стольких рейсах, сколько дней у него распланировано, и без него строка списка размножилась
      // бы по числу дней. Прежде так жил только линейный заказ, с ADR 0207 — любой: дни перестали
      // быть свойством типа ТС. План показывает своя ручка (`GET /:id/days`) — массив дней в
 ...
 * (ADR 0100 §12).
 *
 * Вторая половина условия появилась вместе с линейной техникой, но линейности не принадлежит и
 * тогда: у заказа, который ведут днями, назначение — машина по умолчанию (ADR 0100 §4), а работали
 * в конкретные дни другие единицы, и отбор по одному назначению отвечал бы на вопрос «где ходила
 * ТС-341» неверно в обе стороны сразу: показывал бы заказы, на которые она только назначена, и
 * прятал бы те, которые она на самом деле отработала. С ADR 0207 днями ведут любой заказ техники
 * на объект, и половина эта стала общей — спрашивать её по признаку типа было бы уже неверно.
 *
 ...
 * рейсов у заказа, который ведут сроком стояния на площадке, нет вовсе. Поэтому в режиме `history`
 * у заявки с историей вместо равенства назначению спрашивается пересечение отрезков истории со
 * сроком ([assignment-read.ts](../services/assignment-read.ts)).
 *
 * Вторая половина — дни заказа — остаётся в обеих ветвях нетронутой, и это не осторожность: история
 * хранит заявке машину по умолчанию, а работали днями другие единицы (ADR 0100 §12). Замени мы и её —
 * отбор перестал бы находить то единственное, ради чего он и заводился. С ADR 0207 это касается уже
 * не только линейных заказов: дни завелись и у тех, чья история про машину площадки не врёт, — но и
 * там день честнее назначения, потому что бумагу выписали на машину рейса.
 *
 ...
 *
 * Тем же правилом с ADR 0207 отсюда уходит и обычный заказ, чей день распланирован: у дня есть
 * свой рейс, и 4-П выписан на его машину — история про этот день говорила бы о машине площадки,
 * расходясь с бумагой. Спрашивают этот источник вторым, после рейса дня (`toDtos`).
 */
  ...
  const base = {
    ...
    route:
      r.routeId && r.routeNum !== null && r.routePosition !== null
        ...
        : null,
    // Бумага заявки целиком, а не бумага её рейса: `route.hasWaybill` выше молчит у заказа
    // техники на объект — своего грузового рейса у него нет, — а листы у него есть, и дверь
    // отката обязана знать о них до того, как спросит причину. Считается выражением выборки,
    // одним условием с сервером (`activeWaybillOfRequest`).
    hasActiveWaybill: r.hasActiveWaybill,
    files: fileList,
    version: r.version,
 ...
 *   список и подписи в нём отвечали бы про разные сутки. Тот же день выбирает и машину строки у
 *   заказа, чей день поставлен в рейс (ADR 0100, а с ADR 0207 — у любого заказа техники на
 *   объект): срез отвечает «что на площадке сегодня», а не «чем заказ взяли в работу вообще»;
...
async function toDtos(
  ...
  return rows.map((row) =>
    toDto(
      ...
      {
        /*
         * Машина дня: сперва рейс этого дня — у любого заказа, чей день распланирован. Режима
         * заявки этот вопрос больше не спрашивает (ADR 0207 §1): дни и 4-П получает и заказ на
         * объект, который прежде вёлся одними неделями, и назови колонка его машину назначения,
         * она разошлась бы с выписанным листом — а лист печатает машину рейса.
         *
         * Дня нет — отвечают по-прежнему двое, и ответы у них разные по смыслу. У линейного
         * заказа это `null`: машина вечером уехала на базу, нераспланированный день означает, что
         * на площадке никого (ADR 0100 §12), и подставлять сюда назначение нельзя. У остальных
         * отвечает свёртка истории на день среза (Ф3), а молчит она — поля нет вовсе, и колонка
         * показывает назначение, как и до разреза срока.
         */
        dayVehicle: q.onDate
          ? (dayVehicles.get(row.id) ?? (row.isLinear ? null : historyDayVehicles.get(row.id)))
          : undefined,
 ...
 * срок, статус и назначенную машину (признак линейности в нём остался, но дверь им больше не
 * воротят, ADR 0207 §1), — и берут его отдельным запросом: DTO отвечает «что это за заявка», а не
 * «можно ли распланировать её день».
 */
 ...
 * грузоперевозки он один (Р7), а у заказа техники на объект их столько, сколько дней
 * распланировано (с ADR 0207 — у любого такого заказа, а не у одного линейного), и правка
 * контакта или комментария задевает их все.
 *
...
async function attachToRoute(
  ...
  params: {
    request: {
      ...
      /**
       * Линейный ли заказанный тип (ADR 0100 §1). Укладку в рейс он больше не решает — правило
       * `canJoinRoute` его не читает (ADR 0207 §1), — но идёт туда дальше вместе с остальным
       * состоянием заявки: признак остался тем, чем был, режимом ведения и ЭСМ-2.
       */
      isLinear: boolean;
  ...
  if (!requirement.formCode) {
    ...
    // техники на объект своего рейса не существует — там период стояния машины на площадке. Рейсы
    // у него бывают, но не здесь и не один: дни планируют после перевода в работу — по одному или
    // пачкой, из карточки заявки (ADR 0100 §8, ADR 0207 §4), разными машинами и разными
    // водителями. Спрашивается вид заявки, а не линейность: признак типа дверь дней больше не
    // воротит (ADR 0207 §1), и заказ, которому дни открылись вчера, обязан получить тот же ответ,
    // что линейный, — иначе портал услышал бы «маршрут не ведётся» там, где ведутся дни.
    if (params.route) {
      throw err.unprocessable(
        params.request.requestType === 'special_equipment'
          ? 'Дни заказа техники на объект планируются после перевода в работу — из карточки заявки, по одному или пачкой: на разные дни выходят разные машины'
          : (requirement.reason ??
  ...
  // Грузовая строка заявки — та единственная, у которой дня нет: день несёт сам рейс. Дни сюда не
  // попадают вовсе (они бывают у заказа техники на объект, а досюда доходит грузоперевозка), но
  // спрашивать их «заодно» нельзя — ответом стал бы первый попавшийся день.
 ...
 * 2. **День заказа, поставленный в рейс**, — бланк рейса по машине **рейса**, а не назначения:
 *    тем же ответом колонка называет машину дня (Р16), и отбор с колонкой обязаны говорить об
 *    одной машине. Перегон идёт по 4-П независимо от типа, грузовой рейс — по бланку типа своей
 *    машины. Режима заявки эта ветка не спрашивает, и с ADR 0207 это уже не потому, что день
 *    бывает только у линейного заказа: день бывает у любого заказа техники на объект, а бланк ему
 *    даёт рейс. Отсюда и строка, у которой в наборе стоят сразу два бланка: неделя ЭСМ-2 из
 *    ветки 1 и 4-П дня отсюда (ADR 0207 §2) — бумаги две, работа одна, и отбор обязан находить
 *    такую строку по любому из двух.
 * 3. **Лист ЭСМ-2 по требованию на распланированный день** (ADR 0100 §5) — второй бланк той же
 *    работы: неделю заказа, который ведут днями, называет человек, и день закрывается сразу
 *    двумя бумагами.
 ...
 * (Р6).
 *
 * Условием `EXISTS`, а не join'ом к работам дня: у распланированного дня бывает две работы разных
 * бланков сразу — рейс и недельный лист, — и join показал бы такую заявку в выдаче дважды. Пустой
 ...
 * и с ADR 0207 это правило строки, а не линейных заказов: рейс дня отвечает у всякого заказа,
 * чей день распланирован, потому что на машину этого рейса и выписан лист.
 */
...
export default async function vehicleRequestsRoutes(app: FastifyInstance): Promise<void> {
  ...
  const listWhere = (p: Principal, q: VehicleRequestListQuery) =>
    and(
      ...
      // заказа, отработанные этой машиной, — одним общим выражением на все пять выдач.
      requestVehicleWhere(q.vehicleId),
      q.num ? eq(vehicleRequests.num, q.num) : undefined,
      approvedFilter(q.approved),
      ...dateFilters(q.requestType, q.dateFrom, q.dateTo),
  ...
  r.get(
    ...
    async (req) => {
      ...
      // дню, по которому отбирались строки. Он же выбирает машину строки у заказа, чей день
      // распланирован (ADR 0100 §12, ADR 0207): срез отвечает про сегодняшнюю площадку, а не про
      // назначение вообще.
  ...
  r.patch(
    ...
    async (req) => {
      ...
      const applyEdit = async (tx: Tx): Promise<void> => {
         ...
         * сборкой дня. Их бывает несколько: у грузоперевозки рейс один, а у заказа техники на
         * объект столько, сколько дней распланировано (Р7), и правка задевает их все — поэтому берутся по
        ...
        if (body.requestType === 'special_equipment') {
          ...
          // Задание дня заказа на объект печатается контактом заявки и её комментарием (Р9б, Р11): у
        ...
        if (periodEdited) {
          ...
          /*
           * Дни заказа, которых рейс не отдал (ADR 0100 п. 11, Р11). У обычной правки
   ...
   *
   * Право — то же, каким открывается сама заявка (ADR 0122): контакт человека, который приедет на
   * площадку, видит каждый, кто видит заявку. Прежнее `waybills.read` закрывало его как
   * персональные данные путевого листа, и заказчик — тот, кто встречает машину, — оставался
   * единственным, кто не мог позвонить водителю; звонили диспетчеру, чтобы тот назвал номер.
   * Граница ПДн осталась там, где ей место: карточка водителя (`drivers.read`) со СНИЛС и
   * удостоверением и сам бланк по-прежнему закрыты (ADR 0037 п. 13).
   *
   * Отдельной ручкой, а не полем DTO: контакта у заявки нет — он вычисляется из работы, и лишний
   * join в списке заявок незачем. Источник зависит от вида работы: у грузоперевозки водитель
  ...
  r.post(
    ...
    async (req) => {
      ...
      if (!backdated) {
        /*
         * Снимок на всю выписку и повтор на конфликте сериализации (Б5, В4) — та же пара, что у
         * каркаса команд, и по той же причине: бумага строгой отчётности собирается десятком чтений
         * (заявка, срок, машина, документы машиниста, действующие листы, строка серии номеров), и в
         * `READ COMMITTED` каждое из них берёт свой снимок. Правка справочника, закоммиченная
         * посреди выписки, попала бы в бланк вместе с состоянием, которого целиком не существовало
         * ни в один момент, — и заметить это по ответу было бы нечем.
         *
         * ПОВТОР ЗДЕСЬ БЕЗОПАСЕН, И ЭТО ПРОВЕРЯЕТСЯ ПОСТРОЧНО. Транзакция ничего не делает наружу:
         * письмо, файл и чужой сервис ей не нужны вовсе, а номер бланка берётся строкой серии в
         * той же транзакции — откатится вместе с ней. Аудит выписки пишется **после** коммита, по
         * списку `issued`, и вторая попытка переписывает этот список целиком, а не дополняет:
         * событий в ленте ровно столько, сколько листов действительно родилось. Планирование при
         * повторе целиком новое: недели, машину, документы и занятость листов `issueEsm2OnDemand`
         * читает сам, внутри замыкания.
         *
         * Единственное, что посчитано **до** транзакции, — периоды, по которым спрошено право
         * (`periods`). Их повтор не пересчитывает, и потому они и едут внутрь `guardedPeriods`:
         * разъехавшись со сроком заявки между попытками, они дадут конфликт с объяснением, а не
         * бланк прошедшей недели без права и без причины.
         */
        issued = await withAssignmentRetry(ESM2_ON_DEMAND_DOOR, () =>
          db.transaction((tx) => issueInTransaction(tx, null), SNAPSHOT_ISOLATION),
        );
      } else {
        ...
        /*
         * ЗАДНИЙ ХОД ОСТАЁТСЯ НА ИЗОЛЯЦИИ ПУЛА, И ЭТО РЕШЕНИЕ, А НЕ ЗАБЫВЧИВОСТЬ (Б5).
         *
         * Транзакцию здесь открывает не эта дверь, а `runCorrection` — общий вход всех команд
         * задним числом (ADR 0101), и живут в нём семь видов операций, к бумажному плану отношения
         * не имеющих. Поднять уровень там значило бы раздать `40001` дверям, у которых обёртки
         * повторов нет, — то есть обменять редкий гибридный снимок на регулярные пятисотки в
         * соседних ручках.
         *
         * А снаружи повтором накрыть эту ветку нельзя: `authorize` и периоды (`periods`) считаются
         * **до** транзакции, повтор их не пересчитывает, и вторая попытка применяла бы решение о
         * праве, принятое по снимку, которого PostgreSQL уже объявил устаревшим. Снимок без
         * повтора — регрессия, повтор без пересчёта — бумага по чужому состоянию; поэтому здесь не
         * делается ни того, ни другого, а ветка остаётся такой, какой была до Б5. Закрывается это
         * не настройкой транзакции, а переносом расчёта внутрь — работой размером с отдельную
         * волну.
         */
        const outcome = await runCorrection(
          {
  ...
  r.patch(
    ...
    async (req) => {
      ...
      /*
       * Снимок на весь переход и повтор на конфликте сериализации (Б5, В4).
       *
       * ПОЧЕМУ ЭТОЙ ДВЕРИ СНИМОК НУЖЕН НЕ МЕНЬШЕ, ЧЕМ КАРКАСУ. Переход зовёт ту же недельную сверку
       * (`syncEsm2Waybills`), что и двери истории, и она собирает бланки десятком чтений — срок,
       * назначение, машину, документы машиниста, действующие листы, строку серии номеров. В
       * `READ COMMITTED` каждое из них берёт свой снимок, и правка справочника, закоммиченная между
       * двумя из них, уходит в бумагу строгой отчётности состоянием, которого целиком не было ни в
       * один момент времени. Плюс отпечаток последствий: он сверяется здесь же, `buildEsm2SyncPlan`
       * на одном снимке, а сверка печатает по другому — и 409 «посмотрите заново» приходил бы на
       * состояние, которого никто не менял.
       *
       * ПОВТОР У ЭТОЙ ДВЕРИ ПРИМЕНИМ ЦЕЛИКОМ, И ВОТ ПОЧЕМУ. Наружу транзакция не делает ничего:
       * все три события аудита (`vehicle_request.status`, назначение, закрытие), событие рейса и
       * сверка дней пишутся **после** коммита, писем и файлов у неё нет вовсе, а номера бланков
       * берутся строкой серии внутри и откатываются вместе с транзакцией. Всё, чем дверь решает,
       * пересчитывается внутри замыкания: режим, тип, блокировки, отпечаток последствий, сверка
       * бумаги и план дней. Единственное, что посчитано снаружи, — `before` и `today`, и они не
       * план, а посылка команды: расхождение `before` с базой ловится сверкой `version` в том же
       * `UPDATE`, то есть повтор кончится честным 409, а не записью по устаревшему состоянию.
       * Переменная `routeDriver` при повторе **перезаписывается** целиком, а не дополняется, —
       * иначе вторая попытка оставила бы событие рейса от первой.
       */
        ...
        await withAssignmentRetry(REQUEST_STATUS_DOOR, () =>
          db.transaction(async (tx) => {
            ...
            if (rollbackToWork) {
              ...
              if (
                statusPreviewFingerprint(planned?.input ?? null, req.body) !== previewFingerprint
              ) {
                throw err.conflict(
  ...
  r.patch(
    ...
    async (req) => {
      ...
      if (!correction) {
        /*
         * Снимок на всю смену техники и повтор на конфликте сериализации (Б5, В4) — ровно та же
         * пара, что у статусной двери, и по той же причине: переоформление листов зовёт общую
         * недельную сверку, а она собирает бланк десятком чтений. Машина из одного снимка и
         * документы машиниста из другого дали бы гибрид в документе строгой отчётности — молча.
         *
         * ПОВТОР ПРИМЕНИМ, НО ТОЛЬКО У ОБЫЧНОЙ СМЕНЫ (её ветка — эта). Наружу транзакция не ходит:
         * события назначения, рейса и сверки дней пишутся после коммита, писем и файлов нет,
         * номера бланков живут строкой серии внутри. План пересчитывается весь: `applyAssignment`
         * заново берёт режим, блокировки, отпечаток последствий (`planReassignCommand` — внутри
         * транзакции), сверку бумаги и план дней. Мутабельные `esm2`, `days`, `routeDriver` и
         * `assigned` вторая попытка **перезаписывает**, а не накапливает, — иначе события после
         * коммита рассказали бы о работе первой, откатившейся.
         *
         * У ветки коррекции (`else` ниже) этого свойства нет, и она остаётся как была.
         */
        assigned = await withAssignmentRetry(REQUEST_ASSIGNMENT_DOOR, () =>
          db.transaction((tx) => applyAssignment(tx, null), SNAPSHOT_ISOLATION),
        );
      } else {
        /*
         * КОРРЕКЦИЯ НАЗНАЧЕНИЯ ОСТАЁТСЯ НА ИЗОЛЯЦИИ ПУЛА И БЕЗ ПОВТОРОВ — ЗАПРЕТ, А НЕ ПРОПУСК (Б5).
         *
         * Повторять эту ветку нельзя ни при каком уровне изоляции: её план (`plan`) посчитан
         * **до** транзакции — какие листы будут названы разблокированными, какую глубину прошлого
         * она задевает, какие подписи объекта снимет и какая у неё эффективная дата (Р36). Вторая
         * попытка применила бы ровно то, о чём протокол повторов говорит «применять нельзя»:
         * решение, принятое на снимке, который PostgreSQL уже объявил устаревшим. Пока мы стояли в
         * очереди, названный лист мог быть аннулирован своей ручкой, а день — подписан объектом; по
         * такому плану операция сожгла бы не тот номер и сняла бы не ту подпись, а человек не узнал
         * бы об этом вовсе — потому и остановились здесь, а не «доделали по аналогии».
         *
         * Снимок без повтора эта ветка тоже не берёт: транзакцию ей открывает `runCorrection` —
         * общий вход всех семи видов операций задним числом, — и поднятый там уровень раздал бы
         * `40001` соседним дверям, у которых обёртки повторов нет. Правильное закрытие — перенос
         * `planAssignmentCorrection` и `authorize` внутрь транзакции, то есть перевод двери на
         * каркас команд; это отдельная работа, а не настройка транзакции.
         */
        const outcome = await runCorrection(
          {
  ...
  // ── Дни заказа техники на объект (ADR 0100, ADR 0207) ──
  // Заводились дни ради линейной техники: она вечером возвращается на базу, а за день успевает
  // поработать на нескольких площадках, и каждый день срока — отдельный выезд, который кладут в
  // рейс машины на эту дату и печатают в его 4-П. Признак типа эту дверь больше не воротит
  // (ADR 0207 §1): день и рейс получает любой заказ на объект на своей живой машине, а тип ТС
  // остался тем, чем был, — режимом ведения и основанием недельного ЭСМ-2.
  // Дверей к дню две: эта — по одному дню, и пачка на диапазон
  // ([vehicle-request-day-batch.ts](vehicle-request-day-batch.ts), ADR 0207 §4). Обе живут у
  // заявки, а не у рейса (ADR 0100 §8): день знает свой срок и свою заявку, а из карточки
  // маршрута ту же заявку пришлось бы разыскивать по объекту среди всех работающих, и срока
  // заказа рейс не знает вовсе. Своей таблицы у дней нет — перечень выводится из срока (§2).
   ...
   *
   * Право — то же, каким открывается заявка (ADR 0122). Прежнее `waybills.read` закрывало план
   * целиком ради ФИО водителей, а вместе с ними прятало от заказчика ответ на вопрос «кто и на
   * чём выйдет ко мне завтра» — тот самый, ради которого заказ и ведут по дням. Читают
   * план теперь все, кто видит заявку; **планируют** дни по-прежнему двое
   * (`waybills.read` + `vehicleRequests.status`) — дверьми ниже, а не этой.
   */
  r.get(
    '/:id/days',
    {
  ...
  r.post(
    ...
    async (req): Promise<VehicleRequestDaysDto> => {
      ...
      const planned = await db.transaction(async (tx) => {
        ...
        const check = canJoinRoute(
          {
            ...
            // Ответ заказа на объект о дне — отрезок срока и уже занятые дни: какой именно день
        ...
        /*
         * День заказа на объект — такая же строка задания, как ездка, и место в порядке объезда у
        ...
        // дни заказов (Р11), — а до постановки дня их на одну меньше.
        await assertRoutePlacement(tx, {
          routeId: route.id,
          formCode: (
