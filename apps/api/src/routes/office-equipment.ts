...
// ── Проверки и вставка карточки парка живут в `services/office-equipment-write.ts` (план
// `docs/office-equipment-candidate-plan.md`, Р14) ──
//
// Переехали они туда потому, что у справочника появился ВТОРОЙ вход: подтверждение кандидата
// заканчивается тем же заведением карточки, с теми же проверками типа, модели, номеров и области.
// Копия этих проверок во второй ручке разошлась бы с оригиналом на первой же новой проверке, причём
// молча — тесты справочника про вторую дверь ничего не знают. Здесь они остались именами: маршрут
// зовёт те же функции, и поведение `POST`, `PATCH` и перемещения не изменилось ни в одном отказе.
//
// Аудит помощник НЕ пишет и не будет (Р14): у двух входов разная семантика журнала, и выбор одной
// из них за оба сломал бы один. Обычное заведение по-прежнему пишет `writeAudit` после транзакции —
// метаданные для него собирает `insertEquipmentCard` и возвращает готовыми.
...
export default async function officeEquipmentRoutes(app: FastifyInstance): Promise<void> {
  ...
  r.post(
    ...
    async (req, reply) => {
      const p = requirePrincipal(req);
      const b = req.body;
      // Проверки, область и сама вставка — общим помощником (Р14): вторая дверь в парк,
      // подтверждение кандидата, зовёт его же, и разойтись им нечем.
      const created = await db
        .transaction(async (tx) => insertEquipmentCard(tx, p, b))
      ...
      // Журнал пишется ПОСЛЕ транзакции и обычным `writeAudit` — сегодняшняя семантика заведения:
      // потерянная запись не должна ронять заведённую карточку. Решение по кандидату пишет свои две
      // строки строгим `writeAuditTx` внутри транзакции, и поэтому аудит остался у вызывающих, а не
      // уехал в помощник (Р14).
      await writeAudit({
        actorUserId: p.id,
        action: 'officeEquipment.create',
        entityType: 'officeEquipment',
        entityId: created.id,
        metadata: created.auditMetadata,
      });
      reply.code(201);
   ...
   * `serviceRequests.read` со своей областью. Без второго блок молчал бы у диспетчера, которому
   * справочник открыт, а модуль обслуживания — нет; выдать ему ремонты «заодно со справочником»
   * значило бы обойти модуль через карточку техники.
   *
   * Менеджер в этом примере стоял до 10.09.2026: круг заявителя оргтехники выдали ему РОЛЬЮ
   * ([ADR 0181](../../../../docs/adr/0181-manager-service-requester.md)), и `serviceRequests.read`
   * у него теперь есть в матрице. Правило от этого не изменилось — изменился лишь тот, на ком его
   * видно: у диспетчера круг по-прежнему приходит набором, и без выдачи блок ему закрыт.
   */
  r.get(
    '/:id/requests',
    {
