 ...
 *
 * Своё здесь одно: руководители отдела. Они не хранятся в карточке, а выводятся из привязок с
 * признаком `is_head` — это та же связь `user_departments`, прочитанная со стороны справочника.
 * Поэтому правка набора отсюда меняет область учётки и гасит её сессии так же, как правка из
 * карточки пользователя.
 *
 * Признак, а не роль: до миграции 0149 руководителями считались учётки роли «Руководитель отдела»,
 * и слияние ролей опустошило бы справочник молча (§11.1 плана реструктуризации прав). Кандидатов
 * сужает портал, сервер же принимает любую живую учётку — прав признак не даёт.
 */
