 ...
 */
import { officeEquipmentCandidates, serviceRequestExecutors, serviceRequests } from '../db/schema';
 ...
 *
 * **Тоже одна ось из трёх** (Р2): существующую строку ею одной не проверяют — для этого есть
 * `assertServiceRequestVisible`. Экспорт остался ради заведения заявки: у `POST /` строки ещё нет, и
 * область спрашивается по её ПРЕДМЕТУ (площадка единицы и отдел-заказчик) — ни подрядчика, ни
 * назначения там не существует, и приписать их было бы нечему.
 */
...
export async function assertServiceRequestVisible(
  ...
  if (can(p, 'serviceRequests.execute') && (await isNamedExecutor())) return;
  assertServiceRequestScope(p, place, place.id);
