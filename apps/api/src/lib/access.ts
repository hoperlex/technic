...
/**
 * Видимость заявок на технику по области учётки (ADR 0025, ADR 0040, ADR 0201). Объектная ось
 * сравнивается с объектом заявки; у роли отдела осей две — её отдел и её площадки, — и они
 * складываются. Пустая область означает «не видит ничего», а не «видит всё»: активировать такую
 * учётку API не даёт, но выборка не должна зависеть от того, удержалась ли эта проверка.
 *
 ...
 *
 * Дизъюнкция, а не выбор одной из двух: у заявки отдела объекта нет вовсе, у заявки площадки нет
 * отдела, и сравнение «по обеим колонкам сразу» не нашло бы ни одной строки. Журнал путевых листов
 * спрашивает ту же ось этой же функцией (`waybillVisibilityWhere`, ADR 0192): колонки ей передают
 * параметрами как раз затем, что там условие уходит внутрь подзапроса по заявкам рейса.
 *
 ...
 * руководитель строительства отвечает за свои объекты, руководитель отдела — за свои отделы и за
 * площадки этих отделов (ADR 0201, `departmentOwnsRequest`), и чужие заявки не согласовывает ни
 * тот, ни другой. Предикат, а не проверка с отказом: им же решается, снимать ли визу при правке
 * заявки.
 */
...
/**
 * Заявка на технику принадлежит области учётки (ADR 0040, ADR 0201): объектная роль работает со
 * своими объектами, роль отдела — со своими отделами И с площадками этих отделов, остальные не
 * ограничены. Ответ роли отдела считает `departmentOwnsRequest` — тот же, что у видимости, визы и
 * подписи под сменами: разойдись они, заявка либо пряталась бы от того, кому открыта, либо
 * открывалась по прямой ссылке.
 *
 * «Заявка чужой оси чужая» осталось правдой там, где осей по-прежнему одна: у объектной роли
 * заявка отдела объекта не имеет вовсе, и «объект не мой» здесь — не придирка, а единственно
 * верный ответ. Отдел без площадок отвечает так же, но по другой причине: площадок у него ноль, а
 * не «ось ему закрыта», — и отказ ниже эти два состояния различает словами.
 */
...
/**
 * Тип заявки доступен роли (ADR 0040, решение 10), и коридор роли РАСШИРЯЕТ ЕЁ ОБЛАСТЬ (ADR 0201):
 * отдел заказывает грузоперевозки всегда, а спецтехнику — пока за его отделами закреплена хоть одна
 * площадка. Заказчиком у такой заявки остаётся объект (CHECK
 * `vehicle_requests_department_freight_check`), и площадки — единственное, что делает этот заказ
 * адресуемым; отдел, с которого сняли последнюю, отвечает как прежде.
 *
 * 403, а не 422: дело не в состоянии заявки, а в том, что этой учётке такой заказ не положен вовсе.
 */
 ...
 * ограничений». У арендодателя и водителя площадки нет вовсе. У ОТДЕЛА ОНА ЕСТЬ — с ADR 0201 он
 * работает на своих площадках в «Заказе ТС» наравне со штабом, — и «ни одной» здесь означает не
 * «нечего считать», а принятое решение: неделю он ЧИТАЕТ, но не ведёт. Читает ровно те же площадки
 * (`weeklyRequestReadScope`, ветка `department`), а собирает и подписывает неделю штаб: документ
 * описывает занятость площадки целиком, а не заказ одного подразделения на ней.
 *
 * Пара этому ответу — матрица: прав `weeklyRequests.create`/`update`/`approve` у обеих ролей отдела
 * нет (`ROLE_PERMISSIONS`), и ветка здесь вторая её половина, а не дубль. Право, дошедшее до отдела
 * набором полномочий, ведения ему не откроет — область ответит «ни одной».
 *
