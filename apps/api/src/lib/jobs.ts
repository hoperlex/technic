 ...
 *
 * Задача НИЧЕГО НЕ РАЗБИРАЕТ САМА: воркер подключён к базе голым `pg`, а словари и правила живут в
 * API — он зовёт внутреннюю ручку перечитывания. Реестр типов задач в этом портале продублирован
 * здесь и в `apps/worker/src/index.ts`, и обе копии обязаны знать новый тип: первая ставит задачу,
 * вторая её исполняет.
 *
 * Удаление прежних наблюдений, разбор и запись исхода идут ОДНОЙ транзакцией. Порядок «удалить →
 * сменить статус → разобрать» отвергнут: выкат, убивший воркера между шагами, оставил бы письмо с
 * удалёнными данными и вне отбора очереди — то есть данные исчезли бы, и этого никто бы не увидел.
 */
export const JOB_REPARSE_DEVICE_MESSAGE = 'reparse_device_message';
