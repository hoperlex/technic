...
export type AccessCondition =
   ...
   *   · правка заявки на обслуживание присылает пару срочности всегда — порознь её не принимают ни
   *     схема, ни `CHECK`, — и «поле есть» закрыло бы правом срочности всю форму заявителя (Р10
   *     плана профилей оргтехники). Этот случай сегодня и есть единственный живой держатель вида.
   *
 ...
 *
 * Незанятых видов сейчас нет: `effectConditionalPermissions` пустовал с заморозки склада
 * автозапчастей (три его строки стояли у ручек обслуживания и уехали вместе со складским расходом,
 * план `docs/auto-part-receipts-plan.md`, Р2, Р3) и снова занят — правкой заявки на обслуживание,
 * где право срочности спрашивается по разнице со строкой, а не по присланному полю (Р10 плана
 * профилей оргтехники). Тем и оправдалось решение оставить пустующий вид описанным: приём «право по
 * факту, а не по телу» не пришлось переизобретать. Вид без единого маршрута — состояние законное, и
 * тест реестра его допускает поимённо, а не молча.
 */
...
export const ACCESS_MANIFEST = {
  ...
  // Рубильники событий почты модуля: читает их тот же, кто видит адресатов, — список рядом с
  // копиями в той же вкладке, и прятать «уходит ли письмо вообще» строже, чем «кому копия», не за
  ...
  // Сообщения об отсутствующей технике (план кандидатов, Р9, Р12–Р15, §8). Ручек здесь ШЕСТЬ:
  // очередь, карточка, правка и три решения проверяющего.
  //
  // ОБЛАСТИ В УСЛОВИИ НЕТ НИ У ОДНОЙ, и это не пропуск: манифест отвечает на вопрос «что нужно,
  // чтобы обработчик вообще начал работать», а «какие сообщения видно» — три основания Р9
  // (`officeEquipmentCandidateScopeWhere`), и живут они в обработчике. Ровно поэтому вне видимости
  // ответ 404, а не 403: страж пропускает, а строки для смотрящего не существует.
  'GET /api/v1/office-equipment-candidates': {
    kind: 'permissions',
    allOf: ['officeEquipment.review'],
  },
  // Карточка сообщения — ВЫБОР ИЗ ДВУХ ПРАВ, потому что читателей у неё две стороны с разными
  // правами: заявитель и все, кому видна его заявка (`serviceRequests.read`), и проверяющий, у
  // которого своё право и которому связанная заявка может быть не видна вовсе. Записанное
  // конъюнкцией, условие отобрало бы ручку у обеих сторон сразу — у первой нет `review`, у второй
  // может не быть чтения заявок. Что смотрящему доступна ИМЕННО ЭТА строка, условие не описывает:
  // это дело предиката Р9 в обработчике.
  'GET /api/v1/office-equipment-candidates/:id': {
    kind: 'anyOf',
    anyOf: ['serviceRequests.read', 'officeEquipment.review'],
    why: 'Карточку сообщения читают участники видимой заявки и проверяющий: права у сторон разные',
  },
  // Правка заявленных реквизитов до решения (Р12) — только проверяющий. Автору она закрыта после
  // отправки намеренно, и держит это отсутствие у него `officeEquipment.review`, а не отдельная
  // проверка «не автор ли»: кандидат есть свидетельство о том, что человек видел в кабинете, и
  // переписанное задним числом свидетельство ничего не доказывает. Условия «только в `pending`» в
  // манифесте нет по тому же правилу, что у ходов закупки: это коридор состояний, а не доступ, и
  // держит его условная запись с `ROW_COUNT = 1`.
  'PATCH /api/v1/office-equipment-candidates/:id': {
    kind: 'permissions',
    allOf: ['officeEquipment.review'],
  },
  // Три решения (Р13, Р15) — одно и то же право, и второго рядом с ним нет, хотя подтверждение
  // ЗАВОДИТ КАРТОЧКУ ПАРКА. `officeEquipment.write` держателю `review` и так принадлежит: его
  // требует `PERMISSION_REQUIRES` (Р8), и набор без него не собирается вовсе — проверяет это
  // `access-conditions.test.ts`. Записанное здесь вторым правом, оно означало бы, что требование
  // прав живёт в двух местах, и первое же расхождение читалось бы как «решать можно без ведения
  // справочника».
  //
  // Условий «только в `pending`», «версия совпала» и «цель объединения активна» в манифесте нет по
  // тому же правилу, что у ходов закупки: это коридор состояний, а не доступ. Держат его условная
  // запись с `ROW_COUNT = 1` и проверки обработчика, а манифест отвечает на вопрос «что нужно,
  // чтобы обработчик вообще начал работать».
  ...
  // Опрос аппаратов по сети (ADR 0205). То же право, что у очереди и правил: это снятие показаний
  // тем же кругом людей с тех же аппаратов. Область видимости карточек здесь ни при чём — цели
  // заданы настройкой окружения, а не справочником, и человек, которому телеметрия открыта,
  // опрашивает их все.
  'GET /api/v1/device-poll/targets': {
    kind: 'permissions',
    allOf: ['officeEquipment.telemetry'],
  },
  // Единственная ручка портала, которая ходит в сеть от имени сервера: право у неё то же, а
  // перечень адресов закрыт настройкой — из интерфейса адрес задать нельзя (ADR 0205, §Последствия).
  'POST /api/v1/device-poll/targets/:key/poll': {
    kind: 'permissions',
    allOf: ['officeEquipment.telemetry'],
  },
  ...
  // из выданных наборов убрано отдельным выпуском (план профилей оргтехники, Э9, миграция E) — в
  // словаре и в матрице администратора оно осталось, но не открывает ни одного маршрута отсюда.
  //
  ...
  /*
   * Срочность спрашивает своё право и в ОБЩЕЙ правке, а не только у своей ручки (план профилей
   * оргтехники, Р10; находка Н1). Прежде пара `isUrgent`/`urgencyReason` приезжала сюда вместе с
   * `serviceRequests.update` — то есть всякому, кто правит заявку: заявитель ставил себе «Срочная»
   * правкой собственной «Новой», а право срочности отбирало у него одну лишь ручку
   * `PATCH /:id/urgency`. Дверь была вторая, и закрыты обязаны быть обе.
   *
   * ПО ЭФФЕКТУ, А НЕ ПО ТЕЛУ, и это не оттенок. Форма правки шлёт пару ВСЕГДА
   * (`apps/web/src/pages/service/serviceRequestSubmit.ts:96-119`), потому что порознь её не
   * принимают ни схема, ни `CHECK` базы: «сняли флаг, забыли причину» — состояние, которого не
   * бывает. Условие «поле прислали» на такой форме означало бы «правку заявки закрыли правом
   * срочности целиком» — заявитель потерял бы и описание, и телефон, и заказчика, а не только
   * красную метку. Поэтому право спрашивается по РАЗНИЦЕ со строкой заявки: прислал то же, что
   * было, — решения об очереди не принимал.
   *
   * Снятие срочности при этом закрыто ровно так же, как постановка: снять — значит изменить флаг,
   ...
   * не выражается вовсе — его доказывает направленный тест из `provenBy`.
   */
  'PATCH /api/v1/service-requests/:id': {
    ...
    provenBy: 'test/service-request-flow.db.test.ts',
  },
  ...
  // Вкладка «Статистика» (план `docs/waste-stats-tab-plan.md`, Р8): право то же, что у списка —
  // стоимость закрытия эта роль уже видит колонкой в журнале, и своё право означало бы, что
  // администратор обязан раздать его всем, кто и так видит те же деньги строкой ниже.
  //
  // Область манифест не описывает, как обычно: площадочную область ставит предикат внутри слоя, а
  // отказ роли от контрагента и кабинету работника живёт в обработчике (`assertWasteStatsAudience`)
  // — он не открывает маршрут, а закрывает свод площадок тому, кто видит модуль своими заявками.
  'GET /api/v1/waste-requests/stats': { kind: 'permissions', allOf: ['wasteRequests.read'] },
