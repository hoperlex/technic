...
export type ServiceSide =
  ...
  | 'customer'
  /**
   * Сторону задаёт САМА КОМАНДА, а не ручка: у пакетного маршрута девять вариантов операции, и у
   * каждого сторона своя — отмену и приёмку держит «Ведение», назначение — распределяющий, «принять
   * в работу» — назначенный исполнитель. Спрашивается она построчно, тем же шагом, что и у
   * одиночной ручки (план массовых действий, Р2).
   *
   * Это НЕ `'any'`: там сторона не спрашивается вовсе, здесь — спрашивается всегда, просто разная.
   * Объявить такую дверь `'any'` значило бы записать в манифест «стороны нет» ровно у той ручки,
   * которая берётся выполнить девять чужих ходов подряд.
   */
  | 'byOperation'
...
/**
 * Все стороны, перечисленные для прогона. Union рантайму не виден, а тест обязан уметь пересчитать
 * их поимённо, — поэтому реестр записан руками, а полноту его держит компилятор (`sideListed`
...
export type ServiceRouteAccess =
  ...
  | {
      /**
       * Область — НЕ ЗАЯВКА, А АВТОР ЗАПИСИ: строка читается только тем, кто её и завёл
       * (`actor_user_id = subject.id`), а чужой ключ отвечает 404. Заведено под состояние пачки
       * (план массовых действий, §6.4): заявок эта ручка не отдаёт вовсе — она отдаёт отчёт,
       * который сама же пачка и сложила уже под областью каждой строки.
       *
       * Третьим видом, а не `'visibility'` и не `'none'`: первое было бы неправдой (области заявки
       * ручка не спрашивает), второе — тоже (область у неё есть, просто своя). Что она и правда
       * есть, доказывает разбор: в теле обязан стоять отбор по `actorUserId`.
       */
      readonly scope: 'actor';
      /** Чем именно ограничен читатель — словами, а не ссылкой на строку кода. */
      readonly why: string;
      readonly side: ServiceSide;
      readonly state: ServiceStateGate | null;
    }
...
export const SERVICE_ACCESS_MANIFEST = {
  ...
  /*
   * Пакетная ручка сохраняет ПОСТРОЧНУЮ проверку видимости: область спрашивает тот же
   * `requireEditable`, что и одиночная, — внутри доменного шага, по каждой строке отдельно. Чужая
   * строка отвечает своим исходом в отчёте и соседей не закрывает (находка Н7 плана: отсев стороны
   * обязан оставаться построчным).
   *
   * `state: null` — своего состояния у маршрута нет: у девяти операций оно разное (коридор, замок
   * предъявления, `canStartServiceWork`, удаляемость), и назвать его ОДНИМ именем нечем. Каждое из
   * них спрашивается своим шагом и объявлено строкой ЭТОЙ ЖЕ таблицы у одиночной ручки.
   */
  ...
  'GET /api/v1/service-requests/bulk/:key': {
    scope: 'actor',
    why: 'отчёт пачки читает только её автор: область — `actor_user_id`, а не заявка; чужой ключ — 404',
    // Сторона у чтения отчёта не спрашивается: пачку заводит кто угодно из допущенных к массовому
    // режиму, а её отчёт — про его собственную работу.
    side: 'any',
    state: null,
  },
  ...
  // Уборка пакетных операций — тот же системный субъект и та же дверь. Область не спрашивается и
  // здесь: пачки чистятся по срокам, а не по тому, чьи они, — за уборкой не стоит ни один человек.
  'POST /internal/service-requests/bulk-sweep': {
    scope: 'none',
    why: 'системный субъект по общему секрету: уборка идёт по срокам, а не по области субъекта',
    side: 'any',
    state: 'MATURE',
  },
