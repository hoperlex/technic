 ...
 */
const SERIES_CODE = `zz_accept_race_${RUN}`;
 ...
 * действительного листа в задание дня не входит и строки ожидания не даёт.
 */
...
/** Грузовой рейс с живой заявкой и выписанным по нему листом: без обоих дня в кабинете нет. */
async function makeRoute(opts: {
  ...
  await ctx.db.insert(schema.vehicleRequestTrips).values({
    requestId: request!.id,
    num: 1,
    fromLocation: 'г Москва, ул Погрузочная, д 5',
    toLocation: 'г Москва, ул Разгрузочная, д 6',
    volumeM3: '10.000',
  });
...
describe.skipIf(!DB_URL)('приём дня: что он гарантирует и что нет (живая схема)', () => {
  beforeAll(async () => {
    ...
    const [series] = await db
      ...
      .values({
        code: SERIES_CODE,
        name: `Тестовая серия (гонка приёма) ${RUN}`,
        prefix: 'ГНК-',
      })
