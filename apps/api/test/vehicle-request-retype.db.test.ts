...
describe.skipIf(!DB_URL)('переоформление заявки в другой тип (живая схема)', () => {
  ...
  afterAll(async () => {
    if (ctx?.db) {
       ...
       * случай заводит заказ — за прогон в ней оседало по десятку заказов вместе с их деталями,
       * ездками и историей.
       *
       ...
       *
       * Детали обоих видов, ездки, файлы и история заказа уходят каскадом вместе с ним.
       */
      const ourUsers = sql`
