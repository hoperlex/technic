...
/**
 * Отказ ДОПУСКА к массовому режиму (план массовых действий, Р5): страж пакетной ручки пускает по
 * любому из шести прав, а обработчик спрашивает сверх него две вещи — продуктовый допуск
 * `canUseServiceBulk` (закрывает Н11: держателя одного `serviceRequests.delete` в пачку не пускают)
 * и точное право названной операции. Отказы у них разные по смыслу, но начинаются одинаково — так
 * их и различает эта подстрока, одна на маршрут.
 */
const BULK_REFUSAL = 'Массовый режим';
...
const FIXTURES: Partial<Record<ManifestRouteKey, RouteFixture>> = {
  ...
  // Отбор машинистов на периоды листа (ADR 0190): периоды обязательны, и без них валидация ответит
  // 400 раньше стража прав — доказывать было бы нечего. Формат тот же, что шлёт форма ЭСМ-2.
  'GET /api/v1/drivers/machinists': { query: 'periods=2026-09-01..2026-09-07' },
  ...
  // ── Справочник прицепов ──
  // Тело нужно валидное: схема разбирается до `preHandler`, и пустой запрос вернул бы 400 раньше,
  // чем страж успел бы ответить 403, — перебор доступа проверял бы не то.
  'POST /api/v1/vehicle-trailers': {
    payload: { kind: 'semi_trailer', model: 'ШМИТЦ SPR-24', registrationNumber: 'ВХ933277' },
  },
  'POST /api/v1/vehicle-trailers/:id/hitch': {
    payload: { vehicleId: RECORD_ID, position: 1 },
  },
  ...
  /*
   * МАССОВЫЕ ДЕЙСТВИЯ (план `docs/office-equipment-bulk-actions-plan.md`, §6.4).
   *
   * Тело валидно и нарочно взято самым дешёвым вариантом команды: схема стоит ДО стража, и пачка
   * без `operation`/`rows` дала бы 400 раньше 403 — перебор доказывал бы разбор тела вместо права.
   * Заголовка `Idempotency-Key` фикстура не шлёт, и это верно: он спрашивается в обработчике, то
   * есть уже за стражем, и положительный случай упирается в 400 — не в отказ доступа.
   *
   * `selfRefusal` обязателен: страж пускает по ЛЮБОМУ из шести прав, а обработчик отбивает и
   * держателя одного `delete` (допуск), и владельца не того права (точное право операции). Без
   * него положительный случай варианта «только `assign`» читался бы как отказ стража.
   *
   * У состояния пачки свой `:key` — это ключ идемпотентности, а не раздел портала: общее значение
   * `key: 'objects'` схема отбила бы как «Invalid UUID» ещё до стража.
   */
  'POST /api/v1/service-requests/bulk': {
    payload: { operation: 'urgency_off', rows: [{ id: RECORD_ID, version: 0 }] },
    selfRefusal: BULK_REFUSAL,
  },
  'GET /api/v1/service-requests/bulk/:key': { params: { key: RECORD_ID } },
  /*
  ...
  // ── Аудит распознавания талонов (ADR 0137) ──
  // Период задан явно, хотя без параметров ручка тоже дошла бы до стража: умолчание в 30 суток
  // она достраивает сама. Явный период страхует от дня, когда границы станут обязательными, —
  // схема стоит ДО стража, и с пустой строкой запроса негативный случай начал бы доказывать
  // работу Zod вместо работы права. Длина в пределах 92 дней (§4.3): отказ по длине приходит из
  // обработчика, то есть уже за стражем, и положительный случай выглядел бы 400 вместо ответа.
  'GET /api/v1/waste-requests/ticket-audit/summary': { query: 'from=2026-07-27&to=2026-08-26' },
  'GET /api/v1/waste-requests/ticket-audit/cohorts': { query: 'from=2026-07-27&to=2026-08-26' },
  // Лента и выгрузка: период тот же и по той же причине. Фильтров в строке запроса нет намеренно —
  // они отбор сужают, а до стража доходит любой; лишний параметр только притворялся бы, что перебор
  // проверяет ещё и их. Предел в 50 000 строк положительному случаю не мешает: он считается по
  // ответу БД, а БД в переборе подменена — до него запрос не доходит вовсе.
  'GET /api/v1/waste-requests/ticket-audit/events': { query: 'from=2026-07-27&to=2026-08-26' },
  'GET /api/v1/waste-requests/ticket-audit/events.csv': { query: 'from=2026-07-27&to=2026-08-26' },
  // Точность (§5.5): период тот же и по тем же причинам — она разбирает его общим помощником.
  'GET /api/v1/waste-requests/ticket-audit/blind': { query: 'from=2026-07-27&to=2026-08-26' },
  // А у состояния подсистемы (§5.4) фикстуры нет — и завести её было бы ошибкой. Строка запроса у
  // него пустая и строгая: любой параметр схема отвергает 400-м раньше стража, поэтому фикстура с
  // периодом сделала бы все случаи маршрута четырёхсотыми, и перебор доказывал бы работу Zod
  // вместо работы права. Без фикстуры запрос идёт голым путём — ровно тем, который схема и ждёт.
  ...
  // Аннулирование: версия и причина. Строк расхода в теле нет — их нет у акта вовсе с заморозки
...
function variantsOf(key: ManifestRouteKey, condition: AccessCondition): Variant[] {
  ...
  // Условие ПО ЭФФЕКТУ (`effectConditionalPermissions`): перебор проверяет только базовую
