 ...
 * сверки, чтобы подмена на отрезковый разрез не прошла незаметно. Три его ожидания уже переписаны
 * месячным разрезом (ADR 0142): срез недели концом месяца и замок отработанного листа по дням. Обёртка двух режимов ему не
...
describe('ЭСМ-2, срез срока на недели (`esm2Periods`)', () => {
  ...
  it('неделя через Новый год дробится концом декабря — как всякая переходная', () => {
    // Пн 28.12.2026 — вс 03.01.2027: два документа, «28–31 декабря» и «1–3 января» (ADR 0142).
    // Года как отдельной границы у листа по-прежнему нет — режет месяц, а Новый год лишь его
    // частный случай.
    expect(esm2Periods('2026-12-28', '2027-01-03')).toEqual([
      { from: '2026-12-28', to: '2026-12-31' },
      { from: '2027-01-01', to: '2027-01-03' },
    ]);
    // Срок, начатый в середине недели перед Новым годом, режется обеими границами сразу.
  ...
  it('месячный срок — шесть листов: понедельники, начало срока и конец августа', () => {
    // Шесть, а не пять: последняя неделя срока переходная, и месяц режет её на «31–31 августа» и
    // «1–3 сентября» (ADR 0142). Заказ на месяц забирает теперь на номер больше — это цена
    // решения, названная вслух.
...
describe('ЭСМ-2, сверка бумаги с заявкой (`esm2SyncPlan`)', () => {
  ...
  it('два листа в одной неделе: отработанный запирает свои дни, а соседний переоформляется', () => {
    // Бывшая странность, снятая ADR 0142. У заявки два отрезка одной недели: «пн…ср» отработан,
    // «чт…вс» выписан на машину, которую с тех пор сменили. Прежде замок считался понедельником,
    // и защита первого листа запирала неделю целиком: второй горел без замены, оставляя
    // четверг…воскресенье без документа — молча. Теперь замок держит **дни** отработанного листа,
    // и переоформление соседа доводится до конца.
    const plan = auto({
      today: '2026-08-06',
      wanted: [
        { from: MON, to: WED },
