...
describe('привязанный файл живёт по правилам своей заявки', () => {
  ...
  it('документ заявки на обслуживание открыт тому, кто читает этот модуль (ADR 0085)', () => {
    ...
    for (const role of [
      'admin',
      // Менеджер — заказчик с 10.09.2026 (ADR 0181): круг заявителя выдан ему матрицей роли, и
      // вложение своей заявки он обязан открывать наравне со штабом. Область самой заявки, как и у
      // прочих, посчитана раньше — здесь решается только «положен ли модуль вообще».
      'manager',
      'shtab',
      'rukstroy',
      'department',
  ...
  it('роль без модуля обслуживания документа не получает, даже ведя справочник оргтехники', () => {
    // У диспетчера есть `officeEquipment.read` — он ведёт карточки техники, — но заявок с суммами,
    // актами и счетами не ведёт вовсе (Р7): круг заявителя приходит ему набором, а роль сама по
    // себе модуля не открывает. Комендант закрыт и от справочника. Менеджер из перечня уехал
    // выше — ADR 0181 выдал ему круг заказчика ролью.
    for (const role of ['dispatcher', 'commandant'] as Role[]) {
