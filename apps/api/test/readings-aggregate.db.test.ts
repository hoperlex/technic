...
describe.skipIf(!DB_URL)('агрегат показаний: месяцы, итог периода и качество данных', () => {
  ...
  it('сумма месяцев равна итогу периода: пара засчитана месяцу своего снимка', async () => {
    ...
    //
    // В день, когда раскладки с границей не существует (см. `daysAcrossMonth`), ряд берётся внутри
    // месяца — и это осознанный выбор вызывающего, а не умолчание помощника. Равенство «сумма
    // месяцев = итог периода» верно и на одном месяце, а остальные утверждения случая — разности
    // между снимками, пропуск посреди ряда, простая сумма литров — про границу не знают вовсе.
    // Пропускать такой день значило бы терять их все ради одного ослабленного.
    const days = daysAcrossMonth(5, 2) ?? series(ago(4), 5);
    /**
  ...
  it('перенос рейса уносит ожидание на новую дату, а число остаётся в месяце своей', async ({
    ...
    // Здесь граница месяца — не украшение случая, а сам его предмет: сдали показание в одном
    // месяце, рейс уехал в другой, и проверяется, что снимок остался у своего месяца, а ожидание
    // ушло к соседнему. Ряда с границей внутри окна записи в 31-й день 31-дневного месяца не
    // существует, и единственный честный ответ на такой день — пропуск с причиной: ряд внутри
    // одного месяца превратил бы «сцену не собрать» в падение утверждения про ключи месяцев, то
    // есть в ложную тревогу о продукте, а бросок — в красный прогон там, где ломать нечего.
    const days = daysAcrossMonth(2, 1);
    skip(
      days === undefined,
      `граница месяца не попадает в окно записи (${STAFF_SUBMIT_PAST_DAYS} дней): сегодня ` +
        `${TODAY}, за дни раньше ${EARLIEST} показания уже не вносят — прошлый месяц целиком вне окна`,
    );
