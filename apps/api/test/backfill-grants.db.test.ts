 ...
 * разваливал и счётчики, и вердикт. Сводить базу к нулю тем более нельзя: `user-grants-single-write`
 * **намеренно** держит назначение без надстройки посреди своего сценария (проверка повторной
 * выдачи), и это его законное промежуточное состояние, а не мусор. С шага 1d так выглядит и любая
 * новая выдача вообще: в старую таблицу не пишет никто.
 *
 ...
 *
 * **Обратная половина того же соседства решена с той стороны.** Надстройка **без** назначения —
 * это то, что перенос не показывает, а переписывает, и тест, державший такую строку
 * зафиксированной, этот прогон бы и поймал. Поэтому свой легаси-сценарий
 * `user-grants-single-write` держит внутри незакоммиченной транзакции (`inRolledBackTx` там же):
 * блокировка `user_role_addons IN SHARE MODE`, которую берёт `run`, показывает переносу только
 * зафиксированные правки надстроек, а незавершённых он не видит ни при каком расписании.
 *
