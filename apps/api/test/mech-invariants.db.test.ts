...
describe.skipIf(!DB_URL)('механизация: инварианты базы (ADR 0152, миграция 0238)', () => {
  ...
  it('арендодатель чужого типа не проходит ни проверкой типа, ни составным ключом', async () => {
    await inScene(async (tx, scene) => {
      ...
      // Арендодатель ТС — законный арендодатель механизации (Р6): компания, уже заведённая под
      // своим типом, сдаёт и то, и другое, а менять ей тип значило бы сломать её технику в парке.
      expect(
        await refusal(
          tx,
          insert(scene, {
