 ...
 *   5. переназначение из «В работе» статуса не трогает (ADR 0187): повтор того же состава
 *      отбивается `422`, и смета с `warranty_months` остаётся нетронутой; смена подрядчика смету
 *      сносит, оставляя заявку в работе (расхождение с Н10, см. комментарий случая);
...
describe.skipIf(!DB_URL)('гарантии в переходах заявки на обслуживание (Э1 плана приёмки)', () => {
  ...
  describe('переназначение из «В работе»', () => {
    it('тот же состав отбивается, а смета и обещанные месяцы остаются нетронутыми', async () => {
      /*
       * СЛУЧАЙ ПЕРЕПИСАН ПОД ADR 0187, и вместе с ним сменился сам вопрос. Прежде этот запрос был
       * осмысленным ходом: состав не менялся, заявка уходила в «Новую» — то есть возвращалась к
       * назначенным, — и проверялось, что обещанные месяцы такой возврат переживают (Н10, §4.2).
       * Отката больше нет, назначение статуса не трогает, и тот же состав стал повтором нажатия:
       * ручка отвечает `422` и не пишет ничего.
       *
       * Смета проверяется после отказа намеренно: отказ обязан быть ПОЛНЫМ. Пройди он хоть
       * наполовину — ветка `handedOver` стоит ниже по той же функции и снесла бы строки вместе с
       * обещанными месяцами.
       */
    ...
    it('СМЕНА ПОДРЯДЧИКА, наоборот, сносит смету вместе с обещанными месяцами', async () => {
      ...
      // Заявка осталась в работе: статуса назначение не меняет (ADR 0187) — снесло только смету.
      expect(dto.status).toBe('in_work');
