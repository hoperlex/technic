...
describe.skipIf(!DB_URL)('показания: состояние дня, журнал и сводка', () => {
  ...
  describe('сводка по парку и журнал машины', () => {
    ...
    it('в сводке есть заправленное, расход сверки и ни одного производного показателя', async () => {
      const row = await statsOf(chain.vehicle, chain.days[0]!, chain.days[2]!);
      expect(row).not.toBeNull();
      // Состав строки перечислен целиком: «на 100 км» и «на моточас» портал не считает, и
      // отсутствие таких колонок — утверждение, а не умолчание. Цифра, поделённая на пробег, —
      // самостоятельная оценка, которой никто не заказывал.
      //
      // Сам расход в строке теперь есть (ADR 0194): решение 12 ADR 0103 снято, но снято вместе со
      // своей причиной — расход пришёл как половина сверки с нормой, и рядом с ним стоят норма,
      // счётчики охвата и признак заведённой нормы. Четыре числа сверки аддитивны; отклонение и
      // процент среди них отсутствуют намеренно — они считаются из пары на каждом уровне отдельно.
      //
      // Снимки счётчиков (Р17) — не производные: это сами показания приборов за период, а не
      // что-то, поделённое на пробег.
      //
      ...
      const sheet = ctx.stats.readingStatsSheet([row!], '2026-08-01 – 2026-08-31', 5);
      ...
      // Выгрузку читают рядом с порталом, и колонки у неё те же, что на экране. Производных в ней
      // по-прежнему нет: число, поделённое на пробег, в чужой книге с формулами зажило бы своей
      // жизнью — а расход рядом с нормой читается ровно как расход.
      expect(header.join(' ')).not.toMatch(/на 100|\/100|л\/км|на моточас/iu);
      // У машины нормы нет: три колонки сверки молчат прочерком, охват — «0 из 0».
    ...
    it('в выгрузке прочерк остаётся прочерком, а не нулём', async () => {
        ...
        [
          {
            ...
            fuelFilledLiters: 0,
            gaps: 2,
            // Сверка с нормой (ADR 0194): у машины её не заводили, и колонки молчат прочерком —
            // ноль в них читался бы как «не жгла топлива», а не как «сверять нечем».
            fuelSpentLiters: 0,
            fuelNormLiters: 0,
            verifiedShifts: 0,
            shiftsWithFuel: 0,
      ...
      expect(sheet.rows[1]).toEqual(['Машина', '—', '—', '0,0', '2', '—', '—', '—', '0 из 0']);
    });
     ...
     * несуществующий вопрос. Отсутствие ИМЕННО ЭТИХ ДВУХ колонок в сводке и в месяцах карточки —
     * решение, а не отложенная работа, и караул держит его от «доделаем и тут, для единообразия».
     *
     * Расход — другое дело: он не уровень, а разность уровней за смену, и с приходом норм
     * (ADR 0194) он в обеих формах есть — вместе с нормой и счётчиками охвата, четырьмя
     * аддитивными числами. Складывать их за период можно, и именно поэтому им здесь место.
     *
     ...
     */
    it('остатки в баке в сводку и месяцы не попали, а расход сверки — попал', async () => {
