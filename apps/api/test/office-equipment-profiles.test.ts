...
describe('Реестр профилей модуля «Орг.техника» (Р3)', () => {
  ...
  it('код профиля «Заявитель» — модульный набор из семи прав заказчика', () => {
    ...
    /*
     * Роль осталась одна — `dispatcher` (ADR 0181). Ролям-заказчикам набор не нужен: права у них
     * уже есть, и набор, ничего не дающий, запрещён тестом каталога. С 10.09.2026 в это же
     * положение попал и менеджер: круг заявителя выдан ему матрицей роли, и строка `manager` здесь
     * означала бы выдачу, не меняющую ни одного права.
     */
    expect([...grant.roles].sort()).toEqual(['dispatcher']);
    /*
 ...
 *
 * У НЕВЛИЯНИЯ ПОЯВИЛОСЬ РОВНО ОДНО ИСКЛЮЧЕНИЕ — `manager` (ADR 0181, 10.09.2026). Круг заявителя
 * выдан ему матрицей роли, и правило теперь ПРО НЕГО: без второго признака страж пропустил бы
 * права, пришедшие ролью, целиком — менеджер видит заявки всей компании и правил бы любую чужую.
 * Исключение названо поимённо и разбирается своим случаем ниже: перебор, из которого субъект молча
 * выпал бы, перестал бы что-либо доказывать.
 */
describe('Страж стороны заказчика (Р6)', () => {
   ...
   */
  it('ни один действующий субъект портала, кроме менеджера, не сужен', () => {
    for (const subject of [...ACCESS_PROFILES, { role: null } as AccessSubject]) {
      // Менеджер — то самое исключение (ADR 0181), и разбирается оно своим случаем ниже. Пропуск
      // идёт по РОЛИ и пустым надстройкам, а не вычёркиванием субъекта из перебора: тот же
      // `manager` приходит сюда ещё и в паре с «Ведением», и эта пара правилом не сужается.
      if (subject.role === 'manager' && (subject.addons ?? []).length === 0) continue;
  ...
  /**
   * ИСКЛЮЧЕНИЕ ИЗ НЕВЛИЯНИЯ, ОНО ЖЕ ВТОРОЙ ПРИЗНАК СТРАЖА (ADR 0181): у менеджера круг заявителя
   * приходит РОЛЬЮ, и правило обязано работать без всякого набора.
   *
   * Проверяется вместе с обратной стороной — диспетчером без набора: прав модуля у того нет вовсе,
   * и сужать его не за что. Иначе правило «всякая роль без оси сужается» прошло бы этот тест и
   * отобрало бы действия у ролей, которым модуля никто не выдавал.
   */
  it('менеджер сужен САМОЙ РОЛЬЮ: своя строка — да, чужая — нет', () => {
    const manager: AccessSubject = { role: 'manager' };
    expect(can(manager, 'serviceRequests.update')).toBe(true);
    expect(actsAsRequestCustomer(manager, { isAuthor: false, inCustomerScope: false })).toBe(false);
    expect(actsAsRequestCustomer(manager, { isAuthor: true, inCustomerScope: false })).toBe(true);
    const dispatcher: AccessSubject = { role: 'dispatcher' };
    expect(can(dispatcher, 'serviceRequests.update')).toBe(false);
    expect(actsAsRequestCustomer(dispatcher, { isAuthor: false, inCustomerScope: false })).toBe(
      true,
    );
  });

  /**
   * «Ведение» снимает правило автора и с менеджера, чей круг пришёл ролью: оператор ведёт чужие
   * заявки по должности, и источник заявительского слоя тут ни при чём.
   */
  it('код «Ведения» снимает правило и с менеджера без набора «Заявитель»', () => {
  ...
  it('те же права без кода набора правило не включают', () => {
    const assembled: AccessSubject = {
      // Роль взята та, у которой круга заявителя в матрице нет: у менеджера правило включает сама
      // роль (ADR 0181), и собранный набор доказывал бы на нём не то, о чём этот тест.
      role: 'dispatcher',
      grantPermissions: [
        'officeEquipment.read',
        'serviceRequests.read',
