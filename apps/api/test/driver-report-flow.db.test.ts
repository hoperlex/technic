...
/**
 * Грузовой рейс с одной живой заявкой в составе. Заявка обязательна: рейс без единой живой заявки
 * в задание дня не попадает вовсе (`loadRouteEntries`), а строки ожидания строятся из задания.
 */
...
describe.skipIf(!DB_URL)('отчёт дня: приёмка и разбор расхождений (живая схема)', () => {
  ...
  afterAll(async () => {
    ...
    // рейс держит строку `RESTRICT`, а заявку — состав рейса.
