...
describe.skipIf(!DB_URL)('профили модуля «Орг.техника»: разделение набора ИТ-службы', () => {
  beforeAll(async () => {
    ...
    /*
     * МЕНЕДЖЕРУ «ЗАЯВИТЕЛЬ» БОЛЬШЕ НЕ ВЫДАЁТСЯ (ADR 0181, миграция 0304): круг заказчика приходит
     * ему матрицей роли, и барьер выдачи отвечает на такую попытку `roleNotAllowed` (409) — набор,
     * не дающий ни одного права, каталог роли не обещает.
     *
     * Т17 от этого не ослаб, а стал строже: сцена та же — роль без площадок и отделов с полным
     * кругом заказчика, — но проверяет она теперь права, пришедшие РОЛЬЮ, и стража, который
     * опознаёт заявителя не по коду набора. Именно на этой паре и ломался бы выпуск: пропусти
     * страж роль, менеджер правил бы любую чужую заявку компании.
     *
     * Выдача осталась в фикстуре ниже ровно одна — «Ведение» тому же человеку (третья часть Т17),
     * и она проверяет, что два слоя не смешиваются.
     */
    void requesterGrantId;
  ...
  /**
   * ПРОФИЛЬ, ВЫДАННЫЙ МОДУЛЬНЫМ НАБОРОМ, НЕ ПРОВЕРЯЕТСЯ НА МАРШРУТАХ НИЧЕМ, и закрывать эту дыру
   * приходится живыми учётками (§11.1 плана, строка «Обмен справочником (файл)»).
   *
   * Общий перебор доступа (`ACCESS_PROFILES` в `permissions.ts`, по нему идут
   * `access-conditions.test.ts` и соседи) собран из пар «роль × тип контрагента» и «роль ×
   * надстройка». Наборы `office_equipment_requester` и `office_equipment_executor` — МОДУЛЬНЫЕ:
   * надстройкой ни один из них не выражен, в перебор они не попадают вовсе, и два профиля модуля
   * из четырёх не имеют доказательств ни на одной строке матрицы. Единственное место, где эти
   * профили существуют по-настоящему, — учётки этого файла, поэтому случай стоит здесь.
   *
   * ПОЧЕМУ ИМЕННО ОБМЕН СПРАВОЧНИКАМИ — самый дорогой частный случай из закрытых профилю. Выгрузка
   * не сужается областью ни на одну строку: она уносит справочник ЦЕЛИКОМ (`exportDirectory`), а
   * справочник водителей — это ФИО, телефоны и номера документов живых людей. Загрузка тем же
   * файлом переписывает справочник поверх, и «предпросмотром» она отличается одним полем тела.
   * Права `directories.export`/`.import` не даёт ни одна роль портала, кроме администратора, и
   * выдаются они ОТДЕЛЬНЫМ административным набором «Обмен справочниками»
   * (`ADMIN_GRANTS.directory_exchange`), которому наборы оргтехники не родня.
   *
   * ДВЕРЕЙ ТРИ, И ПРОВЕРЯЮТСЯ ВСЕ ТРИ. Список справочников со счётчиками (`GET /api/v1/directories`)
   * стоит под тем же `directories.export` и рассказывает, сколько строк лежит в каждом справочнике;
   * выгрузка файлом отдаёт сами строки; загрузка их переписывает. Закрыть две из трёх — оставить
   * дверь, которой хватает, чтобы понять, что уносить.
   *
   * СПРАВОЧНИКА ДВА, И ВТОРОЙ НЕ ЛИШНИЙ: `drivers` — цена ошибки (персональные данные), а
   * `office-equipment` — соблазн («это же мой модуль»). Профилю закрыты оба, и по одной причине:
   * право обмена не про предмет справочника, а про способ выноса.
   *
   * ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ ОБЯЗАТЕЛЕН. 403 у несуществующего маршрута не бывает, зато бывает у
   * маршрута, закрытого для всех: администратор проходит те же самые двери и получает НЕ 403 —
   * иначе случай зеленел бы на сломанной ручке, ничего не доказывая.
   */
  it('обмен справочником закрыт всем четырём профилям и подрядчику: 403 на все три двери', async () => {
    /*
     * Пять субъектов, и это ЧЕТЫРЕ ПРОФИЛЯ ПЛЮС ОТДЕЛЬНАЯ УЧЁТКА ИСПОЛНИТЕЛЯ. Держатель одного
     * лишь `office_equipment_executor` (`ctx.executor`) назван отдельно от сисадмина, у которого
     * тот же код лежит в паре с ИТ-набором: набор исполнителя выдаётся и в одиночку (этап Э6), и
     * доказывать его надо на учётке, где он один, — иначе отказ объяснялся бы соседним кодом.
     */
    const subjects: [string, Auth][] = [
      ['«Ведение» (office_equipment_operator)', ctx.operator.auth],
      ['сисадмин (it_approver + executor)', ctx.itAdmin.auth],
      ['исполнитель (office_equipment_executor)', ctx.executor.auth],
      ['заявитель (office_equipment_requester)', ctx.office.auth],
      ['сервисный центр (operator + контрагент service)', ctx.serviceUser.auth],
    ];

    /*
     * Тело загрузки — схемно годное и содержательно любое. Схема тела разбирается ДО `preHandler`,
     * то есть до стража: пришли мы пустую строку, отказ был бы 400 «не прошло схему», и случай
     * доказывал бы валидацию вместо права. До разбора книги запрос не доходит вовсе.
     */
    const importBody = {
      dryRun: true,
      filename: `справочник ${RUN}.xlsx`,
      contentBase64: Buffer.from('это не книга Excel').toString('base64'),
    };

    for (const [who, auth] of subjects) {
      const list = await inject('GET', '/api/v1/directories', auth);
      expect(list.statusCode, `${who}, список справочников: ${list.body}`).toBe(403);

      // `drivers` первым намеренно: это тот справочник, ради которого случай и написан.
      for (const key of ['drivers', 'office-equipment'] as const) {
        const exported = await inject('GET', `/api/v1/directories/${key}/export`, auth);
        expect(exported.statusCode, `${who}, выгрузка «${key}»: ${exported.body}`).toBe(403);

        const imported = await inject(
          'POST',
          `/api/v1/directories/${key}/import`,
          auth,
          importBody,
        );
        expect(imported.statusCode, `${who}, загрузка «${key}»: ${imported.body}`).toBe(403);
      }
    }

    // — Положительный контроль: те же три двери от лица администратора.
    const adminList = await inject('GET', '/api/v1/directories', ctx.admin.auth);
    expect(adminList.statusCode, adminList.body).toBe(200);
    const keys = (adminList.json() as { items: { key: string }[] }).items.map((row) => row.key);
    expect(keys).toContain('drivers');
    expect(keys).toContain('office-equipment');

    const adminExport = await inject(
      'GET',
      '/api/v1/directories/office-equipment-types/export',
      ctx.admin.auth,
    );
    // Справочник взят маленький и без персональных данных: предмет контроля — что ручка живая, а
    // не что она умеет собирать книгу на тысячу строк.
    expect(adminExport.statusCode, adminExport.body).toBe(200);

    const adminImport = await inject(
      'POST',
      '/api/v1/directories/office-equipment-types/import',
      ctx.admin.auth,
      importBody,
    );
    /*
     * 422, а не 403: администратора пропустил страж, и отбило его СОДЕРЖИМОЕ присланного файла —
     * то есть запрос дошёл до обработчика. Ровно это и требуется доказать: отказы выше приходят от
     * права, а не от закрытой на всех ручки.
     */
    expect(adminImport.statusCode, adminImport.body).toBe(422);
  });
