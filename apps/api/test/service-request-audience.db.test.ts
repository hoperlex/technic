...
describe.skipIf(!DB_URL)('карточка заявки на обслуживание в объёме аудитории (живая схема)', () => {
  ...
  function financeRepairCard(): Record<string, unknown> {
    return {
       ...
       *
       * Поле пока ОДНО: `estimateFormat` у фикстуры — `items`, потому что заявка предъявлена
       * строками, а документной подачи не существует (вход закрыт затвором до Э4). Освобождение,
       * спор и происхождение ожидания в карточку ещё не выводятся — их писатели появятся в Э4 и Э5,
       * и тогда в этот эталон добавятся три ключа. Пустые ключи раньше времени здесь были бы хуже
       * отсутствующих: они утверждали бы, что сервер уже отвечает про спор.
       */
      estimateFormat: 'items',
      /*
       * Поля волны освобождения от подписи, которые сервер отдаёт с этапа Э4. Пусты, потому что по
       * фикстуре объём работ предъявлен строками и подписан человеком: заявления об освобождении не
       * было, происхождение ожидания пусто (ожидание погашено подписью). Ключи при этом обязаны
       * стоять в эталоне — сравнение целиком и есть способ заметить, что новое поле уехало в карточку
       * не спросясь.
       */
