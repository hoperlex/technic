...
describe.skipIf(!DB_URL)('карточка заявки на обслуживание в объёме аудитории (живая схема)', () => {
  ...
  /**
   * Эталон вложения. `purpose` — роль связи (волна освобождения от подписи, Р5): у всех файлов
   * фикстуры она одна, `closing_evidence`, потому что счётом здесь ничего не подают. Роль
   * `estimate_basis` бывает только у страницы счёта, и её карточка показывает так же — отдельного
   * случая на неё нет в этом файле намеренно: он про АУДИТОРИИ, а роль вычищению не подлежит и у
   * заявителя (он видит, что бумага есть, но не видит её вида и денег).
   */
  ...
  function financeRepairCard(): Record<string, unknown> {
    return {
      ...
      equipment: {
        ...
        /*
         * Срок гарантии единицы (Ф3 плана кандидата): единственное поле блока, приходящее из живой
         * карточки, а не из снимка заявки. У фикстуры он не заведён — отсюда `null`, и потому один
         * литерал годится всем трём читателям этого случая. Совпадение неслучайно, но и не вечно:
         * поле закрыто от того, кому закрыт справочник (решение владельца 04.09.2026), и заведи
         * фикстура срок — сервисная компания продолжила бы получать здесь `null`, а «Ведение» и
         * ИТ-служба дату. Проверяется это различие своим файлом
         * (`service-request-equipment-warranty.db.test.ts`, раздел 3), где заявка одна, а читателя
         * два; здесь предмет другой — аудитории денег.
         */
        warrantyUntil: null,
      },
      ...
      /*
       * Поля волны «объём работ документом и освобождение от подписи» (план
       * `docs/office-equipment-on-site-and-invoice-estimate-plan.md`). Внесены ОСОЗНАННО, ровно как
       * требует шапка этого сравнения: новое поле карточки — часть решения, а не попутный довесок.
       *
       * `estimateFormat` у фикстуры — `items`, потому что заявка предъявлена строками, а документной
       * подачи не существует (вход закрыт затвором до Э4).
       */
      estimateFormat: 'items',
      /*
       * Поля волны освобождения от подписи, которые сервер отдаёт с этапов Э4 и Э5. Пусты, потому
       * что по фикстуре объём работ предъявлен строками и подписан человеком: заявления об
       * освобождении не было, спора по заявке не было ни разу, происхождение ожидания пусто
       * (ожидание погашено подписью). Ключи при этом обязаны стоять в эталоне — сравнение целиком и
       * есть способ заметить, что новое поле уехало в карточку не спросясь.
       *
       * `dispute` появился в этом эталоне позже остальных, и не «за компанию»: без него карточка не
       * могла сказать про спор ничего, и портал считал бы его признак по свободному тексту причины
       * заморозки. Разрешённый спор при этом остаётся в ответе навсегда — только он объясняет второе
       * окно приёмки и подпись, собранную уже в «Решена».
       */
      ...
      approval: {
        ...
        revision: 1,
        // Чем поставлена подпись (Р11): по фикстуре её ставил человек. Ключ обязан стоять в
        // эталоне — без него карточка читала бы автопринятие как подпись живого человека.
        source: 'human',
      },
      items: [
        {
  ...
  function requesterRepairCard(): Record<string, unknown> {
    return {
      ...
      estimatePendingRevision: null,
      /*
       * Четыре поля волны освобождения заявителю вычищены проекцией аудиторий (ADR 0160): «принято
       * без согласования», «подана счётом» и состояние спора — это деньги и внутренняя очередь
       * службы, а не ответ на вопрос «что с моим принтером». Ключи при этом ОСТАЮТСЯ: карта полей
       * присваивает `null` даже отсутствующему полю, и эталон обязан описывать то, что уходит
       * наружу, а не то, что мы подразумеваем.
       */
      estimateFormat: null,
      estimatePendingSource: null,
      exemption: null,
