...
const FULL: ServiceRequestDto = {
  ...
  /*
   * Признак повторного обращения (план `docs/office-equipment-repeat-request-plan.md`). В образце
   * он заполнен, как и всё остальное: карта аудиторий объявляет его `all` — считается он в области
   * САМОГО читателя, и вычитать из него нечего, — а образец обязан доказывать это непустым
   * значением, иначе «поле не режется» и «поля не было» читались бы одинаково.
   */
  repeat: { count: 2, windowDays: 30, lastAt: '2026-08-12T09:00:00.000Z' },
  /*
   * Кем разобрано заявленное расхождение (план перемещения, Р8). В образце заполнено по той же
   * причине, что и соседи: пустое поле не доказывало бы, что проекция его не режет.
   */
