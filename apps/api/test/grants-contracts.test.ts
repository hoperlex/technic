 ...
 * аудит талонов, закупки расходников, заявка без аппарата, шестёрка прав механизации и удаление
 * чека на автозапчасти — последнее единственное, что пополнило запрет, а не назначаемые),
...
describe('невыдаваемые права: шесть, и именно эти', () => {
  ...
  it('удаление чека на автозапчасти выдать полномочием нельзя', () => {
    ...
    // запрет обязан отделять её от уничтожения документа, а не накрывать модуль целиком.
    expect(isGrantable('autoParts.manage')).toBe(true);
    expect(isGrantable('autoParts.stock')).toBe(true);
  });
...
describe('назначаемые права: 70 из 76', () => {
  ...
  it('назначаемых ровно столько, сколько прав минус запрет', () => {
    ...
    // нет намеренно: список запчастей открывает широкий `garage.read`, — а разведены они между
    // собой ради акта обслуживания: движение склада строкой акта требует `autoParts.stock` сверх
    // права на акт, которое есть и у менеджера с диспетчером (Р19). Назначаемыми оба стали сразу.
    ...
    // Семьдесят шестым пришло удаление чека на автозапчасти (`autoParts.delete`, план
    ...
    // и сверка «манифест ↔ факт» этого не заметила бы. Поэтому же назначаемых осталось семьдесят —
    // и это ровно то расхождение, ради которого третье равенство ниже и стоит.
    // Семьдесят шесть и семьдесят расходятся ровно на длину запрета, и это третье
...
describe('разница эффективных прав', () => {
  ...
  it('выдача показывает добавленное в словарном порядке', () => {
    ...
    expect(effectiveDelta(before, after)).toEqual({
      added: [
        // Заведение без аппарата стоит первым по той же причине, по которой закупка стоит
        // последней: словарь перечисляет его сразу за `serviceRequests.create` (ADR 0146,
        // решение 6), то есть раньше решений по заявке. Порядок выдачи в наборе — обратный, и
        // расхождение здесь как раз и доказывает, что предпросмотр читает словарь.
        'serviceRequests.createWithoutEquipment',
        'serviceRequests.assign',
        'serviceRequests.approveEstimate',
        'serviceRequests.status',
...
describe('единая проверка выдачи', () => {
  ...
  it('администратор освобождён от конфликтов, а «весь модуль» не освобождает никого', () => {
    ...
    // Весь модуль обслуживания у сервисной компании: четыре права из четырнадцати у неё уже есть по
    // типу контрагента, остальные десять не защищены — набор вправе выдать их разом. Четырнадцать,
    // а не десять: цикл заявок пополнил модуль заморозкой, срочностью и «может быть назначен
    // исполнителем» (`docs/office-equipment-requests-rework-plan.md`, §7.1), а пак расходников —
    // заведением заявки без аппарата (ADR 0146, решение 6); число здесь стоит затем, чтобы новое
    // право модуля попадало в эту проверку осознанно, а не заодно.
    const wholeModule = PERMISSIONS_BY_MODULE.service;
    expect(wholeModule.length).toBe(14);
    expect(
      codesOf(
        validateGrantAssignment({
          roles: ['operator'],
