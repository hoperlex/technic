...
describe('назначаемые права: 62 из 67', () => {
  /**
  ...
  it('назначаемых ровно столько, сколько прав минус запрет', () => {
    ...
    // Шестьдесят семь и шестьдесят два расходятся ровно на длину запрета, и это третье
    ...
    expect(grantable).toHaveLength(62);
  });
 ...
 * покажет ложную потерю, как и случилось, когда волна В5 дописала надстройке `hold` и `urgency`.
 *
