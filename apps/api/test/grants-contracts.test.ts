 ...
 * невыдаваемых прав», «назначаемых 69 из 75» (в ADR было 50 из 55 — словарь пополнили пара прав
 ...
 * чека на автозапчасти — последнее единственное, что пополнило запрет, а не назначаемые, — а
 * движение остатка автозапчастей из словаря затем ушло, единственным ушедшим за всё время),
...
describe('назначаемые права: 69 из 75', () => {
  /**
  ...
  it('назначаемых ровно столько, сколько прав минус запрет', () => {
    ...
    // Приездов, стало быть, семьдесят шесть, а прав в словаре семьдесят пять. Семьдесят пять и
    // шестьдесят девять расходятся ровно на длину запрета, и это третье равенство здесь не
    // лишнее: без него тест согласился бы с правом, добавленным в словарь и тихо попавшим в
    // запрет.
    ...
    expect(grantable).toHaveLength(69);
  });
