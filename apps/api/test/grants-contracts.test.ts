...
describe('назначаемые права: 63 из 68', () => {
  /**
  ...
  it('назначаемых ровно столько, сколько прав минус запрет', () => {
    ...
    // Шестьдесят восемь и шестьдесят три расходятся ровно на длину запрета, и это третье
    ...
    expect(grantable).toHaveLength(63);
  });
 ...
 * следом — ADR 0146, дописавший ей плановые закупки расходников.
 *
...
describe('единая проверка выдачи', () => {
  ...
  it('администратор освобождён от конфликтов, а «весь модуль» не освобождает никого', () => {
    ...
    // Весь модуль обслуживания у сервисной компании: четыре права из тринадцати у неё уже есть по
    // типу контрагента, остальные девять не защищены — набор вправе выдать их разом. Тринадцать, а
    // не десять: цикл заявок пополнил модуль заморозкой, срочностью и «может быть назначен
    // исполнителем» (`docs/office-equipment-requests-rework-plan.md`, §7.1), и число здесь стоит
    // затем, чтобы новое право модуля попадало в эту проверку осознанно, а не заодно.
    const wholeModule = PERMISSIONS_BY_MODULE.service;
    expect(wholeModule.length).toBe(13);
    expect(
      codesOf(
        validateGrantAssignment({
          roles: ['operator'],
