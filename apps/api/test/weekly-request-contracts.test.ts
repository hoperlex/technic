...
/*
 * ЭСМ2-РАЗРЕЗ (работа не закончена — см. реестр `esm2-week-registry.ts`).
 *
 * Предупреждение `esm2_reissue` считается по **неделе** и говорит «лист ЭСМ-2 недели такой-то будет
 * аннулирован». Пока лист и неделя — одно и то же, это верно. После переключения чтения (этап 5) в
 * неделе законно живут два листа, и текст станет неточным дважды: аннулируется не «лист недели», а
 * тот, что накрывает последний день заказа, и он может принадлежать другой машине.
 *
 * Тестом это сегодня не воспроизвести: `itemWarnings` — чистая функция, листов она не читает вовсе
 * и знает только границы недели. Значит и обёртка двух режимов здесь бессмысленна: расхождение
 * появится вместе с правкой самой функции, а не с режимом чтения.
 *
 * Поэтому случаи ниже фиксируют сегодняшнее поведение как есть, а работа записана в реестр.
 */
