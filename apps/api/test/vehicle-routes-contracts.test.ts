...
/**
 * Заказ техники на объект, годный для рейса дня: у него вместо дня подачи срок работ, а вместо
 * «одного рейса» — по рейсу на распланированный день (ADR 0100).
 *
 * «Linear» в имени и `kind: 'linear'` в ответе остались от ADR 0100, где дни завела линейная
 * техника. Сегодня такой ответ даёт любой заказ на объект (ADR 0207 п. 1), и признак `isLinear`
 * здесь — не условие годности, а лишь значение по умолчанию: правила рейса его не читают, и
 * сценарии, где это и есть утверждение, перебирают оба значения сами.
 */
...
describe('какая заявка годится для рейса', () => {
  ...
  /*
   * Здесь было два утверждения о двух разных отказах. Нелинейному заказу на объект рейс отвечал
   * «в рейс ходят только грузоперевозки» (проверялось словом «грузоперевозки»), и лишь линейному —
   * что дверь у дня своя, карточка заявки. Развилка держалась на признаке типа ТС.
   *
   * Её больше нет: ADR 0207 п. 1 открыл дни любому заказу техники на объект, и обе прежние
   * половины стали неправдой — «ходят только грузоперевозки» неправда (ходит и день такого
   * заказа), «это про линейный» неправда тоже. Текст отказа сведён в один
   * `LINEAR_DAY_DOOR_MESSAGE`, и два утверждения сведены в одно вслед за ним: сегодня это один и
   * тот же случай, и держать его двумя тестами значило бы утверждать несуществующую разницу.
   *
   * Проверяется дверь, а не вид заявки: со стороны рейса заказ на объект не кладут вовсе, потому
   * что рейс не знает, какой день срока в него ставят, — и отвечает он этим обоим заказам
   * одинаково. Перебор по `isLinear` и есть снятое правило: вернись развилка по типу, один из двух
   * ответов разойдётся с `LINEAR_DAY_DOOR_MESSAGE`.
   */
  it('заказ техники на объект отсюда не кладут: дверь у дня одна — карточка заявки', () => {
    for (const isLinear of [false, true]) {
      const check = canJoinRoute(
        { ...goodRequest, requestType: 'special_equipment', isLinear },
        emptyRoute,
      );
...
describe('состав перегона', () => {
  ...
  /*
   * Отказ перегона дню — свой, отдельный от «заявки в перегон не кладут»: кладут-то не заявку, а
   * день, и общий текст здесь ничего бы не объяснил. Это осталось как было.
   *
   * Изменились его слова. Прежде текст говорил про линейную технику, которая «возвращается на
   * базу», и проверялся ровно этой цитатой; после ADR 0207 п. 1 перегон отказывает дню ЛЮБОГО
   * заказа на объект, и обещание про линейность стало бы ложью для половины случаев. Проверяемая
   * суть при этом сохранена целиком — день работ в перегон не кладут, — и сказано это теперь
   * причиной, а не приметой техники: у перегона своя заявка-основание.
   *
   * Отказ сверяется с текстом обычной заявки: он обязан остаться ДРУГИМ. Свести их в один значило
   * бы вернуть ровно ту потерю смысла, ради которой отдельный текст и заводили.
   */
  it('день заказа на объект в перегон не кладут — своими словами, не общими', () => {
    const plain = canJoinRoute(goodRequest, { ...emptyRoute, purpose: 'delivery' });
    expect(plain.ok).toBe(false);
    const plainReason = plain.ok === false ? plain.reason : '';
    for (const isLinear of [false, true]) {
      const check = canJoinRoute(
        { ...goodLinearRequest, isLinear },
        { ...emptyRoute, purpose: 'delivery' },
      );
      expect(check.ok).toBe(false);
      const reason = check.ok === false ? check.reason : '';
      expect(reason).toContain('день работ в него не кладут');
      expect(reason).toContain('заявке-основанию');
      expect(reason).not.toBe(plainReason);
    }
  });
});
