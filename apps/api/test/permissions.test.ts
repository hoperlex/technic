...
describe('служба главного механика: механик смотрит, главный механик ведёт', () => {
   ...
   * рулём (карточки водителей). Своего чтения у склада запчастей в перечне нет и не будет: список
   * позиций открывает тот же `garage.read` (`docs/auto-parts-plan.md`, Р10).
   */
  it('механик читает четыре списка и ведёт два — журнал ТО и склад запчастей', () => {
    ...
    // Действий у роли два — техобслуживание (план «Показания техники», Р14) и склад автозапчастей
    // (`docs/auto-parts-plan.md`, Р10), — и правило, по которому собран перечень выше, теперь
    // такое: всё остальное у механика чтение. Прежде тут стояло «ни одного права, которое не
    // кончается на `.read`»; ТО сняло это осознанно (акты заводит служба главного механика, а не
    // тот, кто её об этом попросит), склад — по той же причине: заказчик назвал держателем учёта
    // механиков. Список исключений именно поимённый: третье действие обязано попасть в ревью, а
    // не приехать молча.
