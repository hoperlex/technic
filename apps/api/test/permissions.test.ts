...
describe('права ролей', () => {
  ...
  it('диспетчер ведёт справочники наравне с менеджером и расходится с ним в обе стороны', () => {
    ...
    /*
     * Обратная половина расхождения — круг заявителя оргтехники (ADR 0181, решение заказчика
     * 10.09.2026). У менеджера он стоит в матрице, у диспетчера приходит набором, и это осознанная
     * асимметрия: `manager` — временная роль-заплатка, и её строка в матрице уйдёт вместе с ней, а
     * поимённая выдача пережила бы удаление роли мусором в `grant_roles` и `user_grants`.
     *
     * Права здесь перечислены, а не взяты из `SERVICE_REQUEST_CUSTOMER_PERMISSIONS`: список,
     * сверяемый сам с собой, не поймал бы ни одного расширения — а расширение круга заказчика
     * обязано приезжать сюда строкой и объясняться, потому что вместе с ролями площадки оно
     * достаётся теперь и всей диспетчерской разом.
     */
    const MANAGER_ONLY: Permission[] = [
      ...
      // Сообщение об аппарате, которого нет в справочнике (ADR 0165): тупик того же человека на
    ...
    ).toEqual([...ROLE_PERMISSIONS.manager].filter((p) => !MANAGER_ONLY.includes(p)).sort());
    ...
    for (const permission of MANAGER_ONLY) {
      expect(can(of('manager'), permission), permission).toBe(true);
      expect(can(of('dispatcher'), permission), permission).toBe(false);
    }
  });
...
describe('заявки на обслуживание оргтехники (ADR 0085)', () => {
   ...
   * закрывать.
   */
  it('прав у модуля шестнадцать, и список модуля выводится из матрицы', () => {
    ...
    //
    // Шестнадцатым — «переписывает объём работ по графам» (`serviceRequests.estimateRewrite`, план
    // `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р2; миграция 0298). Дверь у
    // него своя (`PUT /:id/estimate/breakdown`), а держатель ровно один набор — «Ведение»: ни
    // ИТ-службе, ни исполнителю, ни сервисной компании оно не выдаётся, потому что те объём работ
    // ПИШУТ, а не переписывают чужой. В матрице ролей его тоже нет ни у одной строки.
    //
    // ПЕРВОЕ РАВЕНСТВО ЗДЕСЬ — НЕ ФОРМАЛЬНОСТЬ, И ИМЕННО ОНО ЛОВИТ ЗАБЫТУЮ СТРОКУ. Список модуля —
    // не второй перечень рядом со словарём, а его срез, и новое право модуля обязано приезжать в
    // оба места одной правкой. Разъехавшись, они молчат: словарь раздаёт право, а список модуля о
    // нём не знает — и всякий, кто спрашивает «что есть у этого модуля», получает неполный ответ.
    expect([...SERVICE_REQUEST_PERMISSIONS]).toEqual(
      PERMISSIONS.filter((p) => p.startsWith('serviceRequests.')),
    );
    expect(SERVICE_REQUEST_PERMISSIONS).toHaveLength(16);
  ...
  it('заказчик заводит, правит, удаляет и подшивает — и все четыре роли одинаково', () => {
    ...
    /*
     * Менеджер стал пятым заказчиком (ADR 0181): круг у него тот же самый, что у площадки и
     * отдела, — сравнение идёт с тем же перечнем, а не со своим списком, потому что расхождение
     * между «заказчиком с площадкой» и «заказчиком из офиса» живёт не в правах, а в слое стороны:
     * первого сужает его площадка, второго — авторство (`actsAsRequestCustomer`).
     */
    expect(moduleOf(of('manager'))).toEqual(['create', 'delete', 'files', 'read', 'update']);
    // Диспетчер ведёт справочник оргтехники, но не модуль заявок: круг заявителя приходит ему
    // набором «Оргтехника: заявитель», а не ролью, и в матрице его поэтому нет.
