...
const DECLARED_PROFILE_KEYS = [
  ...
  // Офисная пара «Ведения» (план профилей оргтехники, Р5, миграция B этапа Э7): ведение
  // оргтехники бывает и централизованным, и выдаётся оно ролям БЕЗ своей оси области. Субъекты
  // здесь не ради полноты списка: у роли без оси предикат видимости ничего не сужает, поэтому
  // каждый маршрут, который им открылся, обязан быть назван поимённо — включая тот, где вместе с
  // ходом заявки диспетчеру открылся откат статуса (`requests.rollbackStatus` у него по роли).
  'manager+office_equipment_operator',
  'dispatcher+office_equipment_operator',
...
const SERVICE_REQUEST_CUSTOMERS: ProfileKey[] = [
  ...
  'department_head',
  /*
   * Офис приходит сюда НАДСТРОЙКОЙ, а не ролью (план профилей оргтехники, Р5, Р6): у менеджера и
   * диспетчера прав модуля заявок нет ни одного, и «Ведение» открывает им весь состав заказчика
   * заодно с решениями. Ключи стоят здесь именно потому, что базовых ролей в перечне нет: правило
   * `isAllowed` пропустило бы их только вслед за ролью, а роль на эти маршруты не пускает.
   *
   * Своей заявкой держатель при этом не ограничен — этим он и отличается от набора «Заявитель»
   * (Р6): «Ведение» ведёт и чужие, ради этого профиль и существует. Набор «Заявитель» в матрицу не
   * попадает вовсе — он модульный, а перебор субъектов собирается из ролей, типов контрагента и
   * надстроек; его сужение до своих строк доказывают db-тесты модуля.
   */
  'manager+office_equipment_operator',
  'dispatcher+office_equipment_operator',
];
...
const SERVICE_REQUEST_OPERATORS: ProfileKey[] = [
  ...
  'site+office_equipment_operator',
  'department+office_equipment_operator',
  // Централизованное ведение (Р5, миграция B): те же решения у офисной пары ролей.
  'manager+office_equipment_operator',
  'dispatcher+office_equipment_operator',
];

/**
 * Кто проходит к согласованию объёма работ. Перечень СМЕНИЛ ОСНОВАНИЕ вместе с упраздённой визой ИТ
 * (план упрощения цикла, Р3, Р10): двух подписей по порядку больше нет, а согласует по ответу В2
 * назначенный сотрудник, — и страж ручки стал дизъюнкцией «право согласования **или**
 * `serviceRequests.execute`». Поэтому здесь «Ведение» — у него `approveEstimate`.
 *
 * ИТ-СЛУЖБЫ ЗДЕСЬ БОЛЬШЕ НЕТ (план `docs/office-equipment-access-profiles-plan.md`, Э6): она
 * проходила стража вторым слагаемым — своим `serviceRequests.execute`, — а право уехало в
 * отдельный набор `office_equipment_executor`. Пропажа ключей не означает, что системный
 * администратор перестал согласовывать объём работ: профиль выдаётся ДВУМЯ кодами, и второй ему
 * это и открывает. Просто перебор субъектов (`ACCESS_PROFILES`) собирается из ролей, типов
 * контрагента и НАДСТРОЕК, а надстройки «исполнитель» нет и не будет — набор сразу модульный.
 * Держателей модульных наборов матрица не перебирает вовсе (так же, как аудит талонов), и
 * доказывают их db-тесты модуля.
 */
const SERVICE_REQUEST_ESTIMATE_APPROVERS: ProfileKey[] = [
  'admin',
  'shtab+office_equipment_operator',
  'site+office_equipment_operator',
  'department+office_equipment_operator',
...
/**
 * Работа исполнителя: принять в работу, отказаться, вести и предъявить смету, закрыть работы,
 * написать примечание.
 *
 * Ручки исполнителя закрыты стражем «одно из прав» — `estimate` либо `execute` (план §7.3).
 * **Право пускает к ручке, но не к чужой заявке**: какая дуга доступна, решает коридор по паре
 * «назначен поимённо + `execute`», и держатель права, не назначенный на заявку, получает отказ от
 * коридора — на шаг позже, чем этот тест смотрит. Матрица проверяет именно стража, поэтому здесь
 * перечислены все, кого он пускает.
 *
 * СВОЙ СОТРУДНИК ПРИХОДИТ СЮДА НАБОРОМ `office_equipment_executor`, а не надстройкой (план
 * профилей, Э6): `serviceRequests.execute` уехало из «Оргтехника: ИТ-служба» в отдельный модульный
 * набор, потому что у ИТ-набора сквозная область — и право исполнителя было неотделимо от права
 * видеть все заявки компании. Модульные наборы в переборе субъектов (`ACCESS_PROFILES`) не
 * участвуют: он собирается из ролей, типов контрагента и надстроек. Отсюда и перечень из двух
 * строк — стража держателя нового набора проверяют db-тесты модуля, а не матрица.
 */
const SERVICE_REQUEST_EXECUTORS: ProfileKey[] = ['admin', 'operator/service'];

/**
 * Назначение исполнителей — «Ведение» и ИТ-служба (§6.1 плана переработки): второй набор получил
 * `serviceRequests.assign` волной В5, чтобы взять чужую заявку, назначив себя.
 */
const SERVICE_REQUEST_ASSIGNERS: ProfileKey[] = [
  ...SERVICE_REQUEST_OPERATORS,
  'shtab+office_equipment_it_approver',
  'site+office_equipment_it_approver',
  'department+office_equipment_it_approver',
];
...
/*
 * Заморозки в этом перечне нет намеренно: её ручки закрыты стражем `serviceRequests.read`, а
 * решение принимает предикат `canHoldService` в обработчике (план §7.3) — то есть матрица, которая
 * смотрит на стража, видела бы там всех читателей модуля. Кто откладывает заявку, проверяют
 * контрактные тесты коридоров и db-тесты потока.
 */
...
const CASES: Case[] = [
  ...
  },
  {
    ...
    // Ни штаб, ни отдел без надстройки исполнителей не назначают, иначе назначал бы всякий, кто
    // завёл заявку.
    //
    // ИТ-службы в перечне нет и до волны В5 быть не может: право `serviceRequests.assign` приходит
    // ей составом набора, а состав правится каталогом полномочий — маршрут появляется раньше, чем
    // право на него.
    title: 'заявки на обслуживание — назначение исполнителей',
    method: 'PUT',
    url: `/api/v1/service-requests/${RECORD_ID}/executors`,
    payload: { userIds: [], serviceCounterpartyId: COUNTERPARTY_ID, version: 1 },
    allowed: SERVICE_REQUEST_ASSIGNERS,
  },
  /*
   * СНЯТ кейс «виза отдела ИТ по смете» (`PATCH /:id/it-approval`). Виза упразднена целиком (план
   * упрощения цикла, Р10, ответ В2): двух подписей по порядку больше нет — вопрос «чинить за эти
   * деньги или менять аппарат» задаёт себе тот же человек, что смотрит на объём работ, и «менять»
   * стало исходом согласования (кейс ниже), а не отдельной ручкой. Маршрута не существует, и кейс
   * на него проверял бы 404 вместо доступа. Право `serviceRequests.approveIt` из выданных наборов
   * убрано отдельным выпуском (план профилей оргтехники, Э9, миграция E), в матрице администратора
   * оно осталось — и маршрутов не открывает ни одного: что ходов оно не даёт, проверяет
   * `service-corridors.test.ts`.
   */
  {
    // Согласование объёма работ: пара прав вместо одного (Р3). Подписывает назначенный сотрудник, а
    // не только «Ведение», — поэтому ИТ-служба проходит стража своим `serviceRequests.execute`.
    title: 'заявки на обслуживание — согласование объёма работ',
    method: 'PATCH',
    url: `/api/v1/service-requests/${RECORD_ID}/estimate/approval`,
    payload: { approved: true, version: 1 },
    allowed: SERVICE_REQUEST_ESTIMATE_APPROVERS,
  },
  {
    // Состав расходников заполняет ИСПОЛНИТЕЛЬ, а не заявитель (Р15): заявитель говорит словами
    // («закончился чёрный тонер»), номенклатуру подбирает тот, кто повезёт. Право поэтому сменилось
    // с «заказчик, пока заявку никому не отдали» на пару стороны исполнителя — и заказчика в
    // перечне больше нет вовсе. Кейс заведён ради этой перемены: перепутанное право почти всегда
    // меняет состав ролей, и матрица — то место, где это видно.
    title: 'заявки на обслуживание — состав расходников',
    method: 'PUT',
    url: `/api/v1/service-requests/${RECORD_ID}/consumables`,
    payload: { items: [{ consumableId: RECORD_ID, requestedQuantity: 2 }], version: 1 },
  ...
  {
    // Согласованную смету возвращает в правку сам исполнитель: статуса заявка при этом не меняет —
    // снимается снимок согласования, и дальше смета предъявляется заново с ревизией +1 (Н3).
    title: 'заявки на обслуживание — возврат сметы в правку',
    method: 'PATCH',
  ...
  {
    /*
     * Отказ перестал быть переходом (Р7): дуги `assigned → new` нет, и коридорного отсева у ручки
     * не осталось — сторону считает предикат `canDeclineServiceRequest` по строке заявки, то есть
     * уже за стражем. Поэтому «Ведение» проходит сюда своим `serviceRequests.status`, и в перечне
     * оно теперь есть: матрица смотрит на стража, а не на предикат. Что за исполнителя оператор не
     * отказывается, проверяют контрактные тесты предикатов и db-тест цикла.
     */
    ...
    allowed: [...SERVICE_REQUEST_EXECUTORS, ...SERVICE_REQUEST_OPERATORS],
  },
  {
    /*
     * «Принять в работу» — единственный шаг исполнителя, у которого дуга осталась: после упрощения
     * цикла она ведёт из «Новой» (Р6). Право у маршрута общее, `serviceRequests.status`, и оно есть
     * у оператора оргтехники, — но отсекает его коридор до чтения заявки, и в перечень оператор
     * поэтому не попадает: «взяться за заявку за сервис» не должно работать ни через свою ручку, ни
     * через `/status`.
     *
     * Поимённого исполнителя (И*) в перечне нет по другой причине, и она не про коридор: страж
     * ручки пускает и по `serviceRequests.execute` (план §7.3), но право это приходит **набором
     * полномочий**, а перебираются здесь поставочные профили — роль плюс надстройка. Ни одна роль
     * `execute` не даёт, поэтому в матрице его носителя нет вовсе; что назначенный проходит, а
     * неназначенный получает отказ коридора, доказывает db-тест цикла.
     */
    title: 'заявки на обслуживание — принятие в работу',
    method: 'PATCH',
...
beforeAll(async () => {
  ...
  await app.ready();
  /*
   * Тридцать секунд вместо умолчания в десять — мерка выросшего приложения: сборка со всеми
   * маршрутами занимает около восьми секунд транспиляции, и умолчание пробивается стабильно, а под
   * параллельной нагрузкой — с запасом. Падал при этом хук, а не проверка: матрица доступа
   * краснела, не успев спросить ни об одной ручке.
   */
}, 30_000);
