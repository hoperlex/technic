 ...
 * `serviceRequests.execute`». Поэтому здесь и «Ведение» (у него `approveEstimate`), и ИТ-служба
 * (у неё `execute`, которым она значится в заявке исполнителем).
 *
 * Совпадение состава с `SERVICE_REQUEST_ASSIGNERS` — совпадение сегодняшних наборов, а не одно
 * решение: своим списком, потому что назначение и подпись под деньгами разъедутся на первой же
 * правке любого из двух наборов.
 *
 * Право пускает к ручке, но не к чужой заявке: держатель `execute` без строки в ЭТОЙ заявке
 * получает отказ от предиката `canApproveServiceEstimate` — на шаг позже, чем смотрит матрица.
 */
 ...
 * написать примечание.
 *
 * После волны В5 сюда входит и ИТ-служба: набор «Оргтехника: ИТ-служба» несёт
 * `serviceRequests.execute`, и ручки исполнителя закрыты стражем «одно из прав» — `estimate` либо
 * `execute` (план §7.3). **Право пускает к ручке, но не к чужой заявке**: какая дуга доступна,
 * решает коридор по паре «назначен поимённо + `execute`», и держатель набора, не назначенный на
 * заявку, получает отказ от коридора — на шаг позже, чем этот тест смотрит. Матрица проверяет
 * именно стража, поэтому здесь перечислены все, кого он пускает.
 */
const SERVICE_REQUEST_EXECUTORS: ProfileKey[] = [
  'admin',
  'operator/service',
  'shtab+office_equipment_it_approver',
  'site+office_equipment_it_approver',
...
const CASES: Case[] = [
   ...
   * на него проверял бы 404 вместо доступа. Право `serviceRequests.approveIt` при этом из матрицы и
   * наборов не снято — уборка наборов идёт отдельным выпуском, — но маршрутов оно больше не
   * открывает: что ходов оно не даёт, проверяет `service-corridors.test.ts`.
   */
  {
