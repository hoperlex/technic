...
const CASES: Case[] = [
  ...
  },
  {
    ...
    title: 'заявки на обслуживание — виза отдела ИТ по смете',
    method: 'PATCH',
  ...
  {
    // Отказ от заявки и принятие её в работу — шаги исполнителя, но право у маршрута общее,
    ...
    // ручки с волны В3 пускает и по `serviceRequests.execute` (план §7.3), но право это приходит
    // **набором полномочий**, а перебираются здесь поставочные профили — роль плюс надстройка.
    // Ни одна роль `execute` не даёт, поэтому в матрице его носителя нет вовсе; что назначенный
    // проходит, а неназначенный получает отказ коридора, доказывает db-тест цикла.
  ...
  {
    // Отдельного статуса «Диагностика» больше нет (Н2): взявшийся за заявку стоит в «В работе».
