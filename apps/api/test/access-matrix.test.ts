...
/**
 * Заказчик заявки на обслуживание (ADR 0085): обе объектные роли, обе роли отдела и менеджер. Один
 * перечень на заведение, правку и удаление — это одно решение, и разъехаться они не должны.
 * Комендант сюда не входит: оргтехника на площадке идёт через штаб, как и заказ техники.
 */
const SERVICE_REQUEST_CUSTOMERS: ProfileKey[] = [
  'admin',
  /*
   * Менеджер — заказчик РОЛЬЮ (ADR 0181, 10.09.2026), и потому стоит здесь базовым ключом, а не
   * парой с надстройкой. Матрица маршрутов видит только стража двери, и на этом уровне менеджер
   * теперь неотличим от штаба: правом он проходит на все четыре двери заказчика.
   *
   * Что дальше по коду он ограничен АВТОРСТВОМ (`actsAsRequestCustomer`), эта матрица не проверяет
   * и проверять не должна — сужение живёт в обработчике, за стражем, и доказывают его db-тесты
   * модуля. Ровно та же граница, что у набора «Заявитель» у диспетчера.
   */
  'manager',
  'shtab',
  'rukstroy',
  ...
  'department_head',
  /*
   * Диспетчер приходит сюда НАДСТРОЙКОЙ, а не ролью (план профилей оргтехники, Р5, Р6): прав
   * модуля заявок у роли нет ни одного, и «Ведение» открывает ему весь состав заказчика заодно с
   * решениями. Ключ стоит здесь именно потому, что базовой роли в перечне нет: правило `isAllowed`
   * пропустило бы её только вслед за ролью, а роль на эти маршруты не пускает.
   *
   * ПАРЫ `manager+office_equipment_operator` ЗДЕСЬ БОЛЬШЕ НЕТ, и это не потеря доступа (ADR 0181):
   * менеджер проходит на все четыре двери заказчика базовой ролью, а профиль с надстройкой
   * попадает туда правилом `isAllowed` вслед за ней. Оставь мы ключ, его уронил бы отдельный
   * караул матрицы — «свои маршруты надстройки перечисляются там, где базовой роли отказано»: ключ,
   * дублирующий роль, означал бы, что надстройка на этой двери ничего не решает.
   *
   * Своей заявкой держатель «Ведения» при этом не ограничен — этим он и отличается от заявителя
   * (Р6): оператор ведёт и чужие, ради этого профиль и существует. Сужение заявителя до своих строк
   * матрица не видит вовсе — оно живёт за стражем, в обработчике, и доказывают его db-тесты.
   */
  'dispatcher+office_equipment_operator',
];
