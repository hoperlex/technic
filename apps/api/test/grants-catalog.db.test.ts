...
> = new Map([
   ...
   *
   * Строка одна на весь список, и второй набор модульной части — аудит распознавания талонов
   * (`waste_ticket_audit`, миграция 0211, план `docs/waste-ticket-audit-plan.md` §4.1) — приезжает
   * сюда сам, без правки этого файла. Это и было доводом против пятого списка кодов в контрактах:
   * каждый новый перечень пришлось бы дописывать и в `ALL_SYSTEM_GRANT_CODES`, и сюда, а забытая
   * строка означает не падение, а молчание стража — набор существует, помечен системным, и состав
   * его не сверяет никто.
   */
...
describe.skipIf(!DB_URL)('каталог системных полномочий: база против контрактов', () => {
  ...
  /**
   * Т10 ПЛАНА ПРОФИЛЕЙ: **правило совместимых ролей**, проверенное на строках базы, а не на копии
   * в контрактах (план `docs/office-equipment-access-profiles-plan.md`, Р5 и §10).
   *
   * Правило записано один раз и звучит так: строка `grant_roles` — это ОБЕЩАНИЕ, что выдача пройдёт
   * и что-нибудь даст. Отсюда два условия, и оба проверяются здесь:
   *
   *   1. выдача набора этой роли проходит барьеры 3–5 (`validateGrantAssignment`: клетка «ось роли
   *      × модуль», требования `PERMISSION_REQUIRES` по итогу, конфликты обязанностей по итогу);
   *   2. набор что-то роли даёт — иначе администратор выдаёт строку, не меняющую ничего.
   *
   * ПОЧЕМУ ЧИТАЕТСЯ БАЗА. Соседние тесты (`grants-contracts.test.ts`) проверяют то же правило на
   * объявленных каталогах — и обязаны, потому что это решение. Но выдача считается по строкам
   * `grant_roles`, а их пишут миграции руками: роль, дописанная миграцией мимо контрактов, прошла
   * бы весь контрактный перебор и открыла бы выдачу набора там, где её не задумывали. Сверка
   * составов выше поймала бы и это — но сказала бы «списки разошлись», а здесь скажет, ЧТО именно
   * сломается при выдаче.
   *
   * ПОЧЕМУ ТОЛЬКО НАБОРЫ ОРГТЕХНИКИ. Правило Р5 сформулировано про них, и шире оно неверно: «Виза
   * объекта» (`site_approval`) намеренно выдаётся ПОВЕРХ «Заказа техники» и в одиночку барьер
   * требований не проходит — это её решение, проверенное отдельным случаем в
   * `grants-contracts.test.ts`. Требуй тест самодостаточности от всего каталога, он запрещал бы
   * связанные наборы как класс.
   *
   * `driver` и роли без строк сюда не попадают по построению: перебираются только пары, реально
   * лежащие в базе.
   */
  it('каждая роль из grant_roles проходит барьеры выдачи и что-то получает (Т10)', async () => {
    const { rows } = await client.query<{ code: string; role: Role }>(
      `SELECT g.code, gr.role
         FROM grant_roles gr
         JOIN grants g ON g.id = gr.grant_id
        WHERE g.code LIKE 'office_equipment%' AND g.deleted_at IS NULL
        ORDER BY g.code, gr.role`,
    );
    expect(
      rows,
      'в базе нет ни одной строки grant_roles — миграции каталога не накатаны?',
    ).not.toEqual([]);

    for (const { code, role } of rows) {
      /*
       * Состав берётся ИЗ БАЗЫ, а не из контрактов, и это не мелочь оформления: барьер отвечает на
       * вопрос «что будет при выдаче», а выдаёт сервер по строкам `grant_permissions`. Сверку
       * «база = контракты» держит соседний случай выше; здесь она была бы лишним слагаемым, из-за
       * которого одно расхождение падало бы дважды и в разных словах.
       *
       * Сироты (право, снятое из словаря выкатом) отсеиваются `isPermission` — тем же фильтром,
       * что и у читателя прав (`auth/principal.ts`): доступа они не дают, и барьер обязан считать
       * то же, что считает сервер.
       */
      const permissions = (await permissionsOf(code)).filter(isPermission);
      expect(permissions, `набор «${code}» в базе пуст`).not.toEqual([]);
      // Итог субъекта считает сервер как «права роли ∪ состав набора» — так же и здесь: барьеры 4
      // и 5 смотрят на ИТОГ учётки, а не на состав набора (`office_equipment_operator` сам по себе
      // инвариант нарушает, субъект — нет).
      ...
      expect(
        ...
        `набор «${code}» нельзя выдать роли «${role}», хотя строка grant_roles это обещает`,
      ).toEqual([]);

      const own = permissions.filter((p) => !ROLE_PERMISSIONS[role].includes(p));
      expect(
        own,
        `набор «${code}» ничего не добавляет роли «${role}»: выдача не изменит ни одного права`,
      ).not.toEqual([]);
    }
  });

  /**
   * Вторая половина Р5, и она про ЗАПРЕТ: **роль `operator` не перечисляется ни в одном наборе
   * оргтехники никогда** (Р11 плана профилей).
   *
   * Причина не в осторожности. `operator` — внешний исполнитель, и права модуля приходят к нему
   * типом контрагента (`COUNTERPARTY_TYPE_PERMISSIONS.service`), а видимость — предикатом «заявка
   * назначена МОЕЙ компании». Набор области не несёт и нести не может (ADR 0106, решение 2):
   * выданный подрядчику, он раздал бы права по базовой области роли, которой у подрядчика нет
   * вовсе. Барьер 3 часть этого ловит сам — клетка `officeEquipment × counterparty` запрещена, —
   * но `office_equipment_executor` справочника не даёт, и его состав барьер пропустил бы: у
   * модуля `service` контрагентская ось `scoped`. То есть запрет держится решением, а не матрицей,
   * и потому проверяется отдельной строкой.
   */
  it('роли `operator` нет ни в одном наборе оргтехники (Р11)', async () => {
    const { rows } = await client.query<{ code: string; role: string }>(
      `SELECT g.code, gr.role
         FROM grant_roles gr
         JOIN grants g ON g.id = gr.grant_id
    ...
    expect(
      rows.filter((r) => r.role === 'operator'),
      'набору оргтехники разрешена роль внешнего исполнителя — это выдача прав заказчика подрядчику',
    ).toEqual([]);
    // И заодно — что перебор смотрел не в пустоту: наборы модуля в базе есть и роли у них есть.
    expect(new Set(rows.map((r) => r.code)).size).toBeGreaterThanOrEqual(3);
  });
});
