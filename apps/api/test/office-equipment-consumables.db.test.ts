...
describe.skipIf(!DB_URL)('расходники оргтехники: журнал остатка, права и совместимость', () => {
  ...
  it('несуществующая заявка отбивается внешним ключом — до него доходит только цельное событие', async () => {
     ...
     *
     * Ключей на строке журнала теперь ДВА: этот и составной `office_equipment_consumable_stock_row_fk`
     * на тройку «строка + заявка + позиция» (`0186`, M12 плана переработки заявок). Оба отбили бы
     * эту вставку — выдуманы и заявка, и строка, — а какой сработает первым, Postgres не обещает;
     * случай назван именем `…_service_request_id_fkey` потому, что так он падает фактически.
     * Что ловит именно СОСТАВНОЙ ключ — ссылку на строку чужой заявки или на строку другого
     * расходника, где каждый конец по отдельности цел, — проверяет тест 13 в
     * `service-request-consumables.db.test.ts`: здесь такую пару не собрать, строк заявки этот
     * файл не заводит вовсе.
     */
    const c = await createConsumable({ quantity: 12 });
