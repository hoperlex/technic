...
describe.skipIf(!DB_URL)('витрина учёток отдаёт эффективные права (живая схема)', () => {
   ...
   * шести учёток должно быть столько же, сколько за страницу одной. Чтение по строке дало бы вшестеро
   * больше и провалилось бы здесь; абсолютное же число зависит и от чтения принципала, которое к
   * пачке отношения не имеет, и от числа самих пачек: с планом «полномочия назначаются в окне
   * учётки» страница читает `user_grants` четырежды — принципал, коды, права и назначения формы
   * (`UserAccountDto.grants`). Держателей поэтому шесть, а не четыре: сравнение с их числом обязано
   * оставаться утверждением о пачке, а не о том, сколько пачек сегодня читает страница.
   */
