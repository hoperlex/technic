...
describe.skipIf(!DB_URL)('приём писем аппаратов: внутренние ручки и журнал письма', () => {
  ...
  it('выключенный рубильник: письмо остаётся в ящике, курсор стоит, счётчик не растёт', async () => {
    /*
     * СОСТОЯНИЕ ЗАДАЁТСЯ ЯВНО, И ЭТО ПЕРЕМЕНА ПРОТИВ ПЕРВОЙ РЕДАКЦИИ. Раньше закрытый приём брался
     * умолчанием миграции 0317 — «тест ничего не щёлкал», и этим же доказывалось, что волна ехала
     * выключенной. Итогом набора миграций доказать это больше нельзя: приём открыт миграцией 0328
     * (пилот), и база, накатанная целиком, приходит с ОТКРЫТЫМ рубильником. Проверяемое
     * утверждение не меняется — при выключенном рубильнике письмо обязано остаться в ящике, — а
     * итог набора проверяется строкой ниже: сдвинься он обратно, тест скажет об этом раньше, чем
     * начнёт проверять отказ. Тот же приём и по той же причине стоит у рубильника кандидатов
     * (`candidate-intake-flag.db.test.ts`).
     */
    const initial = await ctx.db.execute<{ is_enabled: boolean }>(
      sql`SELECT is_enabled FROM feature_flags WHERE key = 'device_mail_intake'`,
    );
    expect(initial.rows[0]?.is_enabled, 'миграция 0328 оставила приём открытым').toBe(true);
