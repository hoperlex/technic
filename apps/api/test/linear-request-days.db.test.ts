...
describe.skipIf(!DB_URL)('дни линейного заказа в рейсах (живая схема)', () => {
  ...
  /**
   * ЗДЕСЬ СТОЯЛ СТОРОЖ СНЯТОГО ПРАВИЛА, и снято оно решением заказчика, а не переделкой кода.
   *
   * Прежнее утверждение: «у нелинейного заказа дней нет вовсе» — таблица дней пуста, а блок
   * объясняет пустоту тем, что такая машина стоит на площадке неделями; подённая дверь отвечает на
   * это 422 теми же словами. Держалось оно признаком типа ТС: `linearDaysBlocker` спрашивал
   * `isLinear` и заворачивал всех остальных.
   *
   * [ADR 0207](../../../docs/adr/0207-vehicle-request-day-batch.md) п. 1 этот вопрос снял: 4-П за
   * день просят и у машины, которая неделю стоит на площадке (выезд за топливом, возврат в гараж,
   * требование заказчика), и отказ по типу техники означал «такой бумаги не бывает» там, где её
   * всё равно выписывали мимо портала. Признак типа отвечает на «как заявка ведётся по умолчанию»,
   * а не на «бывает ли у этого дня бумага».
   *
   * Утверждение поэтому перевёрнуто, а не выброшено: граница осталась, но проходит не по типу.
   * Линейность решает БУМАГУ — сам ли портал выписывает недельные ЭСМ-2 (п. 2 того же решения), —
   * и обе половины проверяются здесь же: дни у нелинейного заказа ведутся, а его недельный лист
   * остался на месте. Двойная бумага за один день названа ценой решения, а не дефектом.
   */
  it('дни ведут любому заказу техники на объект — линейность решает бумагу, а не дверь', async () => {
    const plain = await requestInProgress(ctx.plainTypeId);

    const plan = await daysOf(plain.id);
    expect(plan.blocker).toBeNull();
    // Перечень выводится из срока — тот же, что у линейного заказа, и по той же причине.
    expect(plan.items).toHaveLength(8);
    expect(plan.items.filter((d) => d.route !== null)).toHaveLength(0);

    const res = await planDay(plain.id, ctx.dayA, {});
    expect(res.statusCode, res.body).toBe(200);
    const day = (await dayOf(plain.id, ctx.dayA))!;
    expect(day.route).not.toBeNull();
    expect(day.route!.vehicleId).toBe(ctx.vehicleId);

    // Бумага у него при этом прежняя: недельный ЭСМ-2 выписал сам перевод в работу (ADR 0100 §5,
    // ADR 0207 п. 2). Ради этого различия признак и остался в справочнике.
    const esm2 = await ctx.db.execute<{ n: number }>(sql`
      SELECT count(*)::int AS n FROM waybills
       WHERE source_request_id = ${plain.id} AND form_code = 'esm2' AND status <> 'cancelled'`);
    expect(esm2.rows[0]!.n).toBeGreaterThan(0);
  });
