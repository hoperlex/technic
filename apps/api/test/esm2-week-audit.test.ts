...
/*
 * ДЕТЕКТОР ВСТАВКИ СЕГОДНЯ СЛЕП К ОДНОЙ ФОРМЕ ЗАПИСИ, И ЭТО ЗАМЕРЕНО, А НЕ ПРЕДПОЛОЖЕНО.
 * `sheet_insert` ищет `insert(waybills` и `insert(schema.waybills`, а db-тесты со своим стендом
 * зовут таблицу через контекст — `insert(ctx.schema.waybills)`. Таких файлов в дереве 16 при 14
 * пойманных: половина класса `consumer` храповику не видна, и новый потребитель заводится молча.
 *
 * Почему не расширено здесь же: расширение делает аудит красным сразу на 16 файлах, и красноту
 * снимает не правка регулярного выражения, а классификация каждого — по файлу на запись, с разбором
 * того, читает он границы или только ставит их в фикстуре. Это работа волны разреза, а не правки
 * соседнего теста; сделать её походя значило бы наштамповать шестнадцать `note`, написанных не
 * глядя, — то есть ровно ту неправду, ради выявления которой реестр и заведён.
 */
const MARKERS = {
