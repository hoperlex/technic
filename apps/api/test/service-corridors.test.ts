 ...
 * дуги.
 *
 * Осталось здесь ровно то, ради чего таблица оставлена ПУСТОЙ, а не удалённой: право
 * `serviceRequests.approveIt` живёт в матрице и в выданных наборах — уборка наборов идёт отдельным
 * выпуском, — и ходов оно не даёт ни одного.
 */
describe('виза ИТ упразднена: право осталось, ходов у него нет', () => {
  it('таблица визы пуста целиком — из любого статуса', () => {
    for (const status of SERVICE_REQUEST_STATUSES) {
      expect(SERVICE_IT_TRANSITIONS[status], status).toEqual([]);
    }
  });

  /**
   * Ключевой случай упразднения: субъект, у которого из прав модуля есть ТОЛЬКО виза, не двигает
   * заявку никуда. Ветка чтения права в `allowedServiceStatusTransitions` при этом жива — она
   * уйдёт вместе с самим правом, — и без этого случая её возвращение к непустой таблице прошло бы
   * незамеченным.
   */
...
describe('кого ждёт заявка', () => {
  ...
  it('сторона следует из статуса, а два рабочих статуса — из признаков строки', () => {
    ...
    //
    // `it` в перечне осталось, хотя очередь его больше не возвращает (Р10): пока право
    // `serviceRequests.approveIt` живо в выданных наборах, живёт и сторона, которую оно называло, —
    // снимаются они одной уборкой. `approval` стоит рядом своим значением (Р3): согласующий не
    // совпадает ни с одной старой стороной.
  ...
  it('стороны ИТ у заявки не бывает ни в одном состоянии', () => {
    ...
    // Само значение при этом живо, и по субъекту сторона отвечает: право в наборах не снято.
    expect(isWaitingOn(admin, 'it')).toBe(true);
    for (const approver of IT_APPROVERS) {
      expect(isWaitingOn(approver, 'it'), accessProfileLabel(approver)).toBe(true);
    }
