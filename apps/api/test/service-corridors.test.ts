...
describe('коридор исполнителя: ход открывает назначение, а не право', () => {
  ...
  it('не принимает работу, не переназначает, не отменяет и не откатывает', () => {
    ...
    // Возврат работающей заявки в «Новую» и административные откаты. Ход по дуге `in_work → new`
    // остался один — откат «принял в работу» (ADR 0187 снял с неё переназначение), — и исполнителю
    // он закрыт: отматывает своё «взялся» не он, а тот, кто заявку ведёт.
    for (const [a, b] of [
      ['in_work', 'new'],
      ['done', 'in_work'],
      ['accepted', 'done'],
 ...
 *
 * Дуга у таблицы осталась ровно одна, и назначение по ней больше не ходит (Р5, ADR 0187): состав
 * исполнителей пишет `PUT /:id/executors`, статуса он не меняет ни при каком исходе, а видно ли
 * действие, отвечает предикат `canAssignServiceExecutors` (блок предикатов Р11 в конце файла).
 */
describe('коридор распределения: кто делает заявку', () => {
  /**
   * Единственная дуга ведёт из «В работе» в «Новую», и держит её ручной возврат заявки — пункт
   * «Вернуть в «Новую»» (`PATCH /:id/status`).
   *
   * ПОЧЕМУ ОНА ЖИВА, ХОТЯ ПЕРЕНАЗНАЧЕНИЕ СТАТУСА БОЛЬШЕ НЕ ТРОГАЕТ (ADR 0187). У «Ведения»
   * оргтехники есть `serviceRequests.assign` и `serviceRequests.status`, а `requests.rollbackStatus`
   * нет — через `SERVICE_ADMIN_ROLLBACKS` дуга к нему не придёт. Сними мы строку заодно с откатом,
   * ручной возврат пропал бы у тех, кто заявки ведёт.
   */
  it('открывает возврат из «В работе» в «Новую» — и это вся его таблица', () => {
...
describe('виза ИТ упразднена: ни таблицы, ни ветки, ни одного хода', () => {
   ...
   * (волна В5). Право распределения даёт ей дугу `in_work → new`, и это ВЕСЬ её вклад в коридор:
   * само назначение статуса не двигает (ADR 0187), а ручку возврата (`PATCH /:id/status`) от неё
   * закрывает страж `serviceRequests.status`, которого у ИТ-службы нет. Ходов «Ведения» у неё
   * по-прежнему нет: ни приёмки, ни отмены.
   */
  it('у ИТ-службы остаются дуга возврата и заморозка — и ни приёмки, ни отмены', () => {
...
describe('администратор проходит каждую дугу всех коридоров', () => {
  const ADMIN_ARCS: [ServiceRequestStatus, ServiceRequestStatus, string][] = [
    ...
    // Распределение и откаты сошлись на одной дуге (Р13): ходит по ней откат «принял в работу», а
    // приходит она в коридор от обеих таблиц — `SERVICE_ASSIGNER_TRANSITIONS` и откатов
    // администратора. Переназначения среди её ходов больше нет (ADR 0187).
    ['in_work', 'new', 'откатить «принял в работу»'],
...
describe('причина перехода', () => {
   ...
   * объяснения потому, что СНИМАЛА исполнителя, а по новой дуге ходит откат «принял в работу»,
   * который исполнителя не трогает. Переназначение по ней не ходит вовсе (ADR 0187) и причину
   * спрашивает само, у себя в ручке, — необязательную (Р6).
   */
  ...
  /**
   * Дуга `in_work → new` объяснения не требует, и это утверждение, а не следствие таблицы выше:
   * требуй мы причину здесь, откат «принял в работу» молча стал бы строже, чем был (как
   * `in_work → assigned` он не требовал её вовсе).
   */
  it('возврат в «Новую» объяснения не требует: откат «взялся» никого не снимает', () => {
...
describe('сброс при возвратах и откатах', () => {
   ...
   * Это ошибка: **откат «принял в работу»** обязан вернуть заявку назначенным, НЕ ТЕРЯЯ их — прежде
   * эта дуга звалась `in_work → assigned` и не сбрасывала ничего. Сними мы исполнителя, откат
   * «взялся» превратился бы в откат «назначил», то есть в другое действие, которого больше нет.
   *
   * Переназначение по этой дуге больше не ходит (ADR 0187), но довод сохраняет силу и для него:
   * состав пишется ДО перехода (письмо собирается в той же транзакции и читает исполнителей из
   * таблицы), а сброс — это `DELETE ... WHERE request_id = …` по всей заявке.
   *
  ...
  it('возврат в «Новую» из работы не снимает исполнителя и не стирает ничего', () => {
    ...
    // Вторая половина той же ловушки: причины эта дуга не требует (Р13) — откат «взялся»
    // объяснять нечего, исполнители остаются на заявке.
    expect(serviceStatusChangeRequiresReason('in_work', 'new')).toBe(false);
...
describe('действия, у которых больше нет дуги (Р11)', () => {
  ...
  it('отказывается от «Новой» назначенный, а не тот, у кого право', () => {
    ...
    // значило бы расширение, о котором не просили. Такую заявку возвращает в «Новую» тот, кто её
    // ведёт, — пунктом «Вернуть в «Новую»»; переназначение статуса не трогает (ADR 0187).
    for (const status of SERVICE_REQUEST_STATUSES.filter((s) => s !== 'new')) {
      expect(
    ...
    // «Ведение» за исполнителя не отказывается: на этот случай у него переназначение — оно меняет
    // состав, оставляя заявку там, где она есть.
    for (const operator of OPERATORS) {
      expect(
