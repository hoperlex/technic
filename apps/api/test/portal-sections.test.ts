...
describe('состав реестра', () => {
  ...
  it('стартовый порядок — сегодняшний homePath плюс «Орг.техника» в хвосте', () => {
    ...
    expect(byStart).toEqual([
      ...
      'admin',
      'office-equipment',
      // Механизация (план `docs/mechanization-module-plan.md`, Р16) дописана в хвост тем же
      // правилом и по той же причине, что «Орг.техника»: в меню раздел стоит третьим, сразу за
      // «Заказом ТС», а стартовым не становится ни у кого, у кого открыт хоть один прежний раздел.
      // Поставь мы его на место меню — площадка с обоими модулями приземлялась бы в аренду вместо
      // вывоза, то есть волна сменила бы стартовую страницу живым учёткам.
      'mechanization',
    ]);
  });
  ...
  it('порядок меню остаётся прежним и кабинета в нём нет', () => {
    expect(SHELL_SECTIONS.map((section) => section.id)).toEqual([
      'waste',
      'vehicle-requests',
      // Механизация — после «Заказа ТС» (Р16): оба отвечают на «чем работаем на площадке», и стоять
      // они должны рядом. Со стартовым порядком это намеренно не совпадает — там раздел последний.
      'mechanization',
      'waybills',
      'garage',
      'office-equipment',
...
describe('совместимость с сегодняшним homePath', () => {
  ...
  it('на смену пароля уводили держатели оргтехники — их и лечит реестр', () => {
    ...
    // Третьим разделом сюда пришла механизация (Р16): `homePath` о ней не знал вовсе — как не знал
    // и об «Орг.технике», — и держатель одного лишь `mechRequests.read` уезжал бы на смену пароля.
    // Это ровно тот же случай, что модуль оргтехники, а не новое расхождение: реестр лечит его тем,
    // что отвечает на «какие у портала разделы» одним списком.
    expect([...sections].sort()).toEqual(['directories', 'mechanization', 'office-equipment']);
  });
});
