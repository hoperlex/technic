...
describe.skipIf(!DB_URL)('каталог назначаемых полномочий: маршруты на живой схеме', () => {
  ...
  /**
   * Барьер 3 — декартово произведение «роль × право» против матрицы осей. Пример — гараж: срез дня
   * по всему парку не сужается ничем, а у роли `shtab` есть объектная ось, и набор открыл бы ей
   * машины и водителей всей компании.
   *
   * До ADR 0192 тем же примером служил журнал путевых листов. Он перестал им быть единственным
   * допустимым путём: у модуля появился предикат области (`waybillVisibilityWhere`), и клетки
   * `waybills × object|department` сменились на `scoped` вслед за ним. Гараж остался там, где были
   * листы, — предиката у него по-прежнему нет.
   */
  it('запрещённая клетка матрицы осей набор не создаёт', async () => {
    const res = await postGrant(ctx.admin, {
      code: freshCode(),
      name: 'Гараж для площадки',
      permissions: ['garage.read'],
      roles: ['shtab'],
    });
    expect(res.statusCode, res.body).toBe(400);
