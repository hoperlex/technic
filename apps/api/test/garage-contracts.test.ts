...
describe('право на раздел', () => {
  it('гараж открыт тем же, кто ведёт водителей и листы', () => {
    ...
    //
    // Роли РАЗЛИЧНЫЕ, а не список субъектов: с расширением «Ведения» на офисную пару (план
    // профилей оргтехники, Р5) перебор `ACCESS_PROFILES` несёт менеджера и диспетчера дважды —
    // самих по себе и с надстройкой, — а надстройка гараж не открывает и открыть не может.
    // Дубли в ответе означали бы «у роли есть право дважды», то есть ничего.
    expect([...new Set(profilesWith('garage.read').map((s) => s.role))]).toEqual([
      'admin',
      'manager',
      'dispatcher',
      'mechanic',
