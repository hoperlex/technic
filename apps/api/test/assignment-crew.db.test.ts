 ...
 *    молча.
 *
 * ПОЧЕМУ ДЕНЬ РАСЧЁТА ФИКСИРОВАН. `asOf` уходит в команду аргументом — среда текущей недели, — и
 * это не удобство: исход команды (Р32), граница отменяемости листа и изменяемая область (Р21)
 * считаются по нему, и прогон, взявший «сегодня» из часов, менял бы смысл половины случаев в
...
// отработанная неделя (прошлая), и ещё не кончившаяся (текущая), и предстоящая (следующая).
...
/** Понедельник прошлой недели: с него идёт срок, и её лист к среде уже отработан. */
...
describe('якорь машиниста (Р16) и три состояния шкалы (Р19)', () => {
  ...
  it('первая фаза предпросмотра называет границу без человека, вторая — план и отпечаток', async () => {
    ...
    await inScene(options, async (tx, scene) => {
      ...
      // его начала: начало лежит в отработанной неделе, куда команда не дотягивается.
      ...
      const full = await previewCrew(
        ...
        setBody({
          ...
          anchors: [{ effectiveDate: MONDAY, driverPersonId: scene.personB }],
        }),
      );
      expect(full.requiredAnchors).toEqual([]);
      // Якорь на понедельник задевает уже прошедшие понедельник и вторник — и вся команда
      // становится коррекцией: исход считается максимумом по её логическим эффектам (Р32).
      ...
      const failure = await errorOf(() =>
        previewCrew(
          ...
          setBody({
            effectiveDate: NEXT,
            driverPersonId: scene.personB,
            // Среда вместо понедельника: пробел начинается не здесь, и якорь на неё был бы второй
            // дверью в историю — мимо Р12 и матрицы исходов Р32.
...
describe('границы команды (Р12, Р13)', () => {
  ...
  it('прошедшие дни отменой не правятся — только коррекцией', async () => {
    if (!readMode.enabled) return;
    await inScene({ driverAtStart: 'person_a' }, async (tx, scene) => {
      // Плановое решение, которое к среде уже вступило в силу: календарь его отменить не даёт.
      await insertChange(tx, {
        requestId: scene.requestId,
        effectiveDate: MONDAY,
        dimension: 'driver',
