...
describe.skipIf(!DB_URL)('повторное обращение по аппарату: правило и границы (§10.1 плана)', () => {
  ...
  describe('окно', () => {
    ...
    it('НАХОДКА: субмиллисекундная часть `created_at` до предиката не доезжает', async () => {
      /*
       * РАСХОЖДЕНИЕ С §10.1 (случай 1) И МЕЖДУ ДВУМЯ ПОТРЕБИТЕЛЯМИ ПРАВИЛА, записанное как есть —
       * утверждения «как должно быть» здесь нет намеренно.
       *
       * План требует: `P.status_changed_at = R.created_at` → повтор ЕСТЬ (верхняя граница включена).
       * Сегодня это верно только для заявок, чей `created_at` пришёлся ровно на миллисекунду.
       * Причина не в SQL, а в дороге к нему: пакетный счёт получает `R.created_at` строкой
       * JavaScript (`row.createdAt.toISOString()` в `serviceRequestRepeatByRequest`), а `Date` в
       * JavaScript миллисекундой и заканчивается — микросекунды `timestamptz` теряются при разборе
       * ответа драйвером. Обе границы окна поэтому уезжают вниз на ту же долю миллисекунды.
       *
       * ЦЕНА — не в самой доле, а в том, что ОТБОР её не теряет: `serviceRequestRepeatWhere` берёт
       * `created_at` КОЛОНКОЙ, то есть с полной точностью базы. На заявке ниже это видно прямо:
       * метка говорит «повторов нет», а отбор «только повторные» ту же заявку показывает. Это ровно
       * тот класс расхождения, ради которого правило и сведено в один файл (К3), — только цена его
       * микроскопическая: чтобы попасть в зазор, предшественницу надо закрыть в ту же миллисекунду,
       * в которую заведена заявка.
       */
      ...
      // По §10.1 здесь ожидается 1: момент закрытия совпал с моментом заведения.
      expect((await repeatOf(id))?.count).toBe(0);
      // А отбор, считающий по колонке, ту же заявку повторной признаёт.
