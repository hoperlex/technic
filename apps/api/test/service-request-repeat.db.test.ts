 ...
 *
 *   1. обе границы окна включены, за границей — ничего; и рядом сторож дороги данных:
 *      субмиллисекундная часть `created_at` не разводит между собой метку, отбор и ссылку
 *      «предыдущие» — все трое читают дату заведения колонкой базы;
...
describe.skipIf(!DB_URL)('повторное обращение по аппарату: правило и границы (§10.1 плана)', () => {
  ...
  describe('окно', () => {
    it('обе границы включены, а за границей ничего (§10.1, случай 1)', async () => {
       ...
       *
       * ШАГ — МИЛЛИСЕКУНДА, А НЕ МИКРОСЕКУНДА, КАК В §10.1: это выбор шага, а не предел точности
       * признака. Субмиллисекундную часть `created_at` правило различает — тем и занят следующий
       * случай; здесь же спрашивают про сами границы, и миллисекунды им довольно.
       */
    ...
    it('субмиллисекундная часть `created_at` не разводит метку, отбор и ссылку (§10.1, случай 1; К3)', async () => {
      /*
       ...
       * Поэтому проверяется здесь не «правильное число», а СОВПАДЕНИЕ ответов; правильное число
       * проверено соседним случаем на границах окна.
       *
       * СПРАШИВАЮТСЯ ВСЕ ТРИ ПОТРЕБИТЕЛЯ, А НЕ ДВА. Ссылка «предыдущие» ходила за датой в JS дольше
       * прочих, и обещание метки рвалось на ней нагляднее всего: «Повтор ×1» вело в пустой список —
       * ровно тот исход, который §11 плана считает провалом приёмки.
       */
      ...
      const marked = await repeatOf(id);
      ...
      // И ссылка показывает ровно ту заявку, которую метка посчитала. Сверяется СОСТАВ, а не длина:
      // одинаковый размер бывает и у разных наборов, а обещание метки — про сами строки.
      const previous = await previousIds(ctx.adminPrincipal, await subjectOf(id));
      expect(previous).toEqual([prevId]);
      expect(previous.length).toBe(marked?.count);
    });
