...
describe('авторизация маршрутов', () => {
   ...
   * переработки цикла §7.3) и у карточки сообщения об отсутствующей технике (план кандидатов, Р9).
   * Инвариант тот же, что у доступа «по самой записи» выше: дизъюнкция, приписанная соседнему
   * маршруту, — это удвоенный круг допущенных, и заметить её иначе можно только в бою.
   *
   * ВТОРОЙ АДРЕС ПОЯВИЛСЯ ВМЕСТЕ С КАНДИДАТАМИ, и перечислен он поимённо, а не открыт префиксом
   * «всё, где две стороны»: карточку кандидата читают участники видимой заявки
   * (`serviceRequests.read`) и проверяющий (`officeEquipment.review`), которому связанная заявка
   * может быть не видна вовсе, — а вот очередь и правка того же предмета закрыты ОДНИМ правом, и
   * дизъюнкция на них означала бы, что очередь проверки открыли каждому заявителю.
   *
   ...
   */
  it('страж «одно из прав» оставлен ходам заявок и карточке кандидата', () => {
    const ALLOWED_DISJUNCTIVE = [
      '/api/v1/service-requests/',
      '/api/v1/office-equipment-candidates/:id',
    ];
    ...
    for (const route of disjunctive) {
      expect(
        ALLOWED_DISJUNCTIVE.some((prefix) => route.url.startsWith(prefix)),
        route.key,
      ).toBe(true);
