 ...
 *
 * Единица бланка — календарная неделя: семь строк «пн…вс» и недельные итоги. Но не только она:
 * с ADR 0142 срок режет и последнее число месяца — бланк ведут месяцем, и работа, лежащая в двух
 * отчётных периодах, не закрывает ни одного. Неделя 31.08–06.09 — это два листа: «31–31 августа»
 * и «1–6 сентября».
 *
 * Даты в тестах — 2026 года: 04.08 вторник, 31.08 понедельник.
 */
describe('срок заявки, разрезанный на листы', () => {
  ...
  it('неделя через границу месяца — два листа, по одному на месяц', () => {
    const weeks = esm2Periods('2026-08-27', '2026-09-09');
    expect(weeks).toEqual([
      { from: '2026-08-27', to: '2026-08-30' },
      // Ровно случай заказчика: неделя пн 31.08 — вс 06.09 закрывается двумя бланками.
  ...
  it('срок в одну переходную неделю — те самые «31–31 августа» и «1–6 сентября»', () => {
    expect(esm2Periods('2026-08-31', '2026-09-06')).toEqual([
      { from: '2026-08-31', to: '2026-08-31' },
      { from: '2026-09-01', to: '2026-09-06' },
    ]);
  });

  it('неделя внутри месяца режется по-прежнему только воскресеньем', () => {
    expect(esm2Periods('2026-08-10', '2026-08-16')).toEqual([
      { from: '2026-08-10', to: '2026-08-16' },
    ]);
  });

  it('високосный февраль и переход года считаются календарём, а не арифметикой', () => {
    // 29 февраля 2028 года существует, и февральский лист кончается им, а не 28-м.
    expect(esm2Periods('2028-02-28', '2028-03-01')).toEqual([
      { from: '2028-02-28', to: '2028-02-29' },
      { from: '2028-03-01', to: '2028-03-01' },
    ]);
    // Стык года — частный случай стыка месяцев: неделя 28.12–03.01 разрезается 31 декабря.
    expect(esm2Periods('2026-12-28', '2027-01-05')).toEqual([
      { from: '2026-12-28', to: '2026-12-31' },
      { from: '2027-01-01', to: '2027-01-03' },
...
describe('сверка недельных листов с заявкой', () => {
  ...
  it('срок сокращён — крайний лист правится, выпавшая неделя аннулируется', () => {
    ...
    // Первая неделя уже отработана — её не трогают; у второй сдвигается только конец, и правкой
    // это и делается (Р6): номер остаётся при листе, замены не выписывается. Третьей недели не
    // будет вовсе — её лист горит без замены: работы в эти дни не будет.
    expect(plan.cancel).toEqual(['c']);
    expect(plan.trim).toEqual([{ waybillId: 'b', to: '2026-08-12' }]);
...
describe('сверка недельных листов линейного заказа', () => {
  ...
  it('срок сокращён — крайняя неделя правится, выпавшая аннулируется без замены', () => {
    const existing = [
      sheet('a', '2026-08-10', '2026-08-16'),
    ...
    // Подрезанная неделя (`esm2RequestedPeriods`) доводится до нового конца правкой, а не парой
    // «аннулировать плюс выписать» (Р6): у линейного заказа состав листа сверкой не трогается
    // вовсе, значит и здесь меняется ровно последний день.
    expect(plan.cancel).toEqual(['b']);
    expect(plan.trim).toEqual([{ waybillId: 'a', to: '2026-08-12' }]);
    expect(plan.issue).toEqual([]);
  });
...
describe('сверка недельных листов при коррекции', () => {
  ...
  it('названный лист отработанной недели переоформляется, неназванный — нет', () => {
    ...
    expect(untouched).toEqual({ cancel: [], issue: [], trim: [] });
 ...
 * `docs/vehicle-request-actual-end-date-plan.md`). Заказ закрывают фактической датой, срок правят,
 * неделя уходит за границу — и всё это меняет у листа ровно один день: последний. Нового
 * содержания у такой правки нет, значит и нового номера ей не нужно.
 *
 * Правило узкое, и цена ошибки у него в обе стороны бумажная. Сократи сверка лист, который
 * сокращать нельзя, — заказ получит живой документ, разошедшийся с работой: аннулирования не было,
 * следа нет, номер по-прежнему числится за этими днями. Не сократи там, где можно, — сгорит бланк
 * строгой отчётности, которого хватило бы на всю неделю.
 *
 * Пятое условие — про хвост: отнятые дни не должны быть нужны никакой другой неделе набора. Им
 * закрытие отделяется от разреза, и держится оно на печатной графе (Р13, решение В8): напечатанный
 * экземпляр невырезанного периода не теряет, и правка там, где хвост забирает соседний лист, дала
 * бы два действующих бланка с одной и той же датой в графе «Период работы».
 *
 * Области сверки (`scope`) у недельного планировщика нет: он считает бумагу заявки целиком, и
 * четвёртое условие Р6 живёт в отрезковом плане (`esm2-plan.test.ts`).
 */
describe('сокращение недельного листа (Р6)', () => {
  const VEHICLE = 'vehicle-1';
  const DRIVER = 'driver-1';
  const sheet = (
    id: string,
  ...
  it('конец недели сдвинут внутрь — лист правится, и номер остаётся при нём', () => {
    const plan = auto({
      wanted: [{ from: '2026-08-10', to: '2026-08-12' }],
      existing: [sheet('a', '2026-08-10', '2026-08-16')],
    });

    // Замены не выписывается: неделю закрывает тот же документ, только короче.
    expect(plan).toEqual({ cancel: [], issue: [], trim: [{ waybillId: 'a', to: '2026-08-12' }] });
  });

  it('отнятые дни забирает соседняя неделя набора — правки нет, идёт перевыпуск', () => {
    /*
     * Тот же лист и то же «конец ближе», а исход другой: неделю 31.08–06.09 разрезала месячная
     * граница (ADR 0142), и её вторую половину ждёт свой документ. Голова недели правке подходит
     * по составу, началу и концу — но хвост 01–06.09 нужен соседу, и это пятое условие Р6.
     *
     * Причина в бумаге: печатный экземпляр остаётся с прежним, невырезанным периодом (Р13,
     * решение В8), и после правки на площадке лежали бы два действующих бланка, у обоих в графе
     * «Период работы» стояло бы 1 сентября. Аннулирование этого не допускает — старый бланк
     * уходит из оборота целиком.
     */
    const plan = auto({
      today: '2026-08-31',
      wanted: esm2Periods('2026-08-31', '2026-09-06'),
      existing: [sheet('a', '2026-08-31', '2026-09-06')],
    });
  ...
  it('период расширяется — по-прежнему перевыпуск: раздвинуть бланк нечем', () => {
    const plan = auto({
      wanted: [{ from: '2026-08-10', to: '2026-08-16' }],
      existing: [sheet('a', '2026-08-10', '2026-08-12')],
    });

    expect(plan).toEqual({
      cancel: ['a'],
      issue: [{ from: '2026-08-10', to: '2026-08-16' }],
      trim: [],
    });
  });

  it('сменилась машина или машинист — перевыпуск, даже когда период только укорачивается', () => {
    const otherVehicle = auto({
      wanted: [{ from: '2026-08-10', to: '2026-08-12' }],
      existing: [sheet('a', '2026-08-10', '2026-08-16', { vehicleId: 'vehicle-2' })],
    });
    const otherDriver = auto({
      wanted: [{ from: '2026-08-10', to: '2026-08-12' }],
      existing: [sheet('a', '2026-08-10', '2026-08-16', { driverPersonId: 'driver-2' })],
    });

    // Состав правкой периода не чинится: в бланке напечатаны госномер и фамилия.
    const reissued = {
      cancel: ['a'],
      issue: [{ from: '2026-08-10', to: '2026-08-12' }],
      trim: [],
    };
    expect(otherVehicle).toEqual(reissued);
    expect(otherDriver).toEqual(reissued);
  });

  it('сдвинулось начало — перевыпуск: другой первый день это другой документ', () => {
    const plan = auto({
      wanted: [{ from: '2026-08-11', to: '2026-08-16' }],
      existing: [sheet('a', '2026-08-10', '2026-08-16')],
    });

    expect(plan).toEqual({
      cancel: ['a'],
      issue: [{ from: '2026-08-11', to: '2026-08-16' }],
      trim: [],
    });
  });
  ...
  it('отработанный лист не сокращают: работа состоялась, и переписывать её нельзя', () => {
    ...
    // Правка периода — такое же вмешательство в прошлое, как аннулирование, и останавливает её та
    // же неприкосновенность: снимает её только названный коррекцией лист.
    expect(plan).toEqual({ cancel: [], issue: [], trim: [] });
    expect(
      auto({
        today: '2026-08-20',
    ...
    ).toEqual({ cancel: [], issue: [], trim: [{ waybillId: 'a', to: '2026-08-12' }] });
  });

  it('неделя, за которой уже стоит точный лист, соседа к себе не пускает', () => {
    const both = [sheet('a', '2026-08-10', '2026-08-16'), sheet('b', '2026-08-10', '2026-08-12')];
    const wanted = [{ from: '2026-08-10', to: '2026-08-12' }];

    // Порядок листов на ответ не влияет: сокращение решается вторым проходом, когда все точные
    // совпадения уже разобраны. Иначе у заявки оказалось бы два действующих документа 10–12.
    expect(auto({ wanted, existing: both })).toEqual({ cancel: ['a'], issue: [], trim: [] });
    expect(auto({ wanted, existing: [...both].reverse() })).toEqual({
      cancel: ['a'],
      issue: [],
      trim: [],
    });
