...
describe.skipIf(!DB_URL)('переключение признака линейности под работающими заказами', () => {
  ...
  it('замороженная заявка ведётся по-старому, а соседняя нового набора — по-новому', async () => {
    ...
    /*
     * ЗДЕСЬ СТОЯЛ СТОРОЖ СНЯТОГО ПРАВИЛА: `expect(day.statusCode).toBe(422)` под пояснением «дней
     * ей не планируют: у обычного заказа их не бывает вовсе». Держался он признаком типа —
     * `linearDaysBlocker` спрашивал `isLinear`, — и замороженный обычный режим закрывал дверь дней
     * вместе со всеми прочими нелинейными заказами.
     *
     * Решением заказчика ([ADR 0207](../../../docs/adr/0207-vehicle-request-day-batch.md) п. 1)
     * дверь дней линейностью больше не воротится: день в рейс кладут у любого заказа техники на
     * объект, и замёрзший режим на это не влияет — замораживается ведение заявки, а не право на
     * бумагу за день.
     *
     * РАЗНИЦА ДВУХ РЕЖИМОВ ПРИ ЭТОМ ОСТАЛАСЬ, И ОНА В БУМАГЕ — ровно то, ради чего снимок режима и
     * заведён (миграция 0137): замороженной обычной заявке портал выписал неделю ЭСМ-2 сам
     * (проверка выше), а заведённой после переключения линейной не выписал ни одной (проверка
     * ниже). Обе стоят в этом же тесте и несут теперь то, что утверждал снятый сторож.
     */
