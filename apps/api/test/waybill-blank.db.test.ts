...
describe.skipIf(!DB_URL)('пустой путевой лист по рейсу без заявок (живая схема)', () => {
  beforeAll(async () => {
     ...
     *
     * ГОСНОМЕР СПРАШИВАЕТСЯ ОТБОРОМ, А ПОРЯДОК ЗАДАЁТСЯ ЯВНО. Прежде здесь стоял `LIMIT 1` без
     * `ORDER BY`, то есть бралась ПРОИЗВОЛЬНАЯ подходящая строка, — и тест падал в полном прогоне
     * на `data.vehicle_reg_number` пустой строкой, переживая даже последовательный повтор.
     * Виноват не сид: в нём все 114 подходящих машин с номерами. Машину без номера заводит сосед
     * — `vehicle-type-maintenance-basis.db.test.ts` вставляет `('own', тип, 'active', note)`,
     * не указывая `registration_number` вовсе, а бланк 4-П у всякого нового типа стоит
     * умолчанием (ADR 0065). Такая строка подходит под отбор, и выбор её ничем не запрещал.
     *
     * Условие на номер — не обход соседа, а запись того, что тест утверждает строкой ниже: лист
     * печатает госномер, и машина без него для этой проверки не годится. Порядок по `created_at`
     * вдобавок предпочитает посевные строки — их кладут миграции до всякого теста, — поэтому под
     * ногами не окажется машина, которую сосед заведёт и уберёт посреди прогона.
     */
    const vehicles = await db.execute<{ id: string }>(sql`
      ...
      WHERE v.ownership = 'own' AND v.status = 'active' AND v.deleted_at IS NULL
        ...
        AND coalesce(v.registration_number, '') <> ''
      ORDER BY v.created_at, v.id
      LIMIT 1`);
    const vehicle = vehicles.rows[0];
    if (!vehicle)
      throw new Error('В базе нет своей машины с бланком 4-П и госномером: миграции не применены');
