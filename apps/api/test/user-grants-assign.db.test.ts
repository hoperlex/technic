...
describe.skipIf(!DB_URL)('выдача и отзыв полномочия: маршруты на живой схеме', () => {
  ...
  /**
   * Барьер 3 — запрещённая клетка матрицы осей. Гараж не сужается ничем, а у роли `shtab` есть
   * объектная ось: набор открыл бы ей машины и водителей всей компании.
   *
   * До ADR 0192 примером служил журнал путевых листов; у него область появилась (предикат
   * `waybillVisibilityWhere`), и клетка сменилась на `scoped`. Гараж занял его место в этом тесте
   * по той же причине, по какой листы его освободили: предиката у модуля нет.
   *
