  ...
  describe('бумаги и гонки (§6.11–§6.13)', () => {
    ...
    it('§6.12 переназначение и закрытие работ одновременно — ровно один успех', async () => {
      ...
      if (reassigned.statusCode === 200) {
         ...
         * Статус при этом остаётся «В работе»: назначение его не трогает ни при каком исходе
         * (ADR 0187; прежде эта же ручка возвращала заявку в «Новую»). Проверяются оба факта, и
         * первым — отсутствие «Решена»: разъедься поведение ручки с этим ожиданием, случай обязан
