...
describe('надстройка прикрепляется только к своим базовым ролям', () => {
  it('оператор оргтехники — площадке, отделу и офисной паре, остальным ролям нет', () => {
    ...
    //
    // МЕНЕДЖЕР И ДИСПЕТЧЕР ДОПИСАНЫ ЭТАПОМ Э7 плана профилей оргтехники (Р5, миграция B): ведение
    // модуля бывает и централизованным, и правило, по которому роль сюда попадает, записано в Р5 —
    // клетка оси не `forbidden` по каждому модулю состава, итог удовлетворяет `PERMISSION_REQUIRES`,
    // набор что-то роли даёт. Строка едет ВМЕСТЕ с `grant_roles` в базе: страж каталога строит
    // ожидание отсюда, и разъехавшись, надстройка и набор дали бы одному человеку разные права в
    // зависимости от источника доступа.
    expect([...ROLE_ADDON_BASE_ROLES[OFFICE]]).toEqual([
      'shtab',
      'site',
      'department',
      'manager',
    ...
    // ИТ-службе те же две роли не положены (Р5): её набор несёт сквозную область сам, а роль без
    // своей оси сделала бы глобальной ещё и работу исполнителя.
    expect([...ROLE_ADDON_BASE_ROLES.office_equipment_it_approver]).toEqual([
      'shtab',
      'site',
      'department',
    ]);
...
describe('матрица надстроек', () => {
  ...
  it('«Ведению» дано ровно ведение справочника и решения по заявке', () => {
     ...
     */
    /*
     * Одиннадцатым — проверка сообщений об отсутствующей технике (ADR 0165; этап Э6 плана предмета
     * заявки, миграция 0294), и стоит она сразу за перемещением по той же причине, по какой там
     * стоит перемещение: это работа со справочником, и в надстройке она потому, что ЗАКАНЧИВАЕТСЯ
     * записью в парк. `PERMISSION_REQUIRES` требует для неё `officeEquipment.write` — он двумя
     * строками выше, и требование выполняется составом набора, а не базовой ролью.
     *
     * Ровно этим право и отличается от соседнего `officeEquipment.propose`, которого здесь нет и
     * не будет: сообщает заявитель (право даёт роль-заказчик и набор «Оргтехника: заявитель»), а
     * проверяет — «Ведение». Разведены они не для симметрии: проверка выдана тому единственному
     * набору модуля, у которого есть ведение справочника, — ИТ-служба его не получает намеренно,
     * иначе в парк открылся бы второй, необъявленный вход.
     */
    /*
     * Двенадцатым — раскладка объёма работ по графам (`serviceRequests.estimateRewrite`, план
     * `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р2; ответ В9 заказчика от
     * 09.09.2026; миграция 0298), и стоит она сразу за сменой статуса, потому что это работа над
     * ЗАЯВКОЙ, а не над справочником.
     *
     * СЛЕДУЮЩИЙ КЕЙС ЭТОГО ФАЙЛА НЕ ОТМЕНЁН ЭТОЙ СТРОКОЙ, и путать их нельзя. Права сметы
     * (`serviceRequests.estimate`) надстройка по-прежнему не даёт и не даст: оно означает «я АВТОР
     * объёма работ», то есть сторону исполнителя, и рядом с `approveEstimate` двумя строками выше
     * превратило бы согласование в подпись под собственной работой. Раскладка устроена иначе —
     * автором сметы держателя она не делает и открывает ровно одну дверь: перенос присланного
     * подрядчиком перечня из свободной записи в графы. Там, где ревизия уже согласована, дверь эта
     * поднимает ревизию и требует новой подписи — то есть переиздаёт документ, а не правит
     * подписанный.
     *
     * ДЕРЖАТЕЛЬ У ПРАВА РОВНО ОДИН — эта надстройка. Ни ИТ-служба, ни набор исполнителя, ни тип
     * контрагента `service` его не получают: те объём работ пишут, а не переписывают чужой.
     */
