 ...
 * закрывается раньше, чем база исчезает (а `DROP DATABASE ... WITH (FORCE)` страхует и от этого).
 */
...
export function useReadModeDatabase(tag: string): ReadModeDatabase {
  ...
  if (enabled) {
    ...
    afterAll(async () => {
      ...
      await adminClient.connect();
      try {
        // `FORCE` — чтобы забытое соединение соседа не оставляло базу на кластере навсегда: их там
        // и так копится десятками.
