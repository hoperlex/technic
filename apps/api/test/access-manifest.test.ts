...
describe('манифест доступа маршрутов', () => {
   ...
   * одной ручки.
   *
   * ДВЕНАДЦАТАЯ — ПАКЕТНАЯ РУЧКА (план `docs/office-equipment-bulk-actions-plan.md`, §6.4), и она
   * единственная, где дизъюнкция шире одной операции: за одним маршрутом стоят девять команд с
   * разными правами, и страж отвечает лишь «бывает ли у субъекта хоть одна». `execute` в ней есть
   * — массовый «Принять в работу» держит именно он, — поэтому общее правило перебора выполняется
   * без исключения. `estimate` в ней нет намеренно (Р5), и это единственное место, где пакет
   * строже одиночной ручки.
   */
