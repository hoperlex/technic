...
describe('манифест доступа маршрутов', () => {
   ...
   * одиннадцатая такая строка должна попасть в ревью, а не приехать молча вместе с правкой
   * соседнего маршрута.
   *
   * Все десять — ходы исполнителя заявки на обслуживание (план переработки цикла §7.3), и во всех
   * десяти вторым членом стоит `serviceRequests.execute`: ради поимённого исполнителя дизъюнкция и
   ...
   * не теоретическая: `effectConditionalPermissions` остался от замороженного склада автозапчастей
   * (план `docs/auto-part-receipts-plan.md`, Р2, Р3). Три его строки стояли у ручек обслуживания и
   * уехали вместе со складским расходом — вид законно стоит незанятым, а описание приёма «право по
   * факту, а не по телу» осталось для следующей такой ручки.
   */
  it('видов условия восемь, и все они перечислены реестром', () => {
    ...
    // И третья: реестр не копилка. Незанятым стоит ровно один вид — `effectConditionalPermissions`,
    // оставшийся от замороженного склада (план чеков, Р2); всякий другой — либо забытый, либо
    // больше не нужный.
