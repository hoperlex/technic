...
/*
 * Настройки сверки с нормой: ручка читает их перед сборкой книги (docs/fuel-norms-plan.md, §3.1),
 * и здесь они подменены значением — как и расчёт. База в этом тесте отдаёт только строки журнала,
 * и живой запрос за одиночкой настроек ушёл бы в неё «неожиданным запросом».
 */
vi.mock('../src/services/fuel-norms', () => ({
  loadFuelNormSeason: () =>
    Promise.resolve({ winterFromMd: '11-01', winterToMd: '03-31', tolerancePercent: 5 }),
}));
...
function statsRow(over: Partial<VehicleReadingStatsRow> = {}): VehicleReadingStatsRow {
  return {
    ...
    gaps: 2,
    // Сверка с нормой: у фикстуры её нет — машина без норм, и три колонки книги молчат прочерком.
    fuelSpentLiters: 0,
    fuelNormLiters: 0,
    verifiedShifts: 0,
    shiftsWithFuel: 0,
...
describe('выгрузки показаний: один сборщик, вариант параметром', () => {
  it('сводка по парку — тот же лист, что выгружает старая ручка', async () => {
    ...
    expect(book[0]!.rows[0]).toEqual([
      ...
      'Разрывов ряда',
      // Сверка с нормой повторяет экран (docs/fuel-norms-plan.md, §4.3): расход сверяемых смен,
      // норма по ним же, отклонение числом и охват.
      'Расход, л',
      'Норма, л',
      'Отклонение, %',
      'Сверено смен',
    ]);
    // Нормы у машины нет: сверять нечего, и четыре колонки сверки молчат прочерком.
  ...
  it('помесячно по парку — лист на счётчик, колонка на месяц периода', async () => {
    ...
    expect(book.map((sheet) => sheet.name)).toEqual([
      ...
      'Наработка, моточасы',
      'Заправлено топлива, л',
      // Сверка ложится в матрицу двумя листами: расход и норма аддитивны, а отклонение между ними
      // не складывается и листом «машина × месяц» означало бы неправду (docs/fuel-norms-plan.md).
      'Расход по сверке, л',
      'Норма расхода, л',
    ]);
...
/**
 * Производных «на сто километров» книги по-прежнему не заводят — и это уже другое утверждение, чем
 * прежде. Запрет считать расход снят вместе с приходом норм (`docs/fuel-norms-plan.md`, §1.1):
 * расход в книгах есть, но только как половина сверки с нормой, рядом с ней и охватом. А вот
 * число, поделённое на пробег, портал не печатает до сих пор: в чужой книге с формулами оно зажило
 * бы своей жизнью, и спорить о нём пришлось бы уже вне портала.
 *
 * Проверяются заголовки **всех** вариантов сразу — новая производная приедет в этот тест раньше,
 * чем в чужую сводную таблицу.
 */
describe('ни одной производной колонки ни в одной книге', () => {
  it('заголовки всех вариантов не знают «на 100 км» и прочих делений', async () => {
    const queries = [
      'kind=fleetSummary&from=2026-01-01&to=2026-03-31',
      'kind=fleetMonths&from=2026-01-01&to=2026-03-31',
      `kind=vehicleJournal&from=2026-01-01&to=2026-01-31&vehicleId=${VEHICLE_ID}`,
    ...
    for (const query of queries) {
      const book = await download(query);
      const headers = book.map((sheet) => (sheet.rows[0] ?? []).join(' ')).join(' | ');
      expect(headers, query).not.toMatch(/на 100|\/100|л\/км|на моточас/iu);
    }
  });
});
