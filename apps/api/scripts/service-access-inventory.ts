...
const CHECKS = [
  ...
  // `assertRepairKind` снято: функции с таким именем в `src` больше нет — её роль («у этого вида
  // заявки объёма работ не бывает», 422) исполняет `assertEstimateApplies`. Мёртвое имя в закрытом
  // перечне хитов не даёт и потому не падает, а следующему читателю врёт: пустая клетка выглядит
  // как «проверки нет», хотя её просто ищут не по тому имени.
  'lockRequest',
  'executorAssignment',
  ...
  'assertInternalToken',
  /*
   * Пять имён волны «свободный объём работ и область исполнителя» (план
   * `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р3, Р5, Р7, Р9). Каждое
   * добавлено по тому же правилу, что и предыдущие пять: без него строка таблицы читается как
   * «ручка этим не закрыта», а это неправда.
   *
   * `assertServiceRequestActionable` — ОБЛАСТЬ ДЕЙСТВИЙ (Р7), вторая из двух областей модуля. Живёт
   * она внутри `requireEditable`, то есть на общем входе всех изменяющих ручек, и до этой строки
   * снимок инвентаря молчал о ней вовсе: у двух десятков ручек в столбце стояла одна лишь область
   * ЧТЕНИЯ (`assertScope`), и матрица отношений Р7 из снимка не читалась ни в одной клетке. Ровно
   * ради этой проверки этап Э8 снимает инвентарь второй раз.
   *
   * `assertServiceRequestScope` — РУБЕЖ ЗАВЕДЕНИЯ: у `POST /` строки заявки ещё нет, и область
   * спрашивается по её ПРЕДМЕТУ. Клетка этой ручки была пуста с самого Э0 — не потому, что заведение
   * ничем не закрыто, а потому, что имени не было в перечне. Пустая клетка обязана означать пустую
   * клетку в коде.
   *
   * `canPickAnyServiceSubject` — ОСНОВАНИЕ того же рубежа после Р9: «выбирает предмет заявки по всей
   * компании». Названо отдельно от рубежа намеренно: выпуск B убирает `serviceRequests` из карты
   * сквозной области, и снимок обязан показывать, на какую именно карту заведение опирается, — иначе
   * уборка строки выглядела бы в инвентаре как отсутствие изменений.
   *
   * `markPastExecutors` и `markAllPastExecutors` — ЗАПИСЬ СЛЕДА СНЯТИЯ (Р5). Это не проверка, и
   * называть их так нельзя; в перечне они по тому же основанию, что `lockRequest` и
   * `executorAssignment`, которые проверками тоже не являются: столбец отвечает на вопрос «из чего
   * сложена модель доступа этой ручки», а строка «бывший исполнитель» матрицы Р7 держится ровно на
   * этой записи. Ручка, снимающая исполнителя без следа, — находка этапа, и увидеть её можно только
   * здесь: четыре ветки `DELETE` живут в трёх разных обработчиках.
   *
   * ЧЕГО СЮДА НЕ ДОБАВЛЕНО И ПОЧЕМУ. `executorFactsOf`, `namedExecutorRowExists` и
   * `pastExecutorRowExists` — сбор признаков внутри `assertScope`: они приезжают в КАЖДУЮ строку,
   * где есть область, и потому не различают ничего. Имя, стоящее у всех ручек сразу, из инвентаря
   * не сообщает ничего, кроме собственной длины.
   */
  'assertServiceRequestActionable',
  'assertServiceRequestScope',
  'canPickAnyServiceSubject',
  'markPastExecutors',
  'markAllPastExecutors',
] as const;
...
/**
 * Где начинается ТЕЛО функции — первая `{` после списка параметров, **пропуская возвращаемый тип**.
 *
 * ПОЧЕМУ ЭТОГО МАЛО — `indexOf('{', …)`. Возвращаемый тип сплошь и рядом содержит объектный литерал:
 * `): Promise<{ mail: …; mailFailed: boolean }> {`. Наивный поиск первой скобки останавливался на
 * литерале ТИПА, и «телом» помощника оказывалось перечисление полей — то есть текст, в котором нет
 * ни одного вызова. Помощник при этом молчал не с ошибкой, а с пустым списком, и в снимке это
 * выглядело как «ручка ничем не закрыта».
 *
 * ЧТО ЭТО ПРЯТАЛО НА САМОМ ДЕЛЕ (находка при расширении перечня волной свободного объёма работ):
 * `applyTransition` объявлен ровно так — и вместе с ним из инвентаря выпадала ПЕРВАЯ из четырёх
 * веток следа снятия (сброс состава при отмене, `markAllPastExecutors`). Ни одна ручка её не
 * показывала, хотя проходит через неё и отмена, и возврат отменённой, и пакетная операция. Тем же
 * приёмом скрывался `canPickAnyServiceSubject` внутри `resolveEmptySubjectCustomer` — там его спасло
 * лишь то, что два соседних разбора предмета зовут его же.
 *
 * ЧЕМ ОТЛИЧАЕТСЯ ТИПОВАЯ `{` ОТ ТЕЛЕСНОЙ. Слева от типовой всегда стоит знак, после которого тип
 * продолжается: `:` `|` `&` `<` `,` `(` `=`. Слева от телесной — конец типа либо конец списка
 * параметров: `>` `)` `]` или буква имени типа. Скобочный сканер уже умеет пропускать балансно, так
 * что типовой литерал проходится целиком и разбор идёт дальше. Полноценный парсер сюда по-прежнему
 * не берётся — довод тот же, что у всего сканера выше.
 */
const TYPE_CONTINUES = new Set([':', '|', '&', '<', ',', '(', '=']);

function bodyBraceAfter(code: string, paramsClose: number): number {
  let i = paramsClose + 1;
  let prev = ')';
  while (i < code.length) {
    const c = code[i]!;
    ...
    if (c === '{') {
      if (!TYPE_CONTINUES.has(prev)) return i;
      i = matchBracket(code, i) + 1;
      prev = '}';
      continue;
    }
    // Точка с запятой между объявлением без тела (перегрузка, `declare`) и следующим кодом: тела у
    // такого объявления нет вовсе, и приписывать ему соседнее было бы хуже, чем промолчать.
    if (c === ';') return -1;
    prev = c;
    i += 1;
  }
...
function helperChecks(code: string): Map<string, AccessCheck[]> {
  ...
  for (const m of code.matchAll(/\b(?:export\s+)?(?:async\s+)?function\s+(\w+)\s*\(/g)) {
    ...
    const brace = bodyBraceAfter(code, params);
    if (brace < 0) continue;
