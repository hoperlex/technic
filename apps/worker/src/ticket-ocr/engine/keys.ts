 ...
 */
/**
 * Заглушка модели варианта A (Р7): «слаг выбирает оператор прокси». Значение живёт здесь, рядом с
 * ключами, а не в конфиге, потому что решает оно именно про ключи: за этой строкой в разное время
 * стоит разная модель, поэтому попытка с ней не кэшируется и не сравнивается с прошлой.
 */
export const PROXY_CHOOSES_MODEL = 'proxy';
