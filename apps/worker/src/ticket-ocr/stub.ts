 ...
 *
 * Пустой строки среди веток нет намеренно: «написания нет» выражается только `null` (Р4,
 * ADR 0166 п. 1), и контрактная схема, которой заглушка проверяет собственный ответ ниже, пустую
 * строку теперь отвергает — заглушка упала бы на ровном месте.
 *
