...
if (partial) {
  say(`точечный прогон по фильтру: ${filters.join(', ')} — это часть набора, а не набор.`);
  say(
...
/**
 * Миграции накатываются ДО тестов — и полному набору тоже, а следом считается статистика
 * (`ANALYZE`, см. шапку `quality-db.ts`).
 *
 * ТОЧЕЧНОМУ ПРОГОНУ это нужно было всегда: под фильтр может не попасть ни один тест api, а именно
 * они наполняют базу в полном наборе, и тест воркера падал бы на отсутствующей таблице.
 *
 * ПОЛНОМУ НАБОРУ шаг добавлен ради статистики. Прежде здесь стояла оговорка «полный набор этого
 * шага не получает намеренно — иначе тесты, которые проверяют поведение на непромигрированной
 * базе, проверяли бы не то». Оговорка проверена по файлам: таких тестов, работающих на ОБЩЕЙ базе
 * прогона, нет ни одного. Все три, которым нужна база без схемы, заводят свою:
 * `legacy-compat.db.test.ts` (копия через `CREATE DATABASE`), `role-migration-pending.db.test.ts`
 * (`SCRATCH_DB`) и `stock-frozen.db.test.ts`. Накат идемпотентен (`applyMigrations` сверяется с
 * журналом), поэтому первый же тестовый файл, как и раньше, зовёт его сам и не находит работы.
 *
 * Без этого шага общую базу не анализирует никто: миграции ей накатывает первый добежавший тест, а
 * посчитать статистику ему нечем, и планировщик до конца прогона работает по оценкам «по
 * умолчанию». Чем это кончалось — в шапке `quality-db.ts`.
 */
if (!manageDb('migrate')) {
  say('миграции на свежую базу не накатились — прогонять было бы нечего.');
  dropOnce();
  rmSync(reportDir, { recursive: true, force: true });
  process.exit(1);
}
