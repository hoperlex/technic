...
export interface RunState {
  ...
  /**
   * Коммиты, ради которых прогон затеян: очередь, накопленная хуком.
   *
   * Забирается один раз, в начале: пока прогон идёт, в очередь капают новые коммиты, и разбирать
   * их этим же прогоном значило бы гнаться за движущимся деревом — они дождутся следующего.
   */
