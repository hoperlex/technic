...
if [ "$DO_MAINTENANCE" -eq 1 ]; then
  ...
  портал закрыт — безопасная сторона. Логи: ${COMPOSE[*]} logs --tail=50 technic-api"
    ...
    [ "$MAINT_C_DOMAIN" = "503" ] \
      || warn "гейт не отбивает доменную ручку — портал ОТКРЫТ при закрытом на бумаге; проверьте prod.env и логи api"
    [ "$MAINT_C_REFRESH" != "503" ] \
      || warn "обновление сессии отбивается 503 — режим выкинет всех на форму входа вместо тихого продления (план Р4)"
    [ "$MAINT_C_WEB" = "200" ] \
      || warn "веб не отдаёт /maintenance.json ($MAINT_C_WEB) — вкладка не увидит объявления, когда api остановят; проверьте маунт каталога и локацию в spa.conf"
    echo
    maint_print_state
  ...
  sudo rm -f $MAINT_FILE" ;;
    ...
    [ "$MAINT_C_REFRESH" != "503" ] \
      || warn "обновление сессии всё ещё отбивается 503 — люди уйдут на форму входа; разберитесь до конца окна"
    ...
    [ "$MAINT_C_WEB" = "404" ] \
      || warn "веб всё ещё отдаёт /maintenance.json ($MAINT_C_WEB) — вкладки останутся с заглушкой на работающем портале"
    echo
    maint_print_state
