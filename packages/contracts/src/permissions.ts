...
export const PERMISSIONS = [
   ...
   * моточасы правят из строки техники, и делает это `vehicleReadings.write`. Теперь склад
   * автозапчастей (план автозапчастей, Р10): позиции ведёт `autoParts.manage`, остаток двигает
   * `autoParts.stock`. Правило раздела при этом не изменилось — каждое действие открывает своё
   ...
   * список запчастей и карточка позиции, потому что ответить «есть ли на складе фильтр» должен
   * всякий, кому виден гараж.
   */
  'garage.read',

  /**
   * Склад автозапчастей (план автозапчастей, Р10): номенклатура позиций, их остаток и разметка «к
   * чему подходит».
   *
   * Права два, и разведены они по той же границе, что у расходников оргтехники: завести позицию и
   * поправить остаток — разные работы. `autoParts.manage` — заведение, правка, гашение и удаление
   * позиции вместе с её применимостью; `autoParts.stock` — движение склада: ручная корректировка,
   * ненулевой начальный остаток при заведении и ненулевая разница строк акта обслуживания.
   *
   * Последнее слагаемое — то, ради чего пара и разведена (Р19). `vehicleMaintenance.write` есть у
   * пяти ролей, включая менеджера и диспетчера; оставь расход из акта под ним одним — и диспетчер,
   * которому остаток править нельзя, списывал бы строкой акта любое количество. Поэтому реквизиты
   * акта (дата, одометр, номер, сканы) правит всякий держатель права на акт, а движение склада —
   * только держатель `autoParts.stock`; считается оно по фактической разнице строк, а не по
   * наличию поля в теле запроса, иначе PATCH с теми же самыми строками отобрал бы у диспетчера
   * правку номера документа.
   *
   * Чтения среди них нет, и своего чтения у модуля не будет: список и карточка позиции
   * открываются широким `garage.read` — ответить «есть ли на складе фильтр» должен всякий, кому
   * виден гараж, а ни персональных данных, ни денег этот список не раскрывает. Зато оба права
   * объявлены требующими `garage.read` (`PERMISSION_REQUIRES` в `grants.ts`): право править то,
...
/**
 * Склад автозапчастей (план автозапчастей, Р10). Ведение номенклатуры и движение остатка идут парой
 * у обеих ролей службы главного механика: заказчик назвал держателем службу целиком, и роли,
 * которой позиции заводить можно, а остаток править нельзя, в замысле нет. Разведены права между
 * собой не ради ролей, а ради акта обслуживания — `autoParts.stock` спрашивается сверх права на
 * акт, когда строки расхода двигают склад (Р19).
 */
const AUTO_PARTS_PERMISSIONS = ['autoParts.manage', 'autoParts.stock'] as const;
...
export const ROLE_PERMISSIONS: Record<Role, readonly Permission[]> = {
   ...
   *
   * Механик читает четыре списка и ведёт два — техобслуживание и склад автозапчастей. Справочники
   ...
   * справочника (Р14в). Второе — склад автозапчастей (план автозапчастей, Р10): держателем
   * заказчик назвал службу, а не человека, поэтому оба права стоят здесь ролью, и набора
   * полномочий у них нет.
   */
  mechanic: [
    'directories.read',
    'garage.read',
    ...
    'drivers.read',
    ...VEHICLE_MAINTENANCE_PERMISSIONS,
    // Склад автозапчастей (план автозапчастей, Р10) — второе действие роли, и приходит оно ролью, а
    // не набором полномочий: заказчик назвал механиков, а не человека поимённо. Менеджеру и
    // диспетчеру этих прав нет намеренно — акт обслуживания они правят, а склад двигать не должны
    // (Р19).
  ...
  chief_mechanic: [
    ...
    ...VEHICLE_MAINTENANCE_PERMISSIONS,
    // Склад автозапчастей — по той же причине и тем же составом (план автозапчастей, Р10).
    ...AUTO_PARTS_PERMISSIONS,
  ],
};
...
export const ROLE_ADDON_PERMISSIONS: Record<RoleAddon, readonly Permission[]> = {
  ...
  office_equipment_it_approver: [
    'serviceRequests.approveIt',
    /*
     * Остальные четыре — работа ИТ-службы **исполнителем** (плана переработки §7.2): она же и
     * инхаус-ремонт, а не только виза. `execute` открывает ходы исполнителя, но **только на
     * заявках, где она назначена поимённо** — это дизъюнкция коридора, а не право само по себе;
     * `assign` нужно, чтобы взять чужую заявку, назначив себя; `hold` — чтобы придержать её, пока
     * ждут запчасть; `files` — чтобы подшить бумагу.
     *
