...
export const PERMISSIONS = [
   ...
   *
   * Чтения среди них нет, и третьего права не будет: список и карточка позиции открываются широким
   * `garage.read` — ответить «есть ли на складе фильтр» должен всякий, кому виден гараж, а ни
   * персональных данных, ни денег этот список не раскрывает. Зато оба права объявлены требующими
   * `garage.read` (`PERMISSION_REQUIRES` в `grants.ts`): право править то, чего не видишь, — дыра,
