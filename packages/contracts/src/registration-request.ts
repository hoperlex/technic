...
/**
 * Умолчания формы рассмотрения заявки: какую роль подставить, увидев пожелание.
 *
 * **Это таблица подстановки, а не отображение «пожелание = роль»** (§11.1 плана реструктуризации
 * прав). Разница не словесная: решает администратор, а таблица лишь заполняет поле формы до того,
 * как он его тронул. Отсюда и то, что `null` — рабочее значение, а не дыра: «другое» роли не
 * подразумевает вовсе, и форма просто открывается с пустым выбором. Никакой код по этой таблице
 * прав не выдаёт и учёток не активирует — активацией остаётся `PATCH` карточки с явно выбранной
 * ролью.
 *
 ...
 * оба законны: перенацелить умолчание на роль, в которую слили прежнюю, либо поставить `null` —
 * «умолчания нет, администратор выбирает сам». Чего делать нельзя — оставить имя упразднённой
 * роли: форма подставила бы значение, которого в перечне нет, портал показал бы пустой выбор как
 * заполненный, а сервер отверг бы сохранение схемой. Проверку «ни одно умолчание не указывает на
 * несуществующую роль» держит `registration-request-contracts.test.ts` — на случай, если тип
 * когда-нибудь ослабят.
 *
 * Таблица закрытая, наружу смотрит `defaultRoleForRequest` — тем же приёмом, что
 * `requestExpectsCorporateEmail` ниже: читателю нужен ответ на своё пожелание, а не сам словарь,
 * и через функцию к умолчанию можно будет добавить набор полномочий, не переписывая вызовы (§11.1
 * обещает форме «роль плюс системные наборы»).
 *
 ...
 */
const registrationRoleRequestDefaultRole: Record<RegistrationRoleRequest, Role | null> = {
  dispatcher: 'dispatcher',
  /*
   * Три площадочных пожелания перенацелены на `site` шагом prepare этапа 8 (ADR 0113) — тем же
   * релизом, которым упраздняемые роли перестали назначаться вовсе. Порядок здесь обязателен:
   * оставь мы прежние умолчания, форма рассмотрения открывалась бы с ролью, которую сервер
   * отвергает, — то есть заявку нельзя было бы принять, не разобравшись сперва в реформе.
   *
   * ADR 0112 §8 держал умолчания на месте по причине, которая на этом шаге исчерпана: «наборы
   * подставлять форме пока нечем». Их и сейчас нечем — таблица «пожелание → роль плюс наборы» из
   * §11.1 не заведена, — но выбор стоит иначе: либо пустая роль, либо верная роль без наборов.
   * Верная роль лучше: под выбором `site` в карточке стоит пояснение, что заказ техники и виза
   * приезжают полномочиями (ADR 0112, решение 7), а само пожелание видно администратору строкой
   * «При регистрации указал: …» — по нему он и решает, выдать «Заказ техники» или ещё и визу.
   */
  rukstroy: 'site',
  site_staff: 'site',
  commandant: 'site',
...
/**
 * Роль, которой форма рассмотрения заполняется по умолчанию; `null` — умолчания нет, выбор за
 * администратором. Пожелания у заявки может не быть вовсе (учётку завёл администратор сам) — это
 * тот же ответ `null`, а не отдельный случай для вызывающего.
 */
export function defaultRoleForRequest(
  request: RegistrationRoleRequest | null | undefined,
): Role | null {
  return request ? registrationRoleRequestDefaultRole[request] : null;
}
...
export const registrationRequestDetail: Record<RegistrationRoleRequest, RegistrationRequestDetail> =
  {
    ...
    // (`defaultRoleForRequest('other') === null`). Без объяснения своими словами такая заявка не
