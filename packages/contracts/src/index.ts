...
/* Чеки на автозапчасти (`auto-part-receipts.ts`) — сразу за складом, который они заменяют (план
 * `docs/auto-part-receipts-plan.md`, Р1). Оба файла стоят рядом ровно на время перехода: склад
 * заморожен и уезжает выпуском 2 (Р22), а до тех пор соседство честнее удаления — старая вкладка
 * ещё читается, и её контракт обязан остаться живым. Общего у файлов нет ничего: чек не ссылается
 * ни на позицию склада, ни на её потолок количества — своя граница объявлена своим числом. */
