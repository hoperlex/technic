...
export type RouteJoinDay =
  ...
  /**
   * Заказ техники на объект: срок работ (`dateTo` пуст у однодневного) и дни, уже стоящие в его
   * рейсах. Ими правило и отвечает на два вопроса дня: внутри ли он срока и не распланирован ли уже.
   *
   * Имя `linear` осталось от ADR 0100, где дни завела линейная техника; сегодня такой ответ даёт
   * любой заказ на объект. Переименование тронуло бы все вызывающие и ничего бы не объяснило —
   * объясняет этот комментарий.
   */
...
/**
 * Отказ положить заказ техники на объект в рейс со стороны рейса (ADR 0100 §8). Дверь у дня одна —
 * карточка заявки: день знает свой срок и свою заявку, а из карточки маршрута ту же заявку
 * пришлось бы разыскивать по объекту среди всех работающих.
 *
 * Про линейность текст молчит намеренно: дни ведутся у любого заказа на объект (см. шапку
 * `vehicle-request-days.ts`), и обещание «это только про линейный» отправило бы половину
 * диспетчеров искать несуществующий запрет вместо той самой карточки.
 */
export const LINEAR_DAY_DOOR_MESSAGE =
  'Дни заказа техники на объект планируют из карточки заявки: рейс не знает, какой день срока в него ставят';
 ...
 * не заявкой целиком, а одним днём срока. День у грузоперевозки сверяется на равенство (лист
 * печатает задание на день), у заказа на объект — на попадание в срок и на то, что этот день ещё
 * не занят другим рейсом.
 */
export function canJoinRoute(
  request: {
    requestType: VehicleRequestType;
    /**
     * Линейный ли **заказанный** тип (ADR 0100 §1). Это правило его НЕ ЧИТАЕТ: в рейс попадает
     * день любого заказа на объект, и выбирать по типу здесь больше нечего. Поле осталось в
     * аргументе, потому что его передают все вызывающие, и убирать его значило бы править их
     * заодно; смысл признака живёт там, где по нему и правда расходится поведение, — в режиме
     * ведения заявки и в выписке недельных ЭСМ-2.
     */
    isLinear: boolean;
    status: RequestStatus;
    deletedAt: string | null;
    /** Чем заявка отвечает на вопрос о дне: точкой подачи либо сроком заказа на объект. */
    day: RouteJoinDay;
  ...
  // площадок. День заказа на объект в перегон не кладут по той же причине (ADR 0100 §9): перегон
  // едет по своему основанию, а день работ — это обычный рейс дня.
  if (isRelocationPurpose(route.purpose)) {
    return {
      ok: false,
      reason:
        request.day.kind === 'linear'
          ? 'Это перегон техники — день работ в него не кладут: перегон едет по своей заявке-основанию, а день ставят в обычный рейс'
          : 'Это перегон техники — заявки в него не кладут, он едет по своей одной',
    };
  }
  /*
   * Кто вообще ходит в рейс. Грузоперевозка ходит целиком: рейс и есть её работа. Заказ техники на
   * объект — только днём срока, и дверь к этому дню одна: карточка заявки (ADR 0100 §8).
   *
   * Текст здесь один. Прежде их было два — по признаку линейности, — но после того, как дни
   * открылись любому заказу на объект, обе половины стали ложью: «в рейс ходят только
   * грузоперевозки» неправда (ходит и день такого заказа), а «это про линейный» неправда тоже
   * (тип больше ничего не решает). Одна ложь вместо двух не лучше — остался ответ по существу:
   * дверь не эта.
   */
  if (request.day.kind === 'trip' && request.requestType !== 'freight_transport') {
    return { ok: false, reason: LINEAR_DAY_DOOR_MESSAGE };
  }
 ...
 * тот стоит в недельном листе ЭСМ-2 заказа техники на объект, этот — за рулём рейса. Прежде здесь
 * стояло «где выписывается ЭСМ-2, рейса не бывает вовсе, и наоборот» — с приходом дневных 4-П на
 * весь период это стало неправдой: у одного заказа за один день бывают обе бумаги сразу — недельный
 * ЭСМ-2 и дневной лист его рейса (решение 1 ADR 0041 в редакции ADR 0207). Графы от этого не
 * сливаются, а расходятся ещё заметнее: неделю подписывает машинист заявки, дневной рейс везёт
 * водитель этого дня, и разными людьми они бывают законно. Расхождение портал называет вслух —
 * молча подставить одного вместо другого нельзя ни в ту, ни в другую сторону.
 */
