...
import { grantStatementListSchema, type GrantOrigin } from './grants';
...
export const createUserSchema = z
  .object({
    ...
    /**
     * Полномочия, выдаваемые вместе с учёткой (ADR 0106; план «полномочия назначаются в окне
     * учётки», §4). Высказывание о каждом наборе, который форма показала, — `{ id, version,
     * selected }`, — а не список того, что должно остаться: снимаемого набора в списке оставшихся
     * нет по построению, и ни его версию, ни сам факт показа передать было бы нечем (Р3).
     *
     * Отсутствие поля здесь означает «пусто»: новой учётке нечего сохранять (§4.1). Умолчания `[]`
     * поэтому нет — молчание и пустое высказывание при создании ведут к одному итогу, а различать
     * их приходится ровно одной проверке: `grants` вместе с `addons` (ниже).
     */
    grants: grantStatementListSchema.optional(),
    /**
  ...
  /*
   * `grants` и `addons` правят одно множество: пока идут шаги 1a–1e ADR 0106, надстройка — это тот
   * же набор (`SYSTEM_GRANT_CODES`), выданный через `user_grants`. Тело, где сказано и то и другое,
   * задаёт два итога сразу, и вопрос «какой из них главный» ответа не имеет — поэтому 400, а не
   * правило старшинства (§4.1).
   *
   * Конфликтом считается **непустой** список надстроек, и это не послабление, а следствие
   * умолчания: `addons` здесь достраивается до `[]`, то есть отсутствие поля неотличимо от
   * присланного пустого списка. Пустой же список о наборах ничего не утверждает — «надстроек нет» и
   * «надстройки не трогаем» дают у новой учётки один итог. У правки умолчания нет, и там проверка
   * строже (см. `updateUserSchema`).
   */
  .superRefine((v, ctx) => {
    if (!v.grants || v.addons.length === 0) return;
    ctx.addIssue({
      code: 'custom',
      message: 'Полномочия и надстройки задают один и тот же доступ — оставьте что-то одно',
      path: ['grants'],
    });
  });
...
export const updateUserSchema = z
  .object({
    ...
    /**
     * Высказывание о полномочиях (план §4): строка на каждый набор, о котором операция что-то
     * решает, — управляемое назначение, назначение, чьё действие переключает смена роли, и
     * отмеченный, но ещё не выданный набор (§4.2).
     *
     * Отсутствие поля — «назначений не касаемся», как у объектов и отделов. Но законен этот ход не
     * всегда: смена роли переключает гейт совместимости и зажигает либо гасит состав, которого
     * администратор не отмечал и не видел, — такое молчание сервер отклоняет (§4.2). Схеме это
     * недоступно: она не знает ни прежней роли учётки, ни выданных ей наборов.
     *
     * Пустой массив — законное высказывание «решать не о чем»; от отсутствия поля он отличается
     * тем, что сервер сверит его с полнотой: есть управляемые или переключаемые назначения — 400.
     */
    grants: grantStatementListSchema.optional(),
    counterpartyId: uuidSchema.nullish(),
    /**
  ...
  /*
   * Оба поля правят одно множество назначений (см. `createUserSchema`), и здесь проверка ровно та,
   * какой ей положено быть: у правки умолчаний нет, поэтому названы оба поля — значит, запрос задал
   * два итога сразу, включая `addons: []` («снять все надстройки») рядом с высказыванием о наборах.
   */
  .superRefine((v, ctx) => {
    if (v.grants === undefined || v.addons === undefined) return;
    ctx.addIssue({
      code: 'custom',
      message: 'Полномочия и надстройки задают один и тот же доступ — оставьте что-то одно',
      path: ['grants'],
 ...
 */
/**
 * Назначенный учётке набор — то, чем окно учётки рисует чекбокс и чем портал собирает высказывание
 * (план «полномочия назначаются в окне учётки», §4).
 *
 * **Зачем рядом с `grantCodes`, а не вместо него.** `grantCodes` отвечает витрине «Права»: она
 * считает по кодам срезы «у кого есть право» и колонку наборов, и код там — ключ, по которому набор
 * узнаётся в журнале и в реестре выдач. Форме кодов не хватает ни на что: чекбоксу нужен `id`
 * (высказывание говорит идентификаторами — код пользовательского набора правится, `id` нет),
 * подписи и справке — `name` (пользовательский набор витрина сегодня подписывает кодом), сверке
 * версий под блокировками — `version` (Р7), причём именно отсюда: у назначения, несовместимого с
 * выбранной ролью, версии в отфильтрованном каталоге нет вовсе.
 *
 * `roleMismatch` и `origin` объясняют, почему набор показан не как обычная галочка: первый —
 * назначение живо, а прав по нему нет (роль вне `grant_roles`, §13.1 плана реструктуризации), и
 * такой набор форма показывает справкой, а не чекбоксом; второй — назначение взведено переводом
 * ролей (`origin = 'migration'`, ADR 0113), и снимать его в форме нельзя: часть подготовленного
 * перевода снимают в реестре выдач, где видно, что именно снимается (Р4).
 */
export interface UserGrantRefDto {
  /** Набор (`grants.id`) — им форма высказывается о наборе, и он же неизменяем у назначения. */
  id: string;
  /** Код набора: тот же ключ, что в `grantCodes`, — по нему набор ищут в журнале и в реестре. */
  code: string;
  /** Название для подписи чекбокса и справки: администратор читает имя, а не код. */
  name: string;
  /** Версия состава на момент чтения карточки — её портал возвращает в `grants[].version` (Р7). */
  version: number;
  /** Роль учётки вне `grant_roles`: назначение живо, прав по нему набор не даёт. */
  roleMismatch: boolean;
  /** Кто завёл назначение: администратор (`manual`) или перевод ролей (`migration`). */
  origin: GrantOrigin;
}
...
export interface UserAccountDto extends UserDto {
  ...
  /**
   * **Все** живые назначения учётки, включая несовместимые с её нынешней ролью, — порядок по коду,
   * как у `grantCodes`.
   *
   * Все, а не только совместимые: форма обязана высказаться и о наборе, чьё действие переключает
   * смена роли (§4.2), а версию для такой строки взять больше неоткуда — отфильтрованный по роли
   * каталог его не содержит (§6). Отдать здесь только совместимые значило бы сделать полноту
   * высказывания невыполнимой ровно в том переходе, ради которого правило и заведено.
   */
  grants: UserGrantRefDto[];
}
