...
export const WAYBILL_SNAPSHOT_KEYS = [
  ...
  'task_to',
  'task_cargo',
  // Время выезда в двух видах: 4-П держит одну графу, форма № 3 — две, «ч» и «мин» по разным
  // клеткам шириной в две цифры. Разбор ложится на снимок, а не на бланк: строку «08:30» в
  // такую клетку не вписать, а собирать её обратно из двух ключей было бы негде.
  'task_departure_time',
  'task_departure_hh',
  'task_departure_mm',
...
/**
 * Можно ли ещё аннулировать лист (ADR 0037 п. 9 в редакции ADR 0052). По день выезда
 * включительно — да; со следующего дня — нет: рейс состоялся, бланк отработал, и лист стал
 * историей, а не планом.
 *
 * Прежняя граница «строго до даты выезда» писалась, когда лист рождался переводом заявки в работу
 * — часто накануне. С маршрутами документ выписывают с рейса, когда состав собран, то есть утром
 * дня выезда: «нельзя в день выезда» означало бы «нельзя никогда», а испорченный при печати бланк
 * остался бы несписанным вместе с рейсом, который он заморозил.
 */
...
export const WAYBILL_LOCKED_MESSAGE =
  'Лист выписан на прошедший день — рейс состоялся, и аннулировать бланк уже нельзя';
