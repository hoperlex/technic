 ...
 *
 * С выпуском автозапчастей (план `docs/auto-parts-plan.md`) к этим трём добавляется четвёртое:
 *
 * 4. **Акт стал основанием складского расхода** (Р4, Р5): у него появились строки «позиция —
 *    количество», и склад двигает разница между присланным набором и текущим, посчитанная сервером
 *    под блокировкой. Отсюда два следствия в этом файле — расхождение POST и PATCH по `parts`
 *    (Р18) и разведённые DTO акта, краткий и полный (Р23).
 *
...
import { z } from 'zod';
import { AUTO_PART_MAX_QUANTITY } from './auto-parts';
...
/**
 * Строка расхода в акте (Р5). Пара «акт + позиция» уникальна: две одинаковых детали в одном акте —
 * это количество 2, а не две строки.
 *
 * Наименование, код и единица приходят соединением с **текущей** карточкой позиции, а не снимком
 * момента списания: позицию переименовывают и уточняют её код, и старый акт обязан называть деталь
 * так же, как её называет справочник, — иначе на один вопрос «что ставили» портал давал бы два
 * разных ответа. Цены и остатка здесь нет вовсе: акт отвечает, что поставили, а сколько такого
 * осталось — вопрос к складу, и ответ на него меняется каждый день.
 */
export interface VehicleMaintenancePartDto {
  id: string;
  autoPartId: string;
  name: string;
  /** `null` — у позиции нет кода: он необязателен (Р12). */
  code: string | null;
  unit: string;
  quantity: number;
  note: string;
...
/**
 * Акт целиком — реквизиты плюс строки расхода (Р23). Его отдают история, форма и карточка машины,
 * то есть места, где акт открыт по одному.
 *
 * Разведён с `VehicleMaintenanceRecordDto` честно, а не полем «которое иногда пустое»: пустой
 * массив в сводке означал бы «строк нет», а это неправда. Обратное склеивание — общий тип со
 * строками — заставило бы каждое открытие гаража тянуть позиции последнего акта для полусотни
 * машин ради колонки, которой нужны только дата и состояние.
 */
export interface VehicleMaintenanceDto extends VehicleMaintenanceRecordDto {
  parts: VehicleMaintenancePartDto[];
  /**
   * Были ли по акту движения склада (Р6). По нему портал заранее показывает «Аннулировать» вместо
   * «Удалить», а не узнаёт правило из 409 после нажатия: акт с движениями неудаляем — держит
   * `ON DELETE RESTRICT` журнала, — и исправляют его аннулированием с возвратом строк на склад.
   *
   * Это не `parts.length > 0`: строки можно снять правкой, а движения по ним останутся навсегда.
   */
...
/**
 * Строка расхода в теле акта (Р5): какая позиция и сколько её поставили.
 *
 * Остатка склада схема не знает и знать не может: сколько лежит на складе, известно только под
 * блокировкой в момент записи, и «проверенное формой» число к этому моменту уже устарело бы.
 * Поэтому нехватку отбивает маршрут (409 «на складе 3, списываете 5»), а форма лишь подсвечивает
 * превышение до отправки — предупреждением, а не запретом.
 *
 * Потолок количества — общий с остатком склада (`AUTO_PART_MAX_QUANTITY`): списать больше, чем
 * позволено хранить, нельзя по построению, и два разных потолка разошлись бы при первой правке.
 */
export const maintenancePartInputSchema = z
  .object({
    autoPartId: uuidSchema,
    /**
     * Целое и строго положительное (Р9): ноль — это не строка расхода, а её отсутствие, и снимают
     * её удалением из набора. Ноль, принятый как строка, дал бы акт, в котором «поставили нисколько»,
     * — и запись в базу отбил бы CHECK `vehicle_maintenance_parts_quantity_check` пятисоткой.
     */
    quantity: z
      .number()
    ...
    /** Зачем ставили: «замена по регламенту», «течь». Необязательно — деталь чаще говорит сама. */
    note: z.string().trim().max(500).default(''),
  })
  .strict();
export type MaintenancePartInput = z.infer<typeof maintenancePartInputSchema>;

/**
 * Набор строк акта целиком (Р5): форма правится целиком, поэтому и строки приходят полным набором,
 * а не парой ручек «добавить/снять». Разницу «стало − было» считает сервер под блокировкой и сам
 * пишет движения склада — клиенту не приходится знать, что его правка «3 → 1» это возврат двух штук.
 *
 * Дубли отбиваются здесь и словами, а не ключом базы: пара «акт + позиция» уникальна (две одинаковых
 * детали — это количество 2, а не две строки), и `vehicle_maintenance_parts_unique` ответил бы
 * механику именем ограничения. Хуже того, из двух строк по одной позиции сервер не смог бы посчитать
 * разницу однозначно: сложить их и списать вдвое или взять последнюю — это разные ответы, и молчаливо
 * выбирать между ними нельзя.
 *
 * Потолок в 50 строк — про акт, а не про базу: акт обслуживания на полсотни позиций уже не документ,
 * а перечень работ за месяц, и разбирать его надо на несколько актов.
 */
const maintenancePartsSchema = z
  .array(maintenancePartInputSchema)
  .max(50, 'В одном акте не больше 50 позиций — разнесите работы по актам')
  .refine((rows) => new Set(rows.map((r) => r.autoPartId)).size === rows.length, {
    message: 'Позиция указана в акте дважды — сложите количество в одну строку',
  });
...
export const maintenanceInputSchema = z
  .object({
    ...
    /**
     * Расход автозапчастей (Р5). У **нового** акта отсутствие поля означает пустой набор: строк у
     * него ещё нет, и списывать нечего. На правке то же умолчание опасно — там оно переобъявлено
     * (`maintenanceUpdateSchema`), и это единственное место, где две схемы расходятся.
     */
    parts: maintenancePartsSchema.default([]),
  })
  .strict();
 ...
 *
 * **`parts` здесь без умолчания, и это не мелочь оформления (Р18).** Схема правки расширяет схему
 * заведения, а значит унаследовала бы `default([])` — и старая вкладка портала, открытая до выката
 * и присылающая PATCH без нового поля, получила бы «снять все позиции»: весь расход акта молча
 * вернулся бы на склад, а механик увидел бы это в журнале задним числом.
 *
 * Поэтому правило на двух ручках разное, и оно постоянное, а не «на время окна совместимости»:
 * отсутствие поля — «строки не менять», явный `parts: []` — «снять все». Это тот же приём, что у
 * разметки применимости и у `modelIds` расходника: «поля нет» и «поле пустое» — разные просьбы, и
 * склеивать их нельзя.
 *
 * Второе следствие того же различения — право: PATCH, приславший те же самые строки, склад не
 * двигает и `autoParts.stock` не требует (Р19), иначе диспетчер не смог бы поправить опечатку в
 * номере документа у акта с расходом.
 */
export const maintenanceUpdateSchema = maintenanceInputSchema.extend({
  version: versionSchema,
  parts: maintenancePartsSchema.optional(),
});
