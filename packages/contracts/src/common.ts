...
/**
 * Календарный месяц, строго `YYYY-MM`. Отдельная величина, а не пара границ: отчётный месяц —
 * это одно значение, которое человек выбирает одним движением и пересылает ссылкой, и разложи его
 * в `from`/`to` уже на портале — в адресе появилась бы пара, которую можно рассогласовать
 * («с 01.05 по 30.06»), а сервер обязан был бы такую пару либо чинить, либо отвергать.
 *
 * Границы месяца считает сервер (`monthRange`): день, которым кончается февраль, — знание о
 * календаре, а не о запросе.
 */
...
/**
 * Границы месяца включительно: `2026-08` → `2026-08-01 … 2026-08-31`.
 *
 * Рядом со схемой, а не в слое выборки: «последнее число месяца» сервер считает для отбора, а
 * портал — для подписи периода, и двух способов узнать, сколько дней в феврале, проекту не нужно.
 *
 * Счёт идёт в UTC и нулевым днём следующего месяца: часовые пояса в вычислении не участвуют —
 * это календарные сутки, а не моменты времени, — а високосный год считается сам.
 */
