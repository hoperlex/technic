...
/**
 * Тот единственный субъект, которого сужает правило ниже: **держатель сквозной области модуля
 * заявок** (`GRANT_MODULE_WIDE_SCOPE` — сегодня это набор ИТ-службы, то есть половина профиля
 * «Системный администратор»).
 *
 * Спрашивается ОБЛАСТЬ, А НЕ КОД НАБОРА, и это не обобщение впрок. Дыру Н8 открыла именно сквозная
 * область: она превращает «мою площадку» в компанию целиком, и правка чужой заявки досталась
 * профилю не решением, а побочным эффектом видимости. Запиши мы здесь код `…_it_approver`, правило
 * молча разошлось бы с таблицей области в тот день, когда сквозным модуль откроют ещё кому-нибудь —
 * причём разошлось бы в дорогую сторону: новая дыра, а не новый отказ.
 *
 * Два исключения, и оба названы решением заказчика (04.09.2026):
 *
 *   1. держатель «Ведения» (`office_equipment_operator`) ведёт модуль и правит чужие заявки по
 *      должности — операторский слой с авторским не смешивается (Р6 прямым текстом). Пара наборов
 *      «Ведение + ИТ» у одного человека — рабочий случай, а не выдумка: он и координирует, и чинит;
 *   2. `admin` — по тому же основанию, по которому он исключён в `actsAsServiceOperator` и в
 *      стороне обсуждения `it`: права у него от роли, а не от выдачи, кодов наборов у него не
 *      бывает вовсе, а разбор застрявшего за любую сторону — его работа.
 */
function isModuleWideReaderOnly(subject: AccessSubject | null | undefined): boolean {
  if (subject?.role === 'admin') return false;
  if (hasGrantCode(subject, OFFICE_EQUIPMENT_OPERATOR_GRANT)) return false;
  return hasModuleWideScope(subject?.grantCodes, 'serviceRequests');
}
 ...
 * с набором «Заявитель», здесь держатель сквозной области, — и оба обязаны отвечать на своей
...
export function canChangeRequestAsCustomer(
  ...
  if (!isModuleWideReaderOnly(subject)) return true;
  return facts.inCustomerScope || facts.isAuthor;
}
