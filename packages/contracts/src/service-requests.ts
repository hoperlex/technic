...
export const createServiceRequestSchema = withUrgency(
  ...
  /**
   * Правила предмета расходятся надвое — ровно как двусоставный `service_requests_subject_check`
   * (миграция 0230, Р7). У заявки С АППАРАТОМ объект — это «где он стоит», и он ходит парой с
   * пометкой; у заявки БЕЗ АППАРАТА тот же `objectId` означает уже другое — **заказчика**, — и
   * пометке там взяться неоткуда. Одним общим набором условий эти два смысла не описываются:
   * «объект без пометки ничего не объявляет» верно только в первой половине и запрещало бы вторую.
   */
