...
// Только тип: список ролей-заявителей (ADR 0181) обязан состоять из ЗНАЧЕНИЙ словаря ролей, и
// `satisfies readonly Role[]` роняет сборку в тот день, когда роль-заплатку `manager` уберут.
import type { Role } from './enums';
// Значением, а не типом: сквозную область модуля читает предикат правки заявки (Н8) — тем же
// `hasModuleWideScope`, каким её читают область видимости на сервере и подбор заказчика на портале.
// Обратной зависимости у наборов на заявки нет, поэтому цикла модулей не заводится.
import { hasModuleWideScope } from './grants';
...
export const SERVICE_ASSIGNER_TRANSITIONS: Record<ServiceRequestStatus, ServiceRequestStatus[]> = {
  ...
  /*
   * РУЧНОЙ ВОЗВРАТ РАБОТАЮЩЕЙ ЗАЯВКИ В «НОВУЮ» — и больше ничего (ADR 0187).
   *
   * Дуга стояла здесь ради ПЕРЕназначения: оно откатывало заявку из «В работе», чтобы новый
   * исполнитель нажал «Принять в работу» сам. Отката больше нет — назначение статуса не меняет ни
   * при каком исходе, — но дугу это не освобождает: её единственным ходом остаётся пункт «Вернуть
   * в «Новую»» (`PATCH /:id/status`, откат своего же «принял в работу», Р13).
   *
   * УБРАТЬ ЕЁ ЗАОДНО С ОТКАТОМ БЫЛО НЕЛЬЗЯ, и это проверено по составу наборов. У «Ведения»
   * оргтехники (`office_equipment_operator`) есть `serviceRequests.assign` и
   * `serviceRequests.status`, а `requests.rollbackStatus` нет — через `SERVICE_ADMIN_ROLLBACKS`
   * дуга к нему не придёт вовсе. Сними мы строку, ручной возврат пропал бы у тех, кто заявки
   * ведёт, — молча и без единой просьбы об этом.
   *
   * Запрет переназначения под висящим предъявлением живёт в предикате `canAssignServiceExecutors`
   * (Р11), а не здесь: назначение таблицу переходов больше не спрашивает вовсе.
   */
  in_work: ['new'],
  on_hold: [],
  done: [],
...
function baseServiceReset(
  ...
  /*
   * `in_work → new` НЕ СНИМАЕТ ИСПОЛНИТЕЛЯ, и это не пропуск, а условие работоспособности хода,
   * который по этой дуге идёт.
   *
   * Прежняя ветка `assigned → new` снимала — но её звал отказ, а отказ переходом быть перестал
   * (Р7): состав он правит сам, статуса не трогает. Ходов по дуге было двое, остался один, и обоим
   * сброс здесь во вред:
   *
   ...
   * - **переназначение из «В работе»** по этой дуге больше не ходит вовсе (ADR 0187): назначение
   *   статуса не меняет. Довод, впрочем, остаётся верным и для него — состав пишется ДО
   *   `applyTransition`, потому что письмо собирается в той же транзакции и читает исполнителей из
   *   таблицы, а сброс, идущий следом, удалил бы строки по всей заявке (`reset.executor` — это
   *   `DELETE ... WHERE request_id = …`). Появись у назначения новая дуга — она наступит на те же
   *   грабли, и абзац здесь ради этого.
   *
...
/**
 * Роли, которым круг заявителя приходит САМОЙ РОЛЬЮ, а не набором (ADR 0181, решение заказчика
 * 10.09.2026): `manager` — временная роль-заплатка, и всё, что она раздаёт, уходит вместе с её
 * строкой в матрице, а не строкой в `grant_roles` и выдачами в `user_grants`.
 *
 * ЗАЧЕМ СПИСОК ЗДЕСЬ. Страж ниже опознаёт заявителя КОДОМ набора (Р9), и права, пришедшие ролью,
 * прошли бы мимо него целиком: менеджер видит заявки всей компании и правил бы, удалял и подшивал
 * бы файлы к любой чужой. Второй источник профиля требует второго признака — вот он.
 *
 * ИМЕНЕМ РОЛИ, А НЕ СОСТАВОМ ПРАВ, и это не отступление от Р9, а его прямое следствие. «Профиль —
 * это код, а не сумма прав» ровно потому, что собранный руками набор с тем же составом не должен
 * получать чужое ограничение; у роли же кода нет вовсе, и назвать её нечем, кроме имени. Обратное
 * правило — «сузить всякого, у кого есть круг заявителя и нет прав ведения» — это отрицательное
 * основание, которое ломается молча: первое же новое право модуля вывело бы роль из-под стража, и
 * ошибка была бы в неперечисленном.
 *
 * `satisfies readonly Role[]` — сторож удаления роли: в тот день, когда `manager` уйдёт из `ROLES`,
 * эта строка не скомпилируется, и заплатку не придётся искать по дереву.
 */
const OFFICE_EQUIPMENT_ROLE_REQUESTERS = ['manager'] as const satisfies readonly Role[];
 ...
 *   2. доступ пришёл заявительским слоем — выданным кодом «Заявителя» ЛИБО самой ролью, если она
 *      названа в `OFFICE_EQUIPMENT_ROLE_REQUESTERS` (ADR 0181: у `manager` круг заказчика стоит
 *      прямо в матрице). Профиль по-прежнему опознаётся кодом, а не суммой прав (Р9): собранный
 *      руками набор с теми же шестью правами сюда не попадает — сузить его значило бы вывести
 *      профиль модуля из чужого набора, ровно то, от чего Р9 уводит;
 ...
 * нет вовсе, а роли его в списке ролей-заявителей нет и быть не может — права у него от полного
 * перечня, и разбор застрявшего за любую сторону его работа. Это не случайное везение, а то же основание, по которому он проходит в
...
function isGlobalRequesterOnly(subject: AccessSubject | null | undefined): boolean {
  ...
  const byRole =
    ...
    (OFFICE_EQUIPMENT_ROLE_REQUESTERS as readonly Role[]).includes(subject.role);
  if (!byRole && !hasGrantCode(subject, OFFICE_EQUIPMENT_REQUESTER_GRANT)) return false;
...
/**
 * **В модуле он исполнитель и никто больше** — тот единственный профиль, которому назначение
 * область СУЖАЕТ, а не расширяет (план
 * `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р3; ответы В3 и В6 заказчика от
 * 09.09.2026: «исполнитель видит только свои заявки», плюс свои заведённые и снятые с него).
 *
 * Три сомножителя, и каждый отсекает своё: доступ к модулю пришёл ему набором ИТ-службы либо
 * набором исполнителя, то есть работой руками, а не должностью заказчика; роль не `admin`; и у него
 * нет «Ведения».
 *
 * ЗАМЕНИЛА СОБОЙ `isModuleWideReaderOnly`, СТОЯВШУЮ РОВНО ЗДЕСЬ. Форма у них общая — те же два
 * исключения и по тем же причинам: «Ведение» (`office_equipment_operator`) ведёт модуль и правит
 * чужие заявки по должности, операторский слой с авторским не смешивается, а пара «Ведение + ИТ» у
 * одного человека — рабочий случай, он и координирует, и чинит; у `admin` же права от роли, кодов
 * наборов у него не бывает вовсе, и разбор застрявшего за любую сторону — его работа (по тому же
 * основанию он исключён в `actsAsServiceOperator` и в стороне обсуждения `it`). Разошлись они в
 * первом сомножителе, и это и есть решение Р10: прежняя спрашивала КАРТУ области
 * (`hasModuleWideScope(…, 'serviceRequests')`), а карта из модуля уезжает следующим выпуском — вся
 * логика, на неё опёртая, после уборки молча поменяла бы ответ.
 *
 * СПРАШИВАЮТСЯ КОДЫ НАБОРОВ, А НЕ ПРАВО `serviceRequests.execute`, и это не вкусовщина: «по праву»
 * ответило бы неверно с обеих сторон сразу.
 *
 *   1. У роли `admin` право есть по построению (`admin: [...PERMISSIONS]`), и признак отобрал бы у
 *      администратора все заявки портала — сузив ровно того, кто разбирает застрявшее;
 *   2. у держателя ОДНОГО ИТ-набора его нет вовсе: миграция 0262 отдала `execute` отделённому
 *      набору исполнителя. «По праву» оставило бы такому держателю сквозную видимость целиком, то
 *      есть не сузило бы ничего у того, ради кого решение и принято.
 *
 * ПОДРЯДЧИКА ЭТА ФУНКЦИЯ НЕ КАСАЕТСЯ И КАСАТЬСЯ НЕ ДОЛЖНА: у оператора контрагента `service` кодов
 * наборов оргтехники не бывает ни одного (`OFFICE_EQUIPMENT_PROFILE_REGISTRY`, профиль `service` —
 * пустой список), а сужает его своя ось «заявка назначена моей компании». Ответ В7 — «у сервисного
 * центра всё верно уже сегодня», — и трогать его волна не бралась.
 *
 * СЛЕДА СНЯТИЯ ЗДЕСЬ НЕТ: это свойство ЗАЯВКИ, а не субъекта, и живёт оно в области чтения. Функция
 * отвечает только на вопрос «кто он в модуле», и все три сомножителя читаются по самому субъекту —
 * поэтому её одинаково зовут и предикат выборки списка, и проверка по одной строке.
 */
export function actsAsServiceExecutorOnly(subject: AccessSubject | null | undefined): boolean {
  if (!subject) return false;
  if (subject.role === 'admin') return false;
  if (hasGrantCode(subject, OFFICE_EQUIPMENT_OPERATOR_GRANT)) return false;
  return (
    hasGrantCode(subject, OFFICE_EQUIPMENT_IT_GRANT) ||
    hasGrantCode(subject, OFFICE_EQUIPMENT_EXECUTOR_GRANT)
  );
}

/**
 * **Выбирает ли субъект предмет заявки по всей компании** — площадку аппарата, отдел-заказчика,
 * площадку сообщения об отсутствующей технике (план
 * `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р9).
 *
 * ОСНОВАНИЕ ПЕРЕЕЗЖАЕТ С КАРТЫ ЗАЯВОК НА КАРТУ ПАРКА, и в этом всё решение. Сегодня четыре разбора
 * предмета спрашивают сквозную область МОДУЛЯ ЗАЯВОК (`hasModuleWideScope(…, 'serviceRequests')`),
 * хотя ни один из них не про ленту заявок: площадка аппарата, отдел-заказчик и площадка кандидата —
 * это СПРАВОЧНИК, из которого заявку собирают, и вопрос у них один и тот же — «весь ли парк ему
 * показывать». Пока оба ключа стоят у одного набора (ИТ-служба), подмена не меняет ни одного
 * ответа; ровно это и обязан доказать parity-тест, а не рассуждение.
 *
 * ЗАЧЕМ ТОГДА МЕНЯТЬ, ЕСЛИ ОТВЕТ ТОТ ЖЕ. Затем, что `serviceRequests` из карты области уезжает
 * следующим выпуском, и без этой правки уборка строки молча сузила бы ЗАВЕДЕНИЕ заявки: «аппарат
 * вижу, а завести по нему заявку не могу» — не сужение видимости заявок, а поломка сценария, ради
 * которого парк и оставлен видимым целиком (ответ В5). То есть выпуск, названный технической
 * уборкой, поменял бы продуктовые правила заведения. С этим предикатом он остаётся тем, чем назван,
 * — уборкой строки, которую больше никто не читает.
 *
 * `admin` отдельной веткой по тому же основанию, что у соседей: карту читают по кодам наборов, а у
 * него их нет вовсе.
 */
export function canPickAnyServiceSubject(subject: AccessSubject | null | undefined): boolean {
  if (!subject) return false;
  if (subject.role === 'admin') return true;
  return hasModuleWideScope(subject.grantCodes, 'officeEquipment');
}

/**
 * **Распоряжается ли субъект самой записью со стороны заказчика** — то есть открыты ли ему ПРАВКА и
 * УДАЛЕНИЕ этой заявки (план профилей оргтехники, находка Н8, решение заказчика 04.09.2026).
 *
 * ПРАВИЛО ОДНОЙ СТРОКОЙ: правку и удаление открывает сторона заказчика, посчитанная по
 * СОБСТВЕННОЙ оси субъекта (его площадка, его отдел) либо по авторству. Сквозная область стороной
 * заказчика не делает — она отвечает «видно», а не «моё».
 *
 * ЧТО ЭТО ЗАКРЫВАЕТ. Обе половины профиля «Системный администратор» выдаются поверх ролей
 * `shtab`/`site`/`department`, у каждой из которых `serviceRequests.update` и `.delete` есть по
 * матрице как у заказчика, а сквозная область ИТ-набора снимает у них ось. Итог до правила:
 * держатель профиля правил и уводил в архив ЛЮБУЮ «Новую» заявку компании, тогда как матрица §5.1
 * обещает обратное двумя клетками. Правило и есть то место, которое эти клетки теперь держит.
 *
 * СУЖАЕТ ОНО РОВНО ДВЕ ДВЕРИ, А НЕ ЧЕТЫРЕ, И ЭТО ГЛАВНОЕ В НЁМ. Страж авторства
 * (`actsAsRequestCustomer`) стоит на общем входе всех изменяющих ручек, и под ним, кроме правки и
 * удаления, живёт ПОДШИВКА И СНЯТИЕ ДОКУМЕНТОВ. Сисадмина штатно назначают исполнителем на заявки
 * чужих площадок — ради этого у профиля второй набор, — и подшить акт по такой заявке он обязан.
 * Расширь мы существующий страж на сквозную область «в лоб», назначенный исполнитель перестал бы
 * прикладывать документы к заявке, которую сам же чинит. Поэтому правило отдельной функцией и
 * зовётся только с двух дверей; на остальных двух прежнее правило остаётся ровно таким, каким было.
 *
 * ПРЕЖНЕЕ ПРАВИЛО СНИМАЕТСЯ ПЕРВЫМ, а не заменяется этим: субъекты у них разные — там роль без оси
 * с набором «Заявитель», здесь исполнительский профиль модуля, — и оба обязаны отвечать на своей
 * двери. Конъюнкция читается как «сначала прежний вопрос, потом добавка», и порядок этот
 * единственно верный: глобальному заявителю сквозной области не выдают, а сисадмину «Заявитель»
 * не нужен, но собрать одному человеку оба набора администратор вправе.
 *
 * ОСНОВАНИЕ ОТВЯЗАНО ОТ КАРТЫ ОБЛАСТИ (план
 * `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р10). Прежде тот же субъект
 * опознавался здесь через сквозную область модуля заявок, то есть через КАРТУ. Правило от этого
 * работало — назначенный на чужую площадку сисадмин правки и удаления не получал, — но держалось
 * на строке, которую волна как раз и убирает: сними карту, признак стал бы ложным, страж ответил бы
 * `true`, и правка с удалением ЧУЖОЙ заявки открылись бы через назначение. Правило «правка и
 * удаление — своя площадка либо своё авторство» обязано пережить уборку карты без единой правки
 * поведения, поэтому субъект опознаётся кодами наборов (`actsAsServiceExecutorOnly`), а не
 * областью. Правится это ВЫПУСКОМ A, а не B: область действий уже пропускает назначенного по своей,
 * исполнительской причине, и до замены между ней и этим стражем осталась бы щель.
 *
 * НАЗНАЧЕНИЕ ОСТАЁТСЯ ОСНОВАНИЕМ — но своим. Действия исполнителя и подшивка документов открыты
 * назначенному ровно как сегодня (см. абзац про две двери выше); разведены два разных основания:
 * «я это чиню» и «я этим распоряжаюсь». Первое даёт назначение, второе — сторона заказчика, и
 * смешивать их нельзя ни в одну сторону: слитые, они либо отобрали бы у исполнителя акт, либо
 * отдали бы ему чужую заявку целиком.
 *
 * ИЗМЕНЕНИЕ НАЗВАНО ВСЛУХ, А НЕ СПРЯТАНО ЗА «ТОТ ЖЕ СУБЪЕКТ». Держатель ОДНОГО исполнительского
 * набора (без ИТ) сквозной области не имеет и прежним признаком не ловился вовсе — правку и
 * удаление он получал по обычной оси своей роли. Теперь ловится: править и удалять он может свою
 * площадку и свои заявки, чужие — нет. Это правильная строгость, но это ИЗМЕНЕНИЕ поведения, а не
 * его сохранение, и доказывать его обязан отдельный тест.
 *
 * В БАЗУ НЕ ХОДИТ И ХОДИТЬ НЕ МОЖЕТ: признаки приходят посчитанными, как у соседей. Сервер считает
 * их по строке заявки тем же `inServiceRequestCustomerScope`, каким считается подсветка адресата
 * «Заявителю», портал — из готового ответа (`ServiceRequestDto.inCustomerScope` и сторона
 * `customer` в сводке обсуждения). Второй копии правила на клиенте от этого не заводится — обе
 * стороны зовут ЭТУ функцию.
 */
export function canChangeRequestAsCustomer(
  subject: AccessSubject | null | undefined,
  facts: RequestCustomerFacts,
): boolean {
  if (!actsAsRequestCustomer(subject, facts)) return false;
  if (!actsAsServiceExecutorOnly(subject)) return true;
  return facts.inCustomerScope || facts.isAuthor;
}

/**
 * Факты заявки, которыми решается вопрос «можно ли этому субъекту вообще её трогать» (план
 * `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р11).
 *
 * Три поля, и ни одного лишнего. СЛЕДА СНЯТИЯ ЗДЕСЬ НЕТ НАМЕРЕННО: он расширяет область ЧТЕНИЯ
 * (ответ В6 — «заявки, с которых его сняли»), а не действий. Снятый исполнитель заявку видит и в
 * обсуждении по-прежнему участвует, но чинить, назначать, замораживать и подшивать больше не может;
 * приедь след сюда четвёртым полем, «смотреть» и «распоряжаться» снова слились бы в один вопрос —
 * ровно то разделение, ради которого две области и разведены.
 */
export interface ServiceRequestActionFacts {
  /**
   * Рубильник `service_request_executor_scope` включён. Полем факта, а не вопросом внутри
   * предиката: предикаты контрактов чистые и в базу не ходят, на сервере признак приезжает полем
   * принципала тем же запросом, что и остальной субъект, а на портал — списком `features` в ответе
   * сессии. Протаскивай мы флаг параметром через каждого зовущего, о нём было бы где забыть.
   */
  executorScopeEnabled: boolean;
  /**
   * Он завёл эту заявку (`service_requests.created_by`) — то же самое поле, каким сторона
   * заказчика считается в `RequestCustomerFacts`.
   */
  isAuthor: boolean;
  /**
   * Он стоит в ДЕЙСТВУЮЩЕМ поимённом составе исполнителей. Именно действующем: строка назначения
   * при снятии удаляется физически, и «был назначен» — это уже след, то есть другой вопрос и другая
   * область (см. абзац выше).
   */
  isNamedExecutor: boolean;
}

/**
 * **Можно ли этому субъекту вообще трогать эту заявку** — общий первый сомножитель ВСЕХ действий
 * над ней (план `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р11).
 *
 * ПРАВИЛО ТРЕМЯ ВЕТКАМИ:
 *
 *   1. рубильник выключен — `true`, сегодняшнее поведение до единого ответа. Так выглядит выпуск A
 *      до включения и любой откат: строку ключа возвращает `UPDATE`, а не обратный релиз;
 *   2. рубильник включён и субъект — исполнительский профиль (`actsAsServiceExecutorOnly`):
 *      действие открывает действующее назначение либо своё авторство, и больше ничего;
 *   3. все остальные — `true`, без единого изменения. Ветка обязательна, и первые редакции плана её
 *      теряли: общий вход в изменяющие ручки стоит у ВСЕХ субъектов, и правило «назначен ∨ автор»,
 *      поставленное туда без развилки, отобрало бы правку у обычного коллеги с ролью площадки,
 *      который правит заявку своей площадки, автором не будучи. Это была бы переделка всей модели
 *      действий под видом сужения одного профиля.
 *
 * ЗОВУТ ЕГО ДВОЕ, И В ЭТОМ ВЕСЬ СМЫСЛ ОДНОГО ПРЕДИКАТА. Сервер зовёт его из
 * `assertServiceRequestActionable` — общего входа изменяющих ручек. Портал — первым сомножителем во
 * всех своих мутациях: набор пунктов меню, правка и удаление в списке, подшивка и снятие
 * документов, объём работ и расходники. Доступность действия портал считает сам — по праву и
 * статусу, — а про отношение к заявке не знает вовсе: без общего сомножителя бывшему исполнителю
 * рисовалась бы кнопка «Изменить исполнителей», а сервер отвечал бы на неё 403. Обещаний, которых
 * сервер не выполняет, модуль избегает предикатами с ADR 0162, и этот — из того же ряда.
 *
 * СЛУЧАЙ, КОТОРЫЙ ЛОВИТ ТОЛЬКО ОН: бывший исполнитель на СВОЕЙ площадке. Страж стороны заказчика
 * выше отвечает ему `true` по `inCustomerScope` — площадка действительно его, — а область действий
 * отвечает `false`: он не назначен и не автор. Считай портал доступность одним стражем заказчика,
 * он показал бы правку, удаление и загрузку документов там, где сервер отвечает 403 на каждую.
 *
 * ДВА ПРЕДИКАТА НЕ ЗАМЕНЯЮТ ДРУГ ДРУГА, А ПЕРЕМНОЖАЮТСЯ: `canChangeRequestAsCustomer` отвечает «моя
 * ли это запись», этот — «в моей ли она области действий». Вопросы разные, и ни один не выводится
 * из другого.
 */
export function canActOnServiceRequest(
  subject: AccessSubject | null | undefined,
  facts: ServiceRequestActionFacts,
): boolean {
  if (!facts.executorScopeEnabled) return true;
  if (!actsAsServiceExecutorOnly(subject)) return true;
  return facts.isNamedExecutor || facts.isAuthor;
}
 ...
 *
 * ВХОДА ДВА, И ВТОРОЙ — ПАМЯТЬ СПОРА (Р9). Предварительных барьеров у денег заказчик не захотел ни
 * одного (ответы В2, В12 и В13 от 11.09.2026): остался рубильник `service_estimate_exemption`. Но
 * барьер ПОСЛЕ спора — не политика, а исполнение уже принятого решения: если «Ведение» разобрало
 * спор исходом «нужна подпись», подпись по этой заявке собирают, и заявление второй раз освобождать
 * от неё не может.
 *
 * ЧТО БЫЛО БЕЗ ВТОРОГО ВХОДА. Исход спора снимал сам подрядчик и без всякого разбора: возврат в
 * правку (`/estimate/reopen`) гасит ожидание, то же заявление по новой ревизии снова отвечало
 * `applied`, автоподпись вставала на место, работы закрывались, и автоприёмка через сутки принимала
 * заявку. Ни одной человеческой подписи, и требование спора не помнил никто. Функция обязана его
 * спросить — иначе весь разбор спора стоит один возврат в правку.
 *
 * Исходов по-прежнему ровно два, и отказом ни один из них не бывает: заявление со снятым
 * освобождением записывается как `observed` — след остаётся, подпись собирается обычным порядком.
 * Третий исход заводить нельзя: см. `SERVICE_ESTIMATE_EXEMPTION_OUTCOMES`.
 *
 * ОБА ПРИЗНАКА ПРИХОДЯТ ЗНАЧЕНИЯМИ, А НЕ ЧИТАЮТСЯ ЗДЕСЬ. Рубильник сервер знает по строке
 * `feature_flags`, портал — по ответу сессии (`hasFeature`); память спора сервер читает по таблице
 * споров, портал — по полю карточки `dispute`. Общего источника у них нет, и спроси функция базу
 * сама — портал не смог бы позвать её вовсе. От этого она остаётся чистой и проверяемой всеми
 * сочетаниями в четыре строки теста.
 *
 * ОБА ПОЛЯ ОБЯЗАТЕЛЬНЫ. Необязательное со значением по умолчанию `false` означало бы «о споре не
 * спросили» и «спора не было» одним и тем же ответом — то есть новый вызов, забывший про память
 * спора, молча вернул бы дыру, которую этот вход и закрывает.
 */
export function evaluateExemption(input: {
  flagEnabled: boolean;
  /** По заявке есть разрешённый спор с исходом «нужна подпись» (Р9): освобождение больше не даётся. */
  disputeRequiresSignature: boolean;
}): ServiceEstimateExemptionOutcome {
  if (input.disputeRequiresSignature) return 'observed';
...
/**
 * Наименование строки объёма работ: до 2000 символов, переводы строк внутри разрешены (`trim`
 * снимает пробельное только по краям).
 *
 * ПОЧЕМУ ПРЕДЕЛ ВЫРОС С 255 (план `docs/office-equipment-free-estimate-and-executor-scope-plan.md`,
 * Р1; ответ В1 заказчика от 09.09.2026). Подрядчик присылает объём работ письмом — одной записью,
 * перечнем позиций с количествами: «Ремонт МФУ (RICOH MP C 2011 …); Узел фотобарабана в сборе …,
 * 2 шт; Блок проявки в сборе …» и так далее, около 370 символов на пять строк, а цены по позициям
 * не разложены вовсе. Свободный режим ввода кладёт этот перечень в ОДНУ строку сметы
 * (`kind = service`, `quantity = 1`, `unitPrice` = общая стоимость): второго источника суммы в
 * модели не заводится — итог, согласование, акт, факт, реестр гарантий и разбор спора читают
 * строки, и вторая дорога заставила бы каждое из шести мест отвечать, какой источник главный.
 * При пределе 255 письмо в эту строку просто не влезало: человек резал бы перечень руками, то есть
 * терял бы предмет согласования — ровно то, под чем потом стоит подпись.
 *
 * ПЕРЕНОСЫ СТРОК — ЧАСТЬ ЗНАЧЕНИЯ, А НЕ МУСОР. Перечень читают глазами там же, где согласуют, и
 * склеенный в одну строку он становится нечитаемым в том самом месте, ради которого заведён.
 * Поэтому схема их не режет, а таблица показывает наименование с сохранением переносов
 * (`white-space: pre-line`).
 *
 * ОГРАНИЧЕНИЕ ЖИВЁТ ТОЛЬКО ЗДЕСЬ, И МИГРАЦИИ ПОД НЕГО НЕТ. Колонка `service_request_items.name` —
 * `text` без длины, а единственный `CHECK` на ней требует непустоты (`btrim(name) <> ''`), не
 * длины. То есть 255 никогда не было свойством хранилища — это была рамка формы, и поднимается она
 * правкой одной строки контрактов, без окна выката. Обратная сторона названа вслух: сузить предел
 * обратно тоже можно без миграции, но уже нельзя безопасно — в базе к тому времени лежат записи
 * длиннее, и схема начала бы отбивать правку строк, которые сама же и приняла.
 */
const itemNameSchema = z.string().trim().min(2, 'Укажите наименование').max(2000);
...
/**
 * Раскладка свободной записи по графам (`PUT /:id/estimate/breakdown`; план
 * `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р2; ответ В9 заказчика от
 * 09.09.2026). По составу тело то же, что у `putServiceEstimateSchema`, — и это не повод
 * переиспользовать её схему.
 *
 * ЧЕМ ОТЛИЧАЕТСЯ ОТ СОСЕДКИ. Разные держатели, разная дверь и разные последствия. Ту открывает
 * `serviceRequests.estimate` — АВТОР объёма работ, исполнитель, — и правит он черновик: состав
 * заменяется, номер ревизии и подписи не трогаются. Эту открывает
 * `serviceRequests.estimateRewrite` — «Ведение», которое переносит присланный подрядчиком перечень
 * в графы, — и по СОГЛАСОВАННОЙ ревизии она делает другое: поднимает номер, снимает подпись,
 * пересчитывает итог и ставит новое предъявление, потому что подписанное содержимое под прежней
 * ревизией не меняется (ADR 0133). Одна схема на две двери означала бы, что тело запроса перестало
 * отличать правку черновика от переиздания документа.
 *
 * `.strict()` — НЕ УКРАШЕНИЕ, А ПЛАТА ЗА УЖЕ СЛУЧИВШИЙСЯ ДЕФЕКТ. Молчаливое отбрасывание
 * неизвестных полей приёмом модуля стоило ровно одного разбора: портал шлёт поле, сервер про него
 * не знает, ответ 200 — и человек уверен, что сохранил то, чего не сохранилось. У ручки, которая
 * снимает согласование и требует новой подписи, такая тишина дороже всего: опечатка в имени поля
 * прошла бы как успешная раскладка, а заявка ушла бы на второй круг подписей ни за чем.
 *
 * Предел строк тот же (200) — это тот же документ, и разложенная смета не бывает длиннее набранной
 * руками. Пустой список схемой разрешён, как и у соседки: что делать с «разложить в ноль строк»,
 * решает ручка — у неё для этого есть статус заявки и состояние ревизии, а у тела их нет.
 */
export const putServiceEstimateBreakdownSchema = z
  .object({
    items: z.array(serviceEstimateItemSchema).max(200),
    version: z.number().int().nonnegative(),
  })
  .strict();
export type PutServiceEstimateBreakdownInput = z.infer<typeof putServiceEstimateBreakdownSchema>;
...
/** Что режиму «предыдущие» позволено сопровождать: только листание и порядок, но не отбор. */
const REPEAT_FOR_KEEPS = new Set(['page', 'pageSize', 'sortBy', 'sortOrder', 'repeatFor']);

export const serviceRequestListQuerySchema = baseListQuery(SERVICE_REQUEST_SORT_FIELDS)
  .extend({
    ...
    urgent: z
      ...
      .transform((v) => v === 'true'),
    /**
     * ТОЛЬКО ПОВТОРНЫЕ (план `docs/office-equipment-repeat-request-plan.md`, Р10): заявки, у которых
     * по тому же аппарату есть закрытая либо отменённая ремонтная заявка в окне `W` — правило Р1
     * целиком, включая область видимости читателя.
     *
     * ФЛАГОМ, КАК У СОСЕДЕЙ: значение приходит строкой из query и превращается в «нет», пока его не
     * включили, — `undefined` в предикате означал бы фильтр, применённый наполовину.
     *
     * ЧТО ИМЕННО ОН ОТБИРАЕТ, РЕШАЕТ СЕРВЕР, И ТОЛЬКО ОН. Собери портал тот же отбор из
     * `equipmentId` + `status` + `kind`, он показал бы не то, что посчитала метка: окно у каждой
     * строки своё, а число совпадений считается в области смотрящего (Н7, Р8). Одно правило — одно
     * место, и место это `services/service-request-repeat.ts` на сервере.
     *
     * ПРИ ВЫКЛЮЧЕННОМ ОКНЕ (`SERVICE_REQUEST_REPEAT_WINDOW_DAYS = 0`) ОТБОР ПУСТ, А НЕ ОШИБОЧЕН
     * (§10.1, тест 8): «повторных нет» — честный ответ выключенного признака, а 422 на параметр,
     * который портал помнит с прошлого сеанса (ADR 0139), означал бы сломанный список у человека,
     * ничего сегодня не нажимавшего.
     */
    repeat: z
      .enum(['true', 'false'])
      .optional()
      .transform((v) => v === 'true'),
    /**
     * ССЫЛКА «ПРЕДЫДУЩИЕ» (Р10 того же плана): показать те самые заявки, которые посчитала метка
     * повтора у заявки `R`. Не отбор, а РЕЖИМ — потому и живёт отдельным параметром, а не парой
     * `equipmentId` + `status`: окно у каждой строки своё, считается оно в области смотрящего, и
     * собранный порталом «похожий» отбор показал бы не то число, которое человек только что видел
     * в теге.
     */
    repeatFor: uuidSchema.optional(),
    createdFrom: dateOnlySchema.optional(),
    createdTo: dateOnlySchema.optional(),
    archive: archiveFilterSchema,
  })
  /*
   * РЕЖИМ «ПРЕДЫДУЩИЕ» НЕ СМЕШИВАЕТСЯ С ОТБОРОМ (Р10). Ссылка обещает ровно те заявки, которые
   * посчитал тег, и число строк обязано совпасть с `repeat.count`. Позволь мы дописать к ней хоть
   * один фильтр — человек увидел бы «Повтор ×3» и список из одной строки, причём без единого
   * признака, что виноват отбор, оставшийся с прошлого сеанса (списки помнят отборы, ADR 0139).
   *
   * Проверка идёт ОТ ОБРАТНОГО — перечислены разрешённые ключи, а не запрещённые. Перечень
   * запрещённых пришлось бы дополнять при каждом новом фильтре списка, и первый же забытый молча
   * вернул бы ту самую рассогласованную выдачу.
   */
  .superRefine((value, ctx) => {
    if (!value.repeatFor) return;
    const busy = Object.entries(value)
      .filter(([key]) => !REPEAT_FOR_KEEPS.has(key))
      // «Нейтральное» — это не только отсутствие: булев фильтр в выключенном виде приходит `false`,
      // архив по умолчанию — `exclude`. Считать их заданными значило бы запретить саму ссылку.
      .filter(([, v]) => !(v === undefined || v === false || v === '' || v === 'exclude'))
      .map(([key]) => key);
    if (busy.length === 0) return;
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['repeatFor'],
      message: `Список предыдущих заявок показывается целиком и не сочетается с отбором: снимите ${busy.join(', ')}`,
    });
  });
 ...
 *
 * `undefined` В ТИП НЕ ВХОДИТ, и это не придирка к форме. Поле карточки необязательно — сервер его
 * отдаёт, а вот ответ СТАРОГО приложения в окне выката его не несёт, — и пройди `undefined` в
 * аргумент молча, «формата в ответе нет» и «наследственное правило выбрано осознанно» стали бы одним
 * значением. Зовущий пишет `?? null` и
...
// ── Повторное обращение по аппарату (план `docs/office-equipment-repeat-request-plan.md`) ──
//
// ТРЕТЬЕ ПОНЯТИЕ РЯДОМ С ДВУМЯ УЖЕ РАБОТАЮЩИМИ (§4 плана), и путать их нельзя ни в коде, ни в
// словах. «Уже чиним?» отвечают частичные уникальные индексы `service_requests_open_*_unique` —
// они не дают завести вторую НЕЗАКРЫТУЮ заявку того же вида. «Чините по гарантии» отвечает
// `warrantyClaim` — обращение с юридическим содержанием и четырьмя серверными проверками. Здесь
// же — наблюдение и только оно: «по этому аппарату недавно уже была закрытая заявка».
//
// СЕМАНТИКА ПРАВИЛА ЖИВЁТ В КОНТРАКТАХ, А ЕГО SQL — НА СЕРВЕРЕ (Р4). Здесь стоят те части, о
// которых спорят портал, сервер и тесты: какие статусы считаются терминальными, какой вид заявки
// вообще участвует и что означает нулевое окно. Сам предикат собрать этими константами нельзя и не
// нужно — он один на четырёх потребителей (список, карточка, `repeat=true`, `repeatFor`) и живёт в
// `apps/api/src/services/service-request-repeat.ts`: тащить в общий пакет drizzle и схему БД ради
// одного условия значило бы отдать порталу зависимость, которой он не пользуется.

/**
 * ПРЕДЫДУЩАЯ ЗАЯВКА СЧИТАЕТСЯ ЗАКРЫТОЙ В ЭТИХ ДВУХ СТАТУСАХ (Р1, условие 3).
 *
 * Отменённая входит НАРЯДУ с принятой (Р11, развилка В3): «завели, отменили, завели снова» — это и
 * есть обращение второй раз, а причина отмены («дубль», «не подтвердилось») читается в самой
 * заявке. Сузить перечень до одной `accepted` — решение заказчика, а не правка кода: и предикат, и
 * условие частичного индекса `service_requests_repeat_idx` собраны отсюда, и разъехаться им нечем.
 *
 * «Решена» (`done`) сюда НЕ входит намеренно: работы закрыты, но заявку ещё принимают, и повтором
 * по ней считалась бы заявка, заведённая до того, как первую вообще досмотрели.
 */
export const SERVICE_REQUEST_REPEAT_TERMINAL_STATUSES = [
  'accepted',
  'cancelled',
] as const satisfies readonly ServiceRequestStatus[];

export type ServiceRequestRepeatTerminalStatus =
  (typeof SERVICE_REQUEST_REPEAT_TERMINAL_STATUSES)[number];

/**
 * ПРИЗНАК СЧИТАЕТСЯ ТОЛЬКО У РЕМОНТА, И СРАВНИВАЕТСЯ РЕМОНТ С РЕМОНТОМ (Р3).
 *
 * Повторная заявка на расходники — норма, а не сигнал: картридж кончается регулярно, и «Повтор ×5»
 * у принтера, которому пять раз меняли тонер, обесценило бы саму метку. Частота расхода — другой
 * вопрос, и отвечает на него отчёт по расходникам.
 *
 * Значением, а не переписанной строкой `'repair'` в трёх местах: тем же самым видом собраны и
 * условие индекса, и обе половины предиката (`P.kind = R.kind = 'repair'`).
 */
export const SERVICE_REQUEST_REPEAT_KIND = 'repair' as const satisfies ServiceRequestKind;

/**
 * ОКНО ВЫКЛЮЧЕНО. Ноль — не «окно нулевой длины», а рубильник признака целиком (Р5): при нём
 * сервер не ходит в базу вовсе, поля `repeat` в ответе нет ни у одной строки, а отбор
 * `repeat=true` отвечает пустой выдачей.
 *
 * Это же значение — кодовое умолчание `SERVICE_REQUEST_REPEAT_WINDOW_DAYS`: порог берут из данных
 * пилота (§8), и отсутствие переменной окружения не вправе включить эвристику само собой.
 */
export const SERVICE_REQUEST_REPEAT_WINDOW_DISABLED = 0;

/**
 * Признак повторного обращения — то, что портал рисует меткой «Повтор ×N» (Р10).
 *
 * ЧИСЛО ЗАВИСИТ ОТ ЧИТАТЕЛЯ (Р8): у оператора по той же заявке бывает «×3», у заявителя «×1» —
 * совпадения считаются в области видимости смотрящего. Это не расхождение, а то же правило, по
 * которому у них разные списки; главное свойство признака — число совпадает с тем, что человек
 * увидит по ссылке.
 */
export interface ServiceRequestRepeatDto {
  /** Сколько закрытых и отменённых ремонтов по тому же аппарату попало в окно. Ноль — метки нет. */
  count: number;
  /**
   * Окно, по которому считали, — днями. Приходит с каждым признаком не ради красоты: подпись
   * обязана называть период («2 заявки за 30 дней»), а не «недавно», иначе метку нечем оспорить.
   */
  windowDays: number;
  /** Последнее из совпадений — для подсказки «последний раз чинили 12.08.2026». */
  lastAt: string | null;
}

/**
 * ПРИМЕНИМ ЛИ ПРИЗНАК К ЭТОЙ ЗАЯВКЕ ВООБЩЕ — до всякого похода в базу и одинаково у всех
 * потребителей.
 *
 * Три условия из Р1, которые видны в самой строке: окно включено, заявка ремонтная (Р3) и предмет
 * у неё назван карточкой парка. Заявке без аппарата признака нет вовсе — сравнивать не с чем, а
 * `NULL = NULL` в SQL и так ложен: явное условие здесь стоит затем, чтобы такая строка не занимала
 * места в пакетном запросе и чтобы правило читалось словами, а не выводилось из свойств `NULL`.
 *
 * Функцией в контрактах, а не тремя проверками на сервере: тот же вопрос задаёт портал, решая,
 * рисовать ли пустое место под метку.
 */
export function serviceRequestRepeatApplies(
  row: { kind: ServiceRequestKind; officeEquipmentId: string | null },
  windowDays: number,
): boolean {
  return (
    windowDays > SERVICE_REQUEST_REPEAT_WINDOW_DISABLED &&
    row.kind === SERVICE_REQUEST_REPEAT_KIND &&
...
export interface ServiceRequestDto {
  ...
  /**
   * ВИДНА ЛИ ЗАЯВКА ЧИТАТЕЛЮ СО СТОРОНЫ ЗАКАЗЧИКА — по оси его роли (его площадка, его отдел), а
   * не по сквозной области набора (`inServiceRequestCustomerScope`; находка Н8, решение заказчика
   * 04.09.2026). Поле отвечает не про заявку, а про читателя — как `audience` и стороны
   * обсуждения рядом.
   *
   * ЗАЧЕМ ОНО ПОРТАЛУ. Правку и удаление открывает `canChangeRequestAsCustomer`, и второй его
   * признак — эта самая сторона. Вывести её портал не может ничем: `ServiceRequestDto` не
   * раскрывает `createdBy`, а сложить ответ самому — значит завести на клиенте вторую копию
   * правила области, которая разойдётся с серверной молча. Без поля пункты меню пропадали бы у
   * сисадмина на заявке ЕГО ЖЕ площадки, где сервер их разрешает, — расхождение, которое §11
   * плана считает дефектом строки наравне с обещанием несуществующего действия.
   *
   * Обязательное, а не `?`: значение считает сервер на каждом ответе, и `undefined` здесь означал
   * бы строку, про сторону которой никто не решил, — то есть тихое «нет».
   */
  inCustomerScope: boolean;
  id: string;
  num: number;
  ...
  /**
   * ПОВТОРНОЕ ОБРАЩЕНИЕ ПО ТОМУ ЖЕ АППАРАТУ (§6 плана) — наблюдение, а не основание: гарантийным
   * обращением оно не становится и никаких действий не запускает (Р9). У заявки бывают обе пометки
   * сразу, и это честно.
   *
   * НЕОБЯЗАТЕЛЬНОЕ, И РАЗНИЦА СУЩЕСТВЕННА ДЛЯ ПОРТАЛА. Отсутствие поля означает «признак к этой
   * строке не применяется» — окно выключено (`W = 0`), заявка на расходники или заявка без
   * аппарата (`serviceRequestRepeatApplies`); тега нет вовсе. `count: 0` означает другое —
   * «считали, повторов не нашли»: тега тоже нет, но признак работает, и отбор «только повторные»
   * человеку показывать можно.
   *
   * СЧИТАЕТСЯ ПАКЕТНО НА СТРАНИЦУ, А НЕ КОЛОНКОЙ ЗАПРОСА (Р6, Н6): окно у каждой строки своё —
   * оно отсчитывается от её собственного `createdAt`, — поэтому группировка по аппарату здесь
   * неверна, а коррелированное поле в списке `SELECT` drizzle молча переписывает. Сборка зовёт
   * `serviceRequestRepeatByRequest` одним запросом на всю страницу.
   */
...
export const SERVICE_REQUEST_PERMISSIONS: readonly Permission[] = [
  ...
  // Раскладка объёма работ по графам — своя дверь у «Ведения» (план
  // `docs/office-equipment-free-estimate-and-executor-scope-plan.md`, Р2), а не оговорка к
  // `estimate`: порядок здесь повторяет словарь `PERMISSIONS`, и страж модуля сверяет оба перечня
  // строка в строку.
...
export const SERVICE_REQUEST_FIELD_AUDIENCE = {
  ...
  /*
   * Сторона заказчика этого читателя — не деньги и не кухня: она отвечает «моя ли это заявка по
   * оси моей роли», и урезанная аудитория задаёт тот же вопрос, что полная. Спрятать её от
   * `requester` значило бы погасить его собственные кнопки правки и удаления — карта режет
   * финансовую сторону заявки, а не права читателя (Г4).
   */
  inCustomerScope: 'all',
  id: 'all',
  num: 'all',
  ...
  warrantyClaim: 'all',
  /*
   * Признак повтора — не деньги заявки, а факт о её предмете, и заявителю он не открывает ничего:
   * совпадения посчитаны В ЕГО СОБСТВЕННОЙ области видимости (Р8), то есть по заявкам, которые он
   * и так видит списком. Вычитать здесь нечего.
   *
   * Кому метку ПОКАЗЫВАТЬ — вопрос развилки В4, и решается он не этой картой: карта режет готовое
   * DTO, а «не считать признак для такого-то читателя» — решение сборки. Поставь мы здесь
   * `{ requester: undefined }`, заявитель получал бы поле, вычтенное после того, как сервер его
   * посчитал, — то есть цену расчёта без единого его следа на экране.
   */
  repeat: 'all',
