...
export interface AuthUser extends PersonNameParts {
  ...
  /**
   * Контрагент учётки (ADR 0010) — та самая организация, от лица которой человек работает.
   *
   * Приезжает рядом с типом, потому что тип отвечает только на «в каком модуле», а порталу нужен и
   * второй вопрос — «эта запись отдана МОЕЙ компании?». Без идентификатора он отвечал на него
   * приближением («оператор какой-то сервисной компании», Н7 плана аудита исполнителей), и
   * приближение это держалось не собой, а соседним правилом области видимости.
   *
   * Ни правом, ни областью поле не является: доступ по-прежнему решает сервер на своём принципале,
   * где такой же `counterpartyId` читается из базы на каждом запросе. Здесь оно нужно ровно затем,
   * чтобы портал не показывал ход по заявке, назначенной не его компании, — то есть чтобы кнопка
   * не вела в 403.
   *
   * Идентификатор своей же организации — не персональные данные и не чужая запись: учётка и так
   * видит название своего контрагента в каждой своей заявке.
   */
  ...
  addons: RoleAddon[];
  /**
   * Коды назначенных наборов (ADR 0106) — то, ЧТО учётке выдано.
   *
   * Зачем поле, если рядом лежат и права, и надстройки. Портал узнаёт по нему БИЗНЕС-ПРОФИЛЬ
   * модуля (план профилей оргтехники, Р9): сторона обсуждения «Ведение» и «Системный
   * администратор» опознаётся кодом набора, а не суммой прав, — иначе профиль выводился бы из
   * состава чужого собранного набора, а уборка мёртвого `serviceRequests.approveIt` погасила бы
   * сторону молча. Сегодня тот же ответ давали бы `addons`, но надстройки уходят на шаге 1e ADR
   * 0106, и всё, что решает по ним, обязано переехать на коды заранее.
   *
   * Правом поле не является и в `can` не участвует (`AccessSubject.grantCodes`): состав набора
   * живёт в базе, и второй путь к тому же ответу разошёлся бы с первым молча. Строки, а не
   * `SystemGrantCode`: рядом с системными приходят наборы, собранные администратором.
   *
   * Приходит во всех четырёх ответах, устанавливающих сессию (`login`, `refresh`, `/auth/me`,
   * смена пароля) — тем же сборщиком и по той же причине, что список прав ниже.
   */
  grantCodes: string[];
  /**
  ...
  /**
   * **Права назначенных наборов** (ADR 0106) — четвёртый источник субъекта доступа, тот самый, что
   * у принципала на сервере (`Principal.grantPermissions`).
   *
   * ЗАЧЕМ ОТДЕЛЬНО ОТ `permissions`, которые их уже включают. Поля отвечают на разные вопросы и
   * читаются разными механизмами. `permissions` — готовый ответ «что человеку можно», им работает
   * `can` контекста портала. А общие предикаты контрактов — те, что «одна функция на сервер и
   * портал» (`isServiceExecutor`, `allowedServiceStatusTransitions`, `participantSidesOf`) — берут
   * не список, а СУБЪЕКТ (`AccessSubject`) и складывают его четыре источника сами. Субъект без
   * этого поля обрезан: роль и пометки о наборах не рассказывают, а `grantCodes` — это коды, не
   * права.
   *
   * ЧТО БЫЛО БЕЗ НЕГО. Пока `serviceRequests.execute` приходил надстройкой, обрезанный субъект
   * случайно совпадал с полным: надстройка в `AuthUser` есть. Разделение набора ИТ-службы (план
   * `docs/office-equipment-access-profiles-plan.md`, Э6) это совпадение сняло — работа исполнителем
   * уехала в модульный набор `office_equipment_executor`, надстройки у которого нет и не будет, — и
   * портал перестал бы показывать своему сисадмину ходы исполнителя и подшивку документов, при том
   * что сервер их разрешает. Совпадение держалось и у всех прочих наборов каталога (аудит талонов,
   * «Архивариус»): просто их права портал спрашивал только через `can` контекста.
   *
