...
export const PERMISSION_CATALOG: Record<Permission, PermissionCatalogEntry> = {
   ...
   * «Удаляет чек…» рядом с «Ведёт справочник…» читается как другое действие с первого взгляда, а
   * «Управляет чеками» слилась бы с ведением. Читают эту подпись двое: карточка доступа учётки
