...
/**
 * Ответ правки техники — `PATCH /vehicles/:id`.
 *
 * Обёрткой, а не голым `VehicleDto`, потому что у правки есть **второй исход**: перевод машины в
 * «Списана» и смена её типа на бланк «форма № 3» снимают привязки закреплённых прицепов
 * (план `docs/vehicle-trailers-plan.md`, §4.2.3). Это изменение в базе, которого человек не
 * просил, и §7 обещает прямо: «списание машины или прицепа снимает привязку и говорит, сколько
 * сняло». Промолчи ответ — и диспетчер, списавший тягач, узнает об отцепленном полуприцепе из
 * чужой жалобы.
 *
 * Поля «сколько отцепили» нет и не будет в самой карточке: число снятых привязок — про
 * **операцию**, а не про машину. В `VehicleDto` оно жило бы ложью — читалось бы у каждой строки
 * списка, где никто ничего не снимал. Тем же приёмом разведены исход и карточка у соседей:
 * `UserMutationResult` (`{ user, notified }`) и `HitchTrailerResultDto` (`{ trailer, notice }`).
 *
 * Число, а не фраза (в отличие от `HitchTrailerResultDto.notice`): вытесненного жильца сервер
 * обязан **назвать** — его реквизиты есть только у него, — а здесь говорить нечего сверх счёта,
 * и собрать «отцеплено 2 прицепа» портал может сам. Имя поля то же, что у ответа архивации
 * (`DeleteVehicleResult`): одно событие — одно имя, иначе две двери одного правила разойдутся
 * при первой правке.
 */
export interface UpdateVehicleResult {
  /** Машина после правки — та же карточка, что приезжала голым ответом раньше. */
  vehicle: VehicleDto;
  /** Сколько прицепов сошло с машины этой правкой. `0` — ни один, и говорить не о чем. */
  unhitchedTrailers: number;
}

/**
 * Ответ мягкого удаления техники — `DELETE /vehicles/:id`, кнопка «В архив».
 *
 * Третье событие таблицы плана `docs/vehicle-trailers-plan.md` §4.2.3 (два первых — в
 * `UpdateVehicleResult`): уход машины в архив тоже снимает привязки закреплённых прицепов. Случай
 * опаснее списания: удалённая машина исчезает из списков совсем, и оставленная привязка называла
 * бы тягача, которого в портале уже нет. Обещание §7 у обеих дверей одно — «снимает привязку и
 * говорит, сколько сняло», — и молчать архивации не разрешено ровно так же, как правке.
 *
 * Объявлено типом, а не оставлено лишним полем в `{ ok: true }`: пока формы нет в контрактах,
 * сервер число возвращает, а портал о нём не знает — то самое состояние, в котором обещание §7
 * не выполнено, хотя снятие привязок написано верно. Необъявленный ответ немой: его не читают.
 *
 * Карточки здесь нет и не нужно — удалённую запись списки не показывают, — поэтому число стоит
 * рядом с признаком успеха, а не в обёртке при `VehicleDto`. Но причина та же, что у
 * `UpdateVehicleResult`: сколько отцепили — про **операцию**, а не про машину; в `VehicleDto` это
 * поле читалось бы у каждой строки списка, где никто ничего не снимал.
 *
 * `ok` литералом `true` (как у `RejectUserResult`): отказ эта дверь отдаёт ошибкой HTTP, а не
 * телом с `ok: false`, и придумывать порталу несуществующую ветку разбора незачем.
 */
export interface DeleteVehicleResult {
  /** Всегда `true`: неуспех приезжает ошибкой, а не этим телом. */
  ok: true;
  /** Сколько прицепов сошло с машины её уходом в архив. `0` — ни один, и говорить не о чем. */
  unhitchedTrailers: number;
}
...
/**
 * Как машина называется **в строке подбора** — в фильтрах списков и выпадающих списках выбора
 * (ADR 0098). От `vehicleLabel` отличается вторым реквизитом: там, где нужно коротко назвать уже
 * выбранную машину, хватает одного госномера, а выбирают её двумя приметами сразу — «А123БВ797 —
 * КамАЗ 65115». Госномер помнят не для всякой машины, марку — не для всякого госномера, и список
 * из одних номеров заставляет сверяться со справочником в соседней вкладке.
 *
 * Пара та же, какой техника представлена в справочнике: у своей машины — «Госномер» и
 * «Марка/модель», у аренды — «Описание» и «Арендодатель» (у неё госномера нет вовсе, ADR 0018).
 * Отсутствующий реквизит не оставляет пустой половины и не рисует прочерк: строка сокращается до
 * того, что есть, а если нет ни одного — до `vehicleLabel`, где следом идут категория и тип.
 *
 * Порядок реквизитов — он же порядок ввода при поиске: подстрочный поиск по этой строке находит
 * машину и по началу госномера, и по марке.
 */
export function vehicleOptionLabel(v: {
  ...
  const parts =
    v.ownership === 'rental'
      ? [v.description || null, v.lessorName ?? null]
      : [v.registrationNumber, v.modelName];
  const shown = parts.filter((p): p is string => !!p && p.trim() !== '');
  return shown.length > 0 ? shown.join(' — ') : vehicleLabel(v);
}
