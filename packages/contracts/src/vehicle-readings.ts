 ...
 * 3. **Топлива три числа, и они разной природы** (ADR 0163). `fuelStartLiters` и `fuelEndLiters` —
 *    УРОВЕНЬ в баке на концах смены: складывать их за период бессмысленно, поэтому в сводках,
 *    месячных агрегатах и матричных книгах их нет. `fuelFilledLiters` — ПОТОК, заправленное за
 *    смену, и суммируется только он.
 * 4. **Расход считается — но только там, где его есть с чем сверить.** Решение 12 ADR 0103
 *    («расхода портал не считает») снято заказчиком вместе с приходом норм расхода топлива
 *    (`docs/fuel-norms-plan.md` §1.1): расход появился в портале не как показатель сам по себе, а
 *    как половина сверки с нормой, и в колонку идут только те смены, которые сверке поддаются —
 *    с обоими остатками, непрерывной парой снимков и положительной базой. Слива портал не знает до
 *    сих пор, поэтому формула считается без него, и это оговорено в ADR волны.
 */
...
/**
 * Остатки в баке приходят `optional()` **без** `default(null)`, и это договор провода, а не
 * небрежность (ADR 0163, Р13). Отсутствующий ключ означает «оставить как есть», явный `null` —
 * «очистить»; различить их обязательно, потому что показание отправляется целиком, и вкладка
 * старой сборки, не знающая этих полей, иначе стирала бы остатки при всякой правке одометра.
 *
 * Второй довод — идемпотентность. Отпечаток тела считается уже после разбора, канонизатором,
 * который пропускает `undefined` (`services/readings.ts`); дай этим полям `default(null)`, и повтор
 * потерянного запроса, отправленного до выката, получил бы другой отпечаток — то есть 409 вместо
 * ответа на собственный ретрай.
 *
 * Три старых поля остаются с `default(null)`: очистку счётчика все живые клиенты выражают явным
 * `null`, сборок, не знающих одометра, не существует, и переучивать их протокол незачем.
 */
const fuelLevel = readingNumber.nullable().optional();
...
export const readingValuesSchema = z
  .object({
    ...
    /** Остаток в баке на НАЧАЛО смены (ADR 0163, Р1): уровень, а не поток — за период не суммируется. */
    fuelStartLiters: fuelLevel,
    fuelFilledLiters: readingNumber.nullable().default(null),
    /** Остаток в баке на КОНЕЦ смены. Необязателен наравне с началом (Р2). */
    fuelEndLiters: fuelLevel,
    comment: z.string().trim().max(500).default(''),
  })
  .superRefine((v, ctx) => {
    // Правило считает ПРИСУТСТВУЮЩИЕ числа: для него отсутствующий ключ и `null` равнозначны —
    // числа нет. Различаются они позже, при разрешении правки, а не здесь (Р13).
    const filled =
      v.odometerKm !== null ||
      v.engineHours !== null ||
      v.fuelFilledLiters !== null ||
      (v.fuelStartLiters ?? null) !== null ||
      (v.fuelEndLiters ?? null) !== null;
    if (!filled) {
      ctx.addIssue({
        code: 'custom',
...
export interface VehicleReadingDto {
  ...
  /**
   * Топливо в баке на начало смены, литры (ADR 0163). В ответе поле обязательно и всегда равно
   * числу или `null` — необязательность живёт только на вводе, где отсутствие ключа значит
   * «оставить как есть».
   */
  fuelStartLiters: number | null;
  /** Заправлено ЗА СМЕНУ: поток, единственное из трёх чисел топлива, которое можно складывать. */
  fuelFilledLiters: number | null;
  /** Топливо в баке на конец смены, литры. */
...
export interface VehicleReadingStatsRow {
  ...
  /** Сверка с нормой — те же четыре аддитивных числа, что у `ReadingTotals`. */
  fuelSpentLiters: number;
  fuelNormLiters: number;
  verifiedShifts: number;
  shiftsWithFuel: number;
  /**
   * У машины есть живая норма, действующая хотя бы на один день периода (Р15а).
   *
   * Признак нужен, чтобы различить два одинаковых с виду прочерка: «норму не заводили» — вопрос к
   * тому, кто ведёт справочник, «нормы есть, но остатков не сдавали» — к тому, кто принимает
   * показания. В `ReadingTotals` его нет и быть не может: булево не складывается по месяцам.
   */
  hasNorm: boolean;
}
...
export interface IntakeIssue {
  /**
   * Что случилось: `overdue`, `waiting`, `no_data`, `shift_tail_missing` (у показания нет ни
   * одного числа конца смены), `anomaly_*` либо вид расхождения.
   */
  code: string;
  level: 'yellow' | 'red';
  message: string;
...
export interface ReadingTotals {
  ...
  unacceptedShifts: number;
  /**
   * Сверка с нормой расхода — четыре числа, и все четыре **аддитивны** (`docs/fuel-norms-plan.md`,
   * Р12а). Отклонения, процента и признака превышения здесь нет намеренно: процент, сложенный по
   * месяцам, даёт чушь, поэтому он считается из этой пары на каждом уровне отдельно — общей
   * функцией `fuelDeviation`.
   *
   * `fuelSpentLiters` — расход СВЕРЯЕМЫХ смен, а не всех: колонка на экране обязана сходиться с
   * нормой и отклонением, стоящими рядом. Полный расход по всем сменам с остатками живёт в
   * служебной книге, где он построчный.
   *
   * Ноль здесь — не то же, что прочерк на экране: числа обязаны складываться, поэтому месяц без
   * сверяемых смен хранит нули, а печатается прочерком (правило печати, не хранения).
   */
  fuelSpentLiters: number;
  fuelNormLiters: number;
  /** Охват: сколько смен прошло сверку и сколько всего дало расход (знаменатель, §3.4). */
  verifiedShifts: number;
  shiftsWithFuel: number;
}
...
export interface VehicleReadingCardDto {
  ...
  lastEngineHours: { value: number; measuredOn: string } | null;
  /** См. `VehicleReadingStatsRow.hasNorm`: тот же признак, тот же вопрос. */
  hasNorm: boolean;
  /**
   * Допуск сверки, проценты. Едет в ответе вместе с числами (Р12б), а не спрашивается отдельной
   * ручкой: настройки закрыты правом справочников, а карточку открывает право показаний — и без
   * этого числа она не смогла бы ни покрасить превышение, ни объяснить, из чего оно вышло.
   */
