 ...
 * 4. **Расхода портал по-прежнему не считает** — ни колонкой, ни производной «на сто километров».
 *    Основание у запрета с приходом остатков сменилось, и это записано в ADR 0163: раньше расход
 *    был невычислим (`начало + заправлено − конец − слив`, а слагаемых не было), теперь вычислим,
 *    но не заказан. Показатель, поделённый на пробег, читается как расход независимо от подписи, а
 *    слива в данных нет до сих пор.
 */
