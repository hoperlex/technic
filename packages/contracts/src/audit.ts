...
// ── Срез журнала ──
//
// Журнал в портале один (ADR 0088), а читают его срезами: подвкладка «Пользователи → Аудит»
// спрашивает, что происходило с учётными записями, вкладка «Права» — что происходило с каталогом
// полномочий. Срез задаёт **сервер**, а не набор галочек в фильтре: пустой набор означает «фильтра
// нет», и ручка без границы отдавала бы весь `audit_log` — заявки, технику, путевые листы,
// справочники и тысячи входов, среди которых десяток административных событий не найти.

/**
 * Действия, **цель которых — учётная запись**: перечень событий самой учётки плюс выдача и отзыв
 * полномочия. Последние два пишутся на учётку (`entityType: 'user'`, ADR 0106) и отвечают на тот же
 * вопрос разбора — что меняли у этого человека, — поэтому в её ленте им место.
 *
 * Отдельно от `USER_AUDIT_ACTIONS`: тот перечень отвечает на другой вопрос — что портал считает
 * событием самой учётки, — и слить их значило бы переписать реестр каталога полномочий задним
 * числом.
 */
export const USER_TARGET_AUDIT_ACTIONS = [
  ...USER_AUDIT_ACTIONS,
  'grant.assign',
  'grant.revoke',
] as const satisfies readonly AuditAction[];
export type UserTargetAuditAction = (typeof USER_TARGET_AUDIT_ACTIONS)[number];

/** Срезы журнала: учётки, каталог полномочий, весь реестр. */
export const AUDIT_SCOPES = ['user', 'grant', 'all'] as const;
export type AuditScope = (typeof AUDIT_SCOPES)[number];

/**
 * Действия среза. `all` — это весь закрытый **реестр**, а не весь `audit_log`: события заявок,
 * техники, справочников и входов ручка журнала не отдаёт ни в одном срезе. У них другой читатель —
 * история своей карточки (`services/*-history.ts`), — и описателя строки у них здесь нет.
 *
 * Выдача и отзыв полномочия попадают сразу в два среза, и это не оплошность: одно событие отвечает
 * на два вопроса — «что меняли у этого человека» и «кому раздавали это полномочие».
 */
export function auditScopeActions(scope: AuditScope): readonly AuditAction[] {
  if (scope === 'user') return USER_TARGET_AUDIT_ACTIONS;
  if (scope === 'grant') return GRANT_AUDIT_ACTIONS;
 ...
 *
 * Наборы (объекты, отделы, надстройки, полномочия) занимают по два поля — «добавлены» и «сняты», а
 * не одну пару «было → стало»: составы по десять площадок в двух колонках сравнивать глазами
 * невозможно, а вопрос у читателя всегда про разницу — что человеку открыли и что закрыли.
 *
 * `grantsGranted`/`grantsRevoked` стоят рядом с надстройками, а не в конце перечня: полномочия
 * назначаются тем же окном учётки и заняли в форме место надстроек (Р11 плана
 * `docs/account-form-grants-plan.md`), поэтому и в событии читаются на их месте.
 *
 * Пока поле `addons` живо (до шага 1e того же плана), правка **системного** набора даёт в событии
 * сразу две строки — «Надстройки выданы» и «Полномочия выданы», — где первая называет надстройку
 * её подписью, а вторая тот же набор его названием из каталога. Это одно решение администратора,
 * записанное двумя способами: две системные надстройки и есть наборы полномочий (ADR 0106). Двумя
 * разными операциями пара строк не является и станет одной, когда `addons` уйдёт.
 *
...
export const userAuditFieldLabels: Record<UserAuditField, string> = {
  ...
  // Тем же словом, каким выдачу и отзыв называют события каталога («Полномочие выдано»,
  // «Полномочие отозвано»): строки лежат в одной ленте, и третьего названия одной операции у
  // журнала быть не должно.
  grantsGranted: 'Полномочия выданы',
  grantsRevoked: 'Полномочия отозваны',
  person: 'Работник',
...
const fieldSummaries: Record<UserAuditField, string> = {
  ...
  grantsGranted: 'полномочия',
  grantsRevoked: 'полномочия',
  person: 'работник',
  scope: 'объекты и отделы',
  addons: 'надстройки доступа',
};
...
export const auditQuerySchema = baseListQuery(AUDIT_SORT_FIELDS).extend({
  /**
   * Срез журнала; умолчание — учётки. Умолчание живёт в схеме, а не в маршруте: границу видит и
   * клиент, и будущий экспорт, а запрос без параметра не может случайно попросить весь журнал.
   *
   * От `actions` отличается тем, чем граница отличается от удобства: набор действий — это галочки
   * читателя внутри среза, и снятые галочки означают «весь срез», а не «весь `audit_log`».
   */
  scope: z.enum(AUDIT_SCOPES).default('user'),
