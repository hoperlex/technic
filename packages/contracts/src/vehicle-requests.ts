 ...
 *
 * Отдел-заказчик бывает только у грузоперевозки (CHECK `vehicle_requests_department_freight_check`,
 * миграция 0069): спецтехника выходит на площадку, а площадка — это объект, и заказчиком у неё
 * стоит он. Запрет о КОЛОНКЕ, а не о роли: сама роль отдела спецтехнику заказывает (ADR 0201) —
 * от имени своей закреплённой площадки, то есть заявкой, у которой в заказчиках объект.
 */
...
export interface VehicleRequestDriverDto {
   ...
   * День, когда карточку человека сняли из справочника; `null` — карточка жива (ADR 0190).
   *
   * Работу это не отменяет: человек на машине остался, бумага за отработанное ему годится. Но
   * карточка заказа обязана сказать это вслух — по такому заказу выпишется ещё один бланк строгой
   * отчётности на удалённого, и узнать об этом лучше здесь, чем в бухгалтерии заказчика.
   */
...
export interface VehicleRequestBaseDto {
  ...
  /**
   * Заявка стоит в действующем листе любым способом — грузовой строкой, днём или перегоном.
   *
   * Поле заведено ради одной двери: возврат заявки в «Новую» стирает работу, и заявке, попавшей в
   * выданный бланк, сервер этого не даёт (`ROLLBACK_WAYBILL_MESSAGE`). Считает он при этом ровно
   * так же — все три способа попасть в бумагу (`activeWaybillOfRequest`), — а портал до сих пор
   * спрашивал `route.hasWaybill`, то есть лист СВОЕГО рейса. У заказа техники на объект такого
   * рейса не бывает вовсе: его листы висят на рейсах дней, `route` пуст, и признак всегда отвечал
   * «листа нет». Окно отката открывалось, человек заполнял причину — и упирался в 409 сервера.
   *
   * `route.hasWaybill` поле не заменяет и не дублирует: тот отвечает «заморожен ли ЭТОТ рейс»
   * (состав, водитель, реквизиты), а этот — «есть ли у заявки хоть одна действующая бумага».
   * Вопросы разные, и сводить их в один признак значило бы снова получить два ответа на месте
   * одного.
   *
   * Аннулированный лист не считается: испорченный бланк списан, и работа заявки к нему больше не
   * относится. ЭСМ-2 не считается тоже — он привязан к самой заявке, а не к рейсу, и откату не
   * мешает (сверка `syncEsm2Waybills` аннулирует его вместе с работой).
   */
  hasActiveWaybill: boolean;
  files: FileDto[];
  version: number;
...
export interface SpecialEquipmentRequestDto extends VehicleRequestBaseDto {
   ...
   *
   * Читается это как «как заявка ведётся по умолчанию», а не «бывают ли у неё дни»: дни и дневные
   * 4-П признак больше не воротит (`linearDaysBlocker`, ADR 0207 §1) — их ведут у любого заказа
   * техники на объект. За признаком осталась ровно одна развилка — сам ли портал выписывает
   * недельные ЭСМ-2, — и поле показывают из-за неё.
   *
  ...
  /**
   * Машина дня в срезе «На объекте» (ADR 0100 §12). Поля нет вовсе, когда выдача не про конкретный
   * день (список, лента, журнал): там машина строки — назначенная, и второго ответа на этот вопрос
   * не существует. `null` — день среза не распланирован: показывать нечего, и говорится об этом
   * словами (`ON_SITE_DAY_UNPLANNED_MESSAGE`), а не машиной по умолчанию.
   *
   * Линейность здесь больше не условие: дни ведутся у любого заказа техники на объект, и обещание
   * «только у линейного» стало бы ложью на первом же нелинейном заказе с выписанным 4-П.
   */
   ...
   * только при отборе по машине — и у любого заказа техники на объект, а не только у линейного:
   * день с чужой машиной бывает теперь у каждого из них. У грузоперевозки дней нет вовсе, и
   * отвечать там нечем.
   *
