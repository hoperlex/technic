...
// смены, машиночасы, заправка и подпись объекта, что этот день был именно таким. Без подписи по
// всем прошедшим дням заявка не закрывается, а сама заявка не уходит из среза «На объекте».
//
 ...
 * — запрет закрытия, запрет смены машины и предупреждение в срезе. Конкретные даты нужны только
 * в тексте отказа и в самой таблице смен, и там их отдаёт сервер.
 */
export interface VehicleRequestShiftsSummaryDto {
  approvedDays: number;
  /** Дни заказа, которые уже наступили, но не подтверждены. Их и не хватает для закрытия. */
...
/**
 * Почему заявку нельзя завершить: остались дни, за которые никто не расписался. Работу принимает
 * объект — по дням, а не одним итогом в момент закрытия, — и незакрытый день означает, что о
 * машиночасах этого дня ещё не договорились.
 *
 * Следствие, принятое сознательно: заявку с неистёкшим сроком закрыть нельзя — будущие дни не
 * согласуешь. Сначала срок сокращают досрочным завершением (ADR 0044), и только потом закрывают.
 */
export function shiftsCompletionBlocker(r: ShiftSubject): string | null {
  if (r.requestType !== 'special_equipment') return null;
  const pending = r.shifts?.unapprovedPastDays ?? 0;
  if (pending <= 0) return null;
  return `${pendingDaysLabel(pending)} без согласования — заявку нельзя завершить`;
}
