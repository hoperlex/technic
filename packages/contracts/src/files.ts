...
export interface FileDto {
  ...
  createdAt: string;
  /**
   * Файл поставлен в КАРАНТИН (Р6, п. 4 плана освобождения от подписи): аварийный выход для
   * ошибочно загруженного секретного или чужого документа. Содержимое закрыто всем, включая автора,
   * и открыто только по отдельному праву разбора инцидента.
   *
   * У такого вложения `filename` — ПУСТАЯ СТРОКА, и признак здесь затем, чтобы экран отличил
   * «имя скрыто по обращению» от «имени нет». Слово-заглушка вместо имени не годится: в списке
   * файлов она читается как имя файла. Сама строка вложения остаётся — «доказательство скрыто» и
   * «доказательства не было» — разные факты.
   *
   * Признака НЕТ у обычного файла: его отсутствие и означает «не в карантине». Сервер собирает пару
   * «имя + признак» одной функцией на все модули (`services/file-view.ts`), потому что девять
   * сборщиков вложений с собственной проверкой карантина разошлись бы на первой же новой ручке.
   */
  quarantined?: boolean;
}
