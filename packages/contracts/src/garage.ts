 ...
 *
 * Строки рейса при этом мало: занятость даёт рейс, по которому **есть куда ехать** (ADR 0131) —
 * перегон, рейс с непустым составом либо рейс с действующим листом. Грузовой рейс без состава и без
 * листа это заготовка: запись заведена утром, заявки положат позже, — и в срезе дня её нет вовсе,
 * ни занятостью, ни состоянием, ни цифрой сводки. Правило считает сервер (`routeHasWork` в
 * `apps/api/src/services/garage.ts`): отбор по загруженной странице врал бы и в счётчике, и в
 * листании.
 *
...
export interface GarageVehiclesSummaryDto {
  ...
  unavailable: number;
  /**
   * Рейсы этого дня **с работой**, в которых ещё нет водителя. Цифра, ради которой сводку и
   * открывают утром: без водителя лист не выписать, а по состоянию машины этого не видно — она уже
   * «в рейсе».
   *
   * Заготовки (ADR 0131) сюда не входят по тому же правилу, по которому их нет и в занятости:
   * искать водителя рейсу, в котором ни одной заявки, портал не зовёт.
   */
