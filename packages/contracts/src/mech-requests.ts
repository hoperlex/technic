...
// ── Предмет аренды: модель из справочника (Р5, ADR 0156) ──

/*
 * Здесь стояли схема свободной строки, её нормализованный ключ и подсказка ранее введённого
 * (`GET /mech-requests/kinds`). Всё три сняты этапом Э2: вид техники перестал набираться словами и
 * стал ссылкой на справочник моделей (`mechModelId` в теле заведения и правки, миграция 0251).
 *
 * Подсказка не заменена «поиском по справочнику», а именно удалена: она отвечала на вопрос «что уже
 * набирали в этой области», а справочник отвечает «что вообще можно взять», и вторая ручка рядом с
 * первой означала бы, что строгий выбор всё-таки не строгий (ADR 0156, решение 2).
 *
 * Написание заявки (`kindName`) держалось в DTO ещё один этап — им показывалась заявка, которая
 * своей модели в справочнике не нашла. Этап Э3 снял и его: заказчик, которого спросили про такие
 * заявки, выбрал СНЯТЬ КОЛОНКУ, потеряв написания (миграция 0256). Предмет аренды теперь ровно
 * один — `mechModelName` по ссылке, — а у заявки без ссылки названия техники нет вовсе.
 */
...
export const MECH_REQUEST_SORT_FIELDS = [
  ...
  'objectName',
  /**
   * Столбец «Модель». Ключ остался прежним и после того, как написание ушло из схемы совсем (Э3,
   * миграция 0256): это идентификатор СТОЛБЦА, а не имя колонки базы, и переименуй его — открытая
   * вкладка старой сборки, отправившая `sortBy=kindName`, получала бы 400 на сортировку, которую
   * сама же и предлагает. Тот же довод, по которому остался ключ события истории. Сервер
   * сортирует по наименованию модели; заявка без ссылки уезжает в конец — сортировать у неё нечего.
   */
  'kindName',
...
export const mechRequestListQuerySchema = baseListQuery(MECH_REQUEST_SORT_FIELDS).extend({
  ...
  /**
   * Модель — ссылкой на справочник (Р5, ADR 0156). Прежде здесь стояла строка, сравнивавшаяся по
   * нормализованному ключу написания; с Э2 выбор строгий, и фильтр спрашивает то же самое, что
   * форма, — идентификатор строки справочника. Заявки, не нашедшие своей модели при переносе,
   * этим фильтром не отбираются ничем: у них ссылки нет вовсе, и до Э3 их находят поиском.
   */
  mechModelId: uuidSchema.optional(),
...
// ── Журнал закрытых и его итог: вкладка «История» (§7 п. 3, Э3) ──

/**
 * Столбцы сортировки журнала: **весь набор списка плюс факт аренды**.
 *
 * Факт — четыре колонки, которых в рабочем списке нет по существу: у незакрытой заявки возврата,
 * отработанного и суммы не бывает, а в журнале ими и пользуются («что вернули последним», «где
 * дороже всего»).
 *
 * Второй, укороченный перечень столбцов списка здесь не заводится намеренно: набор разошёлся бы с
 * первым на первой же новой колонке, и портал получал бы 400 на сортировку, которую сам и
 * предложил. Цена — `deletedAt`, который в журнале пуст у каждой строки (удалённых в нём нет
 * вовсе); сортировка по нему ничего не переставляет, и это дешевле расхождения.
 */
export const MECH_HISTORY_SORT_FIELDS = [
  ...MECH_REQUEST_SORT_FIELDS,
  /** Фактическая выдача: с этого дня шли деньги. */
  'actualFrom',
  /** Фактический возврат; у отменённой заявки его нет — она возвращается пустой колонкой. */
  'actualTo',
  'actualUnits',
  'finalCost',
] as const;
...
/**
 * Журнал закрытых заявок механизации (§8, Э3) — те же фильтры, что и у списка, кроме трёх.
 *
 * `rental` и `overdue` спрашивают про ДЕЙСТВУЮЩУЮ аренду (Р2), а в журнале действующих нет ни
 * одной: он про то, что кончилось. `archive` нет по той же причине, что и у вывоза (ADR 0135):
 * снесённая заявка живёт вкладкой «Архив» (ADR 0070), а журнал отвечает про состоявшееся.
 *
...
/**
 * Итог журнала за выбранные фильтры (§8, Э3). Считается по тем же условиям, что и сам журнал:
 * сводка, отвечающая не про то, что человек видит в таблице, вводит в заблуждение вернее, чем её
 * отсутствие.
 *
 * Три особенности, и каждая — находка ревью плана:
 *
 * 1. **часы и смены не складываются**: ставка задаётся за час либо за смену (Р7), и «120» без
 *    единицы не значит ничего — поэтому чисел два, по одному на значение `rate_unit`;
 * 2. **арендой считается только выданное** (`actual_from IS NOT NULL`): отменённая до подачи
 *    заявка арендой не была, и в `rentals` её включать нельзя — при том что закрытой она является
 *    и в `closed` с `cancelled` входит;
 * 3. **дни — календарные и включительно** (`dateKeySpan`): аренда «с 1-го по 1-е» — это один день,
 *    а не ноль, и разность дат ответила бы вторым.
 */
export interface MechRequestHistorySummaryDto {
  /** Всего закрытых заявок: «Выполнена» и «Отменена» вместе. */
  closed: number;
  /** Из них были арендами — с проставленной выдачей. */
  rentals: number;
  cancelled: number;
  /** Сумма календарных дней аренды, включительно. */
  days: number;
  /** Отработано там, где ставка за час. */
  hours: number;
  /** Отработано там, где ставка за смену. */
  shifts: number;
  /**
   * Сумма `final_cost`, ₽ — **строкой**: `numeric(14,2)` складывает база точно, а перевод суммы в
   * число с плавающей точкой по дороге к экрану даёт «9600.000000000001» на ровном месте. Строка
   * доезжает до портала ровно тем, что посчитала база.
   */
  cost: string;
}
...
export const createMechRequestSchema = z
  .object({
    ...
    /**
     * Предмет аренды — строка справочника моделей, и только она (ADR 0156, решение 2). Свободного
     * написания в теле не осталось вовсе: строка рядом с выбором означала бы, что справочник ведут
     * «когда удобно», а половина заявок продолжает жить написаниями. Цена решения названа там же:
     * пока модели нет в справочнике, заявку на неё не завести — её заводит держатель
     * `directories.write` на своей вкладке.
     *
     * Что модель существует и не погашена, схема не знает и знать не может: это вопрос к базе, и
     * отвечает на него сервер (`assertMechModelAssignable`).
     */
    mechModelId: uuidSchema,
...
export const updateMechRequestSchema = z
  .object({
    ...
    /** Смена модели — только у «Новой»: после неё за предметом стоит договорённость с арендодателем. */
    mechModelId: uuidSchema.optional(),
  ...
  .superRefine((v, ctx) => {
    ...
    if (v.status === 'completed') {
      ctx.addIssue({
        code: 'custom',
        path: ['status'],
        message: MECH_NO_COMPLETED_STATUS_MESSAGE,
      });
    }
...
/**
 * «Завершена» у механизации не бывает вовсе (Р8): перехода в неё нет ни в одном коридоре, а база
 * держит это отдельным `CHECK (status <> 'completed')`. Один текст на два отказа — схемы смены
 * статуса и фильтра журнала: оба отвечают на одну и ту же попытку назвать несуществующий статус.
 */
export const MECH_NO_COMPLETED_STATUS_MESSAGE = 'У аренды механизации нет статуса «Завершена»';
...
export interface MechRequestDto {
  ...
  /**
   * Модель из справочника (ADR 0156). `null` — заявка старше Э2, чьё написание при переносе не
   * нашлось; с Э3 (миграция 0256) названия техники у такой заявки нет вовсе, и портал показывает
   * на его месте прочерк. Состояние законное и постоянное: заказчик выбрал снять колонку с
   * написаниями, а не заводить таким заявкам модели.
   */
  mechModelId: string | null;
  /** Наименование модели — join'ом со справочником, а не снимком: переименование видно и в старых заявках. */
  mechModelName: string | null;
  plannedFrom: string;
  plannedTo: string;
...
export const mechRequestChangeLabels: Record<string, string> = {
  ...
  // Ключ остался прежним, а подпись сменилась вместе с полем (ADR 0156): история append-only, и
  // переименуй ключ — записи, сделанные до Э2, остались бы в карточке без подписи вовсе. Уборка Э3
  // сняла колонку `kind_name` из базы, но ключа не касается по той же причине: в базе её нет, а в
  // событиях прошлого года она есть, и подписывать их по-прежнему нечем больше.
  kindName: 'Модель',
  plannedFrom: 'Подача',
  plannedTo: 'Возврат',
