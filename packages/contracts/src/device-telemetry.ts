...
const parseRuleCommonShape = {
  ...
  /**
   * Модель, к которой писано правило: одно правило на модельный ряд вместо правила на аппарат.
   *
   * ПУСТАЯ СТРОКА — УСЛОВИЯ НЕТ, как у отправителя и темы. Значение сравнивается подстрокой, и это
   * не небрежность: прошивка пишет `RICOH MP C2011`, справочник — `Ricoh Aficio MP C2011SP`, и
   * условие, требующее точного равенства, не совпало бы ни с одной из двух записей сразу.
   *
   * ЧЕМ ИМЕННО СРАВНИВАТЬ, РЕШАЕТ СЕРВЕР, и порядок источников — часть решения (ADR 0204):
   * модель карточки опознанного аппарата старше модели, названной письмом. Второго носителя у
   * этого правила нет: форма только передаёт строку.
   */
  whenModel: z.string().max(200).default(''),
...
export interface DeviceParseRulePreviewDto {
  /** Условия применимости (профиль, отправитель, тема, модель) совпали. */
  applies: boolean;
...
// ── Письма для проверки правила ──

/**
 * Сколько писем предлагается на выбор в форме правила.
 *
 * Потолок, а не страницы: список нужен, чтобы ткнуть в письмо, на котором правило проверяют, и
 * листание в нём — это уже очередь разбора, у которой свой экран, свой отбор и свой курсор.
 */
export const DEVICE_RULE_SAMPLE_LIMIT = 50;

/**
 * Письмо, на котором правило можно проверить.
 *
 * ОТДЕЛЬНАЯ ВЫБОРКА, А НЕ ОЧЕРЕДЬ РАЗБОРА, и это не дублирование: очередь отвечает на вопрос «с
 * чем человеку надо разобраться» — там нет ни разобранных писем, ни закрытых просмотром. Правило
 * же чаще всего проверяют ровно на удачно разобранном письме, а письмо без сырья не годится
 * вовсе: проверять нечего, и предпросмотр честно отвечает отказом. Поэтому отбор здесь свой —
 * «у письма есть сохранённое сырьё», — и он назван прямо.
 */
export interface DeviceMailSampleDto {
  id: string;
  /** Момент приёма порталом: им же список и упорядочен, новые сверху. */
  receivedAt: string;
  /** Пустая строка — темы нет. Подпись письма собирает портал, а не сервер. */
  subject: string;
  /** Пустая строка — отправитель не назван. */
  fromAddress: string;
  status: DeviceMessageStatus;
}
