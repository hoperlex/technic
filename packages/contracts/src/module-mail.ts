 ...
 * назначении адресовано **людям**: тем, кого на заявку назначили, — и список берётся не отсюда, а
 * из строк `service_request_executors` и учёток оператора назначенного контрагента (план
 * `office-equipment-requests-rework-plan.md`, §7.3, Н13). Строка таблицы у него работает **поверх**
 * назначенных, а не вместо них: «хочу видеть все назначения» — это копия, а не адресация.
 */
...
export const moduleMailEventHints: Record<ModuleMailEvent, string> = {
  ...
  service_request_assigned:
    ...
    'списку и оператору сервисной компании; адрес отсюда получает копию сверх них. Заявителю ' +
    'это письмо не уходит: движение по заявке он видит в портале.',
};
 ...
 * не про базу.
 *
 * `no_recipients` заведён ради **письма о назначении** и только ради него: оно адресовано людям, а
 * не ящику канала, и адресатов у него может не оказаться совсем — заявку отдали сервисной компании,
 * у которой в портале нет ни одной учётки оператора. Слать такое письмо в ящик службы было бы
 * подменой адресата: служба и так назначила, а исполнитель задания не увидит. Молчать при этом
 * молча нельзя — назначивший обязан узнать, что задание никуда не ушло, и позвонить.
 *
...
export const moduleMailOutcomeLabels: Record<ModuleMailOutcome, string> = {
  ...
  no_recipients:
    'Некому отправить письмо: у назначенных нет ящиков в портале — предупредите их сами',
