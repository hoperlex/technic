...
/**
 * Исход почтовой части операции. Продолжает `MAIL_OUTCOMES` (ADR 0087) двумя значениями, и оба
 * означают «операция удалась, письма нет»: заявка не должна падать из-за настройки рассылок или
 * из-за того, что тело письма не собралось.
 *
...
export const moduleMailOutcomeLabels: Record<ModuleMailOutcome, string> = {
  ...
  no_recipients: 'Адресаты не настроены — служба не оповещена',
